...
test('founder runtime auto-registration receives operator requireKey, not member-permissive user-or-key auth', () => {
  const source = fs.readFileSync(new URL('../startup/register-founder-runtime-routes.js', import.meta.url), 'utf8');
  assert.match(
    source,
    /autoRegisterProductModules\(app,\s*\{\s*pool,\s*requireKey,\s*callCouncilMember,/s
  );
  assert.doesNotMatch(
    source,
    /autoRegisterProductModules\(app,\s*\{\s*pool,\s*requireKey:\s*requireUserOrKey,/s
  );
});
