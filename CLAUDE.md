...
**SO-004 — DOCUMENT EVERY CONVERSATION, ROUTED TO WHERE IT BELONGS (founder-ratified, 2026-08-07):** Every substantive conversation with Adam — any chat agent, any surface — must be captured, not left to live only in session context that vanishes at compaction or session end. Route to **both** real, already-existing destinations, never a new parallel system (matches the ARC asset-reuse discipline in `docs/constitution/FOUNDER_PACKET_V2_BUILDEROS_MASTER_ARCHITECTURE.md`): (1) **Product/topic space** — when the conversation is about a specific product or work-in-progress (e.g. communication system, a blueprint, a naming proposal), write a structured capture to `docs/products/<product>/conversations/<date>-<topic>.md`, matching the real, already-used pattern (`docs/products/lifeos/conversations/` has 7 real examples as of 2026-08-07). (2) **Twin/archive space** — also write to `docs/conversation_dumps/<date>-<topic>.md`, the real intake source `scripts/scan-conversation-dumps.mjs` reads and `scripts/import-dumps-to-twin.js` feeds into the Digital Twin (`memory_dump_chunks` → `adam_decisions` → twin profile). A capture is a faithful structured summary — verbatim founder quotes on load-bearing points, real decisions, real positions — not a raw multi-hour transcript dump. Capture at natural boundaries: when a distinct topic concludes, not only at the very end of a marathon session. This is not optional or best-effort — a conversation that only exists in a chat window Adam can't get back is exactly the kind of loss this order exists to close.
