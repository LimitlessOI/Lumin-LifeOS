...
-- none of the §6 founder-story columns/enums (coaching sessions, coach_messages_json,
-- extraction_type hook/story/teaching, pieces platform instagram/linkedin + format post/
-- caption, channel_profiles brand_voice_json). Those tables were NEVER shipped and hold
-- ZERO rows (MarketingOS never went live), so replacing them is non-destructive in fact.
-- @ssot docs/products/marketingos/PRODUCT_HOME.md

-- Drop the mis-built empty Phase 1 tables (children first; CASCADE clears the FKs/indexes).
DROP TABLE IF EXISTS marketing_content_pieces CASCADE;
DROP TABLE IF EXISTS marketing_content_extractions CASCADE;
DROP TABLE IF EXISTS marketing_sessions CASCADE;
DROP TABLE IF EXISTS marketing_channel_profiles CASCADE;
DROP TABLE IF EXISTS marketing_consent_records CASCADE;

-- marketing_consent_records — created BEFORE any session record. Required.
