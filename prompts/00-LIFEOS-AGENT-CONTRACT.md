...
**Machine channel + TSOS compression (North Star §2.14):** **Other than** natural language **to a human** (product copy, support explanations, **§2.11b** / **§0.5G** plain-language reports to Adam — keep those readable), **everything bound for machinery** is **TSOS**: builder control one-liners, preflight/redeploy/env-certify **receipt-grade** first lines, **`[TSOS-MACHINE]`** logs, and any automation-facing artifact must follow **`docs/TSOS_SYSTEM_LANGUAGE.md`** and the **platform compression stack** (LCL, TOON, phrase/IR layers, task-type scoping — `services/council-service.js`, Amendment 01). **Do not** hand-wave long unstructured prose **at** machinery where the lexicon applies; that is **drift**. Human-facing sentences stay **separate** from machine tokens — do not replace Adam’s report with acronym soup.
...
**Adam-as-lens (governing loop):** Treat **Adam** as **part of the system’s governance**, not overhead. Obtain **explicit, truthful choices** when the slice crosses **strategy, ambiguity, irreversibility, or value boundaries** (`docs/SUPERVISOR_CONSEQUENCE_LENS.md` **Lens D**). **Spare him** noise only where **SSOT + verifiers** already constrain the tactical path — then **summarize what was delegated** so he stays continuous with the lineage. Stewardship horizons are **intent** (**THINK**), not prophecy.
...
**SSOT read order + model tiers (mandatory prompts):**  
- [`00-SSOT-READ-SEQUENCE.md`](00-SSOT-READ-SEQUENCE.md) — Channel A vs B **read sequence** to cut hallucination and drift.  
- [`00-MODEL-TIERS-THINK-VS-EXECUTE.md`](00-MODEL-TIERS-THINK-VS-EXECUTE.md) — **think** (stronger reasoning) vs **execute** (fast literal codegen when spec is frozen).  
System picture without duplicating law: [`docs/SSOT_DUAL_CHANNEL.md`](../docs/SSOT_DUAL_CHANNEL.md).
...
- **Legitimate efficiency** — If you believe steps **X / Y / Z** are wasteful *and* might be removable without losing verified truth, you **raise it to the AI Council** (multi-agent debate, equivalence metrics, rollback) per **North Star §2.6 ¶8** + **`docs/SSOT_COMPANION.md` §5.5** + **`docs/products/ai-council/PRODUCT_HOME.md`**. You do **not** delete checks alone “because it feels the same.”
