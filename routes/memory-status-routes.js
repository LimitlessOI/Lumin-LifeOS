...
import { pool } from '../db';
import { requireKey } from '../auth/middleware';

const createMemoryStatusRoutes = ({ pool, rk }) => {
  const router = Router();
  ...
  router.get('/api/v1/lifeos/command-center/memory/status', requireKey, async (req, res) => {
    try {
      ...
      const twentyFourHourCount = await pool.query('SELECT COUNT() FROM self_repair_memory_events WHERE created_at > NOW() - INTERVAL 24h');

      const totalMemoryEvents = totalRowCount.rows[0].count;
      ...
      const memoryEventCount24h = twentyFourHourCount.rows[0].count;
      ...
      const memoryEnabled = totalMemoryEvents > 0;
