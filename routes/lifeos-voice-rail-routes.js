...
export function createLifeOSVoiceRailRoutes({
  ...
  router.post('/message', requireKey, async (req, res, next) => {
    try {
      ...
      const explicitPrivate = Boolean(req.body.private) || mode === 'private';
      const preSubmitClassification = !explicitPrivate && capturePipeline
        ? capturePipeline.classifyVoiceSubmit({ text: req.body.text, mode })
        : null;
      const privateNoSave = preSubmitClassification === 'private_no_save';
      const result = await voiceRail.submitMessage({
        ...
        private: explicitPrivate || privateNoSave,
      ...
      if (capturePipeline && !result.private) {
        ...
        if (inbox_staging?.reason === 'capture_error') {
          return res.status(503).json({
            ok: false,
            error: 'capture_pipeline_failed',
            detail: inbox_staging,
          });
        }
      }
      res.json({ ok: true, ...result, inbox_staging, pre_submit_classification: preSubmitClassification });
    } catch (err) {
