...
export async function registerMarketingSessionRoutes(app, deps) {
    ...
    router.post('/sessions/:id/coach', requireKey, async (req, res) => {
        try {
            ...
            const aiResponseText = await callCouncilMember('marketing-coach', fullPrompt);
    ...
    router.patch('/content/:id', requireKey, async (req, res) => {
        try {
            ...
            if (action === 'approve') {
                ...
                const aiResponseText = await callCouncilMember('marketing-generator', regenerationPrompt);
