...
import { truthGateOutbound } from "./services/truth-enforcement-spine.js";
...
import { createSpineCallAI } from './services/truth-enforcement-spine.js';
...
const runtimeProfile = "full";
const fullRuntimeProfile = true;
...
const { registerServerRoutes: registerServerSurface } = await import("./startup/routes/server-routes.js");
const fullRuntimeServerSurfaceDeps = await (async () => {
...
function broadcastToAll(message) {
  const gated = truthGateOutbound(message, 'websocket');
  const payload = typeof gated === 'string' ? gated : JSON.stringify(gated);
...
const spineCallAI = createSpineCallAI(callCouncilMember, { taskType: 'lifeos.spine' });
...
const autonomyOrchestrator = createAutonomyOrchestrator({
  ...
  callAI: async (_task, prompt) => {
    try {
      return await spineCallAI('autonomy', prompt, { member: 'anthropic' });
    } catch { return ''; }
  },
...
async function mountRuntimeRoutes() {
  const routeRegistrar = (await import("./startup/register-runtime-routes.js")).registerRuntimeRoutes;
...
const ciMonitor = createContinuousImprovement({
  ...
  callAI: async (prompt) => {
    try {
      return await spineCallAI('continuous_improvement', prompt, { member: 'anthropic' });
    } catch {
      return '';
    }
  },
