/**
 * SYNOPSIS: Worker-council subsystem initialization
 * Worker-council subsystem initialization
 * Initializes the worker-council support layer, model router, execution queue,
 * self-programming service, and dependent modules.
...
export async function initializeTwoTierSystem(deps) {
  ...
  try {
    ...
    if (tcoRuntimeEnabled) {
      ...
      logger.info("🛑 [TCO] TCO system bootstrap not initialized (set LIFEOS_ENABLE_TCO_RUNTIME=true to restore)");
    }

    if (legacyTwoTierRouteSpineEnabled) {
      logger.info("🎯 [TWO-TIER-LEGACY] Registering Enhanced Council routes...");
      registerEnhancedCouncilRoutes(app, pool, callCouncilMember, requireKeyFn);
      logger.info("✅ [TWO-TIER-LEGACY] Enhanced Council routes registered");
      ...
      logger.info("✅ [TWO-TIER-LEGACY] Site Builder routes registered");
      createPipelineReportRoutes(app, { pool, requireKey: requireKeyFn });
      logger.info("✅ [TWO-TIER-LEGACY] Site Builder pipeline report routes registered");
      app.use('/api/v1/sites', createDiscoveryRoutes(app, { pool, requireKey: requireKeyFn }));
      logger.info("✅ [TWO-TIER-LEGACY] Site Builder discovery routes registered");
      ...
      logger.info("🛑 [TWO-TIER-LEGACY] Duplicate route spine disabled; canonical runtime routing lives in startup/register-runtime-routes.js");
    }

    console.log("\n╔══════════════════════════════════════════════════════════════════════════════════╗");
    console.log("║ ✅ [WORKER COUNCIL] INITIALIZED                                                   ║");
    console.log("║    Status: Cloud-only worker lanes active                                       ║");
    ...
    try {
      ...
      try {
        ...
        if (auxiliaryBootServicesEnabled) {
          try {
    ...
    if (legacyTwoTierRouteSpineEnabled) {
      ...
      logger.info('✅ [TWO-TIER-LEGACY] All modular routes registered');
    } else {
      logger.info('🛑 [TWO-TIER-LEGACY] Duplicate modular routes not mounted');
    }

  } catch (error) {
    logger.error("⚠️ Two-Tier System initialization error:", { error: error.message });
    logger.error("   System will continue without the duplicate two-tier route spine");
  }
