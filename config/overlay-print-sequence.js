...
export const OVERLAY_PRINT_SEQUENCE = Object.freeze([
  ...
  printStep({
    ...
    spec: 'Add makeAndroidBody so a caller can pass the Android Body without forking the Mind. Real reachability is a SEPARATE step (TALOA-S64-ANDROID-BODY-LIVE-WIRE-001) -- the codegen pipeline authors exactly one target_file per step (confirmed live 2026-08-20: routes/factory-mount-routes.js builds one CODEGEN prompt for one TARGET FILE and can never write a second file), so a cross-file reachability requirement on THIS step was structurally unsatisfiable no matter how many retries -- burned this step\'s entire revive budget on an honest but impossible check before this was caught.',
  ...
  printStep({
    id: 'TALOA-S64-ANDROID-BODY-LIVE-WIRE-001',
    target_file: 'services/general-browser-agent-live.js',
    sandbox_boundary: 'services/**',
    depends_on: ['TALOA-S64-ANDROID-BODY-WIRE-001'],
    task: 'Real reachability companion to TALOA-S64-ANDROID-BODY-WIRE-001, same pattern as TALOA-S64-MACOS-BODY-WIRE-001 for the macOS Body: THIS step\'s target_file is the actual live caller, so its own codegen pass can satisfy the check. runGoalOnSession currently hardcodes browser observe/act (observePage/executeAction from ./general-browser-agent-runtime.js) with no Body selection. Import makeAndroidBody from ./general-browser-agent-runtime.js. Add an optional `body` parameter to runGoalOnSession (default undefined, preserving current browser-only behavior exactly). When body is provided, use body.observe/body.act instead of the hardcoded browser observe/act. Additive only -- do not change default/no-arg behavior.',
    spec: 'Closes the real gap TALOA-S64-ANDROID-BODY-WIRE-001 could not close itself: makeAndroidBody existed but had zero callers anywhere (confirmed by direct grep, 2026-08-20).',
    file_contains: ['makeAndroidBody'],
    behavior_assertions: [
      { type: 'file_contains', path: 'services/general-browser-agent-live.js', must_include: ['makeAndroidBody'], assertion_id: 'reachability:android-body-live-wired' },
    ],
  }),
  printStep({
  ...
  printStep({
    ...
    spec: 'Blueprint §30–32. Reuse capsule_store. Real reachability is a SEPARATE step (TALOA-S64-TEMPLATE-REPLAY-WIRE-001) -- the codegen pipeline authors exactly one target_file per step (confirmed live 2026-08-20), so a cross-file reachability requirement on THIS step was structurally unsatisfiable no matter how many retries.',
    expected_exports: ['createTemplateReplayService'],
    file_contains: ['createTemplateReplayService', 'environment_signature'],
    behavior_assertions: [
      { type: 'file_contains', path: 'services/taloa/template-replay-service.js', must_include: ['createTemplateReplayService', 'environment_signature'], assertion_id: 'self:template-replay-defined' },
    ],
  }),
  printStep({
    id: 'TALOA-S64-TEMPLATE-REPLAY-WIRE-001',
    target_file: 'services/taloa/task-orchestrator-service.js',
    sandbox_boundary: 'services/taloa/**',
    depends_on: ['TALOA-S64-TEMPLATE-REPLAY-001'],
    task: 'Real reachability companion to TALOA-S64-TEMPLATE-REPLAY-001, same pattern as TALOA-S64-MACOS-BODY-WIRE-001: THIS step\'s target_file is the actual real caller, so its own codegen pass can satisfy the check. Import createTemplateReplayService from ./template-replay-service.js and call it at the start of dispatchTask() -- if a matching template replay exists, use it instead of falling through to strategyRouter/bodyAdapter. Additive only, do not rewrite dispatchTask\'s existing flow for the no-replay case.',
    spec: 'Closes the real gap TALOA-S64-TEMPLATE-REPLAY-001 could not close itself: createTemplateReplayService existed but had zero callers anywhere (confirmed by direct grep, 2026-08-20).',
    file_contains: ['createTemplateReplayService'],
    behavior_assertions: [
  ...
  printStep({
    ...
    sandbox_boundary: 'services/taloa/**',
    task: 'JSDoc @ssot docs/products/universal-overlay/PRODUCT_HOME.md. Overlay print §64 item 8: observed page text cannot become instructions. Export assertObservationIsNotAuthority(observation, envelope) with REAL detection logic, not a no-op stub -- a no-op that always passes is a false gate, worse than no gate (it claims coverage it does not have).',
    spec: 'Blueprint §46. Real reachability is a SEPARATE step (TALOA-S64-PROMPT-INJECT-WIRE-001) -- the codegen pipeline authors exactly one target_file per step (confirmed live 2026-08-20), so a cross-file reachability requirement on THIS step was structurally unsatisfiable no matter how many retries -- burned this step\'s entire revive budget on an honest but impossible check before this was caught.',
    expected_exports: ['assertObservationIsNotAuthority'],
    file_contains: ['assertObservationIsNotAuthority'],
    behavior_assertions: [
      { type: 'file_contains', path: 'services/taloa/prompt-injection-authority-gate.js', must_include: ['assertObservationIsNotAuthority'], assertion_id: 'self:prompt-inject-gate-defined' },
    ],
  }),
  printStep({
    id: 'TALOA-S64-PROMPT-INJECT-WIRE-001',
    target_file: 'services/taloa/body-adapter-service.js',
    sandbox_boundary: 'services/taloa/**',
    depends_on: ['TALOA-S64-PROMPT-INJECT-001'],
    task: 'Real reachability companion to TALOA-S64-PROMPT-INJECT-001, same pattern as TALOA-S64-MACOS-BODY-WIRE-001: THIS step\'s target_file is the actual real caller, so its own codegen pass can satisfy the check. Import assertObservationIsNotAuthority from ./prompt-injection-authority-gate.js and call it inside observe(), after the observation is produced and before it is returned, passing the current task\'s authorization envelope. Additive only -- do not rewrite observe()\'s existing flow beyond this.',
    spec: 'Closes the real gap TALOA-S64-PROMPT-INJECT-001 could not close itself: assertObservationIsNotAuthority existed but had zero callers anywhere (confirmed by direct grep, 2026-08-20).',
    file_contains: ['assertObservationIsNotAuthority'],
    behavior_assertions: [
