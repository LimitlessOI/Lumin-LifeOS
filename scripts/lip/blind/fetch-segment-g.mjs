/**
 * SYNOPSIS: Segment G: unused 2021-04..2021-06 pumps (exclude A–F).
 * @ssot docs/products/lip/PRODUCT_HOME.md
 * Segment G: unused 2021-04..2021-06 pumps (exclude A–F).
 */
...
const RAW = path.join(LIP_DATA, 'blind_raw_g');
const CSV_URL =
...
export async function buildSegmentGTimeline(opts = {}) {
  ensureLipDataDir();
  ...
  const pumps = parsePumpCsv(fs.readFileSync(csvPath, 'utf8'))
    .filter((p) => p.date >= '2019-08-01' && p.date <= '2019-09-30')
  ...
  const meta = {
    ...
    date_range: '2019-08-01..2019-09-30',
    ...
    used,
    note: 'Fresh unused slice (2019 Aug–Sep) for both-sides (up trail + down short).',
  };
  fs.writeFileSync(path.join(LIP_DATA, 'blind_g_meta.json'), JSON.stringify(meta, null, 2));
  fs.writeFileSync(path.join(LIP_DATA, 'blind_g_timeline.jsonl'), deduped.map((e) => JSON.stringify(e)).join('\n'));
  return { timeline: deduped, meta };
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  buildSegmentGTimeline({ maxPumps: 30 }).then((r) => console.log(JSON.stringify(r.meta, null, 2)));
}
