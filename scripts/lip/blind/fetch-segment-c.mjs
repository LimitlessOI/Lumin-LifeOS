/**
 * SYNOPSIS: Segment C: unused real pumps (2020-06..2020-12), exclude A+B.
 * @ssot docs/products/lip/PRODUCT_HOME.md
 * Segment C: unused real pumps (2020-06..2020-12), exclude A+B.
 * Extra pre-days of candles so blind reader can hunt the success trail.
 */
...
const RAW = path.join(LIP_DATA, 'blind_raw_c');
const CSV_URL =
...
function loadExcludeKeys() {
  ...
  for (const f of ['blind_timeline_meta.json', 'blind_b_meta.json']) {
...
export async function buildSegmentCTimeline(opts = {}) {
  ensureLipDataDir();
  ...
  const pumps = parsePumpCsv(fs.readFileSync(csvPath, 'utf8'))
    .filter((p) => p.date >= '2020-06-01' && p.date <= '2020-12-31')
  ...
  for (const p of pumps) {
    ...
    if (paths.length < 2) continue; // need enough pre-trail
  ...
  const meta = {
    ...
    used,
    note: 'Unused vs A/B. Extra pre-days for blind success-trail hunting. Real Binance 1m + Sapienza TG.',
  };

  fs.writeFileSync(path.join(LIP_DATA, 'blind_c_meta.json'), JSON.stringify(meta, null, 2));
  fs.writeFileSync(path.join(LIP_DATA, 'blind_c_timeline.jsonl'), deduped.map((e) => JSON.stringify(e)).join('\n'));
  return { timeline: deduped, meta };
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  buildSegmentCTimeline({ maxPumps: 28 }).then((r) => console.log(JSON.stringify(r.meta, null, 2)));
}
