/**
 * SYNOPSIS: Segment D: unused real pumps (2020-04..2020-05 + leftover 2020-08..09), exclude A/B/C.
 * @ssot docs/products/lip/PRODUCT_HOME.md
 * Segment D: unused real pumps (2020-04..2020-05 + leftover 2020-08..09), exclude A/B/C.
 */
...
const RAW = path.join(LIP_DATA, 'blind_raw_d');
const CSV_URL =
...
function dayOffset(dateStr, days) {
  const t = Date.parse(`${dateStr}T12:00:00Z`) + days * 86400000;
...
function loadExcludeKeys() {
  ...
  for (const f of ['blind_timeline_meta.json', 'blind_b_meta.json', 'blind_c_meta.json']) {
    const p = path.join(LIP_DATA, f);
    if (!fs.existsSync(p)) continue;
    const meta = JSON.parse(fs.readFileSync(p, 'utf8'));
    for (const u of meta.used || []) exclude.add(`${u.symbol}|${u.date}`);
  }
  return exclude;
}

export async function buildSegmentDTimeline(opts = {}) {
  ensureLipDataDir();
  ...
  // Fresh slice: Apr–May 2020 then Aug–Sep 2020 leftovers
  const pumps = parsePumpCsv(fs.readFileSync(csvPath, 'utf8'))
    .filter(
      (p) =>
        (p.date >= '2020-04-01' && p.date <= '2020-05-31') ||
        (p.date >= '2020-08-01' && p.date <= '2020-09-30')
    )
    .filter((p) => !exclude.has(`${p.symbol}|${p.date}`))
    .sort((a, b) => a.ts - b.ts);

  const maxPumps = opts.maxPumps ?? 28;
  const timeline = [];
  const used = [];

  for (const p of pumps) {
    if (used.length >= maxPumps) break;
    const days = [dayOffset(p.date, -2), dayOffset(p.date, -1), p.date];
    const paths = [];
    let quote = null;
    for (const d of days) {
    ...
    used.push({ symbol: p.symbol, date: p.date, hour: p.hour, group: p.group, quote, days: paths.length });
  }
  ...
  const meta = {
    ...
    date_range: '2020-04..05 + 2020-08..09',
    ...
    used,
    note: 'Unused vs A/B/C. Blind retest after Segment C lessons.',
  };
  fs.writeFileSync(path.join(LIP_DATA, 'blind_d_meta.json'), JSON.stringify(meta, null, 2));
  fs.writeFileSync(path.join(LIP_DATA, 'blind_d_timeline.jsonl'), deduped.map((e) => JSON.stringify(e)).join('\n'));
  return { timeline: deduped, meta };
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  buildSegmentDTimeline({ maxPumps: 28 }).then((r) => console.log(JSON.stringify(r.meta, null, 2)));
}
