/**
 * SYNOPSIS: Segment F: unused 2021-01..2021-03 pumps (exclude A–E).
 * @ssot docs/products/lip/PRODUCT_HOME.md
 * Segment F: unused 2021-01..2021-03 pumps (exclude A–E).
 */
...
const RAW = path.join(LIP_DATA, 'blind_raw_f');
const CSV_URL =
...
export async function buildSegmentFTimeline(opts = {}) {
  ensureLipDataDir();
  ...
  const pumps = parsePumpCsv(fs.readFileSync(csvPath, 'utf8'))
    .filter((p) => p.date >= '2021-01-01' && p.date <= '2021-03-31')
  ...
  const meta = {
    ...
    used,
    note: 'Fresh unused slice for lessons v3 (short-only, never buy late).',
  };
  fs.writeFileSync(path.join(LIP_DATA, 'blind_f_meta.json'), JSON.stringify(meta, null, 2));
  fs.writeFileSync(path.join(LIP_DATA, 'blind_f_timeline.jsonl'), deduped.map((e) => JSON.stringify(e)).join('\n'));
  return { timeline: deduped, meta };
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  buildSegmentFTimeline({ maxPumps: 30 }).then((r) => console.log(JSON.stringify(r.meta, null, 2)));
}
