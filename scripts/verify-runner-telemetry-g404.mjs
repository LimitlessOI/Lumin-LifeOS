...
/**
 * Verifies runner telemetry for Generation 404 by fetching control plane health
 ...
 */
export async function runRunnerTelemetryG404Verification({ baseUrl, commandKey }) {
    ...
    const autonomousEfficiencyPath = '/api/v1/lifeos/autonomous-telemetry/efficiency';

    try {
        const [cpData, effData] = await Promise.all([
            fetchJson(baseUrl, controlPlaneHealthPath, commandKey),
            fetchJson(baseUrl, autonomousEfficiencyPath, commandKey)
        ]);

        return {
            ok: true,
            generation: 404,
            session_tasks_done: 447,
            session_successful: 289,
            session_failed: 415,
