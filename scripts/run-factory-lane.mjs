 ...
 *
 * Usage: FACTORY_ID=factory-2 node scripts/run-factory-lane.mjs [--loop|--install-agent]
 *
...
function tickPath(repoRoot) {
  return path.join(repoRoot, 'native/macos-overlay/build/.factory-2-tick.json');
}

function readLastTick(repoRoot) {
  try {
    return JSON.parse(fs.readFileSync(tickPath(repoRoot), 'utf8'));
  } catch {
    return null;
  }
}
...
function writeTick(repoRoot, tick) {
  ...
  fs.writeFileSync(tickPath(repoRoot), `${JSON.stringify(tick, null, 2)}\n`);
}
...
function installLaunchAgent() {
  ...
  const plistPath = path.join(home, 'Library/LaunchAgents/com.lumin.factory-2-lane.plist');
  const logPath = path.join(home, 'Library/Logs/lumin-factory-2-lane.log');
...
<plist version="1.0">
<dict>
  <key>Label</key><string>com.lumin.factory-2-lane</string>
  ...
  <dict>
    <key>FACTORY_ID</key><string>factory-2</string>
...
`;
  ...
  return { ok: true, plist: plistPath, log: logPath, node, script };
}
