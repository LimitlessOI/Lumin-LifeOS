...
async function fetchJson(baseUrl, path, commandKey) {
    if (!baseUrl || !path || !commandKey) {
        return { error: true, message: 'Missing baseUrl, path, or commandKey for fetchJson.' };
    }
 ...
 */
export async function runRunnerTelemetryG473Verification({ baseUrl, commandKey }) {
    ...
    const fetchErrors = [];
    if (cpResult.error) fetchErrors.push(`Control Plane Health: ${cpResult.message}`);
    if (effResult.error) fetchErrors.push(`Autonomous Telemetry Efficiency: ${effResult.message}`);
    ...
    return {
        ok: true,
        generation: 473,
        session_tasks_done: 516,
        session_successful: 354,
        session_failed: 446,
