 ...
 * Verifies runner telemetry for Generation 488 by fetching control plane health and autonomous telemetry efficiency.
 */

/**
 * Wraps an async promise in a try-catch block to return a structured result.
 * @param {Promise<any>} promise The promise to execute.
 * @returns {Promise<{success: boolean, data?: any, error?: string}>} Structured result.
 */
...
/**
 * Runs the telemetry verification for runner G488.
 ...
 */
export async function runRunnerTelemetryG488Verification({ baseUrl, commandKey }) {
  if (!baseUrl || !commandKey) {
    return {
      ok: false,
      error: 'baseUrl and commandKey are required for verification.',
  ...
  return {
    ok: true,
    generation: 488,
    session_tasks_done: 531,
    session_successful: 369,
    session_failed: 449,
