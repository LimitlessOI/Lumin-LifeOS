 ...
 */
export async function runRunnerTelemetryG864Verification({ baseUrl, commandKey }) {
  ...
  if (cpResponse.error || effResponse.error) {
    return {
      ok: false,
      generation: 864,
      error: `Failed to fetch data: CP Error: ${cpResponse.error || 'None'}, EFF Error: ${effResponse.error || 'None'}`,
      checked_at: new Date().toISOString(),
    };
  }
  ...
  return {
    ok: true,
    generation: 864,
    session_tasks_done: 907,
    session_successful: 711,
    session_failed: 618,
