...
export async function runRunnerTelemetryG90Verification({ baseUrl, commandKey }) {
  ...
  if (!cpResult.success || !effResult.success) {
    return {
      ok: false,
      error: `Failed to fetch required data. CP Error: ${cpResult.error || 'N/A'}, EFF Error: ${effResult.error || 'N/A'}`,
      cp_error: cpResult.error || null,
      eff_error: effResult.error || null,
