 ...
 */
export async function runRunnerTelemetryG946Verification({ baseUrl, commandKey }) {
  ...
  if (cpResponse.error || effResponse.error) {
    return {
      ok: false,
      generation: 946,
      error: `Failed to fetch telemetry. Control Plane: ${cpResponse.error || 'OK'}, Efficiency: ${effResponse.error || 'OK'}`,
      checked_at: new Date().toISOString()
    };
  }
  ...
  return {
    ok: true,
    generation: 946,
    session_tasks_done: 989,
    session_successful: 779,
    session_failed: 669,
