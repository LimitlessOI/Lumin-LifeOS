#!/usr/bin/env node
/**
 * SYNOPSIS: Prototype V2 — Evidence Fusion & Cognitive Dynamics.
 * Combines transcript, timing, tonality, and history into calibrated
 * confidence estimates for user cognitive/emotional states, then learns
 * per-context modality weights from labeled outcomes.
...
export function extractTranscriptEvidence(text) {
  ...
  if (/\?/.test(text)) {
    // Questions are usually curious unless they signal confusion.
...
export function fuseEvidence(modalities, options = {}) {
  ...
  // Apply calibration offsets and temperature.
...
export function learnWeights(labeledExamples, initialWeights, options = {}) {
  ...
  for (let iter = 0; iter < iterations; iter += 1) {
    for (const ex of labeledExamples) {
      ...
      // Boost weights of modalities whose dominant state matches the true label.
  ...
  // Normalize to keep comparable scale.
...
function selfTest() {
  ...
  const result = fuseEvidence(modalities, { weights: { transcript: 1, timing: 0.8, tonality: 1 } });
  assert.ok(result.state === 'frustrated', 'frustrated should win');
  assert.ok(result.confidence > 0.5, 'confidence above chance');
  assert.ok(result.sources.length === 3, 'three modality sources');
  ...
  assert.ok(learned.weights.transcript >= 0.1, 'transcript weight remains positive');

  console.log('Evidence Fusion V2 self-tests passed.');
}
