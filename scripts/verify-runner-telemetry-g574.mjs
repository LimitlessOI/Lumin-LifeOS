...
async function fetchJson(baseUrl, path, commandKey) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to fetch ${path}: HTTP ${response.status} - ${errorText}`);
    }
 ...
 */
export async function runRunnerTelemetryG574Verification({ baseUrl, commandKey }) {
  ...
  try {
    ...
    return {
      ok: true,
      generation: 574,
      session_tasks_done: 617,
      session_successful: 447,
      session_failed: 496,
