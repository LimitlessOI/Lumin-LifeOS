 ...
 */
export async function runRunnerTelemetryG931Verification({ baseUrl, commandKey }) {
    ...
    try {
        ...
        if (cpResponse.error) {
            throw new Error(`Control Plane Health fetch failed: ${cpResponse.message}`);
        }
        cpData = cpResponse;

        if (effResponse.error) {
            throw new Error(`Efficiency Telemetry fetch failed: ${effResponse.message}`);
        }
        effData = effResponse;
    ...
    return {
        ok: true,
        generation: 931,
        session_tasks_done: 974,
        session_successful: 766,
        session_failed: 660,
