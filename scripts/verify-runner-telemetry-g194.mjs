 ...
 */
export async function runRunnerTelemetryG194Verification({ baseUrl, commandKey }) {
  ...
  const [fetchError, fetchResults] = await tryCatch(
    Promise.all([
  ...
  const [cpData, effData] = fetchResults;
  return {
    ok: true,
    generation: 194,
