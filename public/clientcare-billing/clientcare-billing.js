    ...
    root.innerHTML = `
      ...
      <div class="hero">
        <div>
          ...
          ${isBirthBillMode ? `<div class="card" style="margin-top:14px;background:#16353a;border-color:#2a6b6b;"><strong style="color:#9fe0d8">What each word means here</strong><p class="muted" style="margin-top:8px;color:#c9e8e4;line-height:1.5"><strong>Forever-chase</strong> = unpaid/underpaid stays open forever until paid enough or written no-liability denial.<br/><strong>Next action</strong> = the one thing to do in ClientCare or with the insurer for that row.<br/><strong>Human-in-the-loop</strong> = automation helps; you approve judgment calls. ChargeSlip auto-create is not guaranteed yet.</p>${getBirthBillTenantId() ? `<p class="muted" style="margin-top:8px;color:#9fe0d8;">Practice tenant #${escapeHtml(getBirthBillTenantId())}</p>` : ''}</div>` : ''}
