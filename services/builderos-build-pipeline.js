...
async function runBuildPipeline(opts) {
  ...
  // Additive-patch (opts.additivePatch) and edit-patch (opts.editPatch): the
  // caller spliced only-new-code or surgical find/replace edits into the
  // existing file, so `generatedOutput` is the full merged file with all prior
  // content preserved verbatim. There is no full-file-rewrite stub risk, so
  // skip the Zone 3 hard-block and let the size/stub/antipattern gates below
  // validate the merged result (they still catch any collapse or duplication).
  const isPatched = opts.additivePatch === true || opts.editPatch === true;
  if (zoneResult.zone === 3 && !intakeRewrite && !isPatched) {
