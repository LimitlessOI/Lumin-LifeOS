...
export function modelRoutingForChannel(channel) {
  switch (channel) {
    ...
    case 'lumin':
    case 'chair':
      // SO-003: Chair / counsel is load-bearing — never mark cheap/medium as the tier.
      return { route: 'lumin_chair_native', complexity: 'high', estimated_cost_tier: 'strong' };
    case 'point_b':
...
export async function runLuminChairTurn(ctx, deps) {
  ...
  // Founder Alpha Chat v2: execute any deterministic life_admin action (commitment,
  // note, check-in, etc.) first, then route the result through the Chair model so the
  // reply is generated by the API, not a canned template.
  // GAP-FILL 2026-08-04: resolveChairContext() (services/chair-context-classifier.js)
  // never actually returns the literal channel value 'life_admin' -- personal/life-admin
  // messages resolve to 'chair' (modelRoutingForChannel already treats 'chair' and
  // 'life_admin' as equivalent for model-tier routing, and counselChannels below already
  // groups them). This made the condition below permanently false in production:
  // confirmed live that Chair gave a confident, specific "I can schedule X" reply to a
  // real commitment-creation message, and a later "what are my upcoming commitments"
  // query fabricated plausible-sounding titles -- neither call ever reached
  // captureCommitment/getCommitments, since this whole block never executed. Widened to
  // also match channel 'chair'. Deliberately excludes chatIntent === 'build_request':
  // that classification calls routeBuildRequest(), which POSTs to
  // /factory/execute-step with skip_intake_gate: true -- a real governed-factory
  // trigger that must go through the existing, more careful likelyBuild /
  // requiresPreExecuteClarify gate earlier in this function, not this simpler regex
  // classifier. Verified the fallthrough (chatIntent === 'unknown' or the try/catch) is
  // a no-op for non-matching messages, so this stays safe for governance/build/system
  // messages that also resolve to channel 'chair'.
  if ((channel === 'life_admin' || channel === 'chair') && deps.pool && resolvedUserId) {
    try {
      const {
        classifyIntent: classifyChatIntent,
        executeIntent: executeChatIntent,
      ...
      if (chatIntent !== 'unknown' && chatIntent !== 'build_request') {
        const chatResult = await executeChatIntent({
          db: deps.pool,
          userId: resolvedUserId,
          timezone: 'America/New_York',
        ...
        const agentRes = await runChairDirectAgent({
          ...
          deps: {
            ...
            pool: deps.pool,
            memoryContext: null,
            trySystemAction: () => ({ matched: false, ok: false, error: 'life_admin intent already executed; reply only' }),
          },
          ctx: {
            userId: resolvedUserId,
            userHandle: ctx.userHandle || userHandle || null,
            ...
            intentExecution: {
              ...
              transport: chatResult.transport,
              summary: chatResult.human_summary || chatResult.message,
            },
          },
        });
        if (agentRes && agentRes.reply) {
          if (chatResult?.execution_kind === 'command') {
            agentRes.command_ran = chatResult.ok === true;
            agentRes.action_type = agentRes.action_type || chatIntent;
          }
