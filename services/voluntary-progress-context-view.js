...
export async function activateContextView(pool, { user_id, activation_reason, permitted_reads, forbidden_reads = [], permitted_proposals = [], consent_scope = 'internal_operational', ttl_minutes = 30 }) {
  await pool.query(`CREATE TABLE IF NOT EXISTS voluntary_progress_context_views (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id TEXT NOT NULL,
    activation_reason TEXT NOT NULL,
    permitted_reads JSONB NOT NULL,
    forbidden_reads JSONB NOT NULL DEFAULT '[]',
    permitted_proposals JSONB NOT NULL DEFAULT '[]',
  ...
  const result = await pool.query(
    ...
    [
      user_id,
      activation_reason,
      JSON.stringify(permitted_reads),
      JSON.stringify(forbidden_reads),
      JSON.stringify(permitted_proposals),
      consent_scope,
      ttl_minutes
    ]
  );
...
export function applyContextView(twinReadState, contextViewRow) {
  ...
  return twinReadState.filter(entry =>
    permittedFields.has(entry.field) && !forbiddenFields.has(entry.field)
  );
}
