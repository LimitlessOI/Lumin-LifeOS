...
exp isExplicitDisplayOnlyRequest(text = '', action = 'auto') {
  ...
  // Ensure BuilderOS repair asks for service files are not classified as display-only
...
exp coerceDisplayMisrouteToChair(text = '', chairContext = {}, opts = {}) {
...
exp auditLuminConversationRoutingWiring() {
  const checks = [];
