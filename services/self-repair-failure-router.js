...
export async function routeFailure(pool, { failure_signature, target_path, approach_signature }) {
  ...
  let matched_negative_knowledge = await hasFailedApproach(pool, { target: target_path, approach_signature });
  ...
  let matched_chains = await queryRootCauseChains(pool, { bug_shape_signature: failure_signature });
