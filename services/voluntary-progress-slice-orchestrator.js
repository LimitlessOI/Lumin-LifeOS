...
export async function proposeGoal(pool, { user_id, goal_text, goal_category = 'general' }) {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS voluntary_progress_goals (
  ...
  const result = await pool.query(`
    INSERT INTO voluntary_progress_goals (user_id, goal_text, goal_category, origin, status)
    VALUES ($1, $2, $3, 'user_stated', 'proposed')
    RETURNING *
...
export async function presentAlternativeFutures(pool, { user_id, goal_id, twinKey, moduleKey = null, tenantId = 'default', fields = [] }) {
  ...
  const contextView = await activateContextView(pool, {
    user_id, 
    activation_reason: `Presenting alternatives for goal_id: ${goal_id}`, 
  ...
  if (!twinReadResult.ok && twinReadResult.error === 'forbidden_cross_user') {
...
export async function confirmGoalDecision(pool, { user_id, goal_id, decision_status, modified_goal_text = null, chosen_path_label = null, path_description = null }) {
  ...
  let status = decision_status;
  if (['rejected', 'opted_out'].includes(decision_status)) {
    status = 'declined';
  }
  await pool.query(`
    UPDATE voluntary_progress_goals
    SET status = $1
    WHERE id = $2
  `, [status, goal_id]);

  let experiment = null;
  if (['accepted', 'modified'].includes(decision_status) && chosen_path_label && path_description) {
...
export async function compareExperimentOutcome(pool, { user_id, goal_experiment_id, actual_option, captured_how = 'explicit', twin_subject, twin_proposed_value, truth_grade = 'THINK', evidence = null }) {
  ...
  const twinProposal = await proposeTwinUpdate(pool, {
    ...
    truth_grade,
    evidence,
    rationale: `Compared experiment outcome for goal_experiment_id: ${goal_experiment_id}, actual_option: ${actual_option}`,
    proposed_by: 'voluntary-progress-slice-orchestrator'
  });

  return { ok: true, outcome, twin_proposal: twinProposal };
}
