...
import { validatePostmarkWebhook, validateTwilioWebhook } from '../services/tc-webhook-validation-service.js';
...
const LISTING_SYNC_JOB_TTL_MS = 2 * 60 * 60 * 1000; // 2 hours
...
export function createTCRoutes(
  app,
  { pool, requireKey, coordinator, logger = console, accountManager: injectedAccountManager = null, notificationService: injectedNotificationService = null, callCouncilMember = null, sendSMS = null, sendAlertSms = null, sendAlertCall = null, startAlertLoop = false, managedEnvService = null, } = {}
) {
  ...
  router.post('/access/bootstrap', requireKey, async (req, res) => {
    try {
      const result = await accessService.bootstrapAccess({
        ...
        expOktaPassword: req.body?.exp_okta_