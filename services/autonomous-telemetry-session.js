...
function usefulScoreFromOutcome({ success, action, auditResult }) {
  ...
  const cycleDefs = [
    ...
    },
    {
      ...
      run: async () => {
        ...
        const current = pf?.freshness?.overall === 'CURRENT';
        return {
          wall_clock_ms: Date.now() - t0,
          verification_latency_ms: Date.now() - t0,
          success: current,
          audit_result: pf?.freshness?.overall || 'UNKNOWN',
          proof_status_after: pf?.freshness?.overall || null,
          drift_detected: pf?.freshness?.stale === true,
          useful_work_score: current ? 0.4 : 0.1,
          useful_work_method: 'proof_freshness_read',
          metadata: { stale_count: pf?.freshness?.stale_count ?? null },
        };
      },
    },
    {
      ...
      run: async () => {
        ...
        const useful = usefulScoreFromOutcome({ success: result.audit_result === 'DRY_RUN', auditResult: result.audit_result });
        return {
          wall_clock_ms: Date.now() - t0,
          repair_latency_ms: Date.now() - t0,
          success: result.audit_result === 'DRY_RUN',
          audit_result: result.audit_result,
          stopped_reason: result.stopped_reason,
          proof_status_before: result.audit_before?.proof_freshness_overall,
          useful_work_score: useful.score,
          useful_work_method: useful.method,
          metadata: { steps_planned: result.steps_planned?.length || 0 },
        };
      },
    },
    {
