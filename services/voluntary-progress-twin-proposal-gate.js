...
export async function proposeTwinUpdate(pool, { user_id, subject, current_value = null, proposed_value, truth_grade = 'GUESS', confidence = null, evidence = null, rationale, proposed_by }) {
    ...
    const result = await pool.query(`
        INSERT INTO voluntary_progress_twin_proposals (user_id, subject, current_value, proposed_value, truth_grade, confidence, evidence, rationale, proposed_by, status)
...
export async function approveTwinUpdateProposal(pool, { proposal_id, approved_by, tenantId = 'default', twinKey, moduleKey = null }) {
    ...
    const proposalResult = await pool.query('SELECT * FROM voluntary_progress_twin_proposals WHERE id = $1', [proposal_id]);

    if (proposalResult.rows.length === 0 || proposalResult.rows[0].status !== 'pending') {
...
export async function rejectTwinUpdateProposal(pool, { proposal_id, rejected_by, reason = null }) {
    ...
    const updateResult = await pool.query(`
        UPDATE voluntary_progress_twin_proposals
        SET status = 'rejected', decided_at = now(), rationale = COALESCE(rationale, '') || $1
        WHERE id = $2
        RETURNING *
    `, [reason ? ` Rejected by ${rejected_by}: ${reason}` : ` Rejected by ${rejected_by}`, proposal_id]);
...
export async function getPendingProposals(pool, { user_id, limit = 50 } = {}) {
    ...
    const result = await pool.query(`
        SELECT *
        FROM voluntary_progress_twin_proposals
