...
- NAME: the card's name is the large title text at the very top of the card, above the mana cost. Pre-1994-frame cards print a SEPARATE type-line phrase lower on the card such as "Summon X", "Enchant X", "Interrupt", or "Instant" (e.g. "Summon Merfolk", "Enchant Creature") — that phrase describes the card's TYPE, it is NEVER the card's name. Never return type-line text as the name.
- SET: look for the small set symbol printed on the right side of the type line (same line as "Creature — X" or the old-style type phrase); its shape identifies the exact set and its color indicates rarity (black=common, silver=uncommon, gold=rare, orange/red=mythic). Cards from before Fallen Empires (1994) — Alpha, Beta, Unlimited, Arabian Nights, Antiquities, Legends, The Dark, Revised — have NO set symbol at all; for those, use border color (black border pre-8th-Edition, white border from 8th Edition/2003 onward, with Unlimited and Revised as white-border exceptions) and the copyright/artist line at the very bottom of the card (shows a year) as your identifying evidence instead, and reflect your uncertainty honestly in "confidence" rather than guessing a specific set with high confidence.
- CONDITION: base condition_guess on actually visible wear, not a default. near mint = no visible wear, sharp corners, no scratches. lightly played = minor edge wear or slight corner softening only, still looks good overall. moderately played = visible edge whitening, rounded/worn corners, light surface scratches. heavily played = heavy edge whitening, major corner wear, visible creases or scratches. damaged = creases, tears, water damage, writing, or tape. If wear is not clearly visible in the photo, use "unknown" rather than guessing near mint.
