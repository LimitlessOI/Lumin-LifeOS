/**
 * SYNOPSIS: Exports recordProvenanceClaim — services/self-repair-provenance-ledger.js.
 */
import { execFileSync } from 'node:child_process';

export async function recordProvenanceClaim(pool, { claim, commit_sha }) {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS self_repair_provenance_ledger (
      id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      claim TEXT NOT NULL,
      commit_sha TEXT,
      is_ancestor_of_tip BOOLEAN,
      verified_at TIMESTAMPTZ,
      epistemic_grade TEXT NOT NULL DEFAULT 'GUESS',
      result TEXT,
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    )
  `);

  const result = await pool.query(
    `INSERT INTO self_repair_provenance_ledger (claim, commit_sha, epistemic_grade, result)
     VALUES ($1, $2, $3, $4) RETURNING *`,
    [claim, commit_sha, 'GUESS', null]
  );

  return result.rows[0];
}

export async function verifyProvenanceClaim(pool, { claim_id, repoRoot }) {
  const { rows } = await pool.query(
    `SELECT commit_sha FROM self_repair_provenance_ledger WHERE id = $1`,
    [claim_id]
  );

  if (rows.length === 0) {
    throw new Error('Claim ID not found');
  }

  const commit_sha = rows[0].commit_sha;
  let is_ancestor_of_tip = false;
  let result = 'FALSE_CLAIM';

  try {
    execFileSync('git', ['merge-base', '--is-ancestor', commit_sha, 'HEAD'], { cwd: repoRoot });
    is_ancestor_of_tip = true;
    result = 'verified';
  } catch (error) {
    // Fail closed: if the git command errors, result remains 'FALSE_CLAIM'
  }

  const updatedResult = await pool.query(
    `UPDATE self_repair_provenance_ledger
     SET is_ancestor_of_tip = $1, verified_at = now(), epistemic_grade = $2, result = $3
     WHERE id = $4 RETURNING *`,
    [is_ancestor_of_tip, 'KNOW', result, claim_id]
  );

  return updatedResult.rows[0];
}

export async function getProvenanceLedger(pool, { limit = 50 } = {}) {
  const result = await pool.query(
    `SELECT * FROM self_repair_provenance_ledger ORDER BY created_at DESC LIMIT $1`,
    [limit]
  );

  return result.rows;
}