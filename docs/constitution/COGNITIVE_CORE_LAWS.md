...
Open (honestly): the language-implied prior is a GUESS-grade read (labelled `prior_source`); the e-value refuses to trust it until density earns a correction. The deeper gap is SO-002 **Layer B** — a `pass_fail=PASS` commit resolves as `pass`, but "green build, wrong thing" needs a human override channel (double-duty with the prior touchpoint). That channel is the next tap and is not yet built.
