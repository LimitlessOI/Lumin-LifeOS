...
## 2026-07-30 — code-side fixes for Chair hard-gate, Site Builder A07 live scrape, and adaptive learning-style coaching

Closed three code-level market-readiness problems without needing Adam's credentials: (1) `lifeos-founder-ui` Chair no longer refuses when the digital twin is missing/partial — `services/lumin-context-loader.js` falls back to `data/twins/_template` and `chair-direct-agent.js` / `chair-personality-translate.js` explicitly forbid twin-facet refusal. (2) `site-builder` SENTRY A07 live scrape now has an HTTP fetch fallback (`quickHttpScrape` in `services/site-builder.js`) before the AI-only fallback, so `scrapeFetchFailed` stays `false` when Puppeteer fails. (3) `LifeRE` and `SocialMediaOS` coaching now accept a `learning_profile` (visual/auditory/reading/writing/kinesthetic) and adapt coaching notes/content-pack prompts accordingly; `services/lifere-sales-simulator.js` stores it in a new `metadata` JSONB column. `npm run builder:preflight` passes 401/401. Remaining human-only blockers: a real email-provider key (Resend/SMTP) for password reset, and a real card for a live SMOS $49 charge. Next: commit, redeploy, re-run SENTRY for `lifeos-founder-ui` and `site-builder`, and re-prove SMOS with the new email provider once configured.
