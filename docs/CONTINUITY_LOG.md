...
## 2026-07-30 — Critical tip sweep: auth trio still open + billing amplification

Daily critical-bug automation re-read tip and found the same auth security holes reported 2026-07-24..29 still live because draft PRs #367/#370/#371/#372 never merged. Concrete triggers: (1) member login JWT passes `requireKey` → operator/builder/Railway routes and today's new `POST /api/v1/lifeos/auth/billing/operator-mark-paid` free-tier grant; (2) unauthenticated `POST /set-password` with `{handle,newPassword}` sets password on passwordless accounts; (3) any client sending `x-command-key` (even wrong value) + `return_token:true` on forgot-password gets raw reset token. Also hardened LifeOS billing `/verify` to require `metadata.source=lifeos_billing`, allowlisted tier, `payment_status=paid`, and a matching `lifeos_checkout_sessions` row. Re-fixed on `cursor/critical-bug-investigation-6e44` with regression tests. Remaining known open (not fixed this sweep): Site Builder Stripe product binding (#368), x-forwarded-host checkout URLs, MarketingOS owner_id IDOR, voice-rail body.user IDOR, founder-build Promise.race false FAIL.
