...
## 2026-07-07 — MarketingOS session contract critical bugfix

Deep bug sweep found the latest loop-built MarketingOS Phase 1 route/UI mounted but remained unusable against the corrected §6 schema: consent used an invalid enum, extraction/generation/approval paths referenced non-existent child-table `owner_id` / `source_extraction_id` columns, generation used generic platform/format enums rejected by the database, and the UI expected raw arrays/session objects instead of `{ session }` / `{ pieces }`. Fixed `routes/marketing-session-routes.js` and `routes/marketing-session-ui-routes.js` to match `20260708_marketing_phase1_schema_v2.sql`, added schema-aware end-to-end coverage in `tests/marketing-session-routes-contract.test.js`, wired it into `npm test`, and recorded the MarketingOS product-home receipt. Validation: targeted `node --check`, targeted `node --test`, and full `npm test` pass; `npm run builder:preflight` remains blocked by an unrelated `TRUTH_ENFORCEMENT_AUDIT` direct Anthropic fetch in `services/never-stop-product-factory.js`.
