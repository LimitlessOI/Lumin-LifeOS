...
## 2026-07-12 — Wellness therapist route auth/consent bug sweep

Deep bug sweep found the newly auto-registered Wellness therapist routes were wired against `deps.requireAuth`, while founder-lane auto-registration passes `requireKey`/`requireUserOrKey`; normal account JWTs were not decoded into `req.lifeosUser`, making the new API effectively anonymous/401. The same route also had a permissive brief-share fallback that would mark arbitrary request-body briefs as shared if the route were mounted with an upstream auth injector, bypassing the intended therapist_share consent gate. Fixed `routes/wellness-therapist-routes.js` to consume `requireAuth` or `requireKey`, derive actors from `req.lifeosUser`, require therapist/admin roles for link/brief/share actions, fail closed on consent/link checks, persist session briefs, and share only stored briefs. Added `tests/wellness-therapist-routes-security.test.js` and wired it into `npm test`. Remaining known dependency: `therapist_share` needs a real consent grant/revoke surface before therapist exports can be user-facing.
