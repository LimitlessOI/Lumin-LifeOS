...
## 2026-06-30 — LifeOS auth password-bootstrap hardening

Bug-sweep automation found a concrete account-takeover path in `POST /api/v1/lifeos/auth/set-password`: the route was public and `setAdminPassword()` allowed password creation when `password_hash` was null, so an unauthenticated caller could claim any passwordless handle. Fixed locally by requiring same-handle account JWT or operator key at the route, making passwordless bootstrap operator-only in `services/lifeos-auth.js`, and adding `tests/lifeos-auth-password-security.test.js` for unauthenticated, cross-handle, passwordless, self-service, and operator-bootstrap cases. Targeted regression passes. Full `npm run builder:preflight` is still blocked by the pre-existing `tests/lumin-conversation-routing.test.js` oil-change routing assertion (`actual=lumin`, expected `chair`), unrelated to this auth patch. Next: commit, push `cursor/critical-bug-investigation-abd2`, and open the bug-fix PR.
