...
## [FIX] 2026-07-10 — TC billing route auth/webhook fail-closed patch

Deep bug sweep found a critical regression from the 2026-07-10 TC step-09 auto-registration: `routes/tcBillingRoutes.mjs` was mounted live while `GET /api/tc/billing/status/:agentId` had no auth middleware and `POST /api/tc/billing/webhook` accepted arbitrary JSON, called AI before any Stripe verification, and inserted into a webhook table with incompatible deployed schema variants. **Fix:** status now requires the injected `requireKey`; webhook now requires `stripe-signature` and an injected `validateWebhookSignature`, fails closed with 503 when verifier wiring is absent, never calls AI for webhook parsing, and persists only verified events against either known `stripe_webhook_events` shape. **Regression:** `tests/tc-billing-routes-security.test.js` covers unauth status denial, verified status success, unavailable/invalid webhook verifier side-effect blocking, and verified event persistence; wired into `npm test`. **Validation:** targeted test pending after patch; preflight baseline before patch reached project checks but still fails the known unrelated `tests/lumin-conversation-routing.test.js` assertion (`'lumin' !== 'chair'` on “oil change is conversation not display`).
