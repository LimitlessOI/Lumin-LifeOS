...
| 2026-06-10 | Factory execute-step sandbox traversal hotfix: `factory-staging/factory-core/builder/run-step.js`, canonical `FACTORY-REBOOT-0029/CONTENT/run-step.js`, and standalone `lumin-factory/factory-staging/.../run-step.js` now resolve repo-relative paths before sandbox checks, reject source paths outside the repo, and verify byte-exact SHA before writing. Regression added to execute-step integration; mission hash/acceptance pins updated. | Cron critical-bug audit reproduced a `../` target writing `/workspace/path-traversal-proof.txt` outside the declared sandbox; fix prevents arbitrary out-of-sandbox writes/data loss. | ✅ Targeted repro blocked (`422/BLOCKED_RETURN_TO_BPB`, no file), `npm run factory:acceptance`, generated-bundle `npm run factory:ci` 16/16, main + standalone staging self-checks PASS |
