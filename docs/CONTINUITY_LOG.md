...
| 2026-06-11 | Critical LifeOS migration boot blocker fixed on branch `cursor/critical-bug-investigation-f946`: `db/migrations/20260427_lifeos_decision_review_queue.sql` and `db/migrations/20260427_lifeos_assessment_battery.sql` now reference canonical `lifeos_users(id)` at their original April alphabetical positions, and the decision queue no longer has a `decision_logs` FK. Root cause: the later May repair migration was correct but unreachable on a clean/partially-applied DB because `startup/database.js` aborts at the first failed migration. Builder-first was honored via `npm run builder:preflight` and two non-committing `/builder/task` calls (model `deepseek`); `/execute` was skipped for branch-safety. | Prevent Railway/fresh-DB boot crash before May repair migrations can run. | ✅ static migration-contract check PASS; `npm test` PASS (49 pass, 0 fail, 4 skipped); no active bad FK refs remain. Local VM had no `psql`/Docker, so clean Postgres replay remains unrun. |
