...
## [FIX] 2026-07-03 — Founder chat timeout no longer masks background execution

Deep bug sweep of the 2026-07-02 founder chat hang fix found a real correctness regression: `routes/lifeos-builderos-command-control-routes.js` raced the whole `runLuminChairTurn()` against `CHAIR_TURN_BUDGET_MS` and returned `NO_COMMAND_RAN` / “nothing was committed” on timeout, but the uncancelled promise could continue in the background and still dispatch builder jobs, terminal bridge, blueprint/Point B execution, system actions, or SMOS work. **Fix:** added `shouldUseFounderChairTurnTimeout()` and now only applies the 42s fallback to non-mutating conversational/display turns; execution-capable turns await their real receipt. **Regression:** `tests/lumin-chair-orchestrator.test.js` covers conversational/display timeout allowed and build/mission/system-action/SMOS timeout disallowed. **Next:** deploy after merge; if a future mutating path needs a hard timeout, it must create a durable job/correlation id before returning `RUNNING`/pollable truth, not `NO_COMMAND_RAN`.
