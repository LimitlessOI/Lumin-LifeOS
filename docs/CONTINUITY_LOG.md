...
| 2026-06-18 | Critical bug-hunt fixes landed on `cursor/critical-bug-investigation-5ae5`: kernel-managed `/builder/build` completion authority is enforced again instead of permanently deferred; Action Inbox state mutations are user-scoped; Voice Rail natural-language off-record/private text is treated as private before persistence. Targeted tests added/updated for builder completion and Action Inbox ownership. | Daily high-severity audit found concrete fail-open completion authority, cross-user mutation, and private-retention triggers. | ✅ |
