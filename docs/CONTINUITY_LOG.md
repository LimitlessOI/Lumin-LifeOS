...
## [FIX] 2026-07-04 — Critical memory capsule migration schema conflict repaired

Daily critical bug sweep found a concrete full-runtime boot/apply blocker in the newest memory-system system-build commits. `db/migrations/20260704_create_memory_capsules.sql` attempted to define a second incompatible `memory_capsules(id, owner_id, signal_data, trust_level INTEGER)` table even though the canonical May capsule schema is `memory_capsules(capsule_id, fact_id, trust_level TEXT, retrieval_permission, truth_class, ...)`. On any DB that already has the canonical table, `CREATE TABLE IF NOT EXISTS` no-ops and the following `CREATE INDEX ... owner_id` fails because `owner_id` does not exist; `startup/database.js` rethrows migration errors, so full-runtime route mounting can be aborted on restart. Repaired the migration to assert the canonical `capsule_id` schema and add only safe canonical indexes (`created_at`, `fact_id`, `trust_level`); updated `scripts/verify-memory-system.mjs` with an importable migration contract validator; added `tests/memory-migration-contract.test.js` and wired it into `npm test`; updated the memory-system product home and manifest. Verification: targeted migration contract PASS, `npm test` PASS (258 pass / 4 skipped / 0 fail), product-home/SSOT checks PASS. `npm run builder:preflight` still exits 1 on the pre-existing LifeOS routing assertion (`oil change` expected `chair`, actual `lumin`), unrelated to this memory migration fix.
