...
Adam is right: the chat was useless because every founder-facing call defaulted to Anthropic `claude_sonnet`, and Anthropic returned `credit balance is too low` with no failover. I changed `services/chair-direct-agent.js` to default to `openai_gpt` and cascade through `deepseek`, `gemini_flash`, `claude_sonnet`; I also changed `services/council-prompt-adapter.js` (which backs `createLifeOSChatRoutes` and `POST /api/v1/lifeos/chat/threads/:id/messages`) to do the same. I then fixed the chat's honesty: `services/chair-native-facts.js` now sets `runtimeStatusTurn` for runtime status questions, adds a `live_builder_status.summary` with exact counts, and clears `point_b_status`, `system_knowledge`, `program_context`, `point_b_target`, `strategic_brief`, and `memory_context` so the model cannot answer from stale Point-B product-home text or speculative mission summaries. `services/chair-direct-agent.js` SYSTEM_PROMPT tells the model to start from `live_builder_status.summary`, quote exact numbers, ignore stale conversation, and never invent a `node builderos-reboot/scripts/...` command. I temporarily set `GOVERNED_AUTONOMOUS_SHIP=0` via `POST /api/v1/railway/managed-env/bulk` to stop the commit race while I pushed; I will re-enable after the final deploy. All gates pass (`builder:preflight`, `verify:ci`, `lifeos:bp-priority:verify`, `factory:ci`). Next: commit, push, redeploy, re-enable the governed loop, and verify the live chat gives a factual builder-status answer.
