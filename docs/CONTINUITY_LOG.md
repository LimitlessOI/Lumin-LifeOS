...
## 2026-07-21 — Council cache fence regression fixed

Deep recent-commit sweep found a live paid-content regression introduced in `76269d96f`: `services/council-service.js` changed the closing-fence cache cleaner from zero-or-more trailing whitespace (`\s*$`) to exactly one (`\s$`). Normal fenced JSON ending directly at `` ``` `` was cached with the closing fence left behind; a repeated MarketingOS Generate/Extract request then hit invalid cached JSON and silently emitted transcript-echo fallback pieces. Fixed by centralizing `cleanCouncilResponseForCache()` with the correct EOF-tolerant regex and applying it to all provider cache writes; added `tests/council-cache-cleaning.test.js` and wired it into `npm test`. Validation: focused 2/2, full `npm test`, syntax check, and BuilderOS preflight pass. Branch commit `1cc4ec9a4`; deployment/tip verification pending.
