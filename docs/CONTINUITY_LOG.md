...
## 2026-07-24 — Critical auth regressions restored (requireKey + set-password + tip reset token)

Daily critical-bug sweep found three live account-takeover / privilege-escalation holes that had been fixed earlier then regressed: (1) `requireKey` accepted any valid member JWT as operator; (2) `POST /api/v1/lifeos/auth/set-password` was public and would set a password when `password_hash` was NULL; (3) `forgot-password` returned `tip_reset_token` when any non-empty `x-command-key` header was present. Restored role-gated JWT fallback, authenticated set-password with operator-only bootstrap, and verified-key tip-token disclosure. Regression tests restored under `tests/require-key-operator-jwt.test.js` and `tests/lifeos-auth-password-security.test.js`.

## 2026-07-24 — ChatGPT-style resizable three-pane layout shipped in Lumin drawer

Adam: chat should look and work like ChatGPT/Cursor with a left sidebar of continuous-history jump-cuts, a right report/artifact panel, a composer that can sit top or bottom, resizable panes, mobile horizontal swipe with vertical scroll inside each pane, and a mode-aware LifeRE layout (goals/progress bar on top, conversation in the middle, composer on the bottom). GAP-FILL: `public/overlay/lifeos-app.html` restructured into a three-pane shell (`lumin-left` jump-cut segment list, `lumin-center` conversation with composer and optional top panel, `lumin-right` artifact panel). Added header buttons for left/right panels, composer dock toggle, and mode toggle. Added resizable splitters with mouse/touch drag and localStorage persistence. Added mobile CSS scroll-snap and swipe navigation. Segments are currently auto-titled heuristically (first user message snippet) grouped by 10-minute gaps; true topic naming by model is queued as a follow-up. The LifeRE top panel is a placeholder with a progress bar and goals area until the Chair starts returning structured goal data. `npm run builder:preflight` passed 396/396 after sourcing `.env`. Committed `333c8e7` and redeployed to `https://lumin-web-production-e3a9.up.railway.app`. SENTRY `lifeos-founder-ui` pre-alpha gate: initial run failed because Playwright Chromium was not installed on the VM; `npx playwright install chromium` fixed it, and the gate then PASSed all Layer A + Layer B real-browser checks with 0 findings. Next: prosody/tonal-intelligence pipeline and client feedback clustering.
...
## 2026-07-23 — Tonal reflection principle added to chair prompts

Adam clarified that the system must never mirror a pissed-off or pissy mood. It should identify the tone plainly, ask permission to talk about it, then stay calm, steady, and supportive — reflect, don't react. Updated `services/chair-direct-agent.js`, `services/chair-personality-translate.js`, `services/response-variety.js`, and `docs/constitution/LUMIN_COMMUNICATION_DNA.md` to bake this into the chair's "hat" for every human conversation.
