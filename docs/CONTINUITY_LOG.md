...
## 2026-07-20 — Cognitive Core founder-data authorization boundary

Deep recent-commit audit found the newly mounted Cognitive Core accepted every valid LifeOS account JWT, but its handlers read `req.user` while the auth middleware sets `req.lifeosUser`; ordinary members therefore fell back to founder user `1` and could read or mutate Adam's private journal, outcomes, calibration, and sealed judgment packages. Fixed at the route boundary: account access now requires `founder_admin`, `admin`, or `operator`, while command-key automation remains supported. Added `tests/cognitive-core-routes-security.test.js` and wired it into `npm test`. Validation: focused 3/3, full `npm test`, BuilderOS preflight, product-home verifier, SSOT checks, and syntax pass. Commit `50dcccd48`.
