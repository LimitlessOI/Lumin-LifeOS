...
## 2026-07-06 — Async founder build timeout truth fix

High-severity bug sweep found that `startFounderBuildJob()` used `Promise.race` to mark a background founder build as terminal FAIL after `FOUNDER_BUILD_JOB_TIMEOUT_MS` even though the uncancelled `runFounderBuildWithSelfRepair()` promise could keep running and later commit/deploy. Fixed `services/founder-build-self-repair.js` so the timeout only appends a "still working" progress step; terminal job status now comes only from the real build receipt. Added regression coverage in `tests/founder-build-job-store.test.js`. Verification: `node --check services/founder-build-self-repair.js tests/founder-build-job-store.test.js`, `node --test tests/founder-build-job-store.test.js tests/founder-build-result-truth.test.js`, and `npm test` all pass. `npm run builder:preflight` still fails on the pre-existing LifeOS `tests/lumin-conversation-routing.test.js` `lumin` vs `chair` assertion; changed modules pass the preflight import sweep.
