...
## 2026-07-31 — Critical tip sweep: auth trio + billing amplification still open (7th rediscovery)

Daily critical-bug automation re-read tip and found the same LifeOS auth/money holes still live because draft PRs #367/#370/#371/#372/#373 never merged. Concrete triggers unchanged: (1) member login JWT passes `requireKey` → operator/builder/Railway routes and `POST /api/v1/lifeos/auth/billing/operator-mark-paid` free paid-tier grant; (2) unauthenticated `POST /set-password` with `{handle,newPassword}` sets password on passwordless accounts; (3) any client sending an `x-command-key` header (even wrong value) + `return_token:true` on forgot-password gets a raw reset token; (4) billing `/verify` upgrades tiers without `metadata.source=lifeos_billing` + tracked checkout row. Re-fixed on `cursor/critical-bug-investigation-a259` with the prior regression tests. Remaining known open (not fixed this sweep): Site Builder Stripe product binding (#368), x-forwarded-host checkout URLs, MarketingOS owner_id IDOR, voice-rail body.user IDOR, founder-build Promise.race false FAIL.
