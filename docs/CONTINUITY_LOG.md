...
## 2026-06-28 — Control-plane DONE gate pending-evidence repair

Daily critical-bug sweep found a recent autonomy-closure regression in `services/builderos-control-plane-service.js`: `recordBuildComplete()` ran `canMarkBuildDone()` before writing the completion row, but the gate only read the stale `build_task_ledger` row and ignored token/OIL/end-time evidence already in the completion payload. The gate now merges pending completion evidence before computing missing proof, `BUILDEROS_AUTONOMY_CLOSURE_V1_ACCEPTANCE.json` carries its introducing `git_sha`, and `RECEIPT_TRUTH_VALIDATION.json` reflects the repaired PASS proof. Verification: focused founder/control-plane proof suite PASS (26/26), `node --check services/builderos-control-plane-service.js`, and `npm run builder:preflight` PASS.
