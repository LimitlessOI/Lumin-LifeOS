...
## 2026-07-29 — S00 spend→outcomes (hard gate before more money)

Adam: do not add money until we track production from spend — "$20 → X,Y,Z", ETA+$ from history, efficiency trend, refuse when blind. Extended BuilderOS control plane + kernel linker: `GET /api/v1/builderos/control-plane/spend-outcomes`, `POST .../estimate`, `npm run spend:outcomes`, linkage on health, `SPEND_OUTCOME_STRICT=1`. Still partial until live builds stamp request_id and linkage_rate rises. Next: turn on strict after linkage proven; stamp council calls with task_id.
