...
also one of those laws is other than talking to a human we use the compression tsos

**Assistant:**

Adding an explicit law: non–human-facing communication uses TSOS compression and the machine channel (`TSOS_SYSTEM_LANGUAGE.md`). Updating the contract, read-sequence prompts, and the generated compact rules if the token budget allows.
...
we probalby should store this all in our condenced tsos what do you think i like your ideas as well

**Assistant:**

Checking how the condensed TSOS packet is built so we add this in the right place (generated vs hand-edited).
