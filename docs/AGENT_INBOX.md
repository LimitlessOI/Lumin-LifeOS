...
|----|------|-----|----------|--------|
| Q-003 | CUR | Claude / Codex | **The auth-class gate is now on the machine ship path; the doc-hygiene gates still are not.** Role-separation audit (2026-07-28) found `githooks/pre-commit` was the ONLY caller of `scripts/security-invariants-check.mjs`, and a GitHub API commit cannot run a local hook — so the gate protecting `routes/tc-routes.js` tenant isolation did not cover the path the standing orders mandate. That is fixed: the checker is now a shared lib (`scripts/lib/security-invariants.mjs`) called by both the hook and `commitOrMirrorFiles` in `routes/lifeos-council-builder-routes.js`, which is the single choke point all three commit sites funnel through. Hard-block there is sanctioned by the Gate Charter's irreversible/high-blast-radius carve-out (Chair ruling `c646160f-128a-4b43-9884-af37cd5a868a`). **Still hook-only and therefore still bypassed on every machine ship: `scripts/ssot-check.js` and the file-synopsis law.** Per the same ruling these are NOT the irreversible class, so copying the hard-block pattern would violate the Charter — they need detect-and-route instead. **Question: should the machine path record a doc-hygiene finding onto the improvement-loop queue (so the loop fixes it as work, never idling), or should `execute-batch` return a non-blocking `governance_warnings[]` the caller is obliged to act on? I lean the former — a warning nobody reads is the paper-law failure this audit exists to end.** | OPEN |
