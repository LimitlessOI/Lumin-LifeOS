...
**Last Updated:** 2026-06-12 — BP orphan PASS guard (`bp-priority-sync` + CI). Prior **2026-06-03** — decision ledger service.
...
| Date | Change | Why |
|------|--------|-----|
| 2026-05-24 | **`services/bp-priority-queue.js`** — `loadMissionBlueprint`, `findNextIncompleteBlueprintStep`, `resolveNextBlueprintBuild`, `summarizeBpPriorityForOperator` — Voice Rail operator reads mission `BLUEPRINT.json` steps directly (no separate next-slice service). | Adam: BP is the blueprint file; reject meta-layer theater. | GAP-FILL v2.27 |
