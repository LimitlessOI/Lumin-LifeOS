...
**Last Updated:** 2026-06-14 — kernel-managed `/builder/build` defers only the route-level DONE gate to kernel sequencing; completion authority is route-enforced again before any committed success response. Prior **2026-06-13** — kernel-managed `/builder/build` deferred route-level DONE/completion checks to kernel sequencing.
...
| Date | Change | Why |
|------|--------|-----|
| 2026-06-14 | **Completion authority restored for kernel-managed `/builder/build`:** `routes/lifeos-council-builder-routes.js` still lets the TSOS kernel own DONE-gate proof ordering, but `evaluateBuildCompletionForBuildResponse()` no longer has a `kernelManaged` bypass and `/build` no longer returns `completion_deferred_to_kernel`. Kernel-wrapped committed builds must now receive a real completion grant/receipt or fail closed with 409. | The 2026-06-13 sequencing repair solved a real `build_end_time` ordering bug but accidentally delegated completion authority to a kernel that only records build/DONE proof and never calls `grantBuildCompletion()`. Since `/build` is always mounted through `platformKernel.wrapBuild`, direct automations could treat wrong-outcome commits as success via `ok && committed`. |
