...
**Last Updated:** 2026-06-18 — kernel-managed `/builder/build` still defers only DONE proof sequencing to the kernel; completion authority is enforced before any success response. Prior **2026-06-13** — kernel-managed `/builder/build` deferred route-level DONE/completion checks to kernel sequencing. Prior **2026-06-13** — `/build` DONE-gate enforced in `routes/lifeos-council-builder-routes.js` before success response. Prior **2026-06-13** — DONE gate helper extracted for `/build` wiring.
...
| Date | Change | Why |
|------|--------|-----|
| 2026-06-18 | **`routes/lifeos-council-builder-routes.js`** — supersedes the completion half of the 2026-06-13 kernel deferral: `evaluateBuildCompletionForBuildResponse()` now enforces `grantBuildCompletion()` even on kernel-managed `/builder/build` calls, using the generated output/commit message as immediate commit evidence. The route no longer returns `completion_deferred_to_kernel`; only DONE proof remains kernel-sequenced. **`tests/builderos-completion-authority.test.js`** now proves kernel-managed builds still get a completion receipt. | Critical audit found production `/build` could return `ok:true, committed:true` with `completion_granted:false` because completion was deferred to a kernel step that did not exist. |
