...
|---|---|---|---|---|---|
| 2026-05-24 | **`startup/register-runtime-routes.js`** — `express-rate-limit` deliberation limiter (60 req/min/IP) before `/api/v1/lifeos/deliberation`. **`startup/boot-domains.js`** — `bootDeliberationRepCatalog` logs FAILED (not skipped) + 2s retry on sync failure. | Claude live SENTRY L10/L12 — no throttle; silent empty REP catalog at boot. | AM19 wiring, AM48 feature | pending | pending Railway deploy |
