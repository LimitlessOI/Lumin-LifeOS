...
| GET | `/api/v1/sites/pipeline-report` | Operator pipeline totals for command center/reporting |
| POST | `/api/v1/sites/discover` | Command-key protected prospect discovery; executes the discovery script via argv-only `execFile` |
...
- **KNOW:** `POST /api/v1/sites/discover` no longer constructs a shell command from request JSON; request fields are bounded and passed to `scripts/site-builder-prospect-discovery.mjs` as argv values via `execFile` ✅
- **KNOW:** `tests/site-builder-discovery-routes.test.js` proves malicious `city` input cannot create a temp marker file through shell injection ✅
...
|---|---|---|---|---|
| 2026-05-11 | **Critical security GAP-FILL — `POST /api/v1/sites/discover` command execution hardened.** `routes/site-builder-discovery-routes.js` restored its `@ssot` tag, replaced shell-string `exec(command)` with `execFile(process.execPath, args)`, added bounded string/count validation, and preserves the existing response shape. `tests/site-builder-discovery-routes.test.js` added a regression case whose malicious `city` payload would create a temp marker file if shell interpolation returned. `package.json` now runs the new regression under `npm test`. Builder-first evidence: `npm run builder:preflight` passed; `POST /api/v1/lifeos/builder/build` was attempted for `routes/site-builder-discovery-routes.js` on branch `cursor/critical-bug-inspection-11a6` and failed with HTTP 500 / `Council call failed` / dispatch `HTTP 413`; this direct patch is a GAP-FILL for the blocked builder route. | Recent route mount made `/api/v1/sites/discover` live; the route built `node ... --city="${city}"` from request body fields. Any caller with the command key could inject shell syntax through `city`/`niche`, escalating a discovery request into arbitrary shell execution on the server. | ✅ | `node --check routes/site-builder-discovery-routes.js`; `node --test tests/site-builder-discovery-routes.test.js`; `npm test` |
