...
|---|---|---|---|
| 2026-05-10 | **CRITICAL BOOT FIX — restored `routes/site-builder-routes.js` and `services/prospect-pipeline.js` after repeated builder truncation.** Recent system-build commits (`bb73d6d1` → `149795db` → `b4cc6cfe`) progressively replaced the real route module with partial launch-readiness/discovery snippets and finally left only 3 import lines. The file had no `@ssot` header and no `createSiteBuilderRoutes` named export. After restoring the route, the import chain exposed `services/prospect-pipeline.js` as also truncated at an unterminated SQL string on line 147. Restored both known-good ESM modules from `a3df15e7`, including `createSiteBuilderRoutes(app, ...)`, `/api/v1/sites/*` route mounting, static preview serving, build/prospect/dashboard/analyze/preview-view/email-reply endpoints, warm-lead Slack notification helper, and the complete `ProspectPipeline` class. | Concrete trigger: `server.js` line 95 statically imports `{ createSiteBuilderRoutes }` from the route module; that module imports `ProspectPipeline`. With the 3-line route file, Node ESM module linking fails before Express starts: `SyntaxError: The requested module './routes/site-builder-routes.js' does not provide an export named 'createSiteBuilderRoutes'`. With only the route restored, Node then fails on `services/prospect-pipeline.js:147` with `SyntaxError: Invalid or unexpected token`. Impact: full Railway boot crash / total API outage on any deploy that includes either truncated file. | Reproduced before fix with `node --input-type=module -e "import { createSiteBuilderRoutes } from './routes/site-builder-routes.js'; console.log(typeof createSiteBuilderRoutes)"` → exit 1 missing export, then after route restore → exit 1 prospect-pipeline syntax error. `npm run builder:preflight` PASS after installing local deps. Post-fix: `node --check routes/site-builder-routes.js`; `node --check services/prospect-pipeline.js`; import smoke prints `function`; `npm test` PASS (2 passed, 4 skipped because no local server); `npm run verify:site-builder` now has all syntax checks green and fails only known local env checks `SITE_BASE_URL` + `EMAIL_FROM` (31 passed, 2 failed). |
