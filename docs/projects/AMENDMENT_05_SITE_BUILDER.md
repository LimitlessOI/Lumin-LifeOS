...
|---|---|---|---|
| 2026-05-13 | **Test wiring — `package.json`:** `scripts.test` now includes `tests/site-builder-discovery-routes.test.js` alongside the existing smoke, insurance-card, Postmark, TC digest, operator-runtime, and deployment-service guard tests. | Ensures the command-injection regression runs under the normal `npm test` gate instead of remaining a manual-only test. Current state: code and tests are wired; verification is the next step. | Pending `npm test` |
| 2026-05-13 | **Security regression test — `tests/site-builder-discovery-routes.test.js`:** new Node test starts an in-process Express app, mounts `createDiscoveryRoutes`, sends a malicious `count` value that would create a marker file if interpreted by a shell, and asserts the marker file is never created. | Locks the `/api/v1/sites/discover` command-injection fix so a future change back to `exec(command)` fails locally/CI without touching live Google Places or external services. Current state: test file exists and is tagged to AM05; next file change wires it into `package.json` `scripts.test`. | Pending package test wiring; direct test run follows |
| 2026-05-13 | **Security GAP-FILL — `routes/site-builder-discovery-routes.js`:** added missing `@ssot` header; replaced `child_process.exec(command)` shell-string invocation with `execFile(process.execPath, args)` using discrete argv entries for `city`, `niche`, and `count`; removed command string logging; added a 30s child-process timeout; changed malformed non-array/non-JSON stdout from silent `ok:true`/empty prospects to `502` with `ok:false`. **Builder evidence:** `npm run builder:preflight` passed; `POST /api/v1/lifeos/builder/build` (`target_file: routes/site-builder-discovery-routes.js`, branch `cursor/critical-correctness-bugs-dbd5`) returned HTTP 500 with `detail:"HTTP 413"`, so this local supervisor patch is marked GAP-FILL. | Recent builder commit `6cd63cd4` introduced shell interpolation for `/api/v1/sites/discover`; any authenticated caller with the command key could inject shell metacharacters through `city`, `niche`, or unquoted `count` and execute arbitrary commands as the Railway Node process user. The route must preserve discovery behavior without invoking a shell. Current state: route syntax is patched; regression test + package test wiring are the immediate next file changes. | Pending immediate test addition; run `node --check routes/site-builder-discovery-routes.js` and `npm test` after test file/package update |
...
|---|---|---|---|---|
| 2026-05-13 | **Test wiring — `package.json`:** `npm test` now runs `tests/site-builder-discovery-routes.test.js`. | Makes the Site Builder discovery security regression part of the standard test gate. Next: run syntax checks and full `npm test`, then mark this receipt done or document failures. | ⚠️ IN PROGRESS | Pending `npm test` |
| 2026-05-13 | **Security regression test — `tests/site-builder-discovery-routes.test.js`:** in-process Express regression for `/api/v1/sites/discover`; malicious `count` shell payload must remain inert argv data and must not create a temp marker file. | Prevents future regressions from reintroducing shell-string execution in the discovery endpoint. Next: wire this file into `package.json` `scripts.test`. | ⚠️ IN PROGRESS | Pending package test wiring; direct test run follows |
| 2026-05-13 | **Security GAP-FILL — `routes/site-builder-discovery-routes.js`:** `/api/v1/sites/discover` now calls `scripts/site-builder-prospect-discovery.mjs` with `execFile(process.execPath, args)` instead of shell-building `node ... --city="${city}" --type="${niche}" --count=${count}`. Malformed discovery stdout now returns `502` instead of a false successful zero-prospect response. | Fixes authenticated command injection from request body fields. Builder was tried first and failed with HTTP 500 / `HTTP 413`, so local supervisor patch is allowed as GAP-FILL. Next: add regression test and wire it into `npm test`. | ⚠️ IN PROGRESS | Pending immediate test addition; route syntax/test verification to follow |
