...
<!--
  SPECIFICATION: Cross-Surface Discovery (Deferred Implementation)

  This section outlines future capabilities for enhancing user discovery and navigation
  across the LifeOS Dashboard and Lumin chat interface. Implementation is deferred.

  1.  **Jumping from Dashboard to Lumin with Context Hints:**
      -   Users should be able to initiate a jump from specific dashboard elements (e.g., an MIT, a goal, an event) directly into a Lumin chat thread.
      -   When jumping, Lumin should receive context hints about the originating dashboard entity (e.g., MIT ID, goal name, event details).
      -   Lumin should use these hints to proactively offer relevant actions, information, or conversation starters related to the context.
      -   Example: Clicking an MIT on the dashboard could open Lumin with a prompt like "You clicked MIT 'Finish Q3 Report'. How can I help you with this?"

  2.  **Command Palette Feasibility:**
      -   Explore the feasibility of a global command palette (e.g., activated by Cmd/Ctrl+K) accessible from both the Dashboard and Lumin.
      -   The command palette should allow quick access to common actions, navigation, and entity search.
      -   Actions could include "Add MIT", "Create Goal", "Start new chat", "Jump to Dashboard", "Jump to Lumin".

  3.  **Searchable Entities List (MITs/Goals/Events):**
      -   Implement a unified search mechanism that allows users to find specific MITs, goals, and calendar events.
      -   This search should be accessible from a central point (e.g., the command palette or a dedicated search bar).
      -   Search results should provide direct links to view or interact with the found entity, potentially jumping to Lumin with context.
-->
