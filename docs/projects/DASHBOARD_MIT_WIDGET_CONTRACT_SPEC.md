...
.mit-item.overdue .mit-text {
  color: var(--c-conflict); /* Red color for overdue */
}
.mit-meta {
  font-size: 10px;
  font-weight: 600;
...
.mit-meta.overdue-label {
  color: var(--c-conflict);
}
.mit-meta.recurring-label {
  color: var(--text-secondary);
}
/ ── Quick add ── /
.quick-add {
...
$('mits-list').innerHTML = mits.map(m=> {
  const isKept = m.kept_at;
  const hasDueDate = m.due_at && m.due_at !== '';
  const dueDate = hasDueDate ? new Date(m.due_at) : null;
  const isOverdue = !isKept && hasDueDate && dueDate < now;
  const isRecurring = m.recurrence && m.recurrence !== '';

  let metaHtml = '';
  if (isOverdue) {
    metaHtml += '<div class="mit-meta overdue-label">Overdue</div>';
  }
  if (isRecurring) {
    metaHtml += '<div class="mit-meta recurring-label">Recurring</div>';
  }

  return `
    <div class="mit-item ${isKept ? 'done' : ''} ${isOverdue ? 'overdue' : ''} ${isRecurring ? 'recurring' : ''}"
         data-id="${m.id}"
         data-desc="${(m.description || m.text || '').replace(/"/g, '&quot;')}"
         data-recurrence="${m.recurrence || ''}">
      <div class="mit-check ${isKept ? 'done' : ''}">
      ...
      <div class="mit-content">
        <div class="mit-text ${isKept ? 'done' : ''}">${m.text || m.title || 'Untitled'}</div>
        ${metaHtml}
      </div>
    </div>`;
}).join('');
...
el.classList.toggle('done',!done); // Toggle done class on parent mit-item as well
el.classList.remove('overdue'); // If marked done, it's no longer overdue
try {
