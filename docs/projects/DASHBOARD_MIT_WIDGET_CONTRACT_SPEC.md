...
.mit-widget-item {
    ...
    user-select: none;
    -webkit-user-select: none;
    position: relative; /* For swipe interaction */
}
.mit-widget-item:last-of-type {
    border-bottom: none;
}
.mit-widget-check {
    flex-shrink: 0;
...
.mit-widget-check.done {
    background: var(--c-today);
    border-color: var(--c-today);
}
.mit-widget-check svg {
    display: none;
}
.mit-widget-check.done svg {
    display: block;
}
.mit-widget-check.done svg polyline {
    stroke-dasharray: 20;
    stroke-dashoffset: 0;
    animation: check-draw 0.25s ease forwards;
}
...
.mit-widget-text.done {
    color: var(--text-muted);
    text-decoration: line-through;
}

/* MIT Widget States */
.mit-widget-item--overdue .mit-widget-text {
    color: var(--c-conflict); /* Red for overdue */
    font-weight: 600;
}
.mit-widget-item--overdue .mit-widget-check {
    border-color: var(--c-conflict);
}
.mit-widget-item--recurring .mit-widget-text {
    font-style: italic;
    color: var(--c-mirror); /* Purple for recurring */
}
.mit-widget-item--recurring .mit-widget-check {
    border-color: var(--c-mirror);
}
.mit-widget-item--recurring::after {
    content: '↻'; /* Recurring icon */
    font-size: 12px;
    color: var(--c-mirror);
    margin-left: 8px;
    align-self: center;
...
<!-- NEW WIDGETS ROW -->
<div class="two-col mb-4">
<div id="lifeos-widget-mit" class="dashboard-widget card accent-border-today fade-up delay-3">
    <div class="card-label">Most Important Tasks</div>
    <div id="mit-widget-list">
        <!-- Initial skeleton state for the widget, to be replaced by lifeos-widget-mit.js -->
        <div class="skel-line skeleton w-full"></div>
        <div class="skel-line skeleton w-4/5"></div>
        <div class="skel-line skeleton w-3/5"></div>
    </div>
    <div class="quick-add">
        <input type="text" id="mit-widget-input" placeholder="Add a most important task…">
        <button class="btn-add" id="mit-widget-add-btn">Add</button>
    </div>
</div>
