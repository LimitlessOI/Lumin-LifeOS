ASSUMPTIONS:
- The `/api/v1/lifeos/commitments` endpoint can return `due_date` (ISO string) and `is_recurring` (boolean) fields for each commitment.
- "Swipe vs checkbox" implies the checkbox remains the primary completion mechanism, and no swipe gesture implementation is required for this task.
- "Voice-free MIT entry" refers to the existing text input, requiring no changes.
- Corrected existing syntax errors: `2Math.PIr` to `2*Math.PI*r` and `2 60 1000` to `2 * 60 * 1000`.
