The task asks for a "Specification only" and "no HTML rewrite", but the output contract demands "Generate the complete implementation code" and "Your ENTIRE response must be a complete HTML document" (implying a modified version of the provided HTML). These instructions are contradictory.
