...
| **Next build (product)** | **Overnight dashboard queue** — `npm run builder:preflight`; `npm run lifeos:builder:supervise -- --overnight --overnight-max 2`; or `OVERNIGHT_MAX=2 npm run lifeos:builder:overnight`. Grade commits on `origin/main`; read truthful `docs/projects/DASHBOARD_SHELL_GAP_AUDIT.md`. Victory Vault live verify still outstanding if `/victories` 404 persists. |
...
| **Known gaps** | **Supervisor not yet proven live** — `npm run lifeos:builder:supervise` must be run against the deployed Railway runtime after this patch ships. **Dashboard not in sidebar nav** — dashboard.html is live but the sidebar link in `lifeos-app.html` needs verification. **§2.10 runtime depth** — automated closed-loop observability still mostly CI. **Commitment→execution desk** — specced in backlog, not built. **Ambient sense E2E** — migration applied but no smoke test confirming Neon rows. |
| **⚠️ IN PROGRESS:** | Deploy builder hardening, run supervisor smoke objectives, then verify dashboard sidebar nav and Victory Vault live route (`GET /api/v1/lifeos/victories`). |
...
|---|---|---|---|---|
| 2026-04-29 | **Builder overnight queue & supervisor chain** — `scripts/lifeos-builder-overnight.mjs`, `docs/projects/LIFEOS_DASHBOARD_OVERNIGHT_TASKS.json`, `npm run lifeos:builder:overnight`; `scripts/lifeos-builder-supervisor.mjs` supports `--overnight --overnight-max N`; `.gitignore` `data/builder-overnight-log.jsonl`. **GAP-FILL:** `docs/projects/DASHBOARD_SHELL_GAP_AUDIT.md` — truthful gap analysis (prior `[system-build]` commit claimed ENOENT on SSOT briefs despite server-side file injection — epistemic failure). Overnight task prompts hardened (“never claim missing injected docs”). **`LIFEOS_DASHBOARD_OVERNIGHT_QUEUE.md`** — machine-readable task pointer. | Adam: supervise builder overnight and prove reliability; correct misleading audit committed by council. | ✅ | Railway `OVERNIGHT_MAX=1` task succeeded; truthful audit hand-fix | — `services/council-model-availability.js` now marks Ollama unavailable on Railway unless an explicit non-local endpoint exists, preventing memory/routing from selecting dead local models in production. **`routes/lifeos-council-builder-routes.js`** now treats explicit request `model` as authoritative on `/api/v1/lifeos/builder/build` and skips memory rerouting for that call. Added canonical dashboard supervision artifacts: **`docs/projects/LIFEOS_DASHBOARD_BUILDER_BRIEF.md`**, **`docs/projects/LIFEOS_DASHBOARD_OVERNIGHT_QUEUE.md`**, **`scripts/lifeos-builder-supervisor.mjs`**, and `package.json` script **`lifeos:builder:supervise`**. The brief binds dashboard work to the approved mockup boards, requires desktop sidebar + mobile bottom tabs, and requires both light and dark modes. The supervisor script runs readiness checks plus two deterministic smoke objectives before unattended queue work. | Adam asked for a supervisor pass that makes the system safe to build all night: fix routing drift, tie the builder to SSOT and the image boards, and prove it can follow instructions before overnight dashboard work starts. | ✅ local `node --check`; live supervisor run pending deploy | pending Railway deploy + smoke |
