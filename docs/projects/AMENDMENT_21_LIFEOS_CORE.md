...
| **Next build (product)** | **System-build Lumin chat:** Root cause of the 14-byte HTML incident was **council-service codegen max output = 1500 tokens**; builder now requests scaled `maxOutputTokens` and adds HTML full-file instructions + fence strip + stricter validation. **Deploy** this slice, then rerun `npm run tsos:doctor`, `npm run builder:preflight`, and a large-file `/builder/task` or `/build` smoke (expect `max_output_tokens_requested` in JSON). If the provider still caps below full-file size, response will truncate — next platform step is chunked/delta generation, not silent commit. **E2E household invite verification** — admin creates invite → open in private window → register Sherry → confirm `lifeos_role` admin panel appears for adam only. **Ambient migration smoke test** — confirm `db/migrations/20260423_lifeos_ambient_snapshots.sql` applied on Neon; `GET /api/v1/lifeos/ambient/recent?user=` returns rows after enable in Settings. |
...
| **⚠️ IN PROGRESS:** | Deploy **maxOutputTokens** builder + council-service override; verify large HTML `/build` end-to-end. If outputs still hit model max, design **chunked or delta** builder path (backlog). |
