...
| **Bounded autonomy (~7h)** | **`npm run lifeos:builder:daemon:7h`** — **`--run-for-min 420`**, **`--interval-min 15`**, **`--overnight-max 12`**; bounded default **`OVERNIGHT_CURSOR_WRAP=1`** ( **`OVERNIGHT_CURSOR_WRAP=0`** to opt out ). **Throughput:** **`data/builder-overnight-last-run.json`** + **`daemon_run_limit_reached`** **`KNOW_session_*`** (commits vs wall clock — don’t confuse sleep with **`/build`**). **`BUILDER_DAEMON_SUPERVISE_MODE`**, **`BUILDER_DAEMON_CONSEQUENCE_LENS`**, **`BUILDER_SUPERVISOR_GAPS_*`** · **`BUILDER_OPERATOR_ENV.md`**. Queue **`LIFEOS_DASHBOARD_OVERNIGHT_TASKS.json`**; **`data/builder-daemon-log.jsonl`**. **`GET …/builder/gaps`**. |
...
|---|---|---|---|---|
| 2026-04-30 | **Throughput honesty + bounded queue soak:** **`scripts/lifeos-builder-overnight.mjs`** — writes **`data/builder-overnight-last-run.json`** (`build_commits`, `build_wall_ms_sum`, `runner_wall_ms`, `idle_slice`); idle path explains instant-exit **`queue_slice_empty`**. **`scripts/lifeos-builder-daemon.mjs`** — reads last-run → **`overnightThroughput`** + bounded **`daemon_bounded_session_idle_slice`** + **`daemon_run_limit_reached`** session totals (**`KNOW_session_*`**); **`runForMin>0`** defaults **`OVERNIGHT_CURSOR_WRAP=1`** when unset. **`package.json`** — **`daemon:7h`** **15 min** interval · **`--overnight-max 12`**. **`docs/BUILDER_OPERATOR_ENV.md`**. **`.gitignore`** **`builder-overnight-last-run.json`**. | Adam: **7 h wall clock burned ~30 min of real work** because overnight **idle sliced** once cursor drained — no honesty metric and no recycle default. §2.6 operator truth; §2.11 autonomy must not silently idle. | ✅ | `node --check` daemon + overnight |
