...
| **24/7 + bounded soak** | **Default posture:** **`npm run lifeos:builder:daemon`** = non-stop supervise → **`npm run lifeos:builder:queue`** (**legacy `overnight`**). **Bounded slice only:** **`lifeos:builder:daemon:7h`** — **`--run-for-min 420`**, **`--interval-min 15`**, **`--queue-max 12`** (legacy **`--overnight-max`**); **`BUILDER_QUEUE_CURSOR_WRAP`** / **`OVERNIGHT_CURSOR_WRAP`** bounded default **on** (**`0`** to opt out). **Throughput receipts:** **`data/builder-overnight-last-run.json`**, **`KNOW_session_*`**. Vars: **`BUILDER_DAEMON_SUPERVISE_MODE`**, **`BUILDER_DAEMON_CONSEQUENCE_LENS`**, **`BUILDER_SUPERVISOR_GAPS_*`** — **`BUILDER_OPERATOR_ENV.md`**. Backlog **`LIFEOS_DASHBOARD_OVERNIGHT_TASKS.json`** (**historic path name**); **`GET …/builder/gaps`**. |
