...
**Supreme law:** This section **implements** `docs/SSOT_NORTH_STAR.md` → **Article II §2.6 System Epistemic Oath**, **Article II §2.10**, **Article II §2.11 (code the system / gaps; the system programs amendments & projects; `GAP-FILL` on the platform only)**, **Article II §2.12 (technical decisions → AI Council + best-practice research; consensus / full debate if split; Conductor/Construction supervisor SSOT re-read and drift detection; non-derogable)**, and **Article II §2.14 (TSOS machine-channel lexicon: `docs/TSOS_SYSTEM_LANGUAGE.md` — machinery only; not §2.11b)** for the LifeOS lane and Adam-facing agents. It may add detail; it may **not** weaken §2.6, §2.10, §2.11, **§2.12**, or **§2.14**.
...
| Field | Value |
|-------|--------|
| **Platform (TSOS)** | **TokenSaverOS** — North Star **§2.11a**: the **builder is the meta-product**; **governed self-build** only with receipts. **Machine channel** (builder HTTP, probes, receipt-grade script lines): **§2.14** + **`docs/TSOS_SYSTEM_LANGUAGE.md`** only. **Session reporting** to the operator: **§2.11b** + Companion **§0.5G** (plain language: grade, why A vs B, residue risk) — *orthogonal* to §2.14. |
...
| **Next build (product)** | **System-build Lumin chat:** Railway deploy must expose `GET /api/v1/lifeos/builder/domains` (not 404); then operator runs `npm run lifeos:builder:build-chat` with `PUBLIC_BASE_URL` + `COMMAND_CENTER_KEY` (see `docs/BUILDER_OPERATOR_ENV.md`). **E2E household invite verification** — admin creates invite → open in private window → register Sherry → confirm `lifeos_role` admin panel appears for adam only. **Ambient migration smoke test** — confirm `db/migrations/20260423_lifeos_ambient_snapshots.sql` applied on Neon; `GET /api/v1/lifeos/ambient/recent?user=` returns rows after enable in Settings. **Conversational capture** — extend Lumin threads → structured memory / MIT / commitments with explicit user-visible receipts (no silent writes). |
