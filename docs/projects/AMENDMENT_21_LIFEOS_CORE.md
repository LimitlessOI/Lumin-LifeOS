...
| **Bounded autonomy (~7h)** | **`npm run lifeos:builder:daemon:7h`** — wall-clock **`--run-for-min 420`** ( **`BUILDER_DAEMON_RUN_FOR_MIN`** env); **`BUILDER_DAEMON_SUPERVISE_MODE=probe`** default saves tokens between cycles; **`BUILDER_DAEMON_CONSEQUENCE_LENS=1`** → **`--consequence-lens`** on each supervise leg; **`BUILDER_SUPERVISOR_GAPS_LIMIT`** / **`BUILDER_SUPERVISOR_GAPS_DOMAIN`** widen/filter gap RECEIPT rollup ( **`docs/BUILDER_OPERATOR_ENV.md`** ). Queue **`docs/projects/LIFEOS_DASHBOARD_OVERNIGHT_TASKS.json`**; logs **`data/builder-daemon-log.jsonl`** · state **`data/builder-daemon-state.json`**; **preflight first.** Failures surface in **`GET /api/v1/lifeos/builder/gaps`**. |
