...
| **Last Updated** | 2026-04-15 (Fixed LifeOS verifier/manifest mismatch; made core LifeOS routes required at startup; restored Today health API compatibility; unified LifeOS overlay auth/user-key bootstrap across family, growth, vision, coach, healing, and legacy surfaces) |
...
| 2026-04-15 | LifeOS core stabilization: `scripts/verify-project.mjs` now accepts `project` manifests, Amendment 21 manifest includes `project_id`, core LifeOS routes mount as required, `GET /api/v1/lifeos/health/latest` restored as compatibility alias, and LifeOS overlays now normalize `commandKey` / `lifeos_user` bootstrap across family, growth, vision, coach, healing, and legacy surfaces | Restore trust in the SSOT verification path and eliminate the first-pass runtime mismatches that blocked the core LifeOS daily loop from loading consistently | ✅ | pending |
