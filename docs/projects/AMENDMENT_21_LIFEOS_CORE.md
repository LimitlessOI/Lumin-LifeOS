...
| **Next build (product)** | **`dashboard-import-tokens`** (`LIFEOS_DASHBOARD_OVERNIGHT_TASKS.json` #3): `npm run lifeos:builder:overnight -- --task dashboard-import-tokens` — minimal `<link>` for **`lifeos-dashboard-tokens.css`** in **`lifeos-dashboard.html`**. Preflight ✅; **theme CSS** ✅ on **`main`/prod**. Then **`dashboard-density-commentary`** + **`dashboard-ai-rail-contract`** (docs). Optionally `OVERNIGHT_MAX=3 npm run lifeos:builder:overnight -- --start 2` after verifying import task. Victory Vault **`GET /api/v1/lifeos/victories`** still confirm if 404. |
...
| **Known gaps** | **Supervisor not yet proven live** — `npm run lifeos:builder:supervise` must be run against the deployed Railway runtime after this patch ships. **`/execute`** for `.js` — does not reuse `/build`'s **`node --check`** gate (`THINK`: lower consistency than codegen path); fix only if bad commits recur. **Dashboard not in sidebar nav** — dashboard.html is live but the sidebar link in `lifeos-app.html` needs verification. **§2.10 runtime depth** — automated closed-loop observability still mostly CI. **Commitment→execution desk** — specced in backlog, not built. **Ambient sense E2E** — migration applied but no smoke test confirming Neon rows. |
| **⚠️ IN PROGRESS:** | **Wire tokens into dashboard HTML** (**`dashboard-import-tokens`**). Run supervisor smoke **`npm run lifeos:builder:supervise`** on prod when feasible. Spot-check Victory Vault **`/api/v1/lifeos/victories`**; sidebar Dashboard link KNOW present (**`lifeos-app.html`** `nav-dashboard`). |
