...
| **Next build (product)** | **LifeOS personal dashboard** — `public/overlay/lifeos-dashboard.html` is live at `/overlay/lifeos-dashboard.html`. Needs to be reachable from the app shell navigation in `lifeos-app.html` (it's in PAGE_META but not confirmed linked in sidebar). **Verify Victory Vault routes** — `GET /api/v1/lifeos/victories` should return 200 now that deploy is complete. **E2E household invite verification** — admin creates invite → open in private window → register Sherry → confirm `lifeos_role` admin panel appears for adam only. |
