...
| **Next build (product)** | **Run `npm run lifeos:builder:supervise` after deploy** — builder must pass the doc smoke objective and JS smoke objective before unattended dashboard work. Then use `docs/projects/LIFEOS_DASHBOARD_OVERNIGHT_QUEUE.md` as the task order. After supervision passes: verify dashboard sidebar nav, verify Victory Vault routes (`GET /api/v1/lifeos/victories`), then start constrained dashboard shell tasks only. |
...
| **Completed this session (2026-04-28)** | ✅ **Builder routing hardening** — `services/council-model-availability.js` no longer treats local Ollama as a valid Railway fallback unless a real remote endpoint is configured; this prevents silent selection of `ollama_deepseek_v3` on Railway where it cannot succeed. ✅ **Explicit model authority on `/builder/build`** — `routes/lifeos-council-builder-routes.js` now respects an explicit request `model` instead of allowing memory rerouting to override it. ✅ **Canonical dashboard builder artifacts** — new `docs/projects/LIFEOS_DASHBOARD_BUILDER_BRIEF.md`, `docs/projects/LIFEOS_DASHBOARD_OVERNIGHT_QUEUE.md`, and `scripts/lifeos-builder-supervisor.mjs`; package script `npm run lifeos:builder:supervise` runs readiness + deterministic smoke objectives before overnight work. These artifacts force dashboard work to follow the approved image boards and require both light and dark modes. |
