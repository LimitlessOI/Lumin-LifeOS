...
| **Last Updated** | 2026-04-30 — **Builder:** `routes/lifeos-council-builder-routes.js` — HTML `/build` **`maxOutputTokens`** ceiling raised (defaults + **`BUILDER_HTML_MAX_OUTPUT_TOKENS_CAP`** / **`BUILDER_CODE_MAX_OUTPUT_TOKENS_CAP`**); clearer error when output truncates before **`<body>`**. Prior **NSSOT Article II §2.11a ¶6–8** — constitutional LifeOS **mockup SSOT**, **consumer vs B2B program separation**, **defer Command Center omnibus** until supervised builder maturity; **`## Scope` → `### Constitutional LifeOS UX SSOT`**. Prior: 2026-04-29 — **Truth / reliability bridge** — **`docs/BUILDER_RELIABILITY_EPISTEMIC_BRIDGE.md`** (§2.6 + Am.39 Evidence Ladder + daemon **`reliability_cues`**); **`AMENDMENT_02`** → Am.**39** institutional facts stance. Prior **compound improvement loop** (`evaluate → fix → improve` between slices): **`docs/BUILDER_COMPOUND_IMPROVEMENT_LOOP.md`**. Prior **24/7 daemon:** stale **PID lock** for **PM2**; **`BUILDER_QUEUE_MAX`**; **`docs/BUILDER_24_7_ALPHA_CHECKLIST.md`**. Prior: **`probe` supervise + overnight cursor (`OVERNIGHT_USE_CURSOR`)** → idle when queue drained; **`/ready`** + **`POST /build`** retries (502-class); **`lifeos-builder:probe`**; **gitignored** **`data/builder-overnight-cursor.json`**. Prior: **Autonomous progression (no Adam gate):** **`dashboard-density-commentary`** + **`dashboard-ai-rail-contract`** → **`committed:true`** on Railway (`dda28bf1` line). Transient **502** on first AI-rail **`/build`** — **immediate retry succeeded**. **`LIFEOS_DASHBOARD_OVERNIGHT_TASKS.json`** five tasks complete. Prior: **`scripts/lifeos-builder-supervisor.mjs`** — **`fetchCommittedFile()`** retries `git fetch`/`git show origin/main:<path>` with backoff + optional local fallback after **`POST /build`** commits (fixes false **Doc smoke receipt missing … light** when GitHub lag). **`dashboard-import-tokens`** overnight task **`committed:true`** (`claude_via_openrouter`). Continuous **`npm run lifeos:builder:daemon`** started after **`cycle_ok`** supervise+overnight. Prior: **Inner supervisor protocol:** **`prompts/lifeos-builder-inner-supervisor.md`** + **`npm run lifeos:builder:inner-review`** (`scripts/builder-inner-supervisor.mjs`) — trains cheap **`mode: review`** pass on injected files; **`docs/SYSTEM_CAPABILITIES.md`** **V5**. Prior: **`GET /api/v1/system/builder-health`** (`routes/api-v1-core.js`): `requireKey`, reads `data/builder-daemon-state.json`, optional **`log_lines`** tail of `builder-daemon-log.jsonl` (cap 80); **`@ssot`** tag on route module. **KNOW:** Railway container usually has no daemon-written receipts unless that process shares `data/` — empty state is explicit, not green-washed. **`docs/SYSTEM_CAPABILITIES.md`** matrix **B6**. Prior: **Dashboard token CSS shipped:** **`public/shared/lifeos-dashboard-tokens.css`** via **`POST /api/v1/lifeos/builder/build`** overnight task **`dashboard-theme-foundation`** (`npm run lifeos:builder:overnight -- --task dashboard-theme-foundation` → `committed:true`, `model_used`: `claude_via_openrouter`); prod **`GET /shared/lifeos-dashboard-tokens.css`** HTTP **200**, **2607** bytes (= local). Prior: 2026-04-25 — **Builder `autoWireRoute` FS mirror:** after `commitToGitHub` updates `startup/register-runtime-routes.js`, `mirrorCommittedContentToRepoRoot(REGISTER_PATH, current)` syncs container disk for chained `files[]` (same gap class as pre-mirror `/build`). Prior: 2026-04-29 — **Overnight builder supervision:** `scripts/lifeos-builder-overnight.mjs` + `docs/projects/LIFEOS_DASHBOARD_OVERNIGHT_TASKS.json` + `npm run lifeos:builder:overnight`; supervisor chains `--overnight` after smoke; **GAP-FILL** replaced `[system-build]` `DASHBOARD_SHELL_GAP_AUDIT.md` that falsely claimed SSOT missing (council ignored injected file bodies). Prior: 2026-04-28 — **Builder supervision hardening:** `services/council-model-availability.js` disables Ollama on Railway unless a real non-local endpoint is explicitly configured; `routes/lifeos-council-builder-routes.js` now treats explicit `model` on `/api/v1/lifeos/builder/build` as authoritative and skips memory rerouting for that request. Added canonical dashboard builder artifacts: `docs/projects/LIFEOS_DASHBOARD_BUILDER_BRIEF.md`, `docs/projects/LIFEOS_DASHBOARD_OVERNIGHT_QUEUE.md`, `scripts/lifeos-builder-supervisor.mjs`, and `package.json` script `lifeos:builder:supervise`. Purpose: keep overnight dashboard work tied to SSOT, shell constraints, mobile/desktop intent, and required light/dark themes. Prior: 2026-04-27 — **Railway boot:** `core/SOURCE_OF_TRUTH.md` (runtime mirror) + `server.js` tries `docs/` then `core/` for memory bootstrap; **`services/reminder-cron.js`** — reminder query references no missing `commitments` columns. Prior: **`public/overlay/lifeos-app.html`** — sidebar + **More** sheet: **Dashboard** (`lifeos-dashboard.html`), **Chat** (`lifeos-chat.html`), **Weekly** (`lifeos-weekly-review.html`), **Wins** / Victory Vault (`lifeos-victory-vault.html`); new `--c-dashboard` / `--c-victory`; **PAGE_META** extended. **GAP-FILL:** `POST /api/v1/lifeos/builder/build` failed (`Council call failed` / `fetch failed` from Railway → provider). Prior: **Railway boot:** `startup/register-runtime-routes.js` restores missing `import { createAssessmentBatteryRoutes }`; `routes/lifeos-assessment-battery-routes.js` uses `requireLifeOSUser` from `middleware/lifeos-auth-middleware.js`, `req.lifeosUser.sub`, and correct `saveResult({ userId, … })` call (fixes `ReferenceError: createAssessmentBatteryRoutes is not defined` + broken auth helper import). Prior: 2026-04-25 — **`## Idea Vault → LifeOS-native consolidation`** (variation map; operator **Pewds** hospital context row). Prior: 2026-04-22 — **`docs/SYSTEM_CAPABILITIES.md`** + `npm run system:railway:redeploy` (R1). Prior: **`npm run lifeos:builder:build-chat`**. Prior: **Env certification** + **`ENV_REGISTRY` operator mirror** (screenshots + same-session inventory + `PUBLIC_BASE_URL` SET). Prior: **Operator-supplied Railway evidence (constitutional hard stop):** `ENV_DIAGNOSIS_PROTOCOL` *Operator-supplied evidence*; North Star **§2.3** + TL;DR; Companion **§0.4**; `prompts/00`; `CLAUDE.md` session checklist **#6**; `generate-agent-rules` §4/§6 + regen **`AGENT_RULES.compact.md`** — **forbids** re-asking Adam to set names he already proved **or** “not in prod” from empty IDE shell alone. Prior: **`builder:preflight` → GET `/api/v1/railway/env` name probe**. Prior: 2026-04-25 — **North Star §2.11a / §2.11b split** + Companion **§0.5F** (TSOS+builder) / **§0.5G** (Conductor→Adam report): same repo updates (`QUICK_LAUNCH`, `generate-agent-rules`, `prompts/00`, `CLAUDE.md`, INDEX, Am. 10). Prior: **2026-04-24 — TokenSaverOS (TSOS) + North Star §2.11a** (constitution + Companion §0.5F + QUICK_LAUNCH + INDEX + `prompts/00` + Amendment 10 cross-link): builder = meta-product; **Adam report**; governed self-build. Prior: **Builder preflight** / `GET /ready` / `core/sales-technique-analyzer.js` @ssot. Prior: **Article II §2.12:** constitutional technical-decision + Construction-supervisor anti-drift law; epistemic contract implements §2.12. Prior 2026-04-21 — **Lumin build observability:** `GET /api/v1/lifeos/chat/build/health` + `scripts/lumin-build-smoke.mjs` + `npm run lifeos:lumin-build-smoke` (no AI on health; optional `LUMIN_SMOKE_PLAN=1` for POST `/build/plan`). Prior **§2.11 / §0.5D — code system only (gaps/breakage/Lumin parity); system programs amendments/projects.** Prior **Article II §2.11 + §0.5D (Conductor / GAP-FILL).** Prior **Lumin build panel + build NL + mode `buildContextSnapshot`:** `public/overlay/lifeos-chat.html` (Build, jobs, `/plan` `/draft` `/queue`, Cmd/Ctrl+L); `services/lifeos-lumin.js` mode DB slices. Prior **NSSOT platform law §2.10 + Companion §0.5C:** North Star **Article II §2.10**; Companion **§0.5C**; `### Core LifeOS vs Adaptive Lumin (idea routing)`; `docs/QUICK_LAUNCH.md` + `prompts/00-LIFEOS-AGENT-CONTRACT.md` cross-links. Prior **Lumin shell front door:** `lifeos-app.html` adds visible **Ask Lumin…** strip + raises drawer/FAB z-index; new `### Lumin — companion front door` under Product Layers (conversational-first intent, communication-profile + response-variety, anti-interview / anti-gimmick). Prior **Low-power ambient context:** `db/migrations/20260423_lifeos_ambient_snapshots.sql` (`lifeos_ambient_snapshots`: `id`, `user_id` FK, `signals` JSONB, `created_at`); `services/lifeos-ambient-context.js` (`appendSnapshot`, `listRecent`, `listRecentSafe`); `routes/lifeos-ambient-routes.js` mounted at `/api/v1/lifeos/ambient` — `POST /snapshot` (body `user`, `signals` object ≤16KB, **no AI** on ingest), `GET /recent`; `startup/register-runtime-routes.js` registers ambient router; `public/overlay/lifeos-ambient-sense.js` + Settings toggle in `lifeos-app.html` (localStorage `lifeos_ambient_sense`, default off; 10–20 min cadence while visible, longer on unplugged low battery); `services/lifeos-lumin.js` `buildContextSnapshot` adds `ambient_hints` (last 6 rows). **`lifeos-voice.js`:** always-listen **suspends** when `document.hidden` (mic + wake lock released); screen wake lock **off** by default on `(pointer: coarse)`; `LuminVoice.configure({ wakeLock })`. Prior **2026-04-20 — Household invites + auth UX:** `signup_url` on `POST/GET /api/v1/lifeos/auth/invite(s)` (absolute if `PUBLIC_BASE_URL` or proxy Host, else relative `/overlay/...`); admin **Household invites** section in `public/overlay/lifeos-app.html` Settings; JWT `role`/`tier` synced from access token in `public/overlay/lifeos-bootstrap.js`; `public/overlay/lifeos-login.html` — `?code=` alias for `?invite=`, invite banner, optional display name; `signOut` clears JWT + redirects login; `services/lifeos-lumin.js` — optional context-grounded relational prompts (no fabricated surveillance). **KNOW:** passwords use **scrypt** in `lifeos-auth.js` (not bcrypt). Prior **2026-04-19:** **§2.6 ¶8 HTTP** `/api/v1/lifeos/gate-change` + migration + service + prompt file. Prior: ¶8 SSOT-only. Prior: mandatory ¶5–7 + epistemic cross-links (`prompts/00`, `CLAUDE.md`, Companion, Lumin, Amendment 01). Prior: **Adam ↔ Agent epistemic contract** + Lumin prepend + prompts. Prior: **Commitment→execution desk** graduated autonomy. Prior: full backlog (“Commitment → execution desk…”). Prior: Added `## Agent Handoff Notes`, Amendment 36 cold-start / lane continuity pointers, manifest `lane_read_manifest`. Prior: 2026-04-18 (Pick 1 of 5 follow-up session — Truth Delivery Calibration Learning Loop is live: new `db/migrations/20260418_truth_delivery_calibration.sql` extends `truth_delivery_log` with hour-of-day + emotional-state + joy/integrity-at-time + adds a `truth_deliveries` compatibility view; `services/truth-delivery.js#generate` now captures hour + infers emotional state at delivery time from `daily_emotional_checkins`; new `getCalibrationReport()` aggregates best style / hour / state / topic; `GET /api/v1/lifeos/truth/calibration` route + new "Truth Calibration" card on `lifeos-today.html`. Bug fix: `services/lifeos-scheduled-jobs.js#calibrationTick` queried a non-existent `truth_deliveries` table (real name was always `truth_delivery_log`) — calibration was a silent no-op on every populated DB; query corrected + view added. Earlier 2026-04-18: Money Decisions now mirror into Decision Intelligence — new `services/lifeos-money-decision-bridge.js` + 4 `/finance/decisions/*` routes + "Log as money decision" UI on `lifeos-finance.html` auto-request a Second Opinion when the amount meets the per-user threshold or the move is irreversible, closing the Phase 16 gap on line 526 of the build plan. Earlier 2026-04-18: Zero-Waste AI enforcement hardened on the LifeOS scheduler: `services/lifeos-scheduled-jobs.js` now routes `eventIngestTick`, `earlyWarningTick`, `calibrationTick`, and `weeklyReviewTick` through `createUsefulWorkGuard()` with explicit per-tick prerequisites and SQL work-checks so no AI call fires unless there is real work — directly satisfies the CLAUDE.md "Zero Waste AI Call Rule" and fixes a long-standing drift where these ticks iterated every user on every interval. Earlier 2026-04-18: Phase 5 daily emotional check-in is now a real daily surface: new `db/migrations/20260418_lifeos_daily_emotional_checkins.sql` defines `daily_emotional_checkins` with weather/intensity/valence/depletion-tags + upsert per user per day; `services/emotional-pattern-engine.js` now exports `logDailyCheckin` / `getTodayCheckin` / `getRecentCheckins` / `getTrend`; `routes/lifeos-emotional-routes.js` exposes `POST /daily`, `GET /daily/today`, `GET /daily/recent`, and `GET /daily/trend` (all input-clamped via `safeInt`/`safeLimit`/`safeDays`); `public/overlay/lifeos-today.html` ships a new "Emotional Weather" card + modal between "Your State" and "Attention Loop" so the daily ritual is one tap from the Mirror. This is the capture surface that was missing — the emotional engine, joy score, early-warning notifications, and truth-delivery calibration are all downstream of it. Prior 2026-04-18 work: tracked `services/lifeos-request-helpers.js` as required in `scripts/lifeos-verify.mjs` and in the amendment manifest so the shared input-coercion helper cannot silently disappear; continuing the 2026-04-18 LifeOS input-coercion hardening which added the shared `services/lifeos-request-helpers.js` (`safeInt` / `safeLimit` / `safeDays` / `safeId`) and wired it through `lifeos-core-routes.js`, `lifeos-emotional-routes.js`, `lifeos-copilot-routes.js`, `lifeos-workshop-routes.js`, `lifeos-health-routes.js`, `lifeos-decisions-routes.js`, `lifeos-engine-routes.js`, and `lifeos-identity-routes.js`, so `?days=abc`, `?limit=-5`, and non-numeric `:id` params now return clean 400s or clamp to safe windows instead of reaching the DB as `NaN`; prior 2026-04-18 work normalized shared LifeOS auth/user bootstrap across overlays with `public/overlay/lifeos-bootstrap.js`, taught the shell to use the same context resolver, and extended `scripts/lifeos-smoke.mjs` to verify the bootstrap asset; prior 2026-04-17 work delivered the scoreboard, event stream, notifications escalation, calendar, finance, focus/privacy, and core runtime stabilization) |
...
| **Next build (product)** | **Implement AI rail + density** per **`docs/projects/DASHBOARD_AI_RAIL_CONTRACT.md`** + **`docs/projects/DASHBOARD_DENSITY_INTEGRATION_NOTES.md`** (new **`POST /builder/build`** slices when prioritized — SSOT-grounded **`target_file`** in brief). **FYI:** Victory Vault **`/api/v1/lifeos/victories`** + sidebar **`lifeos-app.html`** **spot-check**: operator optional; autonomous builder does not block on human approval when receipts exist. **`npm run system:railway:redeploy`** if runtime must trail **`main`**. |
...
| **Known gaps** | **`/execute`** for `.js` — does not reuse `/build`'s **`node --check`** gate (`THINK`: lower consistency than codegen path); fix only if bad commits recur. **Dashboard sidebar link** — verify **`lifeos-app.html`** `nav-dashboard` against shipped dashboard. **§2.10 runtime depth** — automated closed-loop observability still mostly CI. **Commitment→execution desk** — specced in backlog, not built. **Ambient sense E2E** — migration applied but no smoke test confirming Neon rows. |
| **⚠️ IN PROGRESS:** | **AI rail implementation** on **`lifeos-dashboard.html`** — blocked by HTML **output truncation** until this session’s **`maxOutputTokens`** cap ships to Railway; **`POST /builder/build`** retry immediately after **`npm run system:railway:redeploy`** (+ deploy settle). **`npm run lifeos:builder:overnight`** / daemon may advance **`dashboard-customization-state`** queue items once rail lands. |
...
|---|---|---|---|---|
| 2026-04-30 | **GAP-FILL — Builder HTML truncation (supervised `/build` false “missing structure”):** **`routes/lifeos-council-builder-routes.js`** — `estimateBuilderMaxOutputTokens(..., targetFile)` uses higher cap when **`target_file`** ends with **`.html`** (default **`BUILDER_HTML_MAX_OUTPUT_TOKENS_CAP`**=**65536**, clamped ≤128k matching `services/council-service.js`); non-HTML codegen cap via **`BUILDER_CODE_MAX_OUTPUT_TOKENS_CAP`** (default **16384**, max 32k). Fallback **`maxOutputTokens`** when estimator returns null and target is HTML uses same HTML ceiling. **`validateGeneratedOutputForTarget`** — explicit message when **`<!DOCTYPE`+`<html`+`<head>`** exist but **`<body>`** missing (truncate-in-head). **`docs/SYSTEM_CAPABILITIES.md`** — B4 notes + changelog. **`docs/BUILDER_OPERATOR_ENV.md`** — server tuning table rows. **`docs/ENV_REGISTRY.md`** — optional builder tuning vars block. Evidence: **`GET /api/v1/lifeos/builder/gaps`** rows **756/758** (`dashboard-ai-rail` **`committed:false`** despite valid prefix). | Adam: **keep supervising — fix platform** so the system keeps building; root cause was **16k completion cap** vs large injected **`lifeos-dashboard.html`** causing **provider cut-off mid-`<head>`**. §2.11 platform GAP-FILL (not IDE product substitution). | ✅ | `node --check routes/lifeos-council-builder-routes.js` |
