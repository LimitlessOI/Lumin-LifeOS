The task specifies implementing the persistent AI rail in `public/overlay/lifeos-app.html`, but the provided file content is for `public/overlay/lifeos-chat.html`. The contract below is written assuming the persistent AI rail will integrate with the existing Lumin chat functionality as seen in `lifeos-chat.html`, but the actual target file for the UI implementation (`lifeos-app.html`) is not available.
...
*   **Interaction Parity:**
    *   **Voice Input (Speech-to-Text - STT):** Leverage the existing `window.LuminVoice` utility (as seen in `lifeos-chat.html`) to enable speech input directly into the rail's input field. This includes displaying interim transcript text and providing visual feedback (e.g., a pulsing mic icon) when active.
    *   **Text Input:** Utilize the existing `luminSend` function (from `lifeos-chat.html`) for sending messages and `luminAutoResize` for managing the input `textarea`'s height.
...
1.  **Refactor `lumin-drawer`:** Modify the HTML structure and CSS of the existing `lumin-drawer` (or a new equivalent in `lifeos-app.html`) to function as a persistent rail container, supporting flexible positioning and sizing.
...
1.  **Voice Input (STT):** Integrate `window.LuminVoice.startForInput()` with the rail's input field. Implement visual feedback for active listening, such as a pulsing microphone icon and displaying interim transcript text via the `lumin-voice-interim` event.
