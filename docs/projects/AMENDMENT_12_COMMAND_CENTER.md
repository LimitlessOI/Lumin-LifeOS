...
|---|---|---|---|---|---|
| 2026-05-24 | **`db/migrations/20260524_self_repair_memory_events_fix.sql`** (NEW recovery migration, `CREATE TABLE IF NOT EXISTS`): prior migration failed on Railway because builder injected invalid `ssot_tag` constraint on non-existent column — Rails marked it applied anyway (migration runner line 80 marks-applied on failure to avoid boot loops); table was never created. Fix uses new filename + idempotent SQL to recover. | Migration runner marks-applied on failure — fixed file would not re-run without manual DB clear; new migration file is the only safe path. | ✅ idempotent SQL | pending | Railway boot log: `Applied migration: 20260524_self_repair_memory_events_fix.sql` |
