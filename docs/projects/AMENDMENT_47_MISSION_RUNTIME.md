...
| **Status** | **PHASE 2 COMPLETE + 2026-06-04 CRITICAL FIX** — All 7 owned files DONE. 10/10 verifier checks PASS. AIC DISCUSSION-6 resolved. Household board add-commitment path now normalizes BPB payloads against the legacy commitments schema. |
...
- ~~**CRITICAL CORRECTION: household Add Commitment still posted BPB fields that did not satisfy legacy `commitments` constraints**~~ — **RESOLVED 2026-06-04**: `createCommitment()` now normalizes `{ owner, text, mission_id: "MISSION-0001" }` before insert: slug mission IDs resolve through `missions.slug`, owner handles resolve through `lifeos_users.user_handle`, `text` backfills required `title`, `title` backfills BPB `text`, and `status` defaults to `open`. Prior 2026-06-02 fix only added columns to the allowlist; it did not transform the household overlay payload, so normal Add Commitment submissions could still fail with legacy `user_id`/`title` NOT NULL errors or UUID parse errors for slug mission IDs.
...
**Current state as of 2026-06-04 (post-critical-fix):** PHASE 2 routes remain production-verified from 2026-06-02. A 2026-06-04 bug sweep found the household board Add Commitment path could still fail on new submissions because the overlay sends BPB-style fields (`owner`, `text`, slug `mission_id`) into the legacy `commitments` table that requires `user_id`, `title`, and UUID `mission_id`. `services/mission-ledger.js` now normalizes that payload before insert. Local regression + syntax + configured smoke suite pass; production route smoke should run after this branch deploys.
...
- `services/mission-ledger.js` — `createCommitment` now includes `user_id` + `title` in allowed cols and normalizes BPB/overlay payloads into legacy schema requirements (`owner`→`user_id`, `text`→`title`, slug `mission_id`→UUID, default `status='open'`)
...
|---|---|---|---|---|
| 2026-06-04 | **CRITICAL FIX: household board Add Commitment payload normalization.** `services/mission-ledger.js`: added `resolveMissionId()` and `resolveOwnerUserId()` helpers; `createCommitment()` now copies fields, resolves non-UUID `mission_id` slugs through `missions.slug`, resolves `owner` handles through `lifeos_users.user_handle`, backfills `title` from `text` and `text` from `title`, includes/defaults `status='open'`, then applies the existing allowlist before INSERT. `tests/mission-ledger.test.js`: new regression coverage for the exact household-board payload and coded unknown mission/owner failures. **GAP-FILL evidence:** builder preflight passed, but two `POST /api/v1/lifeos/builder/task` attempts for `services/mission-ledger.js` returned the same unsafe cached partial rewrite: undeclared `uuid`/`pg-sql2` imports, wrong `lifeos_users.user_id` column, and truncated output starting with only a replacement function instead of the complete service file. The output was rejected and not executed. | Cron critical bug sweep found normal `public/overlay/lifeos-household.html` submissions send `{ owner, text, mission_id: "MISSION-0001" }`; the 2026-06-02 fix only allowed `user_id`/`title` when explicitly supplied, so fresh UI submissions could still fail on legacy NOT NULL columns and UUID parsing. | `services/mission-ledger.js`, `tests/mission-ledger.test.js`, `docs/projects/AMENDMENT_47_MISSION_RUNTIME.md` | ✅ `node --test tests/mission-ledger.test.js` (3/3 pass); `node --check services/mission-ledger.js`; `node --check tests/mission-ledger.test.js`; `npm test` (49 pass, 0 fail, 4 skipped); `node scripts/ssot-check.js --all` exit 0 with pre-existing missing-tag inventory |
