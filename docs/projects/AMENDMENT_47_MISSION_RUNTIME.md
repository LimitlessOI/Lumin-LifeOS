...
- ~~**CRITICAL: `createCommitment` missing `user_id` in allowed cols**~~ — **RESOLVED 2026-06-02**: `services/mission-ledger.js` `createCommitment` cols list did not include `user_id`. The `commitments` table has `user_id BIGINT NOT NULL`, so all INSERTs failed. Fixed by adding `user_id` to the cols array. Seeding now works.
