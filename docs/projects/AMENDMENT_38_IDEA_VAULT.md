...
|------|--------------|-----|
| 2026-06-13 | **Cursor session archive pipeline** — `scripts/archive-cursor-transcript.mjs`; session `e9b7659e` → raw jsonl + master index + `by-product/*` buckets + receipt; indexed in `CONVERSATION_DUMP_IDEAS_INDEX` §12; product pointers in LIFEOS/LIFERE. | Adam: save every conversation back-and-forth for history/receipts; product topics in product files. | ✅ 741 pairs | re-run for new sessions |
