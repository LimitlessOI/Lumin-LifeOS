...
**Last Updated:** 2026-05-27 (self-repair session — live stub-gate repair)
...
| Date | File | What | Why |
|---|---|---|---|
| 2026-05-27 | `services/builderos-pbb-plan.js` + `services/builderos-governed-loop-executor.js` | PBB fix: export name parser (`getBuilderOSC2CommitProof` not `a`), gemini_flash for JS targets, anti-truncation spec. Executor fix: verify fresh builder `output` before stale on-disk file. | First commit-path test: builder committed but groq truncated file → verifier_failed; verifier was reading stale disk not response output. |
