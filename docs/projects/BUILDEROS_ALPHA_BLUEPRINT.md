...
| Date | File | What | Why |
|---|---|---|---|
| 2026-06-08 | `scripts/governed-overnight-backlog-run.mjs` | **Critical runner portability fix:** removed hard dependency on `/bin/zsh` from target classification and syntax verification. `targetLineCount()` now uses `readFileSync`; `classifyLocalTarget()` uses `existsSync`; `syntaxCheck()` uses `execFileSync(process.execPath, ['--check', filePath])`. | Scheduled critical-bug audit found that standard Linux/Railway images can lack `/bin/zsh`; the runner then misclassified existing targets as missing Zone 1 files and reported false syntax failures, causing unsafe full-file regeneration decisions and false health signals. |
