...
| Date | File | What | Why |
|---|---|---|---|
| 2026-06-09 | `builderos-reboot/MISSIONS/FACTORY-REBOOT-0001/BLUEPRINT.json` + `ACCEPTANCE_TESTS.json` | Refreshed stale `S004` hash for current `builderos-reboot/HANDOFF.md` (`a844afe...`) and regenerated the mission acceptance file. Current state: the unrelated bootstrap handoff drift no longer blocks aggregate `factory:acceptance` / `factory:ci`. Next: rerun factory CI after all security-source-of-truth updates. | Factory CI should fail only on real current drift; the old Handoff hash was pre-existing verifier drift uncovered while validating the security fix. |
| 2026-06-09 | `builderos-reboot/MISSIONS/FACTORY-REBOOT-{0004,0005,0006,0007,0010,0013,0015,0029}/...` | **Blueprint rematerialization fix:** copied hardened `factory-staging/server.js`, `factory-staging/startup/register-routes.js`, and `factory-staging/factory-core/builder/run-step.js` into every mission CONTENT payload that byte-exact pins those runtime files; refreshed `BLUEPRINT.json` hash pins and regenerated `ACCEPTANCE_TESTS.json` for those missions. Current state: `factory:duplication` can no longer rematerialize the vulnerable old dispatcher/route files over the hardened runtime; affected hashes now point to `run-step.js` `32b07da...`, `register-routes.js` `bc1bbd...`, and `server.js` `a7f7e0...`. Next: rerun acceptance/duplication/cutover/readiness/factory CI and fix any remaining bundle drift. | Validation exposed a critical second-order bug: the duplication proof deleted the patched runtime files and rematerialized old vulnerable CONTENT from mission blueprints, proving the first commit fixed runtime files but not the factory source-of-truth. |
