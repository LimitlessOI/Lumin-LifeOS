...
| Date | File | What | Why |
|---|---|---|---|
| 2026-06-01 | `scripts/governed-overnight-backlog-run.mjs` + `tests/governed-overnight-backlog-run.test.js` + `package.json` | Final verification receipt for critical runner fallback-churn fix. Checks passed: `node --check scripts/governed-overnight-backlog-run.mjs`, `node --check tests/governed-overnight-backlog-run.test.js`, focused `node --test tests/governed-overnight-backlog-run.test.js` (2/2 pass), and `npm test` (51 pass, 0 fail, 4 skipped). Current state: branch contains two implementation commits (`48e53e80`, `3d938b9f`) and the fix is pushed to `cursor/critical-bug-investigation-bee2`; no runtime Railway call was needed because the bug was reproduced from local runner logs and pure generator logic. Next: open PR with impact/root-cause/fix/validation. | Close the deep bug-finding loop with honest evidence: the resource-leak trigger is concrete and log-proven, the code path is patched, and regression coverage is in the default suite. |
| 2026-06-01 | `scripts/governed-overnight-backlog-run.mjs` | Follow-up testability fix: removed the top-level `import 'dotenv/config'` dependency and replaced it with a best-effort built-in `.env` loader (`loadLocalEnvFile()`) that runs before `PUBLIC_BASE_URL` / command-key constants are read. Current state: direct runner execution still loads local `.env` when present and still fails closed if URL/key are missing; test imports no longer require `node_modules/dotenv`. Next: rerun syntax, focused test, and `npm test`. | Verification failed because this checkout has no installed `dotenv` package, so importing the runner in the new regression test crashed with `ERR_MODULE_NOT_FOUND` before tests could exercise the generator logic. |
| 2026-06-01 | `package.json` | Added `tests/governed-overnight-backlog-run.test.js` to the `npm test` script while preserving all existing protected test entries (`repo:sync-check`, `lifeos:verify:ui-map`, and the deployment guard's required regression files remain unchanged). Current state: the governed runner regression now executes in the default test suite. Next: run `npm test` plus targeted syntax checks before commit. | Ensure the unbounded-runner-loop regression is not only a one-off local test; it must run with the existing default test command so future package/test changes do not silently drop coverage. |
| 2026-06-01 | `tests/governed-overnight-backlog-run.test.js` (NEW) | Added two focused `node:test` regressions for the runner generators: blueprint fallback proof tasks are emitted only once across generations when the underlying exact build key was already attempted, and support self-improvement telemetry fallback is emitted only once with the stable `scripts/verify-runner-telemetry.mjs` target. Current state: tests import the runner safely because `main()` is now direct-run guarded; they use synthetic blueprints/support inputs and do not call Railway, DB, or Git. Next: run `node --check` and the new test file, then commit/push if green. | Locks the critical unbounded-loop fix so future edits cannot reintroduce generation-scoped fallback churn that burns C2/builder/council resources and floods remediation commits. |
| 2026-06-01 | `scripts/governed-overnight-backlog-run.mjs` | Critical bug fix for the continuous blueprint runner: fallback blueprint proof tasks now use stable `attemptedKeys` (`proof-fallback:<signal>`) before queuing, support self-improvement telemetry uses stable id/target (`runner-self-improvement`, `scripts/verify-runner-telemetry.mjs`), `callApi()` now aborts hung HTTP calls after `GOVERNED_BACKLOG_API_TIMEOUT_MS` (default 120s), and the script now guards `main()` behind a direct-run check so generator functions can be imported by tests. Current state: the runner can still stay alive and idle/retry as designed, but it no longer generates generation-scoped proof/telemetry tasks forever after stable blueprint/support work is exhausted. Next: add and run focused regression tests proving second-generation fallback calls return zero duplicate work. | Deep bug-finding automation found log-proven resource churn: identical blueprint proof tasks were emitted as `proof-g6`, `proof-g7`, `proof-g8` for the same sources, causing repeated C2 jobs/commits/token spend; unbounded execute waits could also hold the lock forever. |
