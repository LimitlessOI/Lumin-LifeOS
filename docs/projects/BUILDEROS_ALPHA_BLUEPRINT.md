...
**Last Updated:** 2026-06-14 — `/builder/build` completion authority restored for kernel-managed committed responses; DONE gate remains kernel-sequenced
...
| 2026-06-14 | `routes/lifeos-council-builder-routes.js` + `tests/builderos-completion-authority.test.js` | Critical completion-authority regression fix: kernel-managed `/builder/build` calls no longer bypass completion authority with `completion_deferred_to_kernel`. The route still defers only the DONE gate to the TSOS kernel for ledger proof ordering, but committed responses now require `grantBuildCompletion()` to pass or return 409. Regression test changed to assert kernel-managed builds fail closed on missing completion evidence. | Recent sequencing repair made every mounted `/builder/build` call return `ok:true, committed:true` with completion deferred to a kernel path that does not grant completion; direct automations use `ok && committed` as success, so wrong-outcome commits could be marked complete. |
