...
**Last Updated:** 2026-04-07 (prospect save no longer re-requires file upload after OCR; if payer/member were already read from the card, the VOB panel saves that draft directly to history; VOB prospect CTA switches to “Save card to history” after a successful read; VOB still clones card Files so rerender does not drop uploads; OCR merges into data completeness when subscriber matches; Save prospect keeps group/subscriber)
