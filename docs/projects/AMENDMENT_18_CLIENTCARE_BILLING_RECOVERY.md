...
**Last Updated:** 2026-04-07 (legacy preview/estimate wiring is removed from the operator overlay and the copy now explicitly states that only the real ClientCare VOB matters; asset version bumped to `20260407e`; real ClientCare VOB results now persist visibly in the panel after the run finishes instead of disappearing on dashboard reload; the real ClientCare VOB workflow is moved into the main workspace instead of the narrow utility rail; primary work panels are collapsible; the fake estimate VOB panel is removed from the main workflow; the real ClientCare VOB is now the single primary path in the VOB card; the real ClientCare flow now accepts many card images at once for the same patient and posts them as a multi-file upload; fixed live ClientCare VOB browser runtime bug `phase is not defined`; card subscriber name now triggers an automatic live ClientCare directory search when the loaded backlog does not contain the client; exact/unique matches auto-select the file without operator hunting; prospect save still no longer re-requires file upload after OCR; VOB prospect CTA switches to “Save card to history” after a successful read; VOB still clones card Files so rerender does not drop uploads; OCR merges into data completeness when subscriber matches; Save prospect keeps group/subscriber)
