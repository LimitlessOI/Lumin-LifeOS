...
**Last Updated:** 2026-04-07 (fixed the actual `phase is not defined` production error: `clickFirstMatchingButton()` returned `phase` from inside `page.evaluate()` without passing it into the browser context; the real ClientCare VOB path now passes `phaseLabel` explicitly so click bookkeeping stops throwing mid-run; asset URL bumped again to `20260407j`; the real ClientCare VOB button is now hard-blocked on account mismatch or in-flight directory search, and exact existing-client name matches auto-select instead of leaving stale account context active; frontend API calls now fail fast instead of hanging forever, the real ClientCare VOB route has a server-side deadline, and live ClientCare directory search now times out with a visible warning instead of silently spinning forever; previous ClientCare VOB fixes remain in place)
