...
| **Manifest** | `docs/projects/AMENDMENT_41_MARKETINGOS.manifest.json` |
...
- **Creator Media OS (Amendment 23)** — avatar-based video, likeness capture, cinematic production, channel-scale automated generation from likeness profiles. Amendment 23 is a sibling module, not a duplicate. MarketingOS uses real footage; Amendment 23 generates synthetic likeness video.
- **Video Pipeline (Amendment 07)** — the shared Replicate/FFmpeg render stack is a dependency, not owned by MarketingOS. Do not rebuild it.
- **Outreach CRM (Amendment 08)** — lead contact management, follow-up sequences, CRM records. MarketingOS generates content; Amendment 08 manages the people it reaches.
- **Productized Sprint (Amendment 27)** — the manual delivery service offer layer. Amendment 27 is the Phase 0 revenue vehicle. MarketingOS is the platform that eventually automates what Amendment 27 currently delivers manually.
- **LifeOS Communication OS (Amendment 21)** — personal relationship and communication intelligence. Different buyer, different purpose. No crossover.
...
└── MarketingOS (AMENDMENT 41) ← you are here
...
├── Amendment 07 — Video Pipeline (Replicate, FFmpeg, Kling, Wan)
├── Amendment 08 — Outreach CRM (lead management)
├── Amendment 23 — Creator Media OS (avatar/likeness video — separate product)
├── Amendment 27 — Productized Sprint (Phase 0 manual revenue)
└── Amendment 21 — LifeOS Core (personal data wall — constitutional boundary)
...
**Revenue:** $997 via Amendment 27 "Build My Thing" offer. Stripe payment link only.
...
**Excluded:** Building websites (Amendment 05 Site Builder owns that). Paid ad management, budget management (Phase 10).
...
**Rule 2 — No duplicate ownership with Amendment 23.**
Creator Media OS (Amendment 23) owns: avatar-based video generation, likeness capture profiles, `creator_likeness_profiles`, `creator_channels`, `creator_scripts`. MarketingOS does not build these. If a task would duplicate Amendment 23 scope, HALT and surface the conflict to Adam.
...
|---|---|---|---|---|---|
| 1 | `docs/projects/AMENDMENT_41_MARKETINGOS.md` | This document — SSOT foundation | None | `test -f docs/projects/AMENDMENT_41_MARKETINGOS.md` | File exists, all 13 sections complete |
| 2 | `docs/projects/AMENDMENT_41_MARKETINGOS.manifest.json` | Machine-readable manifest | Task 1 | `node scripts/verify-project.mjs --project marketingos` | Manifest valid, verification passes |
...
- `docs/projects/AMENDMENT_23_CREATOR_MEDIA_OS.md` — concept-stage, 0% built, no owned files exist
- `docs/projects/AMENDMENT_07_VIDEO_PIPELINE.md` + `services/video-pipeline.js` — built, FFmpeg on Railway UNCONFIRMED
- `docs/projects/AMENDMENT_27_PRODUCTIZED_SPRINT.md` — Phase 0 revenue vehicle, fully specced, ready to execute
...
```bash
test -f docs/projects/AMENDMENT_41_MARKETINGOS.md
test -f docs/projects/AMENDMENT_41_MARKETINGOS.manifest.json
node scripts/verify-marketing-data-wall.mjs  # no LifeOS imports in marketing services
grep -r "AMENDMENT_41" routes/marketing-routes.js  # @ssot tag present
```
...
```
docs/projects/AMENDMENT_41_MARKETINGOS.md
docs/projects/AMENDMENT_41_MARKETINGOS.manifest.json
...
services/word-keeper-transcriber.js — transcription backbone (Amendment 16)
services/video-pipeline.js          — shared render stack (Amendment 07)
...
|---|---|---|---|---|---|
| 2026-06-08 | **Critical regression coverage wired.** Added `tests/marketing-critical-regressions.test.js` and included it in `package.json` `test`. The test asserts the MarketingOS migration is real executable SQL for all five SSOT tables, the session overlay has no auto-consent timer and no `innerHTML` user rendering, and the governed runner no longer contains `/bin/zsh` local-check dependencies. | Lock in the high-severity bug fixes found by the scheduled audit so future builder/autonomous commits cannot silently reintroduce schema truncation, stored XSS, consent bypass, or Linux runner misclassification. | ✅ | ⬜ | `node --test tests/marketing-critical-regressions.test.js` 3/3 PASS; `npm test` 52 pass / 0 fail / 4 skipped |
| 2026-06-08 | **Critical security bugfix — MarketingOS session overlay consent + XSS.** `public/overlay/marketing-session.html` now uses explicit checkbox + **Confirm consent** button before enabling text/audio controls, removes the demo `setTimeout(...updateConsentStatus(true))` auto-consent bypass, and renders chat/transcript messages with DOM nodes + `textContent` instead of `innerHTML`. Required builder attempt (`POST /api/v1/lifeos/builder/build`, task `build-1780916854494-8af2605c`) returned `committed:false` with `generated HTML appears truncated before <body>`, so this was GAP-FILL. | Scheduled critical-bug audit found a public overlay where pasted user text like `<img onerror=...>` would execute as HTML and consent became confirmed after 3 seconds without user action, violating the consent-first recording contract. | ✅ | ⬜ | Browser walkthrough video: pending controls stayed disabled after wait; explicit consent enabled controls; XSS payload rendered as text with no alert. `npm run check:overlay` PASS |
| 2026-06-08 | **Critical bugfix — MarketingOS Phase 1 migration repaired.** `db/migrations/[date]_marketing_schema.sql` was invalid markdown/truncated SQL and then the required builder attempt (`POST /api/v1/lifeos/builder/build`, task `build-1780916826796-41117ac4`, commit `1ae7bb97`, `committed:true`) still committed a 9-line truncated migration ending mid-column. GAP-FILL replaced it with executable idempotent PostgreSQL for the five SSOT Phase 1 tables: `marketing_consent_records`, `marketing_sessions`, `marketing_content_extractions`, `marketing_content_pieces`, and `marketing_channel_profiles`, including the consent FK and indexes. | Scheduled critical-bug audit found that the startup migration runner would retry this file on every boot and never create the MarketingOS Phase 1 schema. This blocks sessions/content and creates persistent boot-time migration failures. | ✅ | ⬜ | Regression test validates all five `CREATE TABLE IF NOT EXISTS` statements, consent FK, indexes, no markdown/prose |
