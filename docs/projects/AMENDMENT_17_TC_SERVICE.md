...
| **Last Updated** | 2026-05-10 — Critical syntax repairs: restored `services/tc-document-validator.js` from the last known-good version after a system-build truncation left TC document QA routes throwing syntax errors, and deleted resurrected orphan `services/tc-webhook-validator.js` after system-build commits reintroduced a truncated 12-line file that failed `node --check`; prior 2026-05-09 cleanup had already established the webhook-validator orphan was never imported and not part of owned TC runtime |
...
|---|---|---|---|---|---|
| 2026-05-10 | **Restored `services/tc-document-validator.js` after builder truncation.** System-build commit `c8c55333` ("TC document validator — missing-signature + required-field detection") replaced the 313-line fail-closed validator with six lines ending inside the `DATE_RE` regular expression. Restored the last known-good `dacd2808` ESM implementation: `createTCDocumentValidator()`, `validateFile()`, `validateFiles()`, required document profiles, fact extraction, OCR fallback, and fail-closed `blocks_upload` behavior. | Concrete trigger: `node --check services/tc-document-validator.js` failed with `SyntaxError: Invalid regular expression: missing /`. TC routes dynamically import this validator for manual QA/upload and inspection package paths, so affected endpoints would throw on use instead of fail-closed validation. | ✅ | ✅ | `node --check services/tc-document-validator.js` reproduced failure before restore. Post-restore: `node --check services/tc-document-validator.js` PASS; import smoke with `services/tc-doc-intake.js` prints `function`; top-level runtime syntax sweep PASS (existing files under `server.js`, top-level `routes/`, `services/`, `startup/`, `config/`, `core/`, `middleware/`, `scripts/`, `tests/`; deleted files skipped); `node scripts/verify-project.mjs --project tc_service` syntax checks PASS but verifier still has 4 pre-existing route-contract failures (400/500 from missing test payload/mailbox), not caused by this restore. |
| 2026-05-10 | **Deleted resurrected orphan `services/tc-webhook-validator.js`.** Recent system-build commits reintroduced the file after the 2026-05-09 cleanup had removed it as truncated and never imported. The resurrected file contained only partial imports and ended mid-module-specifier (`../services/tc-asana-sync`), with no `@ssot` header, no exports, and no route wiring. | Concrete trigger: `node --check services/tc-webhook-validator.js` failed with `SyntaxError: Invalid or unexpected token`. The file is not referenced by runtime imports (`routes/tc-routes.js` owns `/webhooks/postmark` and `/webhooks/twilio`), but leaving broken JS in the tree breaks whole-repo syntax verification and risks future boot failure if imported. Deleting is safer than inventing an unmounted validator implementation. | ✅ | n/a (file intentionally not owned/listed) | `node --check services/tc-webhook-validator.js` reproduced failure before delete. Post-delete: `test ! -e services/tc-webhook-validator.js` prints `deleted`; top-level runtime syntax sweep PASS with deleted files skipped. |
