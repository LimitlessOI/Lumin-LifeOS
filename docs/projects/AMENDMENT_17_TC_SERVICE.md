...
| **Last Updated** | 2026-04-01 (`tc-email-document-service`: multi-mailbox search — INBOX + Sent (include_sent, default on); IMAP server-side TEXT pre-filter via address_hint; each result tagged with mailbox so downloadAttachmentsForEmail opens the correct folder. `tc-routes`: buildInspectionMailboxSearch passes include_sent:true + address_hint from tx.address by default) |
