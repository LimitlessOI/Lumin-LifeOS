...
*   **Library vs. Custom Build:** Decision on whether to leverage an existing, Node.js-compatible BPE library (e.g., via FFI) or to develop a pure JavaScript custom implementation.
...
*   **Task 2.1.2: BPE Library Research:**
    *   Investigate existing Node.js-compatible BPE tokenizer libraries. Evaluate their features, performance, licensing, and ease of integration.
    *   Assess feasibility of using WebAssembly or native Node.js addons for performance-critical components if a pure JS solution is insufficient.
