...
Define the Mongoose schema and model for `LifeCoachingSession`. This slice focuses solely on the data structure and its persistence definition, without introducing apiEPs or business logic.
...
4.  Basic Persistence Test (Optional, but recommended for full proof): Create a new `LifeCoachingSession` instance, save it to the db, and then retrieve it, verifying data integrity.
