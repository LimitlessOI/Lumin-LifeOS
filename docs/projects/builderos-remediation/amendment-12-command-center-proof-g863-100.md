...
Implement a stub for `CommandCenterService` that includes basic initialization and a method to "register" or "log" a BuilderOS task. Define the initial `CommandCenterTask` db schema to support task ID, status, and creation timestamp. This slice focuses on establishing the core service structure and verifying basic data persistence.
...
-   `src/config/db.js`: Ensure the new `CommandCenterTask` model is registered and accessible via the db connection.
...
-   Confirm that the db connection successfully registers the `CommandCenterTask` model.
-   Execute `CommandCenterService.logTask('task-123', 'PENDING')` and verify that a corresponding entry exists in the `commandCenterTasks` collection/table in the db.
-   Retrieve the logged task via a direct db query or a new service method (e.g., `getTaskStatus('task-123')`) and assert its correctness (e.g., `status` is 'PENDING').
...
-   The `CommandCenterTask` model fails to register with the db ORM.
-   `CommandCenterService.logTask()` fails to persist data to the db, or the persisted data is incorrect/incomplete.
