...
-   Data Source for Adam Items: Which specific db table(s) or external service(s) hold the "Adam" items? (e.g., `builder_tasks` table, `adam_reviews` service).
...
1.  Backend API Endpoint: Create a new BuilderOS internal apiEP (e.g., `/api/builder-os/adam-tasks/pending`) that returns:
...
2.  Frontend Data Fetching: In the existing "Pending Adam" panel component, integrate a call to this new apiEP.
...
-   `src/builder-os/services/adamService.js`: (If applicable) Add a function to abstract db queries for Adam items.
...
-   Integration Tests:
-   apiEP `/api/builder-os/adam-tasks/pending` returns a `200 OK` with expected `count` and `items` structure.
...
-   Runtime Checks:
-   Ensure robust errHdl for API failures (e.g., network issues, server errors).
