...
- Verify the emitted event payload contains `campaignId` and `userId`.
- Integration Tests (if applicable, outside this slice):
- Verify the `marketing_campaign_impression` event appears in the `marketing_event_stream` with correct data after a simulated impression.
5. Stop Conditions if Runtime Truth Disagrees
- If unit tests for `MarketingEventService.test.js` fail, indicating incorrect event emission or payload structure.
- If the `marketing_campaign_impression` event is not observed in the `marketing_event_stream` or contains malformed data during integration testing.
- If the `campaignId` or `userId` fields are missing or incorrect in the emitted event payload.