...
The core `CoachingSession` data model definition and the initial API endpoint to create a new coaching session are missing. Specifically, the system lacks the ability to persist the state of a new coaching session and expose an endpoint for users to initiate this process.

### 2. Smallest Safe Build Slice to Close It

Implement the `CoachingSession` data model (e.g., Mongoose schema) and the `POST /api/v1/life-coaching/start` API endpoint. This endpoint will be responsible for:
1.  Receiving a request to start a new session.
2.  Creating a new `CoachingSession` record in the database.
...
3.  **Database Inspection:** Query the database directly (e.g., using MongoDB Compass or `mongo` shell) to confirm that a new `CoachingSession` record exists with the returned `sessionId`, the correct `userId`, `status: 'initialized'`, and a non-empty `currentPrompt` (even if placeholder).
