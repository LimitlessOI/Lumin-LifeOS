...
The smallest safe build slice involves defining the TypeScript type for `AICouncil` and creating the corresponding database schema migration to establish the `ai_council` table. This provides the necessary structural foundation without introducing any business logic or API endpoints.
...
*   **Database Schema Check:** After running migrations, verify the existence of the `ai_council` table in the database.
...
*   **TypeScript Compilation Check:** Ensure the project compiles without errors, confirming `src/types/AICouncil.d.ts` is correctly integrated and recognized by the TypeScript compiler.
    *   Command: `tsc --noEmit`

### 5. Stop Conditions if Runtime Truth Disagrees

*   If the `ai_council` table does not exist in the database after migration.
...
*   If TypeScript compilation fails due to issues related to the `AICouncil` type definition.