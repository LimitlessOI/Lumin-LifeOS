...
*   `services/marketing-integration/src/middleware/marketingConsentMiddleware.js`: (NEW FILE) Implement the middleware logic to fetch consent and enforce rules.
...
*   `services/marketing-integration/src/utils/marketingApiAdapter.js`: (If applicable) Modify any direct API call wrappers to integrate the middleware or consent check.
...
*   **Unit Tests (`marketingConsentMiddleware.js`):**
    ...
    *   Test cases for error handling when `UserPreferencesService` is unreachable.
*   **Integration Tests (`marketing-integration` service):**
    *   Simulate an authenticated user with `marketingConsentStatus: 'denied'` attempting to subscribe to a newsletter via a LifeOS API endpoint that triggers a MarketingOS call. Assert that the MarketingOS call is *not* made or is made with a `denied` flag.
