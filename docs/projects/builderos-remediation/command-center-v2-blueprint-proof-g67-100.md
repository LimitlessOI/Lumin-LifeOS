...
Implement the `CommandDefinition` domain entity and a basic repository for its persistence. This slice focuses purely on defining the data structure and providing fundamental CRUD (Create, Read, Update) operations for `CommandDefinition` entities, without exposing any external API endpoints or UI components. The initial implementation can leverage an in-memory store or a mock database client to prove the data flow and persistence logic.
...
*   Manual inspection (if using an in-memory store or mock) or database queries (if connected to a real DB) should confirm that `CommandDefinition` data is correctly persisted and retrieved according to the repository operations.
