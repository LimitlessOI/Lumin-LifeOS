<!-- SYNOPSIS: Documentation — Amendment 41 Marketingos Proof G421 100. -->
...
-   Discrepancy Rate Threshold: The overall discrepancy rate (sum of all types of discrepancies) must not exceed 0.5% for any given verification cycle.
5. Stop Conditions if Runtime Truth Disagrees
-   Persistent Record Count Mismatch: If the record count mismatch (exceeding 0.01% tolerance) persists for 3 consecutive verification cycles.
-   Persistent Key Field Inconsistency: If critical field mismatches (for the 1000-record sample) persist for 2 consecutive verification cycles, or if the number of inconsistent records exceeds 1% of the sample.
-   Persistent Timeliness Violation: If the timeliness delta (exceeding 5 minutes) persists for 3 consecutive verification cycles for more than 0.1% of sampled records.
-   Unacceptable Discrepancy Rate: If the overall discrepancy rate (sum of all types of discrepancies) exceeds 0.5% for 2 consecutive cycles.
-   Action: In any of these cases, the verification service should:
    -   Trigger a P1 alert to the data engineering and MarketingOS teams.
    -   Automatically pause further data ingestion from the source into MarketingOS until the discrepancy is investigated and resolved.
    -   Log detailed diagnostic information for root cause analysis.