...
Define the `LifeCoachSession` schema, implement a db migration to create the corresponding table, and expose a basic RESTful apiEP (`/api/v1/life-coaching/sessions`) for creating and retrieving these sessions. This slice focuses solely on the persistence and basic access layer for the core entity.
...
-   Database Migration:
-   Run `npm run db:migrate`. Verify that the `life_coach_sessions` table is created in the db with the expected columns and indices.
...
-   apiEPs return 4xx/5xx errors for valid requests.
