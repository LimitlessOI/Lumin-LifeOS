...
The ability to define a `Command` entity with a basic status and expose an internal API endpoint to retrieve this status by ID. This proves the foundational data model and a read-only access pattern for command state.

**2. Smallest safe build slice to close it:**
Implement a basic `Command` data model (e.g., `id`, `name`, `status`) and an internal API endpoint `/api/internal/command-center/:commandId/status` to fetch a command's status. This slice focuses purely on read operations to minimize risk and establish the core data access pattern.
...
*   `src/tests/api/internal/command-center/status.test.js` (New file: Unit/integration tests for the status API endpoint.)
...
*   The API endpoint `/api/internal/command-center/:commandId/status` does not return a 200 OK for a valid, known `commandId`.
