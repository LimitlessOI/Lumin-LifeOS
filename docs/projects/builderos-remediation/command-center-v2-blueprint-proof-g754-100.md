...
Implement a minimal GraphQL schema and resolver to expose a single, hardcoded system health status. This slice focuses on establishing the core GraphQL server infrastructure within the existing Node backend, without requiring complex data integration or frontend development at this stage.
...
1.  Start the Node backend server (e.g., `npm run dev` or `node dist/index.js`).
...
-   The Node server fails to start due to dependency issues, port conflicts, or TS compilation errors.
