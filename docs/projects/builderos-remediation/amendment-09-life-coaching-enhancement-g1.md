...
-   Focus exclusively on foundational data model definitions and database schema updates.
...
-   Implement necessary database schema migrations for these model changes.
-   No API endpoints, UI components, or business logic beyond data persistence.
...
-   `src/db/migrations/<timestamp>-add-coach-profile-and-link-user.js` (new file, database migration script)
-   `src/types/db.d.ts` (amend to add `CoachProfile` type and update `User` type, if TypeScript is in use)

5. Required Verifier/Runtime Checks
-   Successful execution of the database migration script.
-   `CoachProfile` table exists in the database with expected columns.
...
6. Stop Conditions
-   `CoachProfile` data model is fully defined, implemented, and migrated to the database.
...
ASSUMPTIONS:
- The project uses a typical Node.js/ESM structure for database models (e.g., `src/db/schemas`).
- Database migrations are managed via scripts (e.g., `src/db/migrations`).
- TypeScript type definitions are maintained for data models (e.g., `src/types/db.d.ts`) as part of "production-quality" code.