...
*   `src/migrations/2024xxxx_add_governance_model_to_projects.js` (for database schema update and initial data defaulting for existing projects)
...
*   **Data Persistence:** Confirm that a project created or updated with a valid `governanceModel` value successfully persists this value in the database.
...
*   **Migration Check:** After running the migration, verify that existing projects without a `governanceModel` field are correctly defaulted (e.g., to `standard`) in the database.

**5. Stop Conditions if Runtime Truth Disagrees:**
*   If schema validation allows invalid `governanceModel` values to pass.
*   If `governanceModel` values are not correctly persisted or retrieved from the database.
