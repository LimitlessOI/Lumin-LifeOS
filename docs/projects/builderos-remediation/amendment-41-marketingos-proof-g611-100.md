<!-- SYNOPSIS: Documentation — Amendment 41 Marketingos Proof G611 100. -->
...
-   Action: Send a `GET` request to `/api/marketingos/campaigns/{campaignId}`.
-   Assertion 2: HTTP status code is `200 OK`.
-   Assertion 3: The retrieved `MarketingCampaign` data matches the updated payload.
-   Assertion 4: The `isSSOT` flag (or equivalent SSOT marker field) within the retrieved campaign data is `true`.
5. Stop Conditions if Runtime Truth Disagrees
The proof is considered *not closed* if any of the following conditions are met during the execution of the `campaigns.ssot.test.ts` integration suite:
-   Any HTTP request (POST, PUT, GET) returns an unexpected status code (e.g., 4xx, 5xx instead of 2xx/201).
-   The data retrieved via the SSOT query interface (`GET /api/marketingos/campaigns/{campaignId}`) does not precisely match the data sent during creation or update.
-   The `isSSOT` flag (or equivalent SSOT marker field) for a `MarketingCampaign` entity is `false` or missing after creation or update, indicating a failure in the SSOT propagation mechanism.
-   The integration tests fail to execute due to environmental issues, missing dependencies, or unhandled exceptions within the MarketingOS service.
In such cases, the build pass should halt, and the specific test failure details should be escalated for immediate investigation by the MarketingOS team to identify the root cause of the SSOT data inconsistency or API malfunction.