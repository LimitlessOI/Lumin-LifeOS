...
*   `services/marketing-consent-auditor/config.js` (Configuration for audit frequency, thresholds, MarketingOS API endpoint)
*   `models/MarketingConsentAuditLog.js` (New Mongoose/Sequelize model for storing audit results and discrepancies)
*   `routes/internal/marketing-consent-audit.js` (New internal API endpoint for triggering audits manually and checking auditor status/metrics)
*   `tests/services/marketing-consent-auditor.test.js` (Unit and integration tests for the new auditor service)
*   `jobs/runMarketingConsentAuditor.js` (New scheduled job definition to invoke the auditor service)

## 4. Verifier/Runtime Checks

*   **Audit Log Monitoring:** Monitor `MarketingConsentAuditLog` for new entries where `discrepancyCount > 0`.
*   **Alerting System:** Verify that alerts are triggered when `discrepancyCount` exceeds