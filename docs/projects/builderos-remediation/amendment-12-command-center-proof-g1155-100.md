...
Define the Mongoose schemas and models for `Command` and `CommandLog` to establish the core data structures required for Command Center functionality. This slice focuses solely on the data model definition, without implementing any business logic or API endpoints.
