...
Implement a dedicated `logAiCouncilDecision` function within the `aiCouncilService` that leverages an existing immutable logging mechanism (e.g., appending to a specific log file, writing to a blockchain-backed ledger, or a WORM-compliant database table) after each AI Council decision is finalized. This function will encapsulate the decision payload and ensure its persistence.
...
*   `src/lib/immutableLogger.js` (or similar existing immutable logging utility): If not already present, define or extend a utility for writing immutable records. If a database model is used, `src/db/models/aiCouncilDecisionLog.js` would be the target.
...
*   **Integration Tests:** Deploy a test instance. Trigger an AI Council decision via an API. Query the designated immutable log store (e.g., database table, log file, blockchain explorer) to confirm the presence and correctness of the logged decision record.
*   **Manual Inspection:** After a decision, manually inspect the log output or database table for the specific decision ID and its associated immutable record.
