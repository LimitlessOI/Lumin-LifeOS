...
Implement the `WhiteLabelConfig` data model and a `getWhiteLabelConfigByTenantId` function within a new or existing configuration service. This slice focuses purely on data definition and backend retrieval, isolated from apiEPs or UI consumption.
...
-   Integration Test (Manual/Local): After deployment to a dev environment, use a db client to insert a sample `WhiteLabelConfig` for a known `tenant