...
4.  **Idempotency Check:** Emit the *same* event again. Verify that the `updatedAt` timestamp in the database reflects the second processing, but no new records are created if the state is identical, and no errors occur.
5.  **Error Handling Test:** Emit a malformed `MarketingEventConsentUpdate` event (e.g., missing required fields, invalid `status` value). Verify that MarketingOS logs schema validation errors and does *not* update the database for the malformed event.
...
*   **Database State Mismatch:** If the database query for the `testUserId` and `consentType` does not reflect the expected `status` or `updatedAt` timestamp after successful log indications.
*   **Persistence Failure:** If MarketingOS logs indicate successful processing but the database query shows no change or an incorrect change.
