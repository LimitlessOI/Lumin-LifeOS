...
Implement a dedicated event handler for `CampaignConversionEvent` that extracts the `conversionId`, `campaignId`, and relevant metadata (e.g., `timestamp`, `source`) and persists this data to the `marketing_events` database table. This handler must be idempotent and include basic error handling for database operations.
...
*   `src/marketingos/data/marketingEventRepository.js` (to add or extend a method, e.g., `saveConversionEvent({ conversionId, campaignId, timestamp, source })`, for database interaction)
...
*   **Integration Test (End-to-End):** Simulate the emission of a `CampaignConversionEvent` via the `eventBus`. After a short delay, query the `marketing_events` database table directly to confirm the presence of a new record matching the emitted event's `conversionId` and `campaignId`.
...
*   If `campaignConversionEventHandler` logs show a persistent error rate exceeding 0.5% for processed events (e.g., database connection failures, schema validation errors).
*   If the `conversionId` or `campaignId` values stored in the database do not precisely match the values from the original emitted `CampaignConversionEvent` payload.
