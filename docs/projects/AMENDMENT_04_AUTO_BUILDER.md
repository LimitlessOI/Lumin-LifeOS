...
**Last Updated:** 2026-06-29 — BP priority scheduler now honors FOUNDER_STOP even with never-stop enabled and serializes expansion cycles through the scheduler running guard; LifeRE blueprint complete (machine alpha 12/12 PASS); Point B advanced to LifeOS Consumer Auth; BUILDEROS_NEVER_STOP added to managed env allowlist; LifeOS user auth blueprint added (rank 11); scheduler boot-window detection gap fixed; never-stop-product-factory.js + never-stop-product-factory-scheduler.js committed (were missing from git, causing Railway crash).
...
| 2026-06-29 | **Critical scheduler safety repair** — `services/builderos-bp-priority-scheduler.js` now evaluates `FOUNDER_STOP.json` before any never-stop expansion work and returns `founder_stop_active` even when `BUILDEROS_NEVER_STOP=1`; expansion cycles now run through the same `state.running` single-flight guard used by BP queue cycles. Added `tests/builderos-bp-priority-scheduler.test.js`. | Recent never-stop wiring let explicit founder halt be bypassed and allowed interval ticks to overlap long expansion/build/redeploy cycles, risking autonomous over-execution and token/redeploy churn. | ✅ `node --check services/builderos-bp-priority-scheduler.js`; ✅ `node --test tests/builderos-bp-priority-scheduler.test.js` | deploy |
