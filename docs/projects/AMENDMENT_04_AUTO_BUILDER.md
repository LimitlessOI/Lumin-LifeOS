...
**Last Updated:** 2026-06-20 — direct terminal intake now emits gate-readable founder packets and intent baselines
...
| 2026-06-20 | **BUG FIX: `scripts/run-founder-intake-direct.mjs` + `factory-staging/factory-core/arc/department-simulations.js` + `factory-staging/factory-core/arc/foundation/coverage-map.js` + `tests/run-founder-intake-direct.test.js`** — structured Founder Interface requests are now rendered as real `FOUNDER_PACKET.md` sections (`## Problem`, `## Desired Outcome`, `## FOUNDER SUCCESS TEST`, etc.) instead of being hidden inside a raw fenced block; the script also writes `INTENT_BASELINE.json` with `status: HANDOFF_READY`, `direct_terminal_intake: true`, and the minimal `ASSET_REUSE_DECISION.json` required by IDC. CFO simulation and intent coverage now accept only that explicit direct-intake marker as priority evidence outside `BP_PRIORITY`; normal product missions still require the queue. Added a CLI regression test that runs a structured direct intake through development-stage gates and cleans its throwaway mission. | Recent terminal-only founder execution routed plain commands into foundation gates, but the script discarded the normalized structure, never created required pre-ARC artifacts, and left direct founder commands blocked as `department:SNT` / `priority_unconfirmed` / `priority_fit:MISSING` / missing asset reuse before execution. | ✅ structured direct intake reaches development-stage PASS with no first_blocker; product BP queue gating preserved for non-direct missions | `node --check scripts/run-founder-intake-direct.mjs`; `node --check factory-staging/factory-core/arc/department-simulations.js`; `node --check factory-staging/factory-core/arc/foundation/coverage-map.js`; `node --test tests/run-founder-intake-direct.test.js`; `npm test`; `npm run builder:preflight`; `node scripts/ssot-check.js --all` |
...
- 2026-06-20: Critical bug fix complete for direct terminal founder intake. Structured Founder Interface payloads now produce gate-readable `FOUNDER_PACKET.md`, `INTENT_BASELINE.json`, and `ASSET_REUSE_DECISION.json`; development-stage repro passes with no first blocker. Next priority: run live Founder Interface `/founder-interface/message` validation after deploy to confirm production route uses the repaired terminal script.
