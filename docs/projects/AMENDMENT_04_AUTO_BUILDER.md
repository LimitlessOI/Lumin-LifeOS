...
**Last Updated:** 2026-05-09b — Fix Railway GQL `meta` scalar field (was querying subfields; Railway v2 `DeploymentMeta` is a scalar JSON type). Prior: 5 new routes in `routes/railway-managed-env-routes.js` (GET /deployments, /deployments/latest, /deployments/:id/logs, /logs/local, /logs/local/:name). Prior 2026-04-27 — `build-from-latest` endpoint added. Prior: 2026-04-25 (atomic key rotation)
