...
| 2026-06-26 | **FPv2 double-gate fix** — intake clearance in `dispatchTask` + `target_file` on internal `/build` dispatch. | MOS-P1-001: `/build` cleared FPv2 but `dispatchTask` re-blocked INTENT_AMBIGUITY. | ⚠️ deploy + re-run | `blueprint:intake:execute` |
