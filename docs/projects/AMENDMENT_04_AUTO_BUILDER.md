...
**Last Updated:** 2026-06-19 — mission-lib path traversal guard blocks blueprint writes outside sandbox
...
| 2026-06-19 | **CRITICAL BUG FIX: `builderos-reboot/scripts/mission-lib.mjs`** — `writeFileExactStep()` now rejects absolute paths and `.`/`..` traversal segments before source reads, sandbox checks, legacy-spine checks, or target writes. Added `tests/mission-lib-path-safety.test.js` to prove `builderos-reboot/MISSIONS/SAFE/../../../server.js` cannot pass a raw-prefix sandbox and overwrite `server.js`; unsafe `content_source_path` traversal is also blocked before read. | Recent autonomous BP runner/execute wiring made blueprint file-write safety load-bearing; raw string sandbox matching could allow malformed blueprint steps to escape the declared sandbox after `path.join()` normalization. | ✅ traversal target/source blocked locally | `node --test tests/mission-lib-path-safety.test.js` |
