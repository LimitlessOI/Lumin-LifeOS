<!-- SYNOPSIS: Communication-first execution blueprint — 8 phases sequenced from the 2026-08-02 independent audit. Phases 2, 4, 6, 7 complete and verified live; Phase 1 is fully installed except 1.4 (crisis-intervention, deliberately held) and half of 1.1 (needs Adam's own secret, not a decision); Phase 5 split into 5.1a (closed) and 5.1b (open founder decision); Phase 8 (Institutionalization) reclassifies founder-blocked items and pushes back on building new tracking infrastructure before auditing what exists. -->

# Blueprint — Communication First, Everything Gated and Enforced

**Status:** PROPOSED, actively executed — sequenced from the 2026-08-02 independent audit findings. Nothing here is ratified; nothing is built merely by this document existing. Phases 2, 4, 6, and 7 are complete and verified live on `main`. Phase 1 is fully installed and live-verified except two things that are not mine to close: 1.4 (crisis-intervention wiring, deliberately held — the blueprint's own highest-consequence item) and half of 1.1 (a real second-factor secret only Adam can supply). Phase 5 split into 5.1a (closed) and 5.1b (an open founder decision, bigger than the original item). Phase 3 remains a pure product decision. Phase 8 is a live self-application of a founder-relayed critique about shrinking what genuinely needs founder attention.
...
- **1.1**: option (a) logging of every fallback-auth use is shipped and verified live — that half is genuinely done. The remaining half (a real second-factor: SMS/TOTP code, or a second passphrase) cannot be built without Adam personally providing the actual secret — a phone number, or a chosen passphrase value. This is not a decision being deferred; it's an input only Adam can supply. Accepting the current risk as final (no second factor, logging only) is also a valid choice, but it's still Adam's to make.
