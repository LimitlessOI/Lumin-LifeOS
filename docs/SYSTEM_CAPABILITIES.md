...
| **V6** | **Builder operator suite (composite)** | `npm run tsos:builder` → `scripts/builder-operator-suite.mjs` | Runs **V2** + **`lifeos:builder:probe`** + **V4** + **V7** + daemon summary + **V8** sequentially | Same as V2 + V4 + command key (**required** for V8) | **SHIPPED** (script) | **Exit:** first failing step wins. Step **6** = operational grade (**V8**). See `docs/BUILDER_OPERATOR_ENV.md`. |
| **V7** | **Token efficiency scorecard** | `npm run tsos:tokens` → `scripts/tsos-token-efficiency.mjs` | `GET /api/v1/twin/free-tier`, `GET /api/v1/twin/tokens`, `GET /api/v1/twin/tokens/history` | `PUBLIC_BASE_URL`, command key alias; optional `TSOS_ENFORCE_TOKEN_GRADE=1`, `TSOS_MIN_TOKEN_GRADE` | **SHIPPED** (script) | Computes weighted **free-tier remaining %** (request budget), today avg token savings %, saved tokens/USD, and 7-day trend; emits **A/B/C/D/F** efficiency grade and optional fail-closed gate; appends local receipt at `data/token-efficiency-log.jsonl`. **Orthogonal** to **V8** (savings % ≠ deploy/LifeOS surface proof). |
| **V8** | **LifeOS operational grade (0–100)** | `npm run lifeos:operational-grade` → `scripts/lifeos-operational-grade.mjs` | **`/healthz`**, **`GET …/builder/ready`** (**policy_revision** vs repo), **`/domains`**, **`/gaps`**, **`GET …/system/builder-health`**, **`GET …/ambient/nudge`**, public **`/overlay/lifeos-dashboard.html`** + **`lifeos-app.html`** | `PUBLIC_BASE_URL`, **`COMMAND_CENTER_KEY`** (required); optional **`TSOS_MIN_OPERATIONAL_SCORE`** (default **90**), **`TSOS_ENFORCE_OPERATIONAL_GRADE=0`** observe-only | **SHIPPED** (script) | Writes **`data/lifeos-operational-grade-last-run.json`** + JSONL log. Fails closed if critical probe fails (deploy drift, builder missing) or score below minimum. **Shell acceptance:** `docs/LIFEOS_SHELL_ACCEPTANCE.md`. |
| **C1** | **Real multi-model gate-change** | `POST /api/v1/lifeos/gate-change/run-preset` etc. | Council provider keys | `COMMAND_CENTER_KEY` + `PUBLIC_BASE_URL` for CLI | **SHIPPED** | See `npm run lifeos:gate-change-run`. Round 3 synthesis E/K v2.7. |
| **C2** | **Deliberation governance v2.7** | `GET/POST /api/v1/lifeos/deliberation/*` incl. pipeline/debrief/reps; council `/build` auto-seed+finalize | Neon: `20260609_deliberation_governance_v27.sql` + `20260609b_founder_debrief_rep_catalog.sql` | `COMMAND_CENTER_KEY` + `PUBLIC_BASE_URL` | **SHIPPED** | Roster, Hist, CFO, consensus, scorecard, gate, Founder Debrief. `npm run lifeos:deliberation:verify` + `lifeos:deliberation:a-to-z-smoke`. |
| **SB1** | **Site Builder async prospect (build + cold email)** | `POST /api/v1/sites/prospect` → **202** + `clientId`; poll `GET /api/v1/sites/prospects/:clientId/status`; bulk via `POST /api/v1/sites/bulk-prospect`; readiness `GET /api/v1/sites/launch-readiness` | `EMAIL_PROVIDER=smtp` + `EMAIL_FROM` + `SMTP_USER` + `SMTP_PASS` (Lumea Gmail) **or** Postmark; `SITE_BASE_URL`; `STRIPE_SECRET_KEY` for publish path; `DATABASE_URL` for job tracking | `COMMAND_CENTER_KEY`, `PUBLIC_BASE_URL` or `SITE_BASE_URL`; `npm run site-builder:pilot` | **SHIPPED** (code) | Avoids Railway ~100s HTTP timeout on long AI builds. `sync=true` still available for local/debug. Pilot script polls until `sent`/`failed`. |
...
| Date | Change |
|------|--------|
| 2026-07-07 | **SB1 Site Builder async prospect:** `POST /api/v1/sites/prospect` returns **202** immediately; background build+email via `services/site-builder-prospect-runner.js`; poll `GET …/prospects/:clientId/status`; launch-readiness accepts Gmail SMTP or Postmark; `npm run site-builder:pilot` polls to completion. |
| 2026-06-09 | **C2 A→Z completion:** pipeline seed/finalize, Founder Debrief generator, REP boot sync, council `/build` deliberation hook, `npm run lifeos:deliberation:a-to-z-smoke`. |
| 2026-06-09 | **C2 Deliberation governance v2.7:** `/api/v1/lifeos/deliberation/*`, migration `20260609_deliberation_governance_v27.sql`, `npm run lifeos:deliberation:verify`. Gate-change auto-persists roster/Hist/CFO/consensus. |
...
| 2026-04-30 | **B2 / V2:** **`GET …/builder/ready`** **`codegen`** block (**`policy_revision`**, **`supports_max_output_tokens_body`**, **`html_output_estimator`**); **`npm run builder:preflight`** prints it. **`scripts/lifeos-builder-continuous-queue.mjs`** passes JSON task **`max_output_tokens`** into **`POST /builder/build`** (thin **`lifeos-builder-overnight.mjs`** wrapper still works). |
