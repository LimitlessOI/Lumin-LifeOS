...
**Corrected 2026-07-27 (found during a founder-directed constitution/SSOT audit — this section was describing the wrong mechanism):** `scripts/governed-overnight-backlog-run.mjs` is a **manual script** — it is not scheduled or started by `server.js` or any startup file, and does not run on its own. The mechanism that actually runs continuously in production, 24/7, independent of any interactive session, is **`services/governed-autonomous-shipping-loop.js`**'s `startGovernedAutonomousShippingLoop()`, started at boot from `server-founder-runtime.js` and self-gated on the live env flags `GOVERNED_FACTORY_ONLY=1` + `GOVERNED_AUTONOMOUS_SHIP=1`. It self-reschedules via `setTimeout` after every tick (event-driven cadence: fast when there's real work, slow idle poll otherwise) and — as of 2026-07-27 — carries its own watchdog (`state.lastTickAt` + a `setInterval` staleness check) that force-recovers the tick chain if it ever goes quiet without the whole process crashing. Status: `GET /factory/readiness`. Per-model outcome data: `GET /factory/model-rankings`. This is the loop to check first when asking "is the system actually still building" — not the overnight-backlog script.
...
3. **BUILDER-FIRST + PREFLIGHT (§2.11 / §2.11a — non-optional):**
   - **Fast path (recommended when shell has `PUBLIC_BASE_URL` + command key):** `npm run tsos:builder` — chains preflight, supervisor probe, TokenSaverOS doctor, token scorecard, daemon state summary, **LifeOS operational grade** (`docs/SYSTEM_CAPABILITIES.md` **V6–V8**). Surfaces-only proof: `npm run lifeos:operational-grade`. **Or** run steps separately:
...
**Queue updated 2026-07-27 — this file had gone stale since 2026-05-10 (over two months of real shipped work missing below); found during a founder-directed constitution/SSOT audit.** Full detail for everything shipped between the old queue and now lives in the relevant `docs/products/*/PRODUCT_HOME.md` Change Receipts (`builderos`, `site-builder`, `ai-council`, `lifere`, `tc-service`, `lifeos`) — this section names the real current priorities, not a full history.

0. **BuilderOS — builder path + pipeline integrity (constitutional P0, §2.11a)** — still P0, still true. `npm run builder:preflight`; governed loop is `services/governed-autonomous-shipping-loop.js` (see *Continuous Autonomous Operations* above for the corrected description of what's actually running). Per-model outcomes now tracked live: `GET /factory/model-rankings` (Tier-0 — measures, does not yet auto-select).
1. **LifeRE broker-readiness** — founder wants to present the full stack to brokers. Real and shipped: TC Service (admin-gated), CRM (BoldTrail), education/curriculum, 24 coaching modules, commitments tracker, real "Ask Chair" conversation panel. **Confirmed still missing**: the "find their superpowers, route them to matching daily tasks" personalization system (checked directly, does not exist under any name) — build this next.
2. **SENTRY finding classification rollout** — `root_cause_class` (`code_defect` \| `blueprint_defect` \| `unclassified_needs_review`) shipped 2026-07-27 on Layer A/B findings; next is giving Layer B's real-model walkthrough its own explicit schema field to classify root cause directly, rather than defaulting UX findings to `unclassified_needs_review`.
3. **Model benchmarking, full scope (North Star §2.0J)** — `model_capability_ledger` (shipped 2026-07-27) only covers the BuilderOS-execution role. The constitution names many more roles (AIC debate, BPB blueprinting, OIL adversarial review, Verifier, summarizer, historian, founder intent modeling, security review, external research) and more metrics (drift rate, hallucination rate, security catch rate, calibration accuracy) than are tracked today — real, named, unbuilt.
4. **Founder Intent Model / "Adam Simulator" (North Star §2.0H)** — constitutionally required, confirmed to not exist anywhere in the codebase under any name. Not started.
5. **Governance Evolution Law cadence (North Star §2.0G)** — "at fixed cadence, review which laws helped/hurt/caused drift" — no scheduler or mechanism for this exists. Not started.
6. **Mission-object attachment (Companion §0.9)** — "every meaningful action must attach to a mission record" (mission_id, state, predicted outcome, verification result). Real GAP-FILL commits are tracked via PRODUCT_HOME.md receipts today, which is good practice but not the same as the constitutionally-specified mission object. Real, structural gap — not yet reconciled.
7. **TC lane / GLVAR** — GLVAR Clareity MFA + browser-agent login now real and shipped (`services/tc-browser-agent.js`); a real security-invariants pre-commit guard (`scripts/security-invariants-check.mjs`) now protects `routes/tc-routes.js`'s admin-gating after it was regressed twice live in one session.
...
**Enforcement note (2026-07-27):** this contract had been silently unmet for ~2.5 months (see the audit note above) — no mechanical gate currently blocks a commit from shipping without a `QUICK_LAUNCH.md` update, unlike the SSOT/security-invariant checks in `githooks/pre-commit`. That's a real, named gap, not yet closed.

## Latest Completed (update each session)

- **2026-07-27 — constitution/SSOT audit + hard-gate pass (founder-directed: "read our constitution and all the SSOTs... what's missing? what's not hard gated?").** Found and fixed this file's own 2.5-month staleness (this entry); corrected the *Continuous Autonomous Operations* section, which was describing a manual script instead of the real always-on loop; found the SO-002 solution-mandatory rule was computed but never enforced. Same session also shipped: `services/model-capability-ledger.js` (real per-model-tier outcome tracking, `GET /factory/model-rankings`); `root_cause_class` on SENTRY findings (code-defect vs blueprint-defect distinction, `services/sentry-findings-to-improvement-feed.js`); a tick-heartbeat watchdog on the governed autonomous ship loop; a real "Ask Chair" conversation panel + guided tour in the LifeRE dashboard wired to the same engine as LifeOS chat; a commitments/to-do tracker wired into the live production lane; a multi-provider free-tier failover fix for the AI spend-cap gate; and — twice — a critical `requireLifeOSAdmin` regression on all TC Service routes caused by unrelated autonomous-factory commits, closed both times and finally closed structurally with `scripts/security-invariants-check.mjs` (new pre-commit hard gate). Full detail in `docs/products/builderos/PRODUCT_HOME.md`, `docs/products/ai-council/PRODUCT_HOME.md`, `docs/products/lifere/PRODUCT_HOME.md`, `docs/products/tc-service/PRODUCT_HOME.md` Change Receipts.
- **NSSOT audit pass (2026-05-10):** Compliance officer run: 9/10 gates green. Builder hardening: JS min-line floor in `validateGeneratedOutputForTarget()` (rejects <15-line JS as truncated); spec contamination gate (rejects HTML/code in spec); `commitToGitHub()` now returns `{ ok, sha }`; `createAutonomyGoldenTag` creates rollback tags on every `/build` commit. 3 builder-truncated syntax errors fixed (`tc-document-validator`, `tc-webhook-validator`, `site-builder-routes`). Dashboard CSS violations cleared (`lifeos:supervise:static` passes). 1 remaining `ssot_validate` gap self-resolves via builder daemon. See `AMENDMENT_21` + `AMENDMENT_17` + `AMENDMENT_05` Change Receipts.
