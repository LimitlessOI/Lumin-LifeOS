...
### 17a. Scaling ONE instance (within-instance parallelism)

Founder-dictated 2026-07-18: the scaling ceiling isn't worker count, it's the shared serialization points every worker eventually has to land through — one Git branch, one Railway deploy target, one DB, one token budget (evidenced live: this session hit repeated git-lock/merge collisions from just ONE autonomous loop sharing a working directory). The design: **one repo, many branches — each factory builds in its own isolated branch/worktree (closes the working-tree-isolation gap named alongside D1-D6, not yet ratified), each branch gets an ephemeral preview Railway environment + its own Neon DB branch (Neon already does prod/sandbox branching — extend, don't invent) so SENTRY verification (Layer B included) also runs in parallel per-branch, not only against the one live deploy — a single fast automated merge queue serializes the actual landing onto `main` (real conflicts should be rare by construction since lanes already avoid working the same product concurrently; the residual friction hit live this session was almost entirely on incidentally-shared generated files — the synopsis index (D5) and the giant single-line PRODUCT_HOME changelogs (idea #4, still unbuilt) — both become LOAD-BEARING for scaling, not just cleanup) — deploys are time-batched/debounced, not triggered per commit (`coalescedRedeploy` + D2's watchPatterns-skip already do half of this) — DB migrations ride the same merge queue as code, no separate contention path — active concurrency (how many factories run right now) is governed by remaining token budget (SO-003's existing daily cap), not a fixed constant, so it scales up when budget allows and idles down as it's consumed.** THINK not KNOW: whether Railway's actual plan supports per-branch ephemeral environments natively needs confirming before this is scoped, not assumed.

### 17b. Competing dual-instance BuilderOS (a SEPARATE, independent instance — not more of 17a)

Founder vision, 2026-07-18, refined with specifics in a follow-up turn. Important distinction the founder's own framing makes clear: 17a is about parallelism *within* one instance (one repo, many branches, converging to one shared truth via a merge queue) — this is the opposite case. Once this system is proven stable as autonomous building infrastructure (founder's own stated precondition — not yet met), clone it into a second, genuinely **independent** instance: its own repo, own Railway deployment(s), own token budget, own copy of 17a's scaling design once that's built. Unlike 17a, a second repo is the *right* call here, specifically because the two instances are not supposed to converge into shared state — only proven improvements cross over.

**Competition dimensions (founder: "I don't care [which specific one], we want continuous improvement" — multi-dimensional, not a single metric):** efficiency (better results per token spent), idea/product quality, and open-ended systemic improvement — any real gain either instance finds counts.

**Scoreboard:** which instance is doing better, **and why** — reasoning required, not just a number. Directly reuses Section H's reviewed ideas #16 (single founder reality dashboard) and #17 (weekly compounding-trend report, folded into digest work) as the building blocks — this is those two ideas applied across two instances instead of one.

**Sharing gate:** a proven improvement (a fix, a doctrine addition, a pattern) is *required* to propagate to the other instance, but only **after it's been proven and tested** — never a raw/unverified idea. Reuses existing machinery rather than inventing a new gate: the same SO-002 solution-mandatory + proof requirement, D3's `known_bad_signatures` pattern (the inverse — a proven-good signature instead of a proven-bad one), and D7's own reviewed idea of cross-instance fix propagation via the consensus protocol, all already in this doctrine.

Not scoped, not designed, not started. Named here as a real founder-owned direction so it survives to whenever the precondition is met, per §16's own rule (D6): a decision this size is a founder/Architect blueprint call, not something to improvise into existence. Rough shape a future Architect pass would need to resolve: (1) how "improvement" is measured comparably across two instances working on different products, (2) the concrete sharing protocol/format for a proven fix crossing instances, (3) whether the scoreboard itself is a shared artifact both instances can read (and if so, how that one shared thing avoids becoming its own bottleneck).
...
| D7 | **The repair pipeline (precise chain, supersedes the looser D7/D8 first written 2026-07-18): SENTRY finds → SENTRY proposes a solution (SO-002 solution-mandatory rule — no finding without a fix, already law) → SENTRY reports to Chair → Chair reviews the proposed solution; if it disagrees, Chair and SENTRY have a brief discussion to reconcile; Chair never fixes anything itself. If the brief discussion does NOT resolve, it does not stall or get informally flagged — it is hard-gated straight into the EXISTING consensus protocol (§16's own recovery_ladder_v2 `consensus_protocol`: unanimous, dissent-encouraged-not-required, `services/council-service.js` + `services/builder-council-review.js` — the same mechanism, not a new one). Once resolved (by agreement or by consensus), Chair passes it to Architect → Architect corrects the BLUEPRINT to reflect the fix (never touches code directly), sets urgency (urgent = top of queue, next thing worked on), and tells the queue what part of the blueprint is now a repair → the queue applies D6's decision-check (does executing this require a judgment call? yes → back to Architect; no → hand to the Factory to actually build).** Also periodic, not only reactive: SENTRY audits the system on a standing cadence (queue health, escalation depth, doctrine drift), not only at each step's ship-time. | Founder-ratified directive, 2026-07-18 (dictated precisely; corrects an earlier draft of this same doctrine that had Chair "tightening BPs directly," which is wrong — only Architect touches the blueprint. Founder also corrected the unresolved-disagreement path: reuse the existing consensus protocol, hard-gated, rather than a new ad hoc "disputed" flag). Generalizes what caught the LifeOS gap (§12) and the D3 repeat-regression, both found by a one-off manual audit — this makes that audit recurring and gives every finding a defined path to a blueprint fix instead of dead-ending as a report. | **Not yet built** — cadence/scope + the Chair↔SENTRY/Chair↔Architect handoff mechanics need Architect scoping |
| D8 | **Chair proactively interviews the founder to ensure blueprint coverage for every product/idea he's ever raised** — not just the ones already formalized. Directly closes the exact failure mode found live: LifeOS (the founder's own stated #1 priority) has zero documented backlog because nobody was asked what belongs in it. | Founder-ratified directive, 2026-07-18, in direct response to the LifeOS empty-backlog finding (§12). | **Not yet built** — needs an actual interview/capture mechanism, not just a standing instruction |
| D9 | **Housekeeping during Railway redeploy waits.** A deploy-in-flight window (~1-2 min, observed repeatedly this session) is dead time for the system today. Pairs with D2: housekeeping work restricted to non-runtime files (docs/data-only, matching D2's `watchPatterns` skip list) can run in that window without triggering a second, competing redeploy. | Founder-ratified directive, 2026-07-18, generalizing D2's already-shipped skip-list mechanism. | **Not yet built** — needs a concrete task list (candidates: synopsis-index regen, SENTRY findings triage, postmortem writing, PRODUCT_HOME receipt cleanup) and a hook into the redeploy-wait window itself |
| D10 | **Priority scoring — sliding scale + aging, scored by Chair, written by Architect only.** Closes the starvation gap between "urgent → top of queue" and "everything else → pure sequential order" (the LifeOS empty-backlog finding, §12, is the live example — important but never explicitly urgent, so it could sit forever under pure sequencing). Every scored item gets an urgency 1-10; 8+ is urgent (top of queue, preempts); everything else ages up a little each cycle it's not picked, so nothing can be silently forgotten forever, but aging alone caps just below the urgent threshold — only Architect (on Chair's recommendation) can grant true preemption. **Two altitudes, one mechanism:** (a) cross-product — which blueprint matters more right now (`docs/products/PRODUCT_BUILD_PRIORITY.json`, explicitly marked in its own header as a founder business decision); Chair scores/recommends using evidence (revenue proximity, staleness/aging, dependency unlocks, D7 repair urgency), Architect writes the score into the blueprint, a change to the *top-ranked* product stays founder-visible (founder-initiated: "I will reach out... almost always" rather than a system-generated ping) — Chair never edits `PRODUCT_BUILD_PRIORITY.json` directly. (b) within-product step ordering — stays Architect's alone (already owned via `depends_on`/step sequencing); no separate Chair scoring function needed there, per D6 (ambiguous step order = blueprint under-specification, escalates to Architect, who already has the tool to fix it). **Scoring happens inline with D7, not as a separate pass:** when SENTRY reports a finding to Chair, Chair's review of the proposed solution and its urgency scoring (urgent vs. nice-to-have) happen in the same step. **Founder-request pushback (extends D6):** a founder priority request gets genuine Chair scrutiny too, not rubber-stamp — Chair can push back if a request reads as an in-the-moment reaction rather than sound business reasoning. This is advisory, not a veto: founder hears the counter-argument, then decides, because founder authority sits above internal governance layers in the existing hierarchy (NSSOT > Companion > CLAUDE.md > product homes) — not a peer vote between founder and Chair. | Founder-dictated directive, 2026-07-18, across several turns of refinement (sliding-scale request → aging proposal → Chair-scores/Architect-writes division → founder-request pushback). | **Not yet built** — needs Architect scoping (the actual scoring/aging mechanics, and where scores persist on a step/product) |

## 14. Change log

| Date | Change |
|------|--------|
| 2026-07-18 | §17 split into 17a (scaling one instance: isolated build branches, ephemeral per-branch Railway+Neon preview envs, single merge queue, time-batched deploys, budget-governed concurrency) and 17b (competing dual-instance BuilderOS, refined with real competition dimensions, scoreboard, and proven-only sharing gate) |
| 2026-07-18 | D10 added: priority scoring (sliding scale + aging, Chair scores/Architect writes, two altitudes — cross-product and within-product — one mechanism), plus founder-request pushback extending D6 |
| 2026-07-18 | D7: unresolved Chair↔SENTRY disagreement hard-gated into the existing consensus protocol (recovery_ladder_v2), not a new ad hoc mechanism — founder correction |
| 2026-07-18 | Corrected D7/D8 → precise repair pipeline (SENTRY finds+proposes → Chair reviews/reconciles → Architect corrects blueprint only → queue's D6 decision-check → Factory builds), founder-dictated precisely; renumbered D7-D10 → D7-D9 |
