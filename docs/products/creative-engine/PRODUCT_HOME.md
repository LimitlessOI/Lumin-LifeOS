...
| Date | What | Why | State |
|------|------|-----|-------|
| 2026-08-02 | **Creative Director lens + project review runner** — `data/lenses/creative-director.json`, `scripts/creative-engine-project-review.mjs`, and `POST` equivalents in `routes/creative-engine-routes.js` process every active product `PRODUCT_HOME.md` and emit `CREATIVE_BRIEF.md`. | Founder: "our projects need to run through the creative engine." Establishes the creative responsibility as a first-class review path and produces shareable briefs for every product. | ✅ 43 briefs generated |
...
| 2026-07-30 | **Competitor niche analysis + one-click social publish scaffold** — new `services/creative-engine/modes/competitor-niche-analysis.js` (fetches public competitor metadata/oEmbed, calls planner for strengths/weaknesses/gaps/incorporate), `services/creative-engine/modes/social-publish.js` (queue per-platform captions, connection check, dry-run/ready status + fallback fetch from `public_url`/direct `PUBLIC_BASE_URL`/previews path), DB migration `20260730_creative_modes_and_publish.sql` expands `creative_jobs`/`creative_assets` kinds and adds `creative_publish_queue` + `creative_competitor_reports`, `services/creative-engine/index.js` wires both modes into `estimate`, `processJob`, and `render`. | Founder: smart edit must understand competing videos and publish to all socials. | ✅ competitor_analysis live; social publish draft/queue works; real posting needs connected accounts + `LIVE_SOCIAL_PUBLISH_ENABLED` + durable video storage (R2) for multi-instance publish |
| 2026-07-30 | **Smart `footage_edit`** — `services/creative-engine/transcribe-edit.js` (Gemini audio transcription with per-word timestamps), `services/creative-engine/modes/footage-edit-smart.js` (filler/repetition/silence removal, optional content-aware keep/cut via `defaultPlannerCallModel`; falls back to keeping the full video when no speech is detected), `services/creative-engine/providers/ffmpeg-local.js` adds `getDuration`/`extractAudio`/`cutKeepRanges`, `services/creative-engine/index.js` routes `smartEdit` requests and updates `estimate`. Gated on `GOOGLE_API_KEY`/`GEMINI_API_KEY`. | Founder: AI video editing must remove ums/stutters, understand keep/cut, and make brilliant videos. | ✅ code + local end-to-end test (44-word sample, 12 fillers/hesitations removed, cut video produced); no-speech fallback tested on live deploy |
