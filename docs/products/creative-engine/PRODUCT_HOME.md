...
| 2026-07-30 | **Competitor niche analysis + one-click social publish scaffold** — new `services/creative-engine/modes/competitor-niche-analysis.js` (fetches public competitor metadata/oEmbed, calls planner for strengths/weaknesses/gaps/incorporate), `services/creative-engine/modes/social-publish.js` (queue per-platform captions, connection check, dry-run/ready status), DB migration `20260730_creative_modes_and_publish.sql` expands `creative_jobs`/`creative_assets` kinds and adds `creative_publish_queue` + `creative_competitor_reports`, `services/creative-engine/index.js` wires both modes into `estimate`, `processJob`, and `render`. | Founder: smart edit must understand competing videos and publish to all socials. | ✅ code + local test (Rickroll URL analyzed into actionable gaps); social publish needs connected accounts + `LIVE_SOCIAL_PUBLISH_ENABLED` for real posting; tip prove after deploy |
