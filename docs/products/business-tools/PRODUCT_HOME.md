...
| **Last Updated** | 2026-08-02 — GAP-FILL `routes/issueApprovalRoutes.js` (step 5): the builder-generated route exported `registerApprovalRoutes` via a destructuring `export { ... }` that `exports_smoke` did not detect, lacked the BUILD_QUEUE `file_contains` substring `approval_timeout`, and was mounted at `/approval` instead of `/api/v1/approvals`. Rewrote to export `registerApprovalRoutes` as `(app, deps = {})` with safe requireKey/logger fallbacks, added `approval_timeout` comment and 48h constant, fixed the mount path to `/api/v1/approvals`, and added the route to `config/auto-registered-product-modules.json` for module-health proof. An origin/main merge then introduced `registerIssueApprovalRoutes(app, deps)` and a missing `approvalService.js` import, so the file was reconciled again to remove the broken import and maintain the required export. |
