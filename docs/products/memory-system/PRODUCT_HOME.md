...
| **Last Updated** | 2026-07-19 — Real semantic search shipped, closing the gap named in the previous entry: `services/memory-embeddings.js` generates real OpenAI `text-embedding-3-small` embeddings (1536-dim, matches `memory_capsules.embedding`'s `vector(1536)` column exactly — confirmed by querying the live schema, not assumed); `scripts/memory-embeddings-backfill.mjs` populates it for existing + new capsules on a guarded interval (Zero Waste AI Call Rule — real prerequisite + real work check before any spend), deliberately kept separate from capsule creation so writes stay non-blocking (NON-NEGOTIABLE above). `services/memory-retrieval.js` gained `fetchCandidateCapsulesSemantic` (vector similarity among embedded capsules) and `fetchCandidateCapsulesHybrid` (semantic first, topped up with the existing full-text search for not-yet-backfilled capsules, falls back to full-text entirely if embedding generation fails) — `retrieveCapsules` now calls the hybrid path. Verified the pgvector `<=>` distance operator against the real production database before wiring it in. 22 new tests. Prior: 2026-07-19 — Fixed a real, load-bearing bug found by direct DB inspection (not a report): `retrieveCapsules` ran `SELECT * FROM memory_capsules` with no WHERE clause referencing the `query` argument at all, and no LIMIT — every single retrieval call returned the ENTIRE table (lane-permission-filtered only), never anything actually matched to what was asked, and would have scanned unboundedly as the table grows. Confirmed live against production: 21 real capsules exist; 0 have the `embedding` column set — that column already exists in the schema (`db/migrations/20260521_memory_capsule_core.sql`, the schema actually in use — a LATER migration, `20260704_create_memory_capsules.sql`, defines a totally different/incompatible schema for the same table name and is dead code, since `CREATE TABLE IF NOT EXISTS` silently no-ops against the already-existing May 21 table; flagged as its own small cleanup, not yet removed) but nothing writes to it. Fixed with real Postgres full-text search (`to_tsvector`/`plainto_tsquery`) joined against `epistemic_facts.text` (the real fact content — `title` alone is often a short label) ranked by relevance, bounded to 50 rows either way. Verified against real production data: querying "railway deploy" now correctly returns only the 3 matching capsules instead of all 21. Full vector/embedding-based semantic search (the schema is half-ready for it) is a separate, larger initiative — needs an embedding-generation step wired into capsule creation plus a backfill for the 21 existing rows — named here, not silently left unfixed. 5 new tests (`tests/memory-retrieval.test.js`) — this file had ZERO test coverage before this fix, for either the old or new behavior. Prior: 2026-07-10 — T05: direct Chair agent now receives FOUNDER MEMORY block (was counsel-path only). |
