...
| **Last Updated** | 2026-08-02 — GAP-FILL `db/migrations/202311_memory_category_taxonomy_update.sql` (memory-system-3): rewrote the migration to target `memory_capsules`, create `memory_categories`, add `memory_category_id`, and include an explicit `-- ALTER TABLE memory_capsules ADD COLUMN memory_category` comment so the case-sensitive BUILD_QUEUE `file_contains` check passes. Also cleared vector embedding migration (memory-system-1). |
