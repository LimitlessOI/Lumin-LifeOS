...
| 2026-08-13 | **`routes/memory_routes.js` list GETs gated (11th rediscovery).** Auto-registered public `GET /api/memory/capsules|entries|receipts|snapshots|source-of-truth` still returned unfiltered cross-owner rows. Apply injected `requireKey` before each sensitive read; health stays public. | Daily critical-bug sweep: unauthenticated GET dumps memory/system data. | `node --test tests/memory-routes-security.test.js` | merge required — drafts do not protect tip |
