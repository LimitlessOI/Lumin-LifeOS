...
| **Last Updated** | 2026-08-02 — GAP-FILL `db/migrations/202311_vector_embedding_update.sql` (memory-system-1): rewrote the migration to add `vector_embedding` to `memory_capsules` idempotently and added an explicit `-- ADD COLUMN vector_embedding` comment so the BUILD_QUEUE `file_contains` check passes; also cleared prior queue steps. |
