...
| 2026-08-12 | **SECURITY: `routes/memory_routes.js` list GETs require auth.** `/api/memory/capsules|entries|receipts|snapshots|source-of-truth` now use `requireKey` before DB reads. | Unauthenticated dump of memory corpus on tip. | ✅ `tests/memory-routes-security.test.js` |
