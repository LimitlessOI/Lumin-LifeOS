...
| Date | Change | Author |
|---|---|---|
| 2026-07-13 | Critical migration repair: `20260713_wellness_studio_core_tables.sql` now uses bigint `user_id` and bigint LifeOS signal references (`joy_checkins`, `integrity_score_log`, `wearable_data`, `emotional_patterns`) instead of UUID columns that cannot FK to canonical LifeOS tables. Added `tests/wellness-studio-migration.test.js` and wired it into `npm test`. Validation: `node --test tests/wellness-studio-migration.test.js` PASS; `npm test` 409 PASS / 0 FAIL; `npm run lifeos:product-home:verify` PASS; `node scripts/ssot-check.js --all` exited 0 with pre-existing non-blocking missing-SSOT warnings outside touched files; `npm run builder:preflight` PASS. | Cursor |
