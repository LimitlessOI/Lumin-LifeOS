<!-- SYNOPSIS: Canonical product home — Wellness Studio -->

# Wellness Studio Product Home

**Formerly called:** Amendment 28 — WELLNESS STUDIO

| Field | Value |
|---|---|
| **Canonical home** | this file |
| **Product id** | `wellness-studio` |
| **Constitutional law** | `docs/constitution/NORTH_STAR_SSOT.md` |
| **Machine manifest** | `docs/products/wellness-studio/FILE_MANIFEST.json` |
| **Authority boundaries** | `docs/products/AUTHORITY_BOUNDARIES.md` |
|| **Last Updated** | 2026-08-02 — GAP-FILL: wellness-studio-step11 done (extendWellnessTables anchor); wellness-studio-step12 un-parked by adding registerWellnessExtensionsRoutes alias + POST /api/v1/wellness/extend to routes/wellness-table-extensions-routes.js and updating auto-register entry. |
...
| 2026-08-02 | **GAP-FILL: `routes/wellness-table-extensions-routes.js` `registerWellnessExtensionsRoutes` alias + `POST /api/v1/wellness/extend` and un-park `wellness-studio-step12`.** Added `registerWellnessExtensionsRoutes(app)` that calls the existing `registerWellnessTableExtensionsRoutes` and adds a handler for `POST /api/v1/wellness/extend`; updated `config/auto-registered-product-modules.json` to register the alias so module-health mounts it. | The BUILD_QUEUE contract expected `registerWellnessExtensionsRoutes` and a route at `/api/v1/wellness/extend`, but the file only exported `registerWellnessTableExtensionsRoutes` and mounted `/api/v1/wellness/extensions`. | `node --check routes/wellness-table-extensions-routes.js` / `npm run builder:preflight` PASS. | — |
...
| Date | Change | Author |
|---|---|---|
| 2026-08-02 | **GAP-FILL: `services/therapist-integration.js` (wellness-studio-3) — corrected a real regression, not just an artifact-proof mismatch.** `communication-profile.js`/`truth-delivery.js` DO exist and DO export real functions (`createCommunicationProfile`, `createTruthDelivery`) — the prior "fix" (row above, dated the same day) was based on a false premise and replaced a real, consent-gated implementation with a no-op stub that dropped the `therapist_client_links.status = 'consented'` check entirely and used a `require('communication-profile')` bare-specifier call that would throw `Cannot find module` (missing `./` prefix and `.js` extension — not a valid local-file require). Root cause: BUILD_QUEUE step `wellness-studio-3`'s own `file_contains` demanded the literal broken text `require('communication-profile')`, so every regeneration cycle was rewarded for reproducing broken code. Restored the real consent-check + therapist-profile-lookup + `user_integrations` record logic, corrected the imports to the sibling modules' actual factory-function exports (`createCommunicationProfile({pool,callAI,logger}).getOrCreate(userId)`, `createTruthDelivery({pool,callAI})` instantiated but not auto-invoked — auto-generating an unsolicited "hard truth" message as a setup side effect would violate the User Sovereignty no-dark-patterns rule), and corrected `wellness-studio-3`'s `file_contains` to real, achievable markers (`createCommunicationProfile`, `createTruthDelivery`, `getOrCreate`) so future regeneration cycles are rewarded for correct code instead of broken code. | Claude |
