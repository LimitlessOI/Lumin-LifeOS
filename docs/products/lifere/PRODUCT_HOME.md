...
| **Last Updated** | 2026-07-21 — **GRAIL/Voluntary-Progress Wave 1 (9 files) SSOT-tagged to this product home** — see Change Receipts below for the full shipping + verification detail. Prior: 2026-07-20 — **SENTRY LifeRE live probe → fixed a prod 404.** A structural probe of LifeRE's live founder-lane surfaces found 8/9 core endpoints 200/ok, but `GET /api/v1/lifere/sales-coach/scenarios` returned **404**: the sales-coaching simulator router was mounted only in the full-runtime lane (`startup/register-runtime-routes.js`), which Railway never boots — prod runs `founder_builder` only. Added a `registerLifereSalesCoachingRoutes(app, deps)` wrapper export to `routes/lifere-sales-coaching-routes.js` and an `config/auto-registered-product-modules.json` entry so the founder lane mounts it (no composition-root edit; `startup/` is protected). Also confirmed the still-open `phone/status transfer_loop_risk: true` blocker (Adam phone-forwarding decision — unchanged). Prior: 2026-07-18 — BUILD_QUEUE.json step 10 ("implement live phone service" — Vapi/Twilio, `services/livePhoneService.js`) had a spurious `depends_on:["8"]` blocking it behind a demoted, unrelated step (TC blocker-card UI routes, 9 failed revives). No functional relationship between the two — removed the dependency (GAP-FILL correction of a blueprint-authoring defect, not a scope decision) so the governed autonomous loop can actually pick up the live-phone service next cycle. Found via a direct system audit (D6 doctrine applied: an ambiguous/broken blueprint link, not queue improvisation). Prior: 2026-07-16 — Pre-transfer SMS brief now unconditional (25-idea build item #1); live-confirmed transfer-loop risk still active, blocked on Adam's phone-forwarding decision. |
...
|------|--------|-----|-------|------|
| 2026-07-21 | **GRAIL/Voluntary-Progress Wave 1 shipped + SSOT-tagged (overnight autonomous build).** 9 new files under `services/voluntary-progress-*.js` — 3 Phase 0.5 seed-the-clock stores (`ask-ledger`, `attention-telemetry`, `mission-intake-log`, all with `consent_scope`) plus 6 Wave 1 missions (`twin-read`, `context-view`, `decision-step`, `alternative-futures`, `experiment`, `twin-proposal-gate`) — all shipped via the governed factory (`governed_ship_queue`), all missing the required `@ssot` tag until now (fixed here, pointed at this file per the existing `lifere-twin-store.js` convention). `voluntary-progress-twin-proposal-gate.js` (the only sanctioned propose-not-apply Twin-write path) failed its first ship attempt with a real bug — `writeTwin` called with wrong parameter names (`user_id`/`proposed_value` instead of `userId`/`payload`, `twinKey` never passed) — found by reading the shipped code directly, fixed by rewriting the spec with the exact signature spelled out, redispatched, and independently re-verified by executing all 4 exports against a fake pool + a stubbed `writeTwin` that throws on any snake_case/undefined regression (13/13 checks pass). | Adam: "get me from point A to Z overnight, don't ask permission, decide and correct later." | ✅ all 6 Wave 1 + 3 Phase 0.5 files `done` on origin/main; wave1-6 independently execution-verified, not just SENTRY-green | none of the 9 files are wired into a real call path yet (built-but-unwired, same named gap as the 15 Self-Repair Tier-0 modules) — next is wiring the first vertical slice (Twin load → alternative paths → decision → experiment → propose-not-apply update) into a real LifeOS/Chair call path |
| 2026-07-20 | **Sales-coaching simulator 404 fixed in prod (founder lane).** `routes/lifere-sales-coaching-routes.js` gained a `registerLifereSalesCoachingRoutes(app, deps)` auto-register wrapper (mounts `/api/v1/lifere/sales-coach`); `config/auto-registered-product-modules.json` gained the matching entry. Root cause: the router was only mounted in the full-runtime lane, which Railway never boots (prod = `founder_builder`), so `GET /sales-coach/scenarios` → 404. Same class of gap the finance/engine/core routes hit. Caught by a live SENTRY-style LifeRE structural probe (8/9 core endpoints already 200/ok). | "keep at it — get them all ready for market, no theater": prove LifeRE's client-facing surfaces resolve, not just the founder shell. | ✅ node --check + JSON valid; local | redeploy → re-probe `/sales-coach/scenarios` 200; then Adam's phone-forwarding (A/B) is the only remaining LifeRE ops blocker |
