...
| **Last Updated** | 2026-07-30 — LifeRE sales coaching now adapts to learner profile: `services/lifere-sales-simulator.js` accepts `learningProfile` on session start, stores it in `metadata` (new `metadata` JSONB column), and injects learning-style guidance into real-time coaching and debrief prompts. |
