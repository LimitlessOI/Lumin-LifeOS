...
| 2026-07-12 | **Temporary model routing pivot away from Anthropic for BuilderOS code lanes:** `config/task-model-routing.js` `council.builder.code` → `openai_builder_standard`, `council.builder.review` → `openai_builder_escalation`, `council.builder.code_review` → `openai_builder_standard` while Anthropic account is credit-dry. `council-service.js` provider failover still retries through `openai_gpt`, `deepseek`, `gemini_flash` on any HTTP/timeout failure. | BuilderOS `council.builder.code` dispatches were failing with `Anthropic HTTP 400` (credit balance too low) and the daily spend cap was hit, stalling the governed loop. Routing the builder's first code/review call to OpenAI keeps the loop moving and lets the existing failover cascade handle OpenAI or other provider outages. | ⚠️ IN PROGRESS: local config updated; pending restart + push/deploy + monitor for first successful OpenAI builder ship | `node --check config/task-model-routing.js` |
