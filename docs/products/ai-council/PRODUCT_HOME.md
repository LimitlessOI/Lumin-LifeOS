...
| **Last Updated** | 2026-08-15 — Founder-ratified LCL doctrine captured: NON-NEGOTIABLE LCL LAW (round-trip fidelity — a shorter message that changes the decision is corruption, not compression), the 25-item optimization surface consolidated as ratified scope, and explicit Phase 2/3 roadmap gating (build only once Phase 1 is proven insufficient, not because it sounds sophisticated). See "Founder-Ratified LCL Doctrine" subsection under LCL Architecture Vision. Prior: 2026-08-02 — `docs/products/ai-council/BUILD_QUEUE.json` step `8` (`scripts/habLimiter.mjs`) reset from `blocked`/`demoted` to `pending`. Removed the impossible `route: {"path":"/"}` assertion on a script target and provided the deterministic `enforceHABLimit` script. The step had 6 revive cycles on `reload_failed` because the builder tried to mount a `.mjs` script as an Express route. Prior: 2026-07-21 — **Systemic `maxTokens` truncation fix**. |
...
| **Last Updated** | 2026-07-27 — **`TRUSTED_FALLBACK_MODELS` re-reordered, strong-first for real this time**: the 2026-07-14 order (`openai_builder_standard` → `openai_gpt` → `deepseek` → `claude_sonnet` → ...) put `claude_sonnet` (claude-sonnet-4-6, the actual strongest configured model) 4th, behind two `gpt-4o` entries and `deepseek`. Because the codegen dispatcher (`routes/factory-mount-routes.js` `codegenRunner.generate`) stops at the FIRST tier producing syntactically-valid, non-empty output, and gpt-4o rarely fails outright even when it writes correct-looking-but-wrong logic, `claude_sonnet` was effectively never reached in practice — direct root cause of several "shape-valid, logically wrong" bugs found and fixed 2026-07-26 (hero-image flag, lead-capture false-negative, fabricated placeholder). New order: `claude_sonnet` → `openai_builder_standard` → `deepseek` → `openai_gpt` → `openai_builder_escalation` → `gemini_flash` → `groq_llama` → `openai_builder_mini`. `openai_builder_escalation` also confirmed to map to the same `gpt-4o` as `openai_builder_standard` (config/council-members.js) — not a real escalation tier, left in place but moved past tiers that add real strength/provider diversity; a genuine escalation tier is a real, separate follow-up. Prior: 2026-07-14 — Model routing SSOT fixes: `config/council-members.js` `claude_sonnet` default model updated to `claude-sonnet-4-6` (previous `claude-sonnet-4-20250514` was deprecated/invalid); `services/response-cache.js` now refuses to cache empty/whitespace responses, preventing poisoned empty cache entries from being reused for load-bearing codegen. These feed into `factory-staging/factory-core/builder/authoring.js` `DEFAULT_CODEGEN_TIERS` which now reuses `TRUSTED_FALLBACK_MODELS` as the codegen failover chain. |
...
### Founder-Ratified LCL Doctrine (2026-08-15)

Captured verbatim/structured from a direct founder brief, not invented by Claude. Ratifies and consolidates the 25-item optimization list this same section's mechanisms already implement piecemeal, plus one new hard rule that did not exist here before.

**NON-NEGOTIABLE LCL LAW.** Compression is successful only when meaning survives. Saving 90% of tokens while changing 2% of the important meaning may be catastrophic. For important communication, track: `ORIGINAL → COMPRESS → DECOMPRESS/INTERPRET → COMPARE SEMANTIC INTENT`, recording semantic preservation, omitted requirements, changed modality/authority, lost exceptions, ambiguity, downstream decision difference, and token reduction. **A shorter message that causes a different decision is not compression. It is corruption.** `services/lcl-monitor.js` is the existing enforcement point for this — any symbol pair shown to lose meaning must be auto-disabled/quarantined, not tuned quietly.

**The 25-item token/context optimization surface (ratified as the full scope this product covers, not just the codebook):** (1) session codebook — key once, deltas after; (2) domain codebooks — load only symbols relevant to the active domain (`config/codebook-domains.js`); (3) instruction IDs for repeated governance/instruction blocks; (4) mined phrase table — promote high-frequency sequences into versioned shorthand; (5) rolling history — verbatim recent turns, structured-summary older turns with source anchors; (6) structured summary over loose prose for older context; (7) tool-result digest instead of reinjecting huge payloads; (8) retrieval instead of whole-document injection; (9) exact-duplicate turn collapse; (10) semantic deduplication only when meaning is demonstrably unchanged; (11) small-model triage before invoking expensive reasoning; (12) structured prompt IR for repeated scaffolding; (13) delta context — transmit changed state, not stable state; (14) structured compact formats for lists/tables once round-trip fidelity is proven; (15) strong, normalized response cache; (16) shadow/AB testing before widening a new compression rule; (17) minimum tool schemas — expose only relevant fields; (18) two-tier system context — universal core + task-specific append; (19) SSOT claim verification — state requires source/path evidence or stays UNVERIFIED; (20) LCL drift monitoring — a losing pair is auto-disabled; (21) useful-work-guard — no recompression spend without expected value; (22) context hygiene — compact rules first, full constitutional text only when required; (23) compact internal wire payloads, expand for human/UI readability; (24) uncertainty brake — say UNCERTAIN and name the missing evidence rather than spend tokens speculating; (25) governed compression changes — no model relaxes fidelity/drift/truth/verification safeguards solo, ever, to save tokens.

**Roadmap discipline, explicit:** Phase 2 (custom BPE tokenizer) and Phase 3 (LoRA fine-tune baking the codebook into weights) are not implemented merely because they sound sophisticated — reality must first demonstrate they outperform ordinary provider caching, structured context, retrieval, templates, and smaller models at Phase 1. This is a hardening of the roadmap gating already implicit in this section's "FUTURE — needs budget" / "FUTURE — needs A100" Build Plan rows below, made explicit as a rule rather than just a scheduling note.

**Design goal, stated as the reason any of this exists:** the interface between AIs should become efficient enough that Council debate, Builder/Architect exchange, and verification can run routinely without paying repeatedly for duplicated English context — but token efficiency never outranks truth, intent, authority, context fidelity, verification, or absence of drift. Reality is the scorecard, not the token count.
...
| 2026-08-15 | **Founder-ratified LCL doctrine captured into this SSOT** — NON-NEGOTIABLE LCL LAW (round-trip fidelity requirement, enforced via existing `services/lcl-monitor.js`), the 25-item token/context optimization surface consolidated as ratified scope, and explicit Phase 2/3 roadmap gating. No code changed — this documents founder intent that this section's existing live mechanisms (codebook, `prompt-translator.js`, `lcl-monitor.js`) already partially implement, and sets the rule for future compression changes. | Adam, direct brief during a session also covering a ChatGPT relay Body/Capsule blueprint: asked for the LCL doctrine to be captured so it could be shared with ChatGPT and made binding rather than left as chat-only context that evaporates. | Documentation only; `node --check` not applicable (no .js changed) | This doctrine is now the standard any future LCL/compression change (including a Phase 2/3 proposal) must be checked against before shipping. |
| 2026-08-02 | **Creative Director review** — generated CREATIVE_BRIEF.md using the Creative Director lens. | Ai Council reviewed through the BuilderOS creative responsibility; brief written to product home for founder review. | ✅ generated |
| 2026-08-02 | **`db/migrations/persist_provider_cooldowns.sql` added and `docs/products/ai-council/BUILD_QUEUE.json` step `3` claimed by the self-heal artifact-proof gate.** The migration creates `provider_cooldowns` and `free_tier_usage` tables with the expected `CREATE TABLE IF NOT EXISTS free_tier_usage` substring, satisfying the queue step that had stalled on SQL `route: {"path":"/"}` assertions. | The never-stop factory could not build this SQL step because cheap models kept emitting JavaScript inside a `.sql` file; a hand-written migration was the fastest deterministic gap-fill while `services/build-integration-context.js` now carries SQL-specific FILE RULES to prevent recurrence. | `node --check db/migrations/persist_provider_cooldowns.sql`; `source .env && node scripts/self-heal-build-queue.mjs` claims `ai-council::3`; `npm run builder:preflight` PASS. |
| 2026-08-01 | **`services/council-service.js` builder-lane 429/credit-exhausted cascade now tries every free provider.** Replaced the single-provider fallback in `callCouncilMember`'s catch block with `cascadeToFreeTier`, which walks the full `PROVIDER_PRIORITY` chain (Groq → Gemini → Cerebras → Mistral → OpenRouter → Fireworks → Ollama) and tries each provider's council members until one succeeds. This closes the gap where the old code only attempted one next provider and gave up if all of that provider's members failed (e.g., `groq_llama` 413 on a large prompt), causing `council.builder.code` to return `Council call failed` even when working free providers remained. The original provider is still marked exhausted before the cascade, and the `is429`/`isCreditError` detection and `!builderLane` removal from the previous fix remain in place. | SO-003: "never idle because one provider ran dry." The builder lane must keep executing when paid tiers are dry, and a single oversize-prompt failure on one free member must not block the rest of the chain. | `node --check services/council-service.js` clean; `npm run builder:preflight` PASS. | Verify live: `POST /api/v1/lifeos/builder/build` with a code task succeeds and returns a `commit_sha` when paid providers have zero balance. |
| 2026-07-28 | **The codegen tier canary asserted a hardcoded model name and had therefore broken twice; it now asserts the policy instead.** `builderos-reboot/scripts/codegen-dumbpipe-proof.mjs` (the `factory:ci` `authoring_canary` gate) asserted `DEFAULT_CODEGEN_TIERS[0] === 'openai_builder_standard'`. That was true when written on 2026-07-14, but the 2026-07-27 strong-first reorder moved `claude_sonnet` to the front — the correct SO-003 change — and the canary has been failing ever since, which is what kept `factory:ci` red. Rewrote the assertion to compare against the newly-exported `QUALITY_FIRST_FALLBACK_MODELS[0]` from `config/task-model-routing.js`, so reordering the declared policy can never again break the test that exists to protect it. Deliberately did **not** assert "tier[0] must not be a cheap model", which was the obvious reading of SO-003: `TRUSTED_FALLBACK_MODELS_OVERRIDE` is a founder-ratified operational lever, and with Anthropic/OpenAI/Together all `needs_payment` right now production legitimately runs cheap-but-funded providers first — that assertion would have red-flagged a correct decision. Instead the canary branches: with no override set the code default must equal the declared strong-first head; with an override set it only requires a non-empty resolved tier and records the override in the finding. Added a second assertion that the chain holds at least two distinct tiers, so escalation is provably possible rather than assumed. Also corrected two stale comments still describing the pre-SO-003 "cheapest tier first" policy. Canary went 8/12 → **13/13**. | A gate whose assertion drifts every time the thing it guards is legitimately improved will be disabled or ignored — it has to assert the invariant, not the current value. | `node builderos-reboot/scripts/codegen-dumbpipe-proof.mjs` 13/13 PASS; `npm run factory:ci` **ALL PASS** (25/25, was 16/25); `CI=1 npm run verify:maturity` exit 0. | If a genuine cheap-vs-strong classification is ever added to `config/council-members.js`, tighten the no-override branch to assert tier[0] is in the strong set. |

| 2026-07-28 | **Real, concrete answers to "we're out of money, what do we do for now" — checked live via `GET /api/v1/lifeos/provider-key-health` rather than guessing.** Confirmed real picture: Anthropic, OpenAI, Together all `needs_payment` (real, external, founder-only fix). But Groq, Mistral, and DeepSeek are genuinely funded and working right now — and **Mistral had zero entry anywhere in `config/council-members.js`** despite a working key, meaning a real, free, already-paid-for provider was completely unreachable by the actual pipeline. Added `mistral_free` (model `mistral-small-latest`, deliberately an alias not a pinned dated version). Also found the diagnostic tool itself was lying: **Gemini showed `status: error` in the health check, but the real cause was the health-check's own probe using a retired model name (`gemini-2.0-flash`)** — production code already correctly uses `gemini-2.5-flash` everywhere, so Gemini has actually been fine and working all along; fixed the probe to stop giving a false negative. Same fix applied to the Anthropic probe's stale model string (doesn't change its real `needs_payment` verdict, but keeps the diagnostic honest). Added a live, reversible `TRUSTED_FALLBACK_MODELS_OVERRIDE` env-var escape hatch (`config/task-model-routing.js`) so today's free-tier-first reality can be set operationally without hardcoding a temporary hack into source — clearing the env var instantly restores the quality-first default the moment funding returns. Attempting to set it live via `POST /api/v1/railway/managed-env/bulk` correctly hit a real allowlist gate (`services/railway-managed-env-service.js`'s `DEFAULT_ALLOWED_KEYS`) — added it there as a legitimate, permanent, non-secret operational key rather than bypassing the gate. **Left honestly unresolved**: Cerebras is genuinely inaccessible (`model does not exist or you do not have access to it`) using the same model string as production — could not verify a working alternative without guessing, flagged as a real open item rather than a blind fix. | Founder, direct, vulnerable: "I know we're out of coin, and I'm out of money... we're gonna have to operate without it for now... maybe you think above some solutions for lack of money from now just for testing. I don't know. You tell me." | `node --check` clean on all 3 files; every claim (which providers work, which need payment, which model strings are stale) verified against the real live health-check response, not inferred from code alone | Set `TRUSTED_FALLBACK_MODELS_OVERRIDE` in Railway to the free-tier-first order and redeploy; re-run `GET /api/v1/lifeos/provider-key-health` after deploy to confirm Gemini now reports `working`; live-dispatch a real task and confirm `mistral_free` is actually reachable end-to-end, not just registered. |

| 2026-07-28 | **Expanded `model_capability_ledger` from 1 role to 2 real roles — priority item 4, honestly partial, not claimed complete.** North Star §2.0J names ~9 roles; the first version only ever recorded `builderos_execution`. Added a `role` column (composite key with `model_tier`) and wired a second real role, `aic_debate`, into `services/lifere-council-router.js`'s Chair/CFO calls — real success now means a genuine model reply replaced the fallback template, and `theater_detected` is set precisely when it didn't, directly reusing the exact detection this session already built once (the CFO-always-fake bug fix). `GET /factory/model-rankings?role=aic_debate` filters; `known_roles` in the response names all ~9 constitutional roles and honestly marks which 7 are still unwired rather than hiding the gap. | Founder: "Start building them from highest priority to the lowest. Every one of these things have to be done." | `node --check` clean on 3 files; production table confirmed empty before the schema change (checked live, not assumed), so the composite-key redesign needed no data migration | 7 of 9 named roles remain unwired (BPB blueprinting, OIL review, Verifier, summarizer, historian, founder intent modeling, security review, external research) — wire each as its real call site is touched for other reasons, not retrofitted speculatively just to claim full coverage. |

| 2026-07-28 | **CRITICAL FINDING + fix: Anthropic and OpenAI accounts are both out of money — real, external, billing-level, not a code bug — plus one real code bug found alongside it and fixed.** Found using the new `tier_errors` diagnostic (built this same session) on a real live LifeRE dispatch: `claude_sonnet` failed with `Anthropic HTTP 400: "Your credit balance is too low to access the Anthropic API"`; `openai_builder_standard`/`openai_gpt`/`openai_builder_escalation`/`openai_builder_mini` all failed with `HTTP 429: exceeded quota`. This directly confirms the founder's own hypothesis from earlier tonight ("Most likely, it's because I'm out of money") — real, current, and requires his own billing action, not something fixable in code. Reported to him directly and immediately given severity. **Also found in the same trace, and fixed**: `deepseek` was failing every single dispatch with `dsIn is not defined` — a real `ReferenceError`, not a billing issue. Root cause: `dsIn`/`dsOut` were declared with `const` inside a `{ }` block (the "TCO-E01: ledger for DeepSeek" block) and referenced at the `finalizeResponse` call below it, outside that block's scope — classic JS block-scoping bug, silently taking DeepSeek out of the entire codegen tier rotation on every real dispatch, for who knows how long, invisible until the new `tier_errors` diagnostic finally surfaced it instead of hiding it behind whichever tier failed last. Fixed by removing the unnecessary block braces. With Anthropic and OpenAI both out of credit right now, DeepSeek being usable again is materially important, not cosmetic — it's one of the only remaining paid tiers with real capacity. | Founder, mid-session: "make sure the system is building. LifeRE." Investigating why a real, unblocked LifeRE step still wasn't shipping led straight to this — both the account-balance issue and the DeepSeek bug were completely invisible before tonight's `tier_errors` diagnostic fix. | `node --check` clean; confirmed no other `dsIn`/`dsOut` usages elsewhere in the file before descoping (safe, not just plausible) | Once Anthropic/OpenAI credit is restored (founder action), re-verify `claude_sonnet` specifically — everything else in the routing/ordering fixes this session should then work as designed for the first time under real conditions, not just in theory. |
| 2026-07-27 | **Built the real per-model-tier capability ledger — direct answer to founder: "Every model that sits in here needs to be rated... Have we ranked any of them? If not, why not?"** Investigated honestly first: nothing tracked real per-model outcomes anywhere live. `services/self-repair-target-reputation.js` tracks by target FILE, not model, and its only caller (`self-repair-quarantine.js`) is itself never called by anything — fully dormant. `services/cognitive-core-trust.js` tracks advisor delegation confidence, a different concept entirely, not raw LLM quality comparison. New: `services/model-capability-ledger.js` (`recordModelOutcome`/`getModelRankings`, self-bootstrapping `model_capability_ledger` table) hooked directly into `runGovernedAutonomousShipOnce`'s per-step result processing in `services/governed-autonomous-shipping-loop.js` — the one chokepoint every governed-factory codegen dispatch already passes through (autonomous loop and manual `/factory/ship-queue-and-commit` calls alike), so recording cannot be silently skipped by a caller forgetting to opt in. Records `model_tier`, `trust_earned` (the strict self/peer/compare honesty grade, not just `ok:true`), `theater_detected`, and `escalated`, straight from data the pipeline already computes — no new AI calls, no speculative infrastructure. Exposed at `GET /factory/model-rankings` (`routes/factory-mount-routes.js`) so the ranking is actually visible and queryable, not written to a table nobody reads. Ranked by trust-earned rate first (the strictest real signal), then raw success rate, then attempt volume. **Honest scope note**: this is Tier-0 — real, mandatory data capture, wired live — not yet a hard gate on model SELECTION (i.e., `TRUSTED_FALLBACK_MODELS`' order isn't auto-adjusted by this data yet). That's the deliberate next step once there's enough real attempt volume to trust the signal, same discipline as the Creative Engine plan's Phase C capability matrix. | Founder, direct, angry about the pattern of suggestions without enforcement: "Everything I ask needs to be fucking hard gated, or it's just a suggestion... You follow it through until it is absolutely impossible for the system not to do it." | `node --check` clean on all 3 files; could not test against a local DB (local `DATABASE_URL` isn't a live connection in this environment, consistent with the already-known local/production `.env` drift) — will be verified the same way everything else this session was verified: a real governed dispatch after deploy, then `GET /factory/model-rankings` checked for real recorded data, not assumed from the code. | Verify live: dispatch a real step, confirm `GET /factory/model-rankings` shows a fresh row. Then: (1) build the second half of the founder's ask — a distinct SENTRY finding classification for "code correctly matches the blueprint but the blueprint itself was wrong," routed to architect-level review, which does not exist anywhere yet. (2) once there's real attempt volume, use this ledger's data to actually reorder `TRUSTED_FALLBACK_MODELS` by measured trust-earned rate instead of a fixed hand-picked order. |
| 2026-07-27 | **Follow-up: re-verified the spend-gate failover fix live and found the single-attempt cascade wasn't enough — expanded to retry across every free provider, not just the first.** Re-sent the identical real message through the LifeRE "Ask Chair" panel right after deploying the previous fix: no longer "Nothing ran," but the chosen fallback (`groq_llama` / `llama-3.1-8b-instant`) 413'd — `Request too large... TPM: Limit 6000, Requested 6431` — a real per-model rate limit on Groq's smallest free model, hit by the founder-interface's normal system-prompt overhead. All three spend gates shared one underlying limitation: `getNextAvailable()` was called once, and if that one provider's real call then failed, the error just propagated — never tried a second free provider. Extracted a shared `cascadeToFreeTier(blockedMember, reasonLabel)` helper (replacing 3 copies of near-identical inline logic) that loops through up to `PROVIDER_LIMITS` length providers, excluding ones already tried, until one actually succeeds or all are exhausted — real fix to "never idle," not just "try once and call it done." | Direct re-verification caught the first fix was real but incomplete — exactly the "test as the client before citing a pass" standard, applied to my own fix, not just the system's. | `node --check services/council-service.js` clean | Verify live once deployed: resend the same message a third time and confirm a real reply comes back (Gemini's free tier has far more TPM headroom than Groq's 8b-instant and should succeed where Groq didn't). |
| 2026-07-27 | **Fixed the one spend gate (of three) in `services/council-service.js` that had no free-tier failover — the exact gate blocking the founder-interface chat live tonight.** Caught live: a real end-to-end test through the new LifeRE "Ask Chair" panel returned `"Daily spend limit ($30) reached at $30.0003... Nothing ran."` — a hard stop, not a template. Traced to `callCouncilMember`'s third spend check (`spend >= effectiveSpendCap`, ~line 957): the first two spend gates in the same function (`MAX_DAILY_SPEND===0` and `spend >= effectiveShutdownThreshold`) both already cascade to a free-tier provider (Groq/Gemini/Cerebras/etc.) before giving up; this third one — the actual `MAX_DAILY_SPEND` cap itself — just threw, with no fallback. Direct violation of SO-003's own text: *"never idle because one provider ran dry... the only hard stop is the daily budget cap."* Fix: added the same free-tier cascade to this third gate, matching the other two exactly. **The paid-spend cap itself is unchanged and still hard** — no spend beyond `MAX_DAILY_SPEND`, no silent raise, nothing that touches the founder's actual financial constraint. This only ensures reasoning fails over to a $0 provider instead of going fully idle once the paid cap is hit, same as already happens everywhere else in this file. | Founder, live: "Most likely, it's because I'm out of money... I have to wait till I can get some more money" — investigated directly rather than assumed; confirmed true for this path via a real behavioral test, then fixed the narrow, specific gap (missing failover) without touching the cap itself (a real financial decision, explicitly not mine to make). | `node --check services/council-service.js` clean | Verify live once deployed: resend the same real message through the LifeRE "Ask Chair" panel and confirm it now returns a real free-tier reply instead of the spend-limit error (until midnight UTC when the cap resets on its own either way). |
| 2026-07-21 | **Honor `maxTokens` as an output-budget alias (systemic silent-truncation fix).** `services/council-service.js` `scopedMaxTokens` read **only** `options.maxOutputTokens`, so the many live call sites that pass `options.maxTokens` were silently ignored and clamped to the task-type default (400 for `json`/`extraction`, 800 otherwise). Long JSON/content responses were truncated mid-array → callers fell back to templates/heuristics. This was the root cause of the SMOS $49-pack echoing the raw transcript, and it also affected ClientCare VOB extraction (`maxTokens:6144`, taskType `json` → capped 400), LifeRE sales coaching (`1200`), and YouTube research (`2800`). Fix: `const n = options.maxOutputTokens ?? options.maxTokens;` (still clamped to the 128k hard cap). `callCouncilWithFailover` delegates to `callCouncilMember` with the same options, so both paths are covered. | Founder walk found SMOS shipping transcript-echo as premium content; code scan proved the same truncation class hit multiple products. Adam: no theater; keep going on every product. | `node --check services/council-service.js` PASS; SMOS generate re-verified live (real `gemini_flash` copy) | redeploy + re-verify a large-`maxTokens` product path is no longer truncated |
...
| 2026-08-02 | **GAP-FILL: duplicate BUILD_QUEUE step export aliases for `services/generalTaskOptimizer.js`, `services/ollamaInvestigation.js`, and `services/providerCooldownManager.js`.** Added `optimizeGeneralTaskSavings = optimizeGeneralTask`, `investigateTokenUsage = investigateOllamaPrompts`, and `retrieveCooldowns = getCooldowns` to clear HARD false-done audit failures (`ai-council` steps `1`, `step2`, and `ai-council-step4` respectively) so `npm run builder:preflight` stays green. | `audit-false-done-steps.mjs --ci` ratchet found done steps whose target files no longer satisfied their declared `expected_exports`; the canonical implementations already existed, only the alias names differed. | ✅ | `node --check` clean; `npm run builder:preflight` PASS | `bp-priority:once` re-verify |
