...
| **Last Updated** | 2026-07-16 — `services/council-service.js` `recordMetered()` now accepts `outputBaselineTokens` and computes `savedOutputPct` so the ledger honestly credits avoided full-file output tokens when the codegen runner uses additive edit-patch mode (`author_then_patch`). Complements the prior `compressCodeSafe` input-token savings. Prior: 2026-07-16 — `services/token-optimizer.js` `compressCodeSafe()` now applies the phrase table to exposed instruction text after stripping markdown/whitespace and before restoring protected code fences and `old_string`/`new_string` anchors. |
