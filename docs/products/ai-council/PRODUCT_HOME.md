...
| 2026-07-31 | **`services/council-service.js` builder-lane credit-exhausted failover to free tier.** Extended the existing 429/rate-limit cascade in `callCouncilMember` to also trigger on payment/credit errors (`credit balance`, `no credits remaining`, `billing`, `payment`, `insufficient_quota`) and removed the `!builderLane` guard so the **builder/codegen lane** can fall back to funded free providers when paid providers are dry. Previously only non-builder calls cascaded; `council.builder.code` (used by `POST /api/v1/lifeos/builder/build`) hit Anthropic's `credit balance is too low` with no recovery and the never-stop factory stalled. The cascade still honors founder-comms exclusion. | SO-003: "never idle because one provider ran dry." The production `provider-key-health` check already shows Groq, Gemini, Mistral, DeepSeek, and Replicate as working; this lets the builder route use them when Anthropic/OpenAI/Together are `needs_payment`. | `node --check services/council-service.js` clean; `npm run builder:preflight` PASS (429/429). | Verify live: `POST /api/v1/lifeos/builder/build` with a code task returns generated content instead of `Council call failed` when paid providers have zero balance. |
