...
| **Last Updated** | 2026-07-21 — **Council cache fence regression fix**: cached fenced JSON now strips closing fences whether or not the model appends trailing whitespace, preventing repeat MarketingOS Generate/Extract requests from receiving invalid cached JSON and silently falling back to transcript-echo templates. Prior: 2026-07-21 — **Systemic `maxTokens` truncation fix**: `services/council-service.js` now honors `options.maxTokens` as an alias for `maxOutputTokens` (previously ignored → output clamped to 400/800-token task defaults → truncated JSON/content → template fallbacks). Root cause of the SMOS $49-pack transcript echo; also affected ClientCare VOB extraction, LifeRE coaching, YouTube research. |
...
- Council cache fence normalization is fixed on branch `cursor/critical-bug-investigation-4d26` at commit `1cc4ec9a4`: closing fences at EOF no longer poison cached JSON. Focused regression tests pass 2/2, full `npm test` passes, and `npm run builder:preflight` passes. Deployment/tip verification remains pending.
...
| 2026-07-21 | **Fix cached fenced-JSON corruption introduced with the `maxTokens` patch.** The closing-fence cleaner changed from `\s*$` to `\s$`, requiring exactly one whitespace character after the final fence. A normal model response ending directly in `` ``` `` therefore cached as valid JSON plus a stray closing fence. On a repeated MarketingOS Generate/Extract request, the exact cache hit failed `parseCouncilResponse()` and silently produced transcript-echo fallback pieces. Fix: centralize `cleanCouncilResponseForCache()` and restore zero-or-more trailing whitespace handling for every provider cache write. Added `tests/council-cache-cleaning.test.js` for closing-fence-at-EOF, trailing-whitespace, and plain-JSON cases. | Deep bug sweep traced the commit diff through council cache lookup and the live MarketingOS paid-pack parser. | `node --check services/council-service.js`; focused test 2/2; full `npm test`; `npm run builder:preflight` — PASS | deploy and tip-verify repeated MarketingOS generation no longer falls back because of cached fences |
