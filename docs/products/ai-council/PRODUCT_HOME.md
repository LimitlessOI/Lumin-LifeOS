...
| **Last Updated** | 2026-07-03 — **Byte-exact codegen, part 2 — `stripNoise` blank-line collapse (kink #18):** `services/token-optimizer.js` `compress()` called `stripNoise()` **unconditionally** (before the `if (!critical)` guard), and `stripNoise` ran `.replace(/\n{2,}/g, '\n')` — collapsing every blank line in the injected source to a single newline even for critical code prompts. Kink #17's fix skipped `stripMarkdown` (asterisks) but not this. Effect: a BuilderOS surgical edit-patch whose `old_string` anchor spanned a blank line (e.g. `import …household-sync.js';\n\nconst ACCESS_COOKIE_NAME`) got the blank line eaten, so the model faithfully copied the collapsed version and `applyTargetedEdits` rejected it as "anchor not found" (single-line anchors were unaffected, which is why the first edit-patch verify passed but a multi-edit failed). Fix: `stripNoise(text, {critical})` now only normalizes CRLF→LF (safe, matches on-disk LF) when `critical`, skipping the blank-line/indent/trailing-whitespace collapse. Verified: live chat **multi-edit** build (change `ACCESS_COOKIE_MAX_AGE_MS` to 7 days **and** rename `ACCESS_COOKIE_NAME`) went from `edit old_string not found` → both edits applied, `PASS`/`committed:true`, `node --check` OK. Regression: `tests/token-optimizer-critical.test.js` (5 cases — blank-line/indent/asterisk preservation, CRLF normalization, non-critical still compresses); `npm test` 254/0. Prior: 2026-07-03 — **Code-generation is byte-exact (compression bypass):** `services/council-service.js` now treats any `code`/`codegen` task as `isCritical`, so the lossy prompt-compression layers (Layer 1 markdown-strip + phrase-sub, Layer 1.5 LCL codebook symbol substitution, Layer 4 IR) are all skipped for code. Root cause of a BuilderOS build corruption (kink #17): those layers ran on codegen prompts and parsed injected source such as `24 * 60 * 60 * 1000` as **markdown emphasis**, stripping the asterisks to `24 60 60 * 1000` before the model saw it — so the model faithfully reproduced corrupted source (both OpenAI and Claude produced the identical corruption, proving it was upstream of the model, not a fidelity issue). This broke surgical edit-patch `old_string` anchors and could inject subtly-wrong arithmetic/operators into any code build that references an existing file. Non-code tasks still compress normally. Verified: a live chat build's edit-patch anchor went from `24 60 60 * 1000` (rejected) to exact `24 * 60 * 60 * 1000` (matched + committed); `npm test` 249/0. Prior: 2026-07-02 — **Reasoning-model temperature fix:** `services/council-service.js` no longer sends a custom `temperature` to OpenAI reasoning models (`o<n>`, `gpt-5.x/6.x`, matched by `/^(o\d|gpt-[56])/i`). Those models reject any value other than the default (1) with HTTP 400 (`Unsupported value: 'temperature' does not support 0`). Deterministic builder codegen/json tasks set `temperature: 0`, so every BuilderOS `/build` on a gpt-5.x escalation lane hard-failed with "Council call failed" before any code was generated. Non-reasoning models still receive the deterministic temperature. Verified by firing a real `/build` that then proceeded past codegen. Prior: 2026-07-02 — **Gemini thinking-mode disabled on the fast flash lane (founder-chat unblock):** `services/council-service.js` + `factory-staging/factory-core/canon/services/council-service.js` now send `generationConfig.thinkingConfig.thinkingBudget = 0` (override via `options.thinkingBudget` / `GEMINI_THINKING_BUDGET`) for `gemini-2.5-flash`. Gemini 2.5 flash enables "thinking" by default, which cost ~7.5s per trivial call and far more on the chair's larger prompts — a founder chat turn stacks several sequential flash calls, so every turn blew past the 45s frontend abort and returned "Lumin timed out … nothing completed." Measured: identical prompt dropped from 7.5s (thinking on) to 0.59s (thinking off). Cheaper too (no billed thinking tokens), matching the free-flash "fast, lightweight reasoning" mandate. Prior: 2026-07-02 — **Railway IPv6 bind fix:** `config/runtime-env.js` `HOST` default changed from `0.0.0.0` (IPv4-only) to `::` (dual-stack). Railway's edge reaches containers over its IPv6 private network, so an IPv4-only bind let the edge complete TLS but receive 0 bytes back — the exact production request-timeout the founder saw on `robust-magic-production.up.railway.app`. Dual-stack accepts both IPv4 and IPv6. Prior: 2026-07-01 — **Builder env hot-reload pass:** `services/council-model-availability.js` now refreshes `.env.builderos` on demand through `config/runtime-env.js`, so local builder/council availability checks stop staying stale after a founder saves or fixes provider keys without restarting the process. Prior: 2026-07-01 — **Builder lane hard timeout pass:** `services/council-service.js` now applies `AbortSignal.timeout(COUNCIL_TIMEOUT_MS)` to the OpenAI-compatible and DeepSeek provider fetch paths, not just Gemini/Anthropic. Builder lanes on Railway can now fail fast with a structured timeout error instead of hanging a founder-facing `/builder/build` request until the edge gives up. Prior: 2026-06-30 — **Builder lane routing truth + Ollama retirement pass:** `services/council-model-availability.js` now reports `openai`, `github_models`, and `fireworks` key presence correctly and hard-blocks `ollama` as `ollama_retired_by_founder_directive`; `services/council-service.js` no longer counts Ollama as a free provider, no longer silently cascades BuilderOS requests into generic free-tier models under spend shutdown, and allows a dedicated `BUILDEROS_MAX_DAILY_SPEND` cap for builder lanes. Follow-up cleanup on 2026-06-30 removed live Ollama member definitions from `config/council-members.js`, stripped retired Ollama wording from active startup/status surfaces, and archived the dedicated Ollama docs/scripts/adapter under `docs/history/legacy-history-salvage/ollama-retired/`. Prior: 2026-06-02 — **Gemini 413 non-retryable fast-fail** in `services/council-service.js`. When Gemini returns HTTP 413, throws `err.code='PROMPT_TOO_LARGE'` + `err.nonRetryable=true` instead of a generic Error — prevents failover retry chains that accumulate past Railway's 60s proxy timeout. Prior: 2026-06-01 — constitutional refactor alignment: AIC clarified as mission-attached deliberation layer with outcome accountability, not automatic truth. Prior: 2026-05-11 — **`services/lifeos-gate-change-council-run.js`** **`@ssot`** corrected **LifeOS → Amendment 01** (gate-change debate is council SSOT; fixes **`npm run ssot:validate`** when only council service changes). Prior: 2026-04-28 — **Railway-aware builder routing hardening:** `services/council-model-availability.js` now marks Ollama unavailable on Railway unless a real non-local endpoint is explicitly configured; this prevents council/builder memory routing from selecting local-only Ollama models in production. Prior: 2026-04-27 — **`services/council-model-availability.js`** committed (was untracked, crashing Railway boot). Prior: 2026-04-26 — **future-back consensus artifact** required for gate-change debate; runtime authority from Memory Intelligence may override static model routing. Prior: 2026-04-25 — **`council.builder.code_execute`** → **`groq_llama`** (builder **`execution_only: true`** + `mode: code`, no `model` override). Prior: 2026-04-24 — **`services/savings-ledger.js` `getSavingsReport`:** exposes full monetization columns from rebuilt `tsos_savings_report` view. Prior: **`maxOutputTokens`** in **`council-service.js`**. Prior: explicit **`council.builder.code` / `plan` / `review`**. Prior: **`@ssot`** council-service. Prior: **`POST /gate-change/run-preset`:** server-side debate. |
