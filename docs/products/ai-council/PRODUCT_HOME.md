...
| **Last Updated** | 2026-07-21 — **Systemic `maxTokens` truncation fix**: `services/council-service.js` now honors `options.maxTokens` as an alias for `maxOutputTokens` (previously ignored → output clamped to 400/800-token task defaults → truncated JSON/content → template fallbacks). Root cause of the SMOS $49-pack transcript echo; also affected ClientCare VOB extraction, LifeRE coaching, YouTube research. Prior: 2026-07-15 — Chat-layer provider failover: `services/chair-direct-agent.js` (Chair/BuilderOS front door) and `services/council-prompt-adapter.js` (legacy Lumin chat) now default to `openai_gpt` and auto-cascade across `deepseek`, `gemini_flash`, `claude_sonnet` on credit/quota/auth/timeout errors. Previously the founder chat was hard-wired to `claude_sonnet`/Anthropic and returned `credit balance is too low` with no recovery. `CHAIR_DIRECT_AGENT_CASCADE` and `CHAT_COUNCIL_CASCADE` env vars let operators override the default cascade order without a code change. |
