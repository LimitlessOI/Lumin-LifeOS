...
| **Last Updated** | 2026-07-16 — `services/token-optimizer.js` `compressCodeSafe()` now applies the phrase table to exposed instruction text after stripping markdown/whitespace and before restoring protected code fences and `old_string`/`new_string` anchors. This raises input-token savings for `codegen` and `code` prompts (the only critical task types that use `compressCodeSafe`) without touching byte-exact code zones. The change targets the lowest `tsos:builder` leg (`token_efficiency`) while preserving codegen correctness. Prior: 2026-07-12 — Chair communication model benchmark (`scripts/benchmark-chair-communication.mjs`) measured `openai_gpt`, `deepseek`, `gemini_flash`, `claude_sonnet` on a realistic founder-style prompt. `gemini_flash` won (score 99, 1473ms, $0), `openai_gpt` second (score 98, 1988ms, $0.0018); `claude_sonnet` and `deepseek` were unreachable in this run. Updated `services/council-prompt-adapter.js` default preferred member to `gemini_flash` and cascade to `gemini_flash,openai_gpt,deepseek,claude_sonnet`. Chair direct agent (`services/chair-direct-agent.js`) default model and cascade updated to match. |
