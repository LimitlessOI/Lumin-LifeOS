...
| **Last Updated** | 2026-08-10 — Real email successfully sent to the founder via AX-driven Gmail compose (verified two independent ways, not self-claimed). Prior same day: fixed `makeAccountConfirmer` self-blocking on Google's own login redirect. Prior: 2026-07-27 **ADAM_GMAIL browser send path** (SMTP confirmed blocked on Railway — independently re-confirmed live tonight). Prior: GLVAR MFA via IMAP + send-as-founder. |
