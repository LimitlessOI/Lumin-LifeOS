...
| **Last Updated** | 2026-08-10 — Fixed `makeAccountConfirmer` self-blocking on Google's own login redirect (`accounts.google.com` now trusted for `*.google.com` targets), unblocking the browser-driven Gmail send path. Prior: 2026-07-27 **ADAM_GMAIL browser send path** (SMTP confirmed blocked on Railway — independently re-confirmed live tonight). Prior: GLVAR MFA via IMAP + send-as-founder. |
