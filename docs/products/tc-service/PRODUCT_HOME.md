...
|---|---|---|---|---|---|
| 2026-07-07 | **`core/notification-service.js` Railway Gmail SMTP hardening:** resolve host to IPv4, `tls.servername`, force port 465 on Railway when env is 587; suppression schema resilient; `outreach_log.sent_at`. | Site Builder cold email failing with `ENETUNREACH` / connection timeout on `:587` IPv6 from Railway. | ✅ | pending manifest | prod resend-outreach |
