...
| **Last Updated** | 2026-07-27 — **GLVAR Clareity MFA via TC IMAP** + `POST /api/v1/tc/email/send-as-founder` (adam@hopkinsgroup.org SMTP). Prior: TC market CTA. |
...
|---|---|---|---|---|---|
| 2026-07-27 | **CRITICAL: the autonomous factory's GLVAR MFA commit (`a17d0bf06b`) silently stripped `requireLifeOSAdmin` off all 121 TC routes and the import itself — full regression of the critical cross-client data-leak fix shipped earlier the same night (`e66673fe57`), live in production before this fix.** Confirmed via `git show a17d0bf06b:routes/tc-routes.js \| grep -c requireLifeOSAdmin` → `0`, and confirmed the vulnerable commit was already deployed live (`GET /api/v1/lifeos/builder/ready` showed `deploy_commit_sha: a17d0bf06b...`) — this was not a theoretical risk, any authenticated LifeOS user could read/write any client's TC transaction data again for the window it was live. Also: the new `POST /api/v1/tc/email/send-as-founder` route (sends real outbound email as `adam@hopkinsgroup.org` via Gmail SMTP) shipped with only `requireKey` — any authenticated user, not just an admin, could impersonate the founder in outbound email. Fixed: re-added the import, re-ran the same scoped regex used for the original fix (`router.<method>('path', requireKey` → `..., requireKey, requireLifeOSAdmin`, matched 121 occurrences this time, function's own `{ pool, requireKey, coordinator, ... }` destructuring confirmed untouched), and confirmed `send-as-founder` now carries `requireKey, requireLifeOSAdmin`. **Real, unresolved governance gap, not yet fixed**: the governed factory has no mechanism to detect or preserve a prior security-motivated middleware addition when it later touches the same file for an unrelated feature — this is the second time tonight a real security property was silently lost by downstream work (the first being the false-shipped-commit issue found earlier). A regression test asserting every `/api/v1/tc/*` route requires admin auth, run in CI or by SENTRY, is the real fix and is not yet built. | Found live while reviewing the GLVAR commit for the sensitive send-as-founder capability — never trust an autonomous commit's message, always read the diff, same standard applied all session. | `node --check routes/tc-routes.js` clean; `grep` confirmed zero routes with `requireKey` but missing `requireLifeOSAdmin` after the fix | Verify live once deployed: unauthenticated + non-admin-authenticated requests to `/api/v1/tc/transactions` should 401/403 again; build the regression test / SENTRY assertion so this class of silent regression is caught automatically next time, not by manual review. |
| 2026-07-27 | **GLVAR MFA auto via IMAP + founder personal send.** `loginToGLVAR` completes Clareity 2FA by polling `adam@hopkinsgroup.org` TC IMAP (or `mfaCode` override). `POST /api/v1/tc/email/send-as-founder` sends as Adam via Gmail SMTP/app password. Browser-agent `site:glvar` passes autoMfa. | Founder: find open-house MLS spots via GLVAR and email listing agents from his inbox — MFA was hard-stopping login. | ✅ node --check | tip login + MLS search + send |
