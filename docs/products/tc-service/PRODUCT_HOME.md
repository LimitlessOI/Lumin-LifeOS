...
|---|---|---|---|---|---|
| 2026-07-07 | **`core/notification-service.js` SMTP on Railway:** IPv4-only DNS lookup, default port **465**, connection timeouts; suppression check resilient to schema drift; `outreach_log` uses `sent_at`. | Site Builder cold email + TC outbound blocked or failing on Railway (`ENETUNREACH` / connection timeout). | ✅ | pending manifest | prod resend-outreach |
