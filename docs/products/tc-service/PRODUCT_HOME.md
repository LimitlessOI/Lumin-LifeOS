...
| **Last Updated** | 2026-07-21 — **TC billing hardened: non-UUID agentId no longer 500s / orphans a Stripe session.** Audit + live prod probe found every `tc_*` table types `agent_id` as `uuid`, but `routes/tcBillingRoutes.mjs` passed the raw client-supplied `agentId` straight into Postgres. A valid UUID worked end-to-end (verified: real `cs_live_` subscription URL created), but a non-UUID id (e.g. `"agent-42"`) threw `invalid input syntax for type uuid` → **500** — and in `/subscribe` that fired *after* the Stripe checkout session was created but *before* the URL was returned (orphan session + leaked DB error). Fix: validate `agentId` is a UUID at the boundary of `/subscribe`, `/success`, `/status/:agentId` and fail clean with a **400** *before* any Stripe call. **Known gap (not fixed here — flagged for factory build):** the only page that calls `/subscribe` (`public/overlay/tc-agent-enroll.html`) is a broken stub — posts `{agentName,email,planTier,paymentMethodToken}` (backend needs `agentId`+`tier`), sends no auth key, reads `data.subscriptionId` (backend returns `session_id`+Stripe `url`), and never redirects to Stripe → TC has **no working self-serve frontend** yet; backend money path is sound for UUID callers. Prior: 2026-07-20 — **Replaced the fake TC billing stub with a REAL Stripe subscription money path.** `routes/tcBillingRoutes.mjs` `POST /api/tc/billing/subscribe` previously asked a council member (AI) to *fabricate* `stripe_customer_id`/`stripe_subscription_id` and never charged anyone (honesty violation + no revenue). Now creates a real `mode:'subscription'` Stripe Checkout session (inline tiered recurring price_data: starter/pro/enterprise, env-overridable), records an `'incomplete'` intent, and grants access only via a new fail-closed `GET /api/tc/billing/success` that verifies the paid session then flips `tc_billing_subscriptions.status='active'` + stores stripe ids. Webhook no longer fabricates via AI (records events for audit). Prior: 2026-07-16 — Deterministic Cloudflare dash-session DNS writer for taloaos.com. |
