...
|---|---|---|---|---|---|
| 2026-07-27 | **⚠️ TC billing (`services/tc-stripe-service.js`) checked and deliberately NOT wired — the code itself is unfinished/unsafe, not just unwired, unlike AI Receptionist and MLS which were genuinely complete.** An audit flagged this as dead (zero callers). Read the actual implementation: `recordClosingFee()` hardcodes `payment_method: 'pm_card_visa'` — Stripe's TEST-MODE dummy card token — so it cannot charge a real customer even if wired live; `createSubscription()` builds a fake customer email (`agent${agentId}@example.com`) instead of the real client's email; `STRIPE_PRICE_IDS` (`price_123456789` etc.) look like placeholder examples, not real configured Stripe price IDs; and both `createSubscription`/`recordClosingFee`/`cancelSubscription` do `new Pool(pool)` — wrapping an already-constructed pg Pool as if it were raw connection config, likely broken. Wiring this in as-is risks a payment flow that silently fails, errors, or behaves unpredictably in production — the opposite of "make it a 10." Left unwired and undeployed; real completion (actual Stripe price IDs, which only the founder can provide — a pricing decision, not a technical one; real client email resolution; the Pool bug) is separate, real, larger follow-up work, not rushed tonight. | Founder: "audit everything... fix gaps/bugs as you find them." Read-before-wire discipline — the same audit that correctly found AI Receptionist/MLS as real-but-unwired also flagged this, and closer reading showed this one is a different, more serious class of gap. | GAP-FILL | — | Direct read of the full 102-line file; not guessed |
| 2026-07-27 | **MLS Deal Scanner (real, DB-backed investor/deal-matching — `mls_investors`/`mls_deal_matches`) was only wired into the dead full-runtime lane, same class of gap as AI Receptionist — wired into the live lane.** Verified `routes/mls-routes.js` is genuinely real logic (not a stub): real parameterized DB queries, a real `services/mls-deal-scanner.js`. Wired `createMLSRoutes` into `startup/register-founder-runtime-routes.js`, admin-gated with the same `requireUserOrKey + requireLifeOSAdmin` composition used for TC Service (investor/deal data is comparably sensitive real client data), not the plain `requireKey` the dead lane used. Also added the missing `@ssot` tag to `routes/mls-routes.js` (had none). | Founder: "audit everything... fix gaps/bugs as you find them... make it fucking happen." | GAP-FILL | — | `node --check` clean |
| 2026-07-27 | **SECURITY — CRITICAL: fixed a real, active cross-client data leak — any authenticated LifeOS user (not just an admin) could read/write ANY client's TC transaction data (PII, purchase price, party emails/phones) by guessing a transaction ID.** `getTransaction(id)` (`services/tc-coordinator.js`) does `SELECT * FROM tc_transactions WHERE id=$1` with zero ownership filter; the ~10 affected routes (`GET /transactions/:id` and siblings — status, workflow, interactions, document-requests, communications, alerts, approvals, showings, `routes/tc-routes.js`) are gated by `requireUserOrKey`, which accepts any valid per-user LifeOS JWT platform-wide, not TC-specific and not admin-only — meaning a real paying customer of a *different* LifeOS product (e.g. a $49 SocialMediaOS pack buyer) with a valid session could reach these routes. Found by an independent audit, verified directly (confirmed the auth binding, confirmed `tc_transactions` has zero tenant-isolation column at all — unlike 5 other TC tables that already use an `agent_id uuid` convention). **Real judgment call on scope, taken to the system's actual Chair/Council deliberation mechanism per founder standing order** (`POST /api/v1/lifere/council/deliberate`, live AI reasoning not templated, receipt persisted at `products/receipts/LIFERE_COUNCIL_1785125990980.json`): recommended a narrow interim fix over a full multi-tenant schema migration, given TC billing is separately confirmed dead (`tc-stripe-service.js` has zero callers) so there is no evidence of real, distinct paying TC clients today. On review, chose an even lower-risk implementation of that same scope than a schema migration would require: since transactions are created via automated email-parsing (not per-user submission) — genuinely single-operator by architecture, not just current usage — added `requireLifeOSAdmin` (an existing, already-used-elsewhere middleware, `middleware/lifeos-auth-middleware.js`) as a second gate on all 120 route-middleware call sites in `routes/tc-routes.js`, precisely verified via a scoped regex replacement (not a blind find-replace, which would have corrupted the function's own parameter destructuring). Zero schema changes, zero risk to existing real transaction data. Closes the actual exploit path completely: a non-admin authenticated LifeOS user now gets 403 on every TC route. | Founder: "audit everything... fix gaps/bugs as you find them... make it fucking happen," and separately: "go to the chair in our system and have a debate... once you guys have consensus, do that." Real crisis-mode security audit; real judgment call routed through the system's actual deliberation mechanism, not decided solo. | GAP-FILL | — | `node --check` clean; direct spot-check of the replaced route (`router.get('/transactions/:id', requireKey, requireLifeOSAdmin, ...)`) confirms correct middleware ordering |
