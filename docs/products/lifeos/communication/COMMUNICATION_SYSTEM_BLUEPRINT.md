...
**Last Updated:** 2026-08-10 — Canonical interface direction captured: the floating sphere / Taloa window supersedes the V0.5 "sandbox" framing (§21.1); real macOS proof-of-concept built and run live (`scripts/prototype-macos-floating-overlay.swift`).
...
**Two concrete, automatic triggers for the deep-verification mode (2026-08-07 — founder-specified, not a design guess):** most turns should stay fast and conversational — the reference feel is an ordinary back-and-forth, not a research session every time. Two conditions should automatically leave that fast path, without waiting for the user to ask for it:

1. **Any request to build or change the system.** This is the Capability Confidence gate above, sharpened: it is not "task requests" generically, it is specifically build/change requests. A reply carries low cost if it's slightly off; a claimed build/change carries the full cost of the founder trusting something that didn't actually happen — the exact repeated failure mode this session found live (a CSS patch nearly misdiagnosed as a no-op, a multipart-encoding bug that looked fine until tested against real production, the same runtime-lane bug silently killing a feature four separate times). "PASS" must mean verified against the real, live result, never just "the commit succeeded."
2. **Brainstorming, planning, and decision-making moments.** These already exist as named moment types in §5.1 — this makes explicit what was implicit: they inherently warrant real research and deeper engagement, not a fast reply budget, because the cost of a shallow answer compounds directly into the user's own thinking and decisions.

Everything else — factual/status inquiries, ordinary conversation — stays on the fast path. The mechanism for "going deep" is not a separate system: it's simply not artificially bounding the investigation (tool calls, verification steps, time) the way a fixed-turn conversational loop otherwise would.
...
### 21.1 Canonical interface direction: the floating sphere / Taloa window (2026-08-10, founder-directed — supersedes V0.5 framing)

**Status: this is not brainstorm.** Founder, live, explicit: *"Everything we have made up to this point has been trash... this is our user interface."* This section defines the target shell for the whole product going forward, not one optional surface among several. It supersedes V0.5's "playable sandbox only" framing below — the sandbox's face/expression work is still valid and gets absorbed into this shell, but the shell itself is now the real target, not a someday polish item.

**The spec, in the founder's own terms:**
- A small sphere/badge sits **above every application, not confined to a browser tab or a single app's window** — visible and reachable regardless of what the user is doing on the machine.
- **Freeform resize, any edge or corner** — the user grabs any part of the boundary and drags to shrink or expand, not fixed preset sizes.
- **Full size continuum**, not a binary badge/expanded toggle: from a tiny corner sphere up through something resembling a familiar AI chat surface (ChatGPT/Gemini-style panel — the founder's own reference points) up to a full application-sized window, and back down, fluidly, at the user's control.
- **On request, it becomes a data surface, not just a chat bubble** — e.g. "pull up the report on that project, what's the token spend, what's the hang-up" should render as a real panel inside this same window, not a separate app or a text answer describing the data.
- **The system decides what's layered above what** when asked — bring one program forward, allow another to sit behind, compose the visible screen around what's actually needed in the moment, matching the existing Fluid, context-narrowing UI principle below but applied to real window layering, not just which UI elements render inside one page.

**Why this is a distinct technical layer, not an extension of what already exists — checked directly against the code, not assumed:**
- The Chrome extension overlay (`extension/content.js`, §19.12) is real and shipped, but it only exists **inside a browser tab** — it cannot appear above Finder, a native app, or another browser window. It cannot satisfy "doesn't matter what app I'm in."
- The V0.5 avatar prototype (`scripts/prototype-avatar-widget.html`) is real and playable, but it is also **a webpage**, not a system-level window — same limitation.
- What "above everything, any app, freely resizable" requires is a **native, always-on-top, transparent compositor window** at the operating-system level — a fundamentally different primitive than either of the above.

**Real proof this is buildable, not speculative — built and run live tonight:** `scripts/prototype-macos-floating-overlay.swift`. A borderless, transparent `NSWindow` set to `.floating` level with `collectionBehavior = [.canJoinAllSpaces, .fullScreenAuxiliary]`, draggable, compiled with `swiftc` and run as a live background process on the founder's actual machine — confirmed rendering above other applications and persisting across window switches. This is the floating-above-everything mechanic proven on real hardware, nothing more.

**What is proven vs. what is still unbuilt, stated plainly so nothing is overclaimed:**
| Proven tonight | Not yet built |
|---|---|
| Borderless, transparent, always-on-top window that stays above other apps and joins every Space | Freeform edge/corner resize (currently fixed size) |
| Compiles and runs on the founder's real Mac, not just a design | Size continuum from sphere → chat-panel → full-app view |
| Basic drag-to-reposition | Face/expression layer from the V0.5 avatar prototype, ported into this window |
| | Any data panel content — this is currently an empty badge, not wired to any LifeOS backend, report, or the token-spend example above |
| | Click-through vs. interactive mode toggling (`ignoresMouseEvents`) |
| | Windows/Linux/iOS equivalents — this proof is macOS-only (`NSWindow`/AppKit); no cross-platform path exists yet |

**Relationship to the rest of this blueprint:** this section defines the *shell* — where Taloa lives on screen and how the user resizes/repositions it. The engines that decide *what Taloa says and does* inside that shell are unchanged: Conversation Composer (§7), cognitive modes (§6), Evidence Fusion (§19.2), and the V1–V5 roadmap (§22) all still apply — they simply render inside this window instead of inside a browser iframe. The V0.5 row in §22's version table is updated below to point here instead of standing alone.

**Governance note:** the native window-compositor code (Swift/AppKit, like the Android accessibility plugin and the Chrome extension's `content.js`) is client-side and hand-authorable under the same precedent already established for those two surfaces — outside SO-001's `services/`/`routes/`/`middleware/` governed-factory scope. Any backend work this shell eventually calls into (pulling a real project/token-spend report, per the example above) does go through the governed factory like any other `services/`/`routes/` module.

**Build receipts (2026-08-10) — all 5 phases from the founder-directed build plan, real code at `native/macos-overlay/`:**

| Phase | What shipped | Real verification |
|---|---|---|
| 1 — Freeform resize | `ContainerView.swift`: edge/corner hit-zone detection (10pt margin) + drag-to-resize math for all 8 edges/corners, min-size clamp | `swiftc` compiles clean; resize algebra checked by hand (each edge case keeps the opposite edge fixed). **Not independently live-tested** — deliberately skipped synthetic mouse-drag automation given 3 unknown-arrangement monitors and live content on screen; real click-and-drag by the founder is still needed to confirm the interactive feel. |
| 2 — Size-continuum embed | `ContainerView.swift`: swaps native `BadgeView` for a `WKWebView` once either dimension crosses 160pt, loading the real production `/lifeos` URL | WKWebView load path verified standalone via `webview-load-test.swift` (see Phase 4) — the threshold-crossing trigger itself shares the same untested-live-resize gap as Phase 1. |
| 3 — Face/expression layer | `BadgeView.swift`: native Core Graphics port of the HTML prototype's 5 expressions (neutral/happy/thoughtful/concerned/surprised) and mode border colors (idle/listening/thinking/speaking/impatient), plus idle bob animation | Compiled and run live; one screenshot taken and immediately deleted after confirming the badge renders correctly in the corner — screenshot not kept since it also captured unrelated content on the founder's screen. |
| 4 — Data-panel behavior | No new code — Phase 2's WKWebView loading the real app *is* the data panel | **Live-verified with a real network round-trip**, not assumed: standalone harness (`webview-load-test.swift`) loaded `https://lumin-web-production-e3a9.up.railway.app/lifeos?native=1...` and got a real `PASS`, `document.title="LifeOS — Sign in"`. Honest finding, not a bug: a fresh `WKWebView` has no session, so first expand shows the real login page — same first-run UX as the already-shipped Capacitor app. `WKWebView`'s default (non-ephemeral) data store persists cookies to disk, so after one login it should stay signed in on later launches — this persistence-across-launches claim is *not yet independently confirmed*, only the standard documented WKWebView behavior it relies on. |
| 5 — Polish | Click-through toggle (Cmd+Shift+T → `window.ignoresMouseEvents`); `SMAppService`-based launch-at-login; real `Taloa.app` bundle (`build.sh`, ad-hoc codesigned) | App bundle builds and launches clean (`build/Taloa.app`, PID confirmed running). Launch-at-login: Apple's own `SMAppService.mainApp.status` self-reported `.enabled` after `register()` — first-party evidence. A second, independent cross-check via `sfltool dumpbtm` did **not** corroborate (empty result unprivileged; a `sudo` attempt hung on an interactive password prompt this session can't answer, and was killed rather than left hanging) — noted honestly as an unresolved secondary-verification gap, not swept under the rug. Click-through toggle and multi-monitor drag are implemented but **not interactively tested**, same reasoning as Phase 1. |

**Addendum (2026-08-10, same session): `SprenView.swift` superseded by `TaloaCharacterView.swift`.** Direct founder feedback after seeing the glowing-ribbon version live: *"the aura's cool, but I don't see a character that I can talk to, and it becomes the face of this program."* A thin animated line read as decoration, not a conversational presence — eyes are the minimum signal that makes something read as a being looking at you, not an effect. Rebuilt as a soft glowing blob body with real eyes (blink cycle, wandering gaze that eases toward random targets, pupils), plus an expression-driven mouth, keeping the same mode-driven glow/color system. Founder also clarified scope: personality expressed through the animation itself (color/motion/gaze) is the right bar for now, not a full behavior-template library. **Live-verified, not assumed:** screenshotted, cropped/zoomed to the character specifically (not just glanced at full-screen), confirmed real eyes with visible pupils mid-gaze-wander and a visible mouth curve — then the screenshot was deleted per the self-verification discipline below.

**Addendum: verification discipline correction (2026-08-10).** Founder feedback, direct: *"there's no point me sending you screenshots anymore. Why are you asking me to? I see it."* — after this session asked him to look and describe results multiple times despite already having working `screencapture` access. Corrected going forward: when a verification tool is available, use it and report the finding; don't offload the check to the founder. Saved as a standing memory (`feedback_self_verify_dont_offload_to_user`), not just a one-off fix.

**Addendum (2026-08-10, same session): `TaloaCharacterView.swift` superseded by `TaloaFairyView.swift`.** Founder ask: *"I really want to be like a character. Maybe even a combination Cortana and Tinkerbell, Syl from The Way of Kings."* Rebuilt as an actual small humanoid figure — glowing head (reusing the eye/blink/gaze/mouth system from the blob version unchanged), a flowing scalloped-hem dress-silhouette body, two translucent wings with a subtle flutter animation, and a Tinkerbell-style sparkle/dust trail — all still procedural shapes with glow, no art assets, same tooling constraint as every prior pass. **Live-verified, not assumed:** screenshotted, cropped/zoomed directly on the figure, confirmed a real recognizable head+body+wings silhouette with visible face. Honest limitation seen in that same crop: the wings currently read as somewhat blocky/rectangular rather than delicate — a real next-pass refinement, not hidden.

**Addendum (2026-08-10, same session): the "no image-generation tool" limitation was real but not permanent — found a working unblock instead of reporting it as a dead end.** Founder, direct, rejecting the limitation framing: *"Don't give me problems... give me what I want despite the limitation... there's no impossible, there's only impossible that somebody figures it out."* Checked again properly rather than repeating the same answer: no image-gen tool available to Claude directly, and none installed on the founder's Mac either (verified: no Draw Things, no local Stable Diffusion checkpoints anywhere on disk, `ollama list` shows only text/vision-*understanding* models like `llava`/`qwen2.5vl`, none of which generate images). The actual unlock: `image.pollinations.ai` is a free, no-API-key, plain-`curl`-reachable text-to-image endpoint; combined with `rembg` (pip-installed locally, U2Net model, no key needed) for background removal, this produces a real transparent character PNG with zero paid services and zero founder decision needed. `TaloaFairyView.swift` (vector humanoid) superseded by `TaloaImageCharacterView.swift`, which loads `Assets/TaloaCharacter.png` (768x768, real generated art: glowing blue-cyan fairy-being, detailed face, hair, wings) and composites it with the same mode-driven colored glow as every prior version. **Live-verified, not assumed:** transparency confirmed by directly reading corner-pixel alpha values (0, not inferred from a preview), then confirmed rendering correctly in the actual running app via a real screenshot, cropped/zoomed on the character, showing it floating cleanly over live desktop content with no background rectangle — then the screenshot was deleted per standing self-verification practice.

**Known real gaps, stated plainly:** this is one static image, not a rigged/animated character — the blink/wandering-gaze/expression-driven-mouth system from `TaloaCharacterView`/`TaloaFairyView` does not apply to a fixed picture; only the background glow color still animates by mode. Getting real expression changes back would mean generating a small set of images (one per mode/expression) and cross-fading between them, or a genuine rigging pass — not done yet, named here so it isn't quietly forgotten. This is also one specific generated look from one prompt/seed, not a finalized "canonical Taloa design" — regenerating variations is cheap (same free pipeline) if this particular face isn't the right one.

**Addendum (2026-08-10, same session): autonomous wandering shipped.** Founder ask: *"can you make it walk around?"* Each per-monitor window now drifts to a new random point on its own screen every 4-9 seconds, easing gently (1.5%/frame at 30fps) rather than snapping -- paused automatically whenever the user is actively dragging/resizing that window (`ContainerView.isUserInteracting`) or once it's expanded into the real app (`isExpanded`), so autonomy never fights a real interaction. **Live-verified with a real before/after diff, not assumed:** two screenshots 8 seconds apart, cropped to the same region -- the character visibly shifted position while floating above a YouTube video, confirming real movement independent of whatever content is underneath it. Screenshots deleted after comparison per standing self-verification practice.

**Talking to it directly -- named as a real, separate, unbuilt gap, not glossed over.** Founder also asked whether he can talk to it. Honest answer given live: the real conversational engine already exists and already works -- expanding the character past ~160pt swaps it into the actual LifeOS app with the real Lumin/Taloa chat, which already has the V1 Voice Presence work built in. What does not exist is talking to the *small floating character* directly without expanding it first -- that needs a real microphone pipeline (capture, permission, speech-to-text, network call to the chat backend, playback) wired from the native small-state view, which is a distinct, larger build not started.

**Addendum (2026-08-10, same session): "cast a spell" flourish shipped, tied to real events.** Founder: *"she can pretend like she's doing a spell when she brings up a new page or a new layout."* `TaloaImageCharacterView.castSpell()` (particle burst + expanding ring + brightened glow, ~0.9s) is called from `ContainerView`'s new `WKNavigationDelegate.didStartProvisionalNavigation` -- fires only on a real page load, never on a timer, so it reads as her causing the change rather than ambient decoration. Also surfaces a small corner version of her for ~1.1s even when already expanded into the real app, so the cast is visible instead of firing while she's hidden. **Live-verified in isolation, not on the live desktop:** built a standalone harness that constructs the real `ContainerView`, resizes it past the expand threshold off-screen, and confirmed the real navigation delegate fired `castSpell()` (`"Taloa: casting (real navigation event)"` printed exactly once, no crash) -- deliberately not tested via synthetic drag on the live multi-monitor desktop, same reasoning as every prior interactive-test gap. Test harness deleted after use; a debug stderr line was kept in `castSpell()` since it's cheap, real operational signal.

**Addendum: the "personality" the founder is asking for already exists in writing, in this document -- it just isn't wired to voice or the visual character yet.** Founder: *"I want it to be somebody we talk to, somebody we relate to... a coherent, nonannoying personality... something people bond with, trust."* That is not an open design question -- §6 (Cognitive modes: Presence/Observation/Reflection/Discovery/Expansion/Guidance/Execution/Safety), §6.2 (Truthful Capability Principle -- never imply capability/certainty that isn't real, the literal mechanism of "somebody who has your back" instead of empty reassurance), and §11 (Anti-Pattern Engine -- the literal mechanism for "nonannoying," detecting repeated openings/validation phrases/question density/framework overuse before they become grating) already specify this in real detail. The gap is integration, not invention: none of §6/§6.2/§11 are wired into the floating character or a voice pipeline yet -- the character currently has zero connection to `chair-lumin-unified.js` or any real conversation turn. Naming this explicitly so "build a personality" isn't treated as starting from zero when a real, detailed spec already exists a few thousand words up in this same file.

**What this means concretely for the founder:** the whole pipeline compiles, runs, and the one genuinely uncertain risk (does the real LifeOS app actually load inside a native WKWebView) is proven with a real network call, not assumed. What's left is exactly the kind of check that needs a real hand on a real mouse: drag an edge, watch it resize; drag past 160pt, watch it become the real app; try Cmd+Shift+T; confirm Taloa reappears after a logout/login of the Mac. None of that can be safely faked from this side without risking an errant click on a live, in-use, multi-monitor desktop.
...
| **V0.5 — Floating sphere / Taloa window (canonical shell, §21.1)** | A native, always-on-top, transparent window — not a browser overlay — that lives above every application on the machine. Freeform-resizable from a small sphere up to a full app-sized panel resembling a familiar AI chat surface; houses the V0.5 avatar's face/expressions once ported in; on request, renders real data panels (e.g. a project/token-spend report) inside itself. | **Elevated from sandbox to the canonical interface direction, founder-directed 2026-08-10** ("everything we have made up to this point has been trash... this is our user interface"). Supersedes the browser-confined framing this row previously described. | Window mechanic proven live: `scripts/prototype-macos-floating-overlay.swift` — borderless/transparent/`.floating`-level `NSWindow`, joins every Space, compiled and run on the founder's real Mac. **Not yet built:** freeform resize, size continuum, avatar face ported in, any real data panel, click-through toggling, non-macOS platforms. Face/expression source material remains `scripts/prototype-avatar-widget.html`. |
| **V1 — Conversational Contracts & Voice Presence** | Promise → completion condition → fulfillment tracking; interruption decay; presence layer (listening, nodding, breathing, thinking sounds). | Highest trust-to-code ratio. Fixes the abandonment failure the founder just experienced. | Proven: `scripts/prototype-conversational-contracts-v1.mjs` passes 39/39 exhaustive tests (`products/receipts/COMMUNICATION_SYSTEM_V1_TEST_TRANSCRIPT.json`); `scripts/prototype-tonality-engine-v1.mjs` passes 15/15 tests (`products/receipts/COMMUNICATION_SYSTEM_V1_1_TONALITY_TEST_TRANSCRIPT.json`). **Real bug found and fixed 2026-08-06** (audit before wiring anything live, not after): `extractContracts` matched promise patterns against the whole turn text unanchored, so a multi-clause turn like "I want pizza and I'll call the restaurant." produced a garbled, overlapping extraction (one contract's promise text swallowed the other clause). Fixed by splitting each turn into clauses (reusing V4's clause-boundary logic) and anchoring each pattern to its own clause; added an exact-text regression test so a future edit can't silently reintroduce the overlap. Production voice-rail wiring still deferred — this was a correctness fix to the unwired prototype, not new wiring. |
...
## 24. Captured ideas not yet scheduled

Real ideas surfaced in conversation, filed in IdeaVault (not lost, not silently built either — see `docs/products/ideavault/PRODUCT_HOME.md`) rather than left only in this doc or in session context, per SO-004:

- **Constitutional Pattern Library** (IdeaVault id `0e72bd1c-9dd3-4f4b-9ec5-a88c1ee66485`, 2026-08-07, `build_priority: 20`, deliberately LATER not NOW): catalog reusable architectural patterns (Conversation State, Evidence Fusion, Consent Management, Decision Qualification, Learning Loop, Metrics, Audit Trail, etc.) as a separate library from complete Blueprint Templates, each entry carrying purpose/inputs/outputs/constitutional requirements/tests/known limitations/why-it-exists — reusing architectural reasoning, not just code. One level above ARC's existing Asset/Parts-Car Requirement (which checks concrete repo files); this would catalog abstracted, reusable contracts instead. **Deliberately sequenced after, not before, V1/V2 are real** — every module this would catalog currently has zero shipped instances; building the library now would mean designing v5 of something with no v1, the exact premature-abstraction risk the Self-Repair Doctrine already named. Correct trigger to revisit: once Conversation State (V1) and Evidence Fusion (V2) are real and proven — extract the first library entries from them, not from a design.
- **Voice UX ideas salvaged from the Voice Rail retro** (IdeaVault id `4181faab-e785-4e00-9589-8e70d1a6b0e8`, 2026-08-07, `build_priority: 20`): (1) TTS playback speed as a sliding bar; (2) a dedicated dictation-only mode — capture a raw thought via voice-to-text with no conversational processing, system reads it back to confirm; (3) live transcript + explicit manual Send control, complementary to (not competing with) V1's auto-detected turn-completion/interruption-decay; (4) tag/address specific council members or offices directly in a voice input; (5) lens selection deliberately kept automatic, not manual — confirms the existing `deriveLenses()` auto-selection in `reasoning-plan.mjs` is already the right design, not a gap to fill. Full context: `docs/products/lifeos/conversations/2026-08-07-v1-voice-presence-build-and-voice-rail-lessons.md`. Natural revisit point: after V1 ships, against V2/V3 scope (#3 and #4 touch the same conversational-turn substrate V1 is building now).
