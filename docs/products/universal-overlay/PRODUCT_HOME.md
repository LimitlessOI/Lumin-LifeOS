...
| **Stability** | `evolving` |
| **Last Updated** | 2026-08-09 — 6 real install-blocking bugs found+fixed via live founder Chrome-extension install (icons, dead backend mount, 2x stale-URL, missing manifest permissions, broken FRAME_ORIGIN/postMessage). Prior: 2026-05-19 native shell; 2026-04-20 founding scaffold. |
...
|---|---|---|---|---|
| 2026-08-09 | **Live Chrome-extension install bugs found + fixed via real founder install session:** (1) `extension/icons/icon-{16,32,48,128}.png` were referenced in manifest but never existed — generated real PNGs. (2) `routes/lifeos-extension-routes.js` was only mounted in the dead full-runtime lane (never loads in production) — mounted directly in `startup/register-founder-runtime-routes.js`, no flag, matching IdeaVault precedent. (3) `extension/background.js` fetched a stale/wrong host (`lumin-lifeos.up.railway.app`) — corrected to real production host. (4) `manifest.json` `permissions` missing `alarms`/`tabs` — `chrome.alarms.create()` in `background.js` threw without it; `node --check` doesn't catch Chrome permission-model errors. (5) `extension/popup.js` had the same stale-URL bug as (3), missed in the first pass. (6) `extension/content.js` `FRAME_ORIGIN` read `chrome.runtime.getManifest().host_permissions[0]`, which is the literal string `<all_urls>` (a permission pattern, not an origin) — truthy, so it short-circuited the intended real-URL fallback and was passed straight to `postMessage`, throwing `Invalid target origin '<all_urls>'` on every page load. Fixed by hardcoding the real Railway URL directly, same as (3)/(5). | Adam: real, live, first-ever install of the extension on his own machine — every bug found via his real screenshots/error reports, not simulated. | ✅ each fix verified live (curl 200s for backend mount + URLs; grep swept whole `extension/` tree for recurrence); (6) confirmed by Adam's own screenshot — popup shows "Connected to LifeOS" / `@adam` / real production host, no Errors badge | all 6 confirmed working live on Adam's real machine |
