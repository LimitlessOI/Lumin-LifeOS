...
| CFO | Fragmented 3x — doctrine role list, Voice Rail seat ("stewardship — speed, spend, ROI, model scorecards"), LifeRE council role; no single clean owner | **Efficiency Officer** — same underlying role, confirmed by the founder as the same office, with an expanded scope beyond the original CFO framing (2026-08-11) | "Efficiency Officer (formerly CFO)" until migration completes; the 3-way fragmentation itself is a separate, pre-existing cleanup this rename doesn't resolve on its own |
...
- Category error on Digital Twin/Imprint and (implicitly) Chair/Conductor — repo silence had been treated as founder rejection. Fixed via §0a's three-column pattern, both terms confirmed directly by the founder (not inferred from either draft) and applied throughout.
...
| 11 | Conductor (implemented as "Chair") pulling Digital Imprint + memory live, every turn | **BP** | Traced call chain `lumin-chair-orchestrator.js` → `command-control-routes.js` → `lumin-context-loader.js` → `data/twins/` |
...
| 39 | Programmatic KNOW/THINK/GUESS enforcement | **BP for BuilderOS claims; DP (instruction only) for Conductor conversation** | `services/truth-ladder.js` is real, tested, wired into the *build/ship* pipeline; zero hits in `council-service.js` (implements the Conductor role under its pre-rename "Chair" code identifiers) |
...
| 42 | "Conductor" | **UK — does not exist** | Zero hits, whole-repo grep |
...
|---|---|
| Current task / goal | Available — Conductor (Chair) already tracks conversational context |
...
| Confidence/consequence of the moment | Partially available — `truth-ladder.js` exists but isn't wired to the Conductor (Chair) conversation (§39) |
...
**Coordination model for splitting a blueprint across multiple simultaneous builders — real governance design, founder-specified 2026-08-11, closing the "who decides the split" question this document originally left open:**

Two separate consensus decisions, not one, each owned by a different pair of already-real roles — nobody unilaterally decides either:

1. **What the parts are.** Conductor proposes a decomposition of the blueprint into independently-buildable parts: what the parts are, who/what works on each, sequencing, and where they recombine. Architect confirms — specifically checking that no two parts' file-sets overlap (the real, already-proven risk this session: git lock contention and staging contamination between concurrent sessions, documented repeatedly in this product's own change receipts) and that nothing is scheduled in parallel while secretly depending on a part not yet built. Consensus is just these two agreeing before anything dispatches — not a full multi-round Council debate, a lightweight two-role check appropriate to a procedural decision.
2. **Whether to spend the capacity at all.** Conductor + Efficiency Officer (see §0a — the founder-confirmed rename of the fragmented "CFO" role, scope now explicitly broader than the original CFO framing) separately decide whether multiple simultaneous factories are actually worth spinning up, weighing priorities, goals, and speed against cost. This is a resource-allocation call, distinct from the decomposition itself.

**The queue is mechanical, not a decision-maker** — once both consensus points are reached, `BUILD_QUEUE.json`/ship-queue dispatches and tracks each part exactly as it already does today; it does not decide what the parts are or whether to use them.

**What's real vs. what would need building:** Conductor (`lumin-chair-orchestrator.js`), Architect (`architect-blueprint-writer.js`), Sentry, and the Builder/ship-queue pipeline are all real, live code today. The Efficiency Officer role has zero backing code (confirmed by repo-wide grep) — it exists only as doctrine. The specific connective mechanism described here — Steward proposes, Architect validates, dispatch follows — does not exist yet as a route or service; it is new, well-scoped work, not an assumption smuggled in as already-built.
...
| 7 | "Conductor" rename (implemented as "Chair" throughout) | **RESOLVED 2026-08-11 — confirmed directly by the founder.** "Conductor" is the ratified target name; used as "Conductor (formerly Chair)" per §0a until the rename actually propagates through the codebase. This document's own text (e.g. §11's reference to "Chair pulling memory") now labels both names together rather than picking the stale one. | Same category error as row 1, caught and fixed the same way — this is not a new kind of mistake, it's the identical one, found twice in one review round, which is exactly why §0a exists as a standing pattern instead of a one-off fix. |
...
## 65b. Chair research packet — adopted 2026-08-11

Second research pass, this time over 2026 literature on multi-agent cognition, agent memory security, authorization propagation, and generative-UI personalization. Full packet with per-item build status: `docs/products/builderos/CHAIR_RESEARCH_DIRECTIVES_2026-08-11.md`. Chair's caveat preserved: this distinguishes productized from research-stage from apparently-unassembled; it does not claim novelty.

**Why this section changes the build rather than only the doctrine.** Seven store contracts in this document are named but unspecified, and they are the reason all sixteen manufacturing slices are blocked. The research does not answer them — only the founder may — but it establishes *what each store is for*, which is the difference between asking the founder to design seven schemas from scratch and asking him to ratify a purpose. The mapping below is normative about purpose and silent about columns.

**A. `CapsuleStore` is a memory admission surface, not a cache.** Memory poisoning is now a live attack class: content planted via a webpage or document that a later agent treats as trusted history. Therefore: nothing enters durable memory because a model saw it. Three zones are required — **Observed** ("we encountered this"), **Provisional** ("may be true"), **Trusted** ("safe enough to influence consequential decisions") — and web content may never write directly to Trusted. A malicious page must be able to produce *"observed claim: Adam changed his bank account"* and must not be able to produce *"Adam's bank account changed."* Every memory answers "why do you believe this?" and is walkable backward: decision ← inference ← memory ← source ← observation. When a source is invalidated, everything derived from it is marked contaminated rather than the single record being deleted — referential integrity for beliefs.

**B. `AuthorityLedger` records envelopes that only ever attenuate.** Authorization propagation is the failure the market is about to discover painfully: ordinary access control does not capture whether authority is still valid once Agent A delegates to B which calls Tool C. Therefore authority travels *with* the task as an immutable envelope (principal, objective, spend ceiling, geography, expiry, may-reserve, may-prepay, may-delegate, may-modify-calendar), no hop may expand what it received, and permissions are bounded by execution count and purpose — "may search Gmail for hotel confirmations for *this* itinerary", not "can access Gmail". This is capability security applied to personal AI, and it is a stronger claim than §14 currently makes.

**C. `ReceiptLedger` must separate a bad decision from a good decision with an unlucky outcome.** A receipt records what was known, what was not, which assumptions were load-bearing, why this option was chosen, the confidence, and the alternatives rejected. Reality later compares outcome against the receipt. Without the assumptions and the alternatives, the system can only learn from results, which teaches it to avoid unlucky good decisions. Calibration is per decision class: what our "80%" means in software architecture is not what it means in forecasting.

**D. `TemplateStore` is the bottom rung of a skill ladder, not the whole thing.** §65a already names template replay the highest value-per-effort item here, and that stands. This adds direction: instance → procedure → skill → general capability, so "change the address on the Southwest Gas site" generalizes to "update identity data in an authorized institution" and a new site inherits the strategy instead of learning from scratch. Failures become curriculum — repeated defeat by modal dialogs during checkout generates adversarial practice on modal dialogs. Each skill carries a competence frontier (proven / likely / experimental / unknown) and autonomy follows competence, which is far better than one global autonomy setting.

**E. `PreferenceStore` must separate identity from preference from state.** Enduring facts, general leanings, and what someone wants right now are three different things, and most personalization collapses them: "usually likes detailed answers" plus "currently driving" must resolve to a short spoken answer. Preference is learned behaviorally, not by asking — an April 2026 generative-UI study found low agreement even among professional designers about what a good interface is, and lightweight preference learning beat direct prompting. So the surface learns density, visual-versus-textual balance, simultaneous choice count, explanation depth and animation tolerance from completion speed, errors, backtracking and dismissals. Two people requesting the same capability get different but stable interfaces. Preferences are versioned, because humans change.

**F. `TaskStore` holds intents that expire and goals that inherit.** "Find me a house" stays relevant for months; "I want pizza" for about forty-five minutes. Every inferred objective carries a half-life, and confidence decays without reinforcement, which is what stops a stale intention from becoming strange autonomous behavior. Subgoals die with their parent — a cancelled trip kills the hotel search, the car search and the restaurant research unless separately retained. And the store must hold **negative goals** ("never contact this person", "never schedule before 9", "don't interrupt while sleeping except emergencies") with stronger persistence than ordinary preferences.

**G. `DeviceRegistry` exists to make one task span devices, not to list hardware.** Because semantic state is independent of the device, a task starts on the desktop, takes authentication from the phone, approval from the watch, and is read aloud in the car — one persistent interaction object rather than five assistant sessions.

**H. Attention is a budget, and restraint is a feature.** Every interruption carries an estimated attention cost, and the system interrupts only when expected benefit exceeds it: a $3 saving does not qualify, a $3,000 mistake does. Low-value items accumulate and compress ("five things happened, none needed you, here is the twenty-second version"). The system must learn when *not* to help — proactive guidance while cooking, silence during deep concentration, a lower threshold when someone is repeatedly failing, a higher one when they are clearly competent.

**I. Minimum necessary context, and transformations instead of binary sharing.** Before any model receives context, strip everything not needed for this exact job — a model booking dinner does not need financial records. And sharing is not share/don't-share: exact value, range, category, anonymous identifier, derived fact, or proof without disclosure. The service usually needs `over_50 = true`, not a date of birth. A small local broker performs redaction, abstraction, minimization and provenance labelling before anything leaves the device, which is a moat that does not depend on which models we use.

**J. The watchdog is not the actor.** A cheap local monitor watches the expensive agent for goal drift, repetition, unexpected domain changes, authority-boundary approach and injection, and wakes the expensive reasoner only when needed — recent work reports large completion gains from automated drift prevention, and it is cheaper than continuous frontier supervision. This is the Factory/Verifier separation extended into runtime: the agent doing the work is not solely responsible for noticing its own drift.

**K. Execution granularity follows risk, and the plan is compiled once.** Batch deterministic safe segments into a single turn (click field, type, tab, type, select) and verify afterward; batch less as risk rises, down to one action per verification. Above that, a high-level plan compiles to a semantic micro-plan once and is then translated per surface into API calls, DOM operations, accessibility actions, CLI or pixels — planning stops being tied to execution mechanics.

**L. Failure is a handoff, not a dead end.** When automation stops, compile the remainder into the smallest possible human action: "everything is done except this CAPTCHA — click here", or "they require a phone call; here is the account number and exactly what to ask." Measure human effort saved (clicks, typing, decisions, waiting, phone time) as a Reality metric, and choose execution economically: an API at 4¢ and 99.8%, a GUI path at 31¢ and 91%, a human handoff at ninety seconds.

**M. Two directions per task, and the anti-addiction metric.** For skills the human wants, assistance decreases — explain, guide, point, observe, nothing — and the KPI is that they no longer need help. For work they hate, automation increases to autonomous. Engagement, session time and daily actives are worth **zero** unless they correlate with human benefit; success can look like the founder not opening the product all day because everything worked. Metrics are classified as human-outcome, operational, diagnostic, or **forbidden optimization target**, so engagement cannot be reintroduced later.

**Umbrella: the Human–AI Contract.** A machine-readable living agreement between one person and their intelligence — what to optimize for, what never to do, when interruption is permitted, which decisions require the human, what may be learned and what must be forgotten, who else may access what, what risk is tolerated, what autonomy has been earned, what must always remain human. Every agent, tool, surface, factory and external service operates downstream of it. The point is stronger than authenticated delegation: not merely proving an AI acts *for* a human, but proving it acts *within the relationship that human deliberately established*.

**Status discipline.** Everything in this section is a normative requirement on the design of these stores and surfaces. None of it is built. It does not answer the seven store contracts and must not be read as answering them — §65 remains open, and the Schema Decision Artifact refuses any answer not ratified by the founder.
...
Ambiguities that were genuinely left open in round 1, now resolved with the founder's direct confirmation rather than left as forks for two Builder teams to disagree over: "Digital Imprint" is ratified (row 1), "Conductor" is ratified (row 7), and "Capsule" as parent class with "Operational Capsule" as subtype (row 2) is refined and closed. What remains genuinely open — rows 3 and 6 — are correctly still open, because they're not close calls resolved by more evidence, they're calls that depend on information (real user data existing, a real Windows device existing) that doesn't exist yet.
