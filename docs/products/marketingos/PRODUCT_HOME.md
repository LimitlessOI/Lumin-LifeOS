...
| **Stability** | `draft` |
| **Last Updated** | 2026-08-02 — GAP-FILL: restored `marketingos` phase 5–7 artifacts, aligned route export names (`registerCampaignRoutes`, `registerPerformanceIntelRoutes`) and `deps.*` handler calls, and hand-authored `routes/marketing-director-routes.js` (s11) with `registerIntelligenceRoutes` for `/api/v1/marketingos/director/recommendations` and `/api/v1/marketingos/director/budget/allocate`. Added the route to `config/auto-registered-product-modules.json` so module-health can prove it mounts. `npm run builder:preflight` passes 442/442. |
| **Owner** | adam |
...
**Last Updated:** 2026-08-08 — Zero-capital economic engine strategy captured (`docs/products/ZERO_CAPITAL_ECONOMIC_ENGINE.md`). See Change Receipts below. Prior: 2026-06-29
...
| 2026-08-08 | **Captured the zero-capital economic engine strategy conversation and filed `docs/products/ZERO_CAPITAL_ECONOMIC_ENGINE.md`.** Adam's real, binding constraint ("I don't want to burn any capital. We don't have any. I've invested everything.") reframed a pasted ChatGPT funding strategy into a concrete zero-upfront-cost plan: lead with the existing SMOS $49 offer (conditional on a fresh SENTRY re-verification — Adam separately flagged "the social media OS needs testing... it looks like it's further along than it is"), then POD/marketplace commerce (Printify/Printful $0-upfront plans, Etsy/eBay/Walmart no-setup-fee listings) as a zero-inventory laboratory, explicitly NOT betting on Amazon first given its per-item/monthly selling-plan cost. Captured real platform fee data, the "10-15% better, not stolen" IP posture (complaint-mining for product improvement, not cloning), the Minimum Viable Experiment kill-switch discipline, and the capital-bucket/bounded-reinvestment rules for the economic engine itself. 10 discrete ideas extracted and filed for real to IdeaVault (`POST /api/v1/ideas`, live UUIDs returned). Not yet built into any mission — strategy/doctrine capture only, per SO-004. | Direct founder request: put the conversation where it belongs, duplicate into the Digital Twin archive, file uncaptured ideas, then give real feedback and a concrete plan. | Full source: `docs/conversation_dumps/2026-08-08-zero-capital-economic-engine-strategy.md`. Twin pipeline run for real: `node scripts/scan-conversation-dumps.mjs --signals` processed 117 files, confirmed the new file cataloged and signal-extracted. 10/10 idea filings returned `HTTP 200`/`ok:true` with real UUIDs. |
| 2026-08-02 | **GAP-FILL: created `routes/marketing-director-routes.js` (s11) and registered it in `config/auto-registered-product-modules.json`.** Exports `registerIntelligenceRoutes` mounting `GET /api/v1/marketingos/director/recommendations` and `POST /api/v1/marketingos/director/budget/allocate`, using `deps.marketingIntelligenceDirector.getStrategicRecommendations` / `allocateBudget` backed by `services/marketing-intelligence-director.js`. | `bp-priority:once` reached s11 but the route file did not exist on `main`; AI providers remain dry so the deterministic wiring was hand-authored from the `BUILD_QUEUE` spec. | `node --check routes/marketing-director-routes.js`; `npm run builder:preflight` 442/442 PASS. |
| 2026-08-02 | **GAP-FILL: aligned `routes/marketing-campaign-routes.js` and `routes/marketing-performance-intel-routes.js` with their `BUILD_QUEUE` artifact-proof contracts.** Added `registerCampaignRoutes` and `registerPerformanceIntelRoutes` alias exports and wired handlers to `deps.marketingCampaignGenerator.generateCampaignAssets` / `deps.marketingMessageLearning.learnFromPerformance` so the expected exports and `file_contains` assertions pass. | `bp-priority:once` artifact proof for marketingos `s4`/`s7` failed because the restored route files exported `registerMarketing*Routes` while the queue expected the shorter names, and the handlers did not match the `deps.*` substrings. These are deterministic naming fixes. | `node --check` on both route files; `npm run builder:preflight` 441/441 PASS. |
| 2026-08-02 | **Merged `origin/main` advance `348633577` — brought in builder-generated `services/marketing-intelligence-director.js` (s10) and remote `docs/products/marketingos/BUILD_QUEUE.json` status updates.** Kept local fixed versions of `routes/marketing-campaign-routes.js`, `routes/marketing-performance-intel-routes.js`, and `services/marketing-campaign-generator.js` (resolved add/add conflicts in favor of our import/filename fixes). | Remote `main` advanced while local GAP-FILL commits were in flight; the two branches both added the phase 5–7 artifacts. Local branch had the corrected imports/filenames; remote had `s10` and updated `BUILD_QUEUE` statuses. Merged to keep both. | `node --check services/marketing-intelligence-director.js` |
| 2026-08-02 | **GAP-FILL: restored `marketingos` phase 5–7 analytics/campaign intelligence artifacts.** Recovered `db/migrations/20260715_marketing_performance_analytics.sql` (performance analytics table), `services/marketing-analytics-ingest.js` (`ingestPerformanceData`), `services/marketing-message-learning.js` (`learnFromPerformance`), `routes/marketing-performance-intel-routes.js` (`/api/v1/marketingos/performance/learnings`), `db/migrations/20260716_marketing_campaign_assets.sql` (campaign assets table), `services/marketing-campaign-generator.js` (`generateCampaignAssets`, `getCampaignAssetsBySessionId`), and `routes/marketing-campaign-routes.js` (`/api/v1/marketingos/campaign/generate`, `/api/v1/marketingos/campaign/assets/:sessionId`). Fixed builder-generated filename casing mismatches (`marketingCampaignGenerator.js` → `marketing-campaign-generator.js`, `marketingMessageLearning.js` → `marketing-message-learning.js`) and added the two new routes to `config/auto-registered-product-modules.json`. | False-done audit found marketingos steps `s1`–`s6` (and blocked `s4`/`s7`) were marked `done` but their artifacts were missing or failed import resolution on `main`. Factory `/build` returned `BLOCKED_TOOLING` / `HTTP 502` due to all AI providers being dry, so the artifacts were deterministically restored from their last builder commit SHAs and the deterministic import/filename errors were hand-repaired. | `node --check` on all restored `.js` files; `npm run builder:preflight` 441/441 PASS. |
...
|---|---|---|---|---|---|
| 2026-07-31 | **`scripts/verify-marketing-phase1-live.mjs`'s `namedInfraBlockers()` was hardcoded — it unconditionally reported both `STORAGE_R2_UNVERIFIED` and `GOOGLE_YOUTUBE_OAUTH_UNVERIFIED` regardless of actual state.** Found by hand: live-tested `GET /marketing/youtube/status` against production and found a real, active YouTube OAuth connection (connected since 2026-07-30 22:24:58) that the script had been falsely reporting as blocked. Fixed `GOOGLE_YOUTUBE_OAUTH_UNVERIFIED` to call the real `/marketing/youtube/status` endpoint instead of assuming; also fixed the stale `robust-magic-production` redirect-URI string to derive from `PUBLIC_BASE_URL`. `STORAGE_R2_UNVERIFIED` now checks env presence, but **honestly still not fully authoritative** — this script's storage check reads its own process env, which only reflects Railway's real state when the script is actually run from/against Railway, not from a local shell; could not find a live storage-status endpoint equivalent to YouTube's `/status` to close that gap the same way. | A verifier that reports blockers unconditionally isn't a verifier — same "built but not actually checking" pattern found repeatedly tonight, this time in a script whose entire job was catching exactly that pattern. | ✅ `node --check` clean; re-ran live against production — `SOCIALMEDIAOS_BLOCKERS.json` now correctly shows only 1 real blocker (`STORAGE_R2_UNVERIFIED`) instead of 2, with the Google one gone because it's actually connected. |
| 2026-07-31 | **`services/marketing-session-export.js` called a nonexistent AI council member, `'strategist'` — every export request (the actual paid deliverable, reached only after ownership+payment checks pass) failed with `Unknown member: strategist`.** `config/council-members.js` has never registered a member or alias named `strategist` (confirmed by grep — zero matches anywhere). Changed to `'gemini_flash'`, the proven, working member name already used for this exact class of task (marketing copy generation) in the sibling file `routes/marketing-session-routes.js`. Found live during hands-on verification of the ownership-scope fix earlier tonight — a real, paid session's export request got past every gate correctly and still failed here, meaning the revenue loop couldn't complete end-to-end even with a real payment. | Found by testing the actual paid-export path as a real client, not by reading the code. Blocks the revenue loop this whole session has been trying to unblock — a customer could pay and still never receive their content. | ✅ `node --check` clean | — | Not yet live-verified against production after this specific fix — do that before considering this closed. |
| 2026-07-31 | **Second incident on the same fix: the broken `getOwnerId(req, db)` pattern was auto-shipped a second time (commit `fe7cc13a0`), overwriting the first manual fix and spreading the bug to all 3 endpoints in the file (export, audio upload, export/status), because the `smos-export-owner-scope-fix` BUILD_QUEUE item was still `status: pending` after the first manual fix — nothing stopped the factory from re-dispatching it.** Live-verified the first fix against production before the regression (real command-key request: nonexistent session → clean `404`, not `500`; a real paid session `019c6b81-...` → ownership + payment checks both passed, reached `buildSessionExport`, which then failed on an unrelated pre-existing bug, `"Unknown member: strategist"` — a separate finding, not fixed here, worth its own BUILD_QUEUE item). Re-applied the correct `toOwnerUuid()` fix to all 3 call sites this time, and — critically — marked the BUILD_QUEUE item `done` with an explicit anti-regeneration note, since leaving it `pending` is what caused the regression in the first place. | The BP-priority-alignment discipline this whole audit is built on doesn't help if a fixed file's queue item stays open — closing the loop, not just the code. | ✅ | — | `node --check` clean; prior fix live-verified against production before being overwritten (see above). Fix #2 not yet re-verified live — do that before considering this closed for real. |
| 2026-07-31 | **Emergency fix: `smos-export-owner-scope-fix` (auto-shipped minutes earlier from my own spec) broke the paid export endpoint for every real customer.** Its `getOwnerId(req, db)` read `req.lifeosUser?.id` (JWTs only ever set `.sub`, confirmed at `services/lifeos-auth.js:423`) and queried a `marketing_users` table that does not exist anywhere in the codebase — so `POST /marketing/session/:id/export` would 500 or always resolve `ownerId: null` and 403 every legitimate paying customer. Replaced with the exact `handle/sub` → `toOwnerUuid()` deterministic-UUID pattern already proven live in the sibling `routes/marketing-session-routes.js`. | Caught by watching the queue-status monitor immediately after the auto-shipped fix landed, before it could break a real customer. | ✅ | — | `node --check routes/marketing-session-export-routes.js` clean. **Not yet hand-verified against a real live paid session — that live check is still owed.** |
| 2026-07-31 | **Audit of Mission 2A found 2 real gaps, filed as spec-only BUILD_QUEUE items (not fixed here — SO-001).** `smos-export-owner-scope-fix`: `POST /marketing/session/:id/export`'s `assertSessionPaid` checks whether a sessionId was ever paid for, not whether the caller owns that session — an IDOR-shaped gap, exploitability limited by session ids being unguessable UUIDv4. `smos-buy-button-session-scope-fix`: the "Buy Content Pack" button (fixed in Mission 2A, commit `f7db5b53b5`) always targets the owner's single most-recent session via `GET /sessions?limit=1`, not the session the user was viewing — can checkout the wrong session if the owner has more than one open. | Direct code audit of Mission 2A/2B commits (`f7db5b53b5`, `77f63ddfd0`) at Adam's request. | — | — | Findings verified against `origin/main` directly (file:line read, not claimed); fixes not yet built. |
| 2026-07-30 | **SMOS checkout route reconciliation (Mission 2A step 1).** `/marketing` dashboard "Buy Content Pack" button now calls `/api/v1/marketing/pack/checkout` (live `cs_live_` Stripe session) using the most recent `marketing_sessions` row, or redirects to `/marketing/session/new` if none. `routes/socialmediaos-routes.js` `POST /content-pack/checkout` returns `410 Gone` with migration pointer. `services/smos-pack-checkout.js` exports shared `isSessionPaid`/`assertSessionPaid` and they are wired into `routes/marketing-session-routes.js` `GET /sessions/:id/export` and `routes/marketing-session-export-routes.js` `POST /marketing/session/:id/export`. `scripts/run-marketingos-layer-a.mjs` and `run-marketingos-layer-b.mjs` resolve the public base URL via `config/public-origin.js` to avoid the stale `robust-magic` base. | Close the live reconciliation gap: the public SMOS UI must use one canonical checkout path that creates real paid entitlements, and all export paths must enforce them. | ✅ | — | `npm run builder:preflight` PASS 404/404; SENTRY `marketingos` Layer A + Layer B PASS 0 findings; live `GET /api/v1/marketing/pack/verify` returns `payment_incomplete` for an open checkout; live `GET /api/v1/marketing/sessions/:id/export` returns `402 payment_required` before payment. |
| 2026-07-29 | **SMOS market-readiness re-prove on current tip.** `products/receipts/SMOS_REAL_CUSTOMER_READINESS.json` updated to `953e202ca9e9`. Password-reset email reaches Postmark sink (`test@blackhole.postmarkapp.com`) with `email_sent: true`; code now has Resend/SMTP fallback when Postmark fails. Stripe Checkout creates a live `cs_live_` session for $49. SENTRY `marketingos` Layer A + Layer B pass 0 findings. Real external-domain reset delivery and a completed live card charge remain blocked by Postmark account approval / missing `RESEND_API_KEY` and the need for a real card, respectively. | Close the July 16 market-readiness blockers on the current deploy tip. | ✅ | — | `npm run builder:preflight` PASS 401/401; `node scripts/sentry-prealpha-gate.mjs marketingos` PASS 0 findings; live Postmark sink send `email_sent: true`; live `cs_live_` checkout created. |
| 2026-07-30 | **Postmark account canceled; `password-reset-email.js` provider-agnostic.** `services/password-reset-email.js` now reads `EMAIL_PROVIDER` (resend|postmark|smtp|disabled) and sends reset emails through the configured provider with a 10s HTTP timeout, falling back to Resend/SMTP when Postmark rejects. `services/env-registry-map.js` marks `POSTMARK_SERVER_TOKEN` UNSET and `RESEND_API_KEY` NEEDED. This closes the blocker where `email_sent` was false because the code only knew Resend/SMTP and ignored the configured Postmark token. | Close the market-readiness blocker named in `SMOS_REAL_CUSTOMER_READINESS.json`: password reset token flow worked but the email itself never sent. | ✅ | — | `npm run builder:preflight` PASS 401/401; `node --check services/password-reset-email.js`; local fake-token Postmark call returns 401 quickly. Live send + SENTRY re-prove pending deploy. | Real external-domain delivery now blocked on `RESEND_API_KEY` + verified sending domain.
| 2026-07-24 | **Phase 5 connect+publish function.** Shipped bank-style connect window (`services/marketing-social-connect-session.js` + routes): user signs in on platform’s real page via screenshot/click/type popup; cookies encrypted into `marketing_social_connections`. Fixed `auth_mode=browser_session`, publisher `getConnection` unwrap + cookie apply; publish route creates browser session itself when live. UI: Connected accounts on `/marketing`, `/marketing/connect/:platform`, Publish on approved pieces. Kill switch still defaults OFF. | Adam: make connect-and-post work like bank linking. | ✅ unit tests | tip redeploy; founder connect one account; flip `LIVE_SOCIAL_PUBLISH_ENABLED` when ready for real posts |
| 2026-07-24 | **SMOS market desk auth repair.** `sharedMarketingClientAuth` now sends Bearer JWT *and* `x-command-key` when both exist (was else-if → stale JWT alone → Unauthorized while chrome said Signed in). On 401: clear JWT, retry with key, repaint chrome; packs/YouTube/suggestions/buy use `marketingFetch`. Operator-only key shows “Operator” not “Signed in”. | Tip browser: Signed in + Unauthorized packs/YouTube/ideas — market-blocker for SMOS. | ✅ | — | tip after deploy |
| 2026-07-21 | **Content-pack quality P0 — real AI copy, not transcript echo.** `routes/marketing-session-routes.js` extract/generate/regenerate passed `maxTokens` to `callCouncilMember`, but `council-service.js` (line ~1346) only honors `maxOutputTokens` — so the option was silently ignored and output clamped to the 800-token default. The 2-3-piece JSON array was truncated (no closing `]`), `parseCouncilResponse` returned null, and **every** piece fell back to a template that echoed the raw transcript (verified live via a real-user browser walk: session `b4b0641d…`, all 6 pieces `generated_by_model: fallback_template`, content = user's own words). The 2026-07-14/15 template-fallback receipts *masked* this by "never returning empty." Fix: pass `maxOutputTokens` (extract 1800 / generate 3000 / regenerate 1500) + surface a non-silent `warning: ai_generate_fell_back_to_template` when all pieces still fall back. Systemic note (not fixed here, governed follow-up): many other callers across the fleet also pass the ignored `maxTokens` key. | Real-user walk: $49 pack delivered the customer's raw transcript instead of platform-adapted marketing copy — not sellable. Adam: no theater. | ✅ | — | ✅ VERIFIED LIVE (commit `55e4f75`, prod SHA parity): re-ran Generate on session `b4b0641d…` after deploy → 9 new pieces `generated_by_model: gemini_flash` (real AI, distinct per-platform: IG "Vegas Home Dream", LinkedIn "Unlocking Homeownership", X <280c w/ hashtags); prior 6 `fallback_template` pieces predate the fix. No cost escalation needed (free tier). |
...
|| 2026-07-17 | **SMOS marketplace checkout on `main`** — `services/socialmediaos-service.js` content-pack Stripe Checkout (`createContentPackCheckout`/`verifyContentPackCheckout`/`getContentPackPricing`); `routes/socialmediaos-routes.js` public `GET /content-pack/pricing`, `POST /content-pack/checkout`, `GET /content-pack/success|cancel`; `public/overlay/marketing-for-you.html` marketplace landing with **Buy Content Pack — $49** button. | Founder: get SMOS marketplace-ready ASAP; `builderos-autonomous` cannot merge into `main`, so surface is ported directly with GAP-FILL receipts. | ✅ | ✅ | `node --check`; `npm run builder:preflight`; `npm run lifeos:bp-priority:verify`; SENTRY + deploy pending |
|| 2026-07-17 | **SMOS `/marketing` home buy-button wiring** — `routes/marketing-session-ui-routes.js` SSR home hero now renders a primary "Buy Content Pack — $49" button and wires it to `POST /api/v1/socialmediaos/content-pack/checkout` using `marketingAuthHeaders()`; the response `checkoutUrl` redirects the browser to Stripe. Unauthenticated guests are sent to `/marketing/signup`. | The existing hero only offered "Start a session" and "Create account", so a marketplace visitor could not click straight through to purchase. | ✅ | ✅ | tip after deploy |
|| 2026-07-17 | **SMOS Layer A SENTRY fixes** — `routes/marketing-session-ui-routes.js` home kicker includes `MarketingOS` so the landing page assertion passes; `services/marketing-title-universe.js` strips markdown fences, requests a single numeric score, and passes `maxOutputTokens: 2000` to prevent Gemini output truncation and parse failures. | SENTRY `ui_marketing_landing` and `api_intel_titles` failed on the live `lumin-web` production URL. | ✅ | ✅ | SENTRY Layer A re-run |
|| 2026-07-17 | **SMOS `/marketing` tour no longer auto-opens** — `routes/marketing-session-ui-routes.js` removes the 600ms auto-launch of the product tour so the primary "Buy Content Pack — $49" button is never covered by the tour modal on first visit. | Playwright buy-button click test failed because `#tourPanel` overlay intercepted pointer events. | ✅ | ✅ | buy-button Playwright re-run |