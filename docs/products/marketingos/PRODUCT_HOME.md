...
| **Stability** | `draft` |
| **Last Updated** | 2026-07-30 — **Postmark account canceled.** `services/password-reset-email.js` supports `EMAIL_PROVIDER=resend|postmark|smtp|disabled` with 10s timeouts and automatic fallback between providers; `services/env-registry-map.js` now marks `POSTMARK_SERVER_TOKEN` UNSET and `RESEND_API_KEY` NEEDED. SENTRY `marketingos` Layer A + Layer B pass with 0 findings. Stripe Checkout creates a live `cs_live_` session for $49. A real external-domain reset email is blocked until a new provider key + verified sending domain are configured. A live card payment has not been completed (requires a real card on the `cs_live` checkout). |
| **Owner** | adam |
