...
|---|---|---|---|---|---|
| 2026-07-08 | **Critical MarketingOS session contract bugfix.** `routes/marketing-session-routes.js` now matches `20260708_marketing_phase1_schema_v2.sql`: consent enum `session_recording`, extraction/platform/format enum values from §6, inserts into child tables without non-existent `owner_id`, generation writes `extraction_id`, and child reads enforce owner via joins to `marketing_sessions`. `routes/marketing-session-ui-routes.js` now emits browser-safe helpers, sends schema-valid consent/session values, reads `{ session }` / `{ pieces }` response shapes, and sends approve/reject actions. Added `tests/marketing-session-routes-contract.test.js` to exercise consent → session → coach → extract → generate → approve → export against a schema-aware fake pool; wired into `npm test`. | The latest loop-built MarketingOS modules mounted but were unusable: first consent submit used an invalid enum and downstream extraction/generation/approval queried columns the corrected schema does not have, causing user-facing 400/500s and blocking Phase 1. | ✅ | ⬜ | ✅ `node --check routes/marketing-session-routes.js routes/marketing-session-ui-routes.js tests/marketing-session-routes-contract.test.js`; ✅ `node --test tests/marketing-session-routes-contract.test.js`; ✅ `npm test`; ⚠️ `npm run builder:preflight` still exits 1 on unrelated `TRUTH_ENFORCEMENT_AUDIT` direct Anthropic fetch in `services/never-stop-product-factory.js` |
