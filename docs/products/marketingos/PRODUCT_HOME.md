...
| **Stability** | `draft` |
| **Last Updated** | 2026-07-29 — **SMOS market-readiness re-proved on current tip (`953e202ca9e9`)**: password-reset email leaves the server via Postmark and returns `email_sent: true` against Postmark's sink (`test@blackhole.postmarkapp.com`); `services/password-reset-email.js` has Resend/SMTP fallback when Postmark fails; SENTRY `marketingos` Layer A + Layer B pass with 0 findings. Stripe Checkout creates a live `cs_live_` session for $49. A real external-domain reset email is still blocked because the live Postmark account is pending approval and no `RESEND_API_KEY` is configured. A live card payment has not been completed (requires a real card on the `cs_live` checkout). Prior: 2026-07-30 — **SMOS password-reset email delivery unblocked:** `services/password-reset-email.js` now respects `EMAIL_PROVIDER` and routes to Postmark/Resend/SMTP with a 10s timeout; `services/env-registry-map.js` marks `POSTMARK_SERVER_TOKEN` SET. Live card-charge proof and SENTRY re-prove in progress. Prior: 2026-07-24 — **Phase 5 Connect+Publish UX live:** bank-style connect popup for Instagram/LinkedIn/X/Facebook (`/marketing/connect/:platform` → secured browser login → encrypted session cookies); Connected accounts panel on `/marketing`; Publish button on approved pieces; publisher unwrap/`auth_mode`/`createBrowserSession` bugs fixed. Live posts still gated by `LIVE_SOCIAL_PUBLISH_ENABLED` (default off). Prior: **SMOS auth chrome fix:** stale JWT alone no longer claims “Signed in” while packs/YouTube 401; client sends JWT+command key together, clears bad JWT and retries. Prior: 2026-07-21 — **Content-pack quality P0 fixed**: generate/extract passed the ignored `maxTokens` key (council-service honors `maxOutputTokens`), clamping output to 800 tokens → truncated JSON → every $49 pack silently echoed the raw transcript via template fallback. Now passes `maxOutputTokens` + surfaces a fallback warning. Verified live via real-user browser walk (all pieces were `fallback_template` before fix). Prior: 2026-07-20 — **SMOS pricing/trial/consent model ratified & locked into `config/smos-pricing.js`** (source of truth): card-required trial (2 free credits, dormant-until-started, verified phone+email, LifeOS 30-day bonus) → Pro $59 / Studio $159 / Studio Plus $459 (video-credit metered) + à-la-carte add-ons; beta 2-yr price-lock; legally-correct TCPA/CAN-SPAM consent split. Recurring-subscription billing engine NOT yet live — spec locked, build via governed factory (SO-001). Detail in `socialmediaos/PRODUCT_HOME.md`. Prior: 2026-07-17 — `/marketing` "Buy Content Pack — $49" button, SENTRY A+B pass on prod. |
| **Owner** | adam |
...
| 2026-07-30 | **SMOS password-reset email delivery unblocked.** `services/password-reset-email.js` now reads `EMAIL_PROVIDER` (postmark/resend/smtp/disabled) and sends reset emails through the configured provider with a 10s HTTP timeout. `services/env-registry-map.js` marks `POSTMARK_SERVER_TOKEN` SET, matching live Railway vault. This closes the blocker where `email_sent` was false because the code only knew Resend/SMTP and ignored the configured Postmark token. | Close the market-readiness blocker named in `SMOS_REAL_CUSTOMER_READINESS.json`: password reset token flow worked but the email itself never sent. | ✅ | — | `npm run builder:preflight` PASS 401/401; `node --check services/password-reset-email.js`; local fake-token Postmark call returns 401 quickly. Live send + SENTRY re-prove pending deploy. | Added Resend/SMTP fallback when Postmark returns pending-approval or same-domain rejection.
