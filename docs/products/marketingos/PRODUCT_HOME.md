...
|---|---|---|---|---|---|
| 2026-07-09 | **Critical owner-binding + schema repair for auto-registered MarketingOS routes.** `routes/marketing-session-routes.js` and `routes/marketing-calendar-routes.js` now resolve owner identity from `req.user`/`req.lifeosUser` for account JWT requests and ignore client-supplied `owner_id`; only command-key fallback can still pass an explicit owner. The same patch removes SQL references to columns absent from the deployed §6/Phase-2 schemas (`owner_id` on child content tables, `source_extraction_id`, nonexistent calendar content columns) and aligns route enums/defaults with migration CHECK constraints. Added `tests/marketing-routes-security.test.js` and wired it into `npm test`. | Recent auto-registration mounted these routes with `requireLifeOSUserOrKey`, which sets `req.lifeosUser`; the handlers ignored it and trusted `body/query.owner_id`, allowing any valid account JWT to read/write another owner's MarketingOS sessions/content by supplying a victim UUID. Once fixed, the pre-existing route/schema drift would have made the same flows 500, so the smallest safe repair covered the coupled schema mismatches. | ✅ | ⬜ | `node --check routes/marketing-session-routes.js && node --check routes/marketing-calendar-routes.js && node --check tests/marketing-routes-security.test.js && node --test tests/marketing-routes-security.test.js` PASS (3/3); full `builder:preflight` blocked pre-existing `TRUTH_ENFORCEMENT_AUDIT DIRECT_LLM_FETCH:services/never-stop-product-factory.js` |
