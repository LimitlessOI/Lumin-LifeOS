...
| **Last Updated** | 2026-07-13 — MarketingOS publish/export security gap closed: publish and alternate export/audio/status routes now scope rows through authenticated owner, publisher unwraps social connections correctly, replay publish requires real evidence before audit success, and the export route's `busboy` dependency is declared. |
...
- State: MarketingOS/SocialMediaOS Phase 1 core loop (consent → coach → extract → generate → approve → export) is live and served at `/marketing` standalone and inside `lifeos-app.html` (`/marketing?shell=1`). Phase 2 surfaces (`/marketing/calendar`, `/marketing/atoms`) are now auth-aware and load/save correctly. 2026-07-13 critical audit found and patched owner-scope gaps in social publish plus alternate `/marketing/session/:id/*` export/audio/status routes; targeted regression, `npm test`, product-home verify, SSOT check, and `builder:preflight` have run on this branch.
...
|---|---|---|---|---|---|
| 2026-07-13 | **Critical MarketingOS publish/export authorization fix.** `routes/marketing-publish-routes.js` now normalizes owner IDs from `req.lifeosUser`/`req.user` first and loads publish pieces only through `marketing_sessions.owner_id`; `routes/marketing-session-export-routes.js` owner-scopes the alternate export, audio upload, and export-status endpoints; `services/marketing-session-export.js` fails closed without `ownerId`; `services/marketing-publisher.js` unwraps `getConnection(...)`, fixes template lookup SQL, and awaits evidence verification before marking replay published. Added `tests/marketing-publish-export-security.test.js`, wired it into `npm test`, and declared the route's existing `busboy` import in the root package manifest/lockfile. | Critical bug sweep proved any authenticated LifeOS user could target another tenant's approved `piece_id` or session UUID on newly mounted MarketingOS money-path routes, while publish itself could fail on connected accounts or falsely record replay success without evidence. The first targeted test run also proved the newly mounted export route had an undeclared `busboy` dependency and could fail import/auto-registration. | ✅ | — | `node --check` touched route/service/test files; `node --test tests/marketing-publish-export-security.test.js` 4/4 PASS; latest `npm test` 409 PASS / 0 FAIL; `npm run lifeos:product-home:verify` PASS; `node scripts/ssot-check.js --all` exited 0 with pre-existing non-blocking missing-SSOT warnings outside touched files; `npm run builder:preflight` PASS. |
