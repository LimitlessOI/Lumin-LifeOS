...
| **Last Updated** | 2026-07-14 — Audit fixes: title dedupe by stem; extract/generate fail-soft; marketing owner_id prefers handle over numeric JWT sub. |
...
|---|---|---|---|---|---|
| 2026-07-14 | **Extract status + approve aliases** — stop writing illegal `status=extracted` (DB check); accept `status:approved` / `approved:true` on content PATCH. | Tip verify: extract 500 on status check; approve 400 Invalid action. | ✅ | — | tip after deploy |
