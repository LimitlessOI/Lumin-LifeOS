...
|**Last Updated:** 2026-07-16 — **Railway branch drift + final checkout verification.** Production had deployed `feat/clientcare-chart-recovery` (`ed6029bb3`) instead of `builderos-autonomous`; triggered `POST /api/v1/railway/managed-env/build-from-latest` with commit `38358e687`. Confirmed on live domain: `POST /api/v1/socialmediaos/content-pack/checkout` returns HTTP 200 + Stripe URL, and `POST /api/v1/lifeos/builderos/command-control/founder-interface/message` with `buy social media os content pack` returns `action: smos_purchase`, `command_truth: COMMAND_RAN`, and a Stripe checkout URL.
|**Last Updated:** 2026-07-17 — **SMOS marketplace landing page wired to paid checkout.** `public/overlay/marketing-for-you.html` now loads `lifeos-bootstrap.js` and includes a `Buy Content Pack — $49` button that calls `POST /api/v1/socialmediaos/content-pack/checkout` (via `fetchWithAuth`) and redirects to the Stripe Checkout URL. `public/overlay/lifeos-app.html` `PAGE_META['marketing']` now loads `/overlay/marketing-for-you.html` as the SocialMediaOS entry point so the buy button is one click from the app shell. The page remains reachable directly at `/socialmediaos` and `/marketing/for-you`.
...
| 2026-07-17 | **SMOS marketplace landing page wired to paid checkout.** `public/overlay/marketing-for-you.html` loads `lifeos-bootstrap.js` and exposes a `Buy Content Pack — $49` button; clicking it uses `fetchWithAuth` to `POST /api/v1/socialmediaos/content-pack/checkout` and redirects to Stripe. `public/overlay/lifeos-app.html` `PAGE_META['marketing']` now points to `/overlay/marketing-for-you.html` so the SocialMediaOS nav opens the marketplace first. | Adam asked for SMOS to be marketplace-ready ASAP; the checkout API worked in chat but had no discoverable UI entry point. | ✅ runtime |
| 2026-07-17 | **Public post-payment return URLs no longer require authentication.** `routes/smos-content-pack-checkout-routes.js` removed `requireKey`/`getOwnerId` from `GET /content-pack/success` and `GET /content-pack/cancel`; `services/socialmediaos-service.js` `verifyContentPackCheckout` now resolves `ownerId` from the pack row when not authenticated and marks the content pack `ready`. A paying customer returning from Stripe lands on a working page. | The Stripe `success_url`/`cancel_url` previously routed to `/api/v1/socialmediaos/content-pack/success` and `/cancel`, which required a JWT or command key and would fail for real customers. | ✅ runtime |
