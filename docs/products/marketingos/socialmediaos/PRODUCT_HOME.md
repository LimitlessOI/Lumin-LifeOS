...
**Last Updated:** 2026-07-20 — **"Creative Engine" doctrine candidate captured** (`docs/CREATIVE_ENGINE_AND_PROGRESS_DOCTRINE.md`): SocialMediaOS is the customer-facing *Creative Engine* — one decision engine, many renderers. Earned-attention + hard-gate-the-reasoning formalized (extends the existing SMOS "earned attention" script framework); pre-ratification (Article VII), not behavior SSOT yet. Also earlier same day: **Pricing model ratified & locked into source of truth** (`config/smos-pricing.js`). Card-required trial (2 free video credits, dormant until started, verified phone+email, LifeOS 30-day all-features bonus) → **Pro $59 → Studio $159 → Studio Plus $459** (video-credit metered, modes as the step-up lever) + cheap à-la-carte add-ons ($5–$39). Beta price-lock (2-yr grandfather). Legally-correct consent split (required Terms box covers email + service msgs; separate optional unchecked call/text/AI-voice box, not a purchase condition). Value anchor: replaces ~$270/mo DIY tool stack or $650–2,798/mo Curaytor. The recurring-subscription billing engine is NOT yet live — spec locked; build goes through the governed factory (SO-001). Prior: 2026-07-17 — Content-pack checkout mounts on founder-builder runtime; `owner_id`→`TEXT`; Stripe errors→503.
