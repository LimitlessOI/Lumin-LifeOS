...
**Last Updated:** 2026-07-30 — **Legacy checkout path deprecated and entitlement shared with MarketingOS.** `routes/socialmediaos-routes.js` `POST /content-pack/checkout` now returns `410 Gone` directing clients to `/marketing/session/new` and `/api/v1/marketing/pack/checkout`. The shared `isSessionPaid`/`assertSessionPaid` helpers from `services/smos-pack-checkout.js` protect all MarketingOS export paths. SMOS public UI/SENTRY Layer A+B re-pass 0 findings.
...
|------|--------|-----|-------|
| 2026-08-22 | **Registered `PRODUCT-SOCIALMEDIAOS-FIRST-PAID-CREATOR-0001` on Abbott, redirected from Costello (`costello-builderos-b`) where its GitHub Actions were billing-blocked and the manual override bridge sat idle.** Same master blueprint (7 slices, SMOS-REV-001..007: verifier truth repair, founder-chat runtime isolation, Creative Brief service, producer-director service, coaching intake extension, generator wiring, zero-deception acceptance harness) — its Costello-only runtime-identity assertions were rewritten for Abbott (`builderos-reboot/MISSIONS/PRODUCT-SOCIALMEDIAOS-FIRST-PAID-CREATOR-0001/`); the product vision/architecture content is unchanged. Wrote the real, working acceptance harness now (`scripts/verify-socialmediaos-first-paid-creator.mjs`, slice SMOS-REV-007) rather than leaving it as a stub — honestly reports `NOT_PROVEN` today since `services/socialmediaos-creative-brief.js` and `services/socialmediaos-producer-director.js` (SMOS-REV-003/004, real service modules that must go through the governed factory per SO-001) don't exist yet; everything else it checks (syntax, verifier isolation, live Phase-1, redispatch proof, checkout path) already passes for real. Registered in `builderos-reboot/BP_PRIORITY.json` rank 34, `verdict: NOT_COMPLETE`. Costello's own copy is parked (`costello-builderos-b/builderos-reboot/POINT_B_TARGET.json`). | Founder: "put abbott to have him finish the socialmediaOS" — Abbott isn't billing-blocked and can actually execute continuously. | `node scripts/verify-socialmediaos-first-paid-creator.mjs`: real, honest FAIL today (2/9 checks not yet satisfied, both service modules queued for the governed factory). `node scripts/verify-bp-priority-guardrails.mjs`: PASS (26 checks). `npm test`: 908/904/0 unaffected. |
| 2026-07-30 | **Deprecated the standalone `/api/v1/socialmediaos/content-pack/checkout` surface.** `routes/socialmediaos-routes.js` now returns `410 Gone` with `next: '/marketing/session/new'` and `migration: '/api/v1/marketing/pack/checkout'`, preventing orphan `socialmediaos_content_packs` rows and keeping paid entitlement creation in the canonical `marketing_pack_checkouts` table. Entitlement enforcement reuses `services/smos-pack-checkout.js` `isSessionPaid`/`assertSessionPaid` shared helpers. | Consolidate SMOS revenue loop under MarketingOS canonical path and eliminate a duplicate, unentitled checkout surface. | `npm run builder:preflight` PASS 404/404; `node --check` on touched files; SENTRY `marketingos` Layer A + Layer B PASS 0 findings. |
| 2026-07-30 | **Adaptive learning-style coaching.** `services/socialmediaos-coaching-service.js` exposes an optional `learningProfile` on session start and stores it in `metadata`. `services/socialmediaos-content-generator.js` injects a `LEARNING STYLE ADAPTATION` instruction into the AI prompt based on visual/auditory/reading/writing/kinesthetic input so generated content packs are formatted for how the user learns. | Founder: LifeOS/LifeRE coaching must adapt to each person's learning style (visual, auditory, reading, kinesthetic). | `npm run builder:preflight` PASS 401/401; `node --check` clean. |
| 2026-07-27 | **SECURITY: fixed a real payment-bypass vulnerability — any valid paid Stripe session could unlock ANY content pack for free.** `verifyContentPackCheckout` (`services/socialmediaos-service.js`) retrieved the Stripe session by ID and checked only `payment_status === 'paid'` — it never verified `session.metadata.contentPackId` matched the `contentPackId` being unlocked. Anyone holding one legitimate paid session ID (e.g. their own past $49 purchase) could replay it against arbitrary `contentPackId` values, flipping any pack to `status:'ready'` for free, repeatedly. Found by an independent audit, verified directly by reading the checkout-creation code (`metadata.contentPackId` is genuinely set at session creation, confirming the check was skippable). Fixed: added an explicit `session.metadata?.contentPackId === contentPackId` match check that throws `checkout_session_pack_mismatch` (400) on any mismatch, before the paid/unpaid check even runs — mirrors the correct pattern already used in the sibling `services/smos-pack-checkout.js` flow. **Correction to two other audit claims, checked and found inaccurate/moot**: the audit also flagged an IDOR in `routes/marketing-publish-routes.js` (`requireUserOrKey`, cross-tenant publish) and an `owner_id` bug making live publishing silently fail — verified directly: that route uses `requireKey` (not `requireUserOrKey` as claimed), and `registerMarketingPublishRoutes` is never called from any startup/core file — the whole route, and `publishApprovedPiece`/`services/marketing-publisher.js` it calls, is genuinely unreachable dead code today. Not fixed (would be wasted effort on code nothing can call); flagged honestly as a real gap if this feature is ever wired to a live route later. | Founder: "audit everything... fix gaps/bugs as you find them... make it fucking happen" — real crisis-mode security audit; one finding acted on after direct verification, two corrected after direct verification found them inaccurate. | `node --check` clean on `services/socialmediaos-service.js`; the dead-code claim confirmed by grepping every startup/core file for the route's export name (zero matches) |
| 2026-07-24 | **Cross-product: Site Builder queued to consume the shared Creative Engine.** New `site_design` mode queued at `services/creative-engine/modes/site-design.js` (BUILD_QUEUE step `cg-site-design-mode`, owned in `docs/products/site-builder/BUILD_QUEUE.json`) — Site Builder will call the SAME `services/creative-engine/index.js` decision engine SocialMediaOS uses, instead of its own separate weaker AI path. Realizes the 2026-07-20 "one decision engine, many renderers" doctrine candidate below as the first real cross-product renderer. | Founder: "Use one creative brain to function for all creative aspects. It's our studio." | ⚠️ queued, not yet built — see site-builder PRODUCT_HOME Change Receipts 2026-07-24 for full detail |
| 2026-07-24 | **Connect+Publish path.** Connected accounts panel + `/marketing/connect/:platform` secured login window + Publish on approved pieces. Session cookies stored encrypted; live post kill-switch default off. | Adam: make bank-style connect/post work. | ✅ local tests; tip after ship |
| 2026-07-24 | **Public `/marketing/for-you` signup-first market door.** Primary CTA Create free account → session → $49; shareable warm-network link. | Chair: SMOS first for cash while cold email/SMS blocked. | ✅ tip Stripe checkout; SMS’d Adam the link |
| 2026-07-20 | **"Creative Engine" doctrine candidate captured.** Multi-AI brainstorm (Adam + GPT-5.6 + Opus) converged on: SocialMediaOS is externally the **Creative Engine** (locked name); internally one **decision engine, many renderers** (website/video/email/social/coaching). Formalized: **earn the next step / Attention Contract** (extends the existing SMOS "earned attention" script framework), **hard-gate the reasoning not the behavior** (Mission/Customer/objection/trust-strategy/next-action/success-metric must exist before render — same shape as BPB-spec + SENTRY-assertions), **Decision Simulator** (persona critique, Hypothesis-grade until calibrated against real outcomes), **competitive-gap engine + confidence scoring**. Synthesis: `docs/CREATIVE_ENGINE_AND_PROGRESS_DOCTRINE.md`; archive: `docs/conversation_dumps/2026-07-20-creative-engine-five-laws-brainstorm.md`. | Founder: capture the converged thinking so it stops living only in chat; save to touched projects + archive. | ✅ doc-only, **pre-ratification (Article VII) — NOT behavior SSOT / not built** |
| 2026-07-20 | **Pricing/trial/consent model ratified & written to source of truth.** `config/smos-pricing.js` now carries the go-forward model alongside the live `pack`: **Pro $59 / Studio $159 / Studio Plus $459** (video-credit metered; creation modes are the step-up lever), **card-required trial** (2 free credits, nudge 2nd to a different mode, dormant-until-started so the offer never expires + the clock starts on first use/open, verified phone+email, **LifeOS 30-day all-features** card-in bonus), à-la-carte add-ons ($5 A/B · $10 research · $15 rapid-response · $39 video pack), **beta price-lock** (2-yr grandfather, enforceable), and a **legally-correct consent split** (required Terms box = ToS+privacy+service msgs+email marketing; separate optional unchecked call/text/**AI-voice** box, not a condition of purchase — TCPA). Re-engagement = AI-voice + email, relationship-framed for non-opt-ins. Value anchor from live 2026 competitor pricing: replaces a **~$270/mo** DIY stack (Buffer+Opus Clip+Descript+Canva+vidIQ+Jasper+HeyGen+BombBomb) or **$650–2,798/mo** Curaytor. Also ratified platform-wide: **"trial everything"** (every product gets a card-in free-taste that seeds the shared Cognitive Core) + first-party in-app cross-product free-trial nudges (own products only, non-manipulative). | Founder pricing-design session 2026-07-20: "Pro $59… beta pricing, lock it in… trial everything… do it the way it's supposed to be legally done." | ✅ config `node --check` + SSOT; ⚠️ **billing engine (recurring subs + credit meter + trial phase + LifeOS grant + consent capture) NOT live — built via governed factory next (SO-001)**; live checkout still serves the one-time `pack` |
...
| 2026-07-19 | **Chair/Architect Competitive Review.** Founder directive: review every revenue blueprint against real competitors, propose additions/removals, cite research. **Market context:** generic social schedulers (Buffer, Hootsuite, Later) compete on scheduling + AI captions (Hootsuite's OwlyWriter, Buffer's AI-to-price ratio) at $5-$399+/seat/mo — that's a commoditized, crowded lane and NOT this product's real competitive set. The closer, more dangerous competitor is **Curaytor** — a real-estate-specific "done for you" platform (website + content + social + email + listing promotion + paid ads) whose standout feature, **"Curaytor Brain,"** analyzes marketing performance across ALL their clients to recommend which subject lines/posts/CTAs actually convert — cross-client learning, not just one account's own history. SocialMediaOS (via this product's existing content-pack/coaching-session data + LifeOS's capsule memory, now with real semantic search) is structurally positioned to build the same thing and currently doesn't. **Real, cited stat to design around:** "listings with video receive 403% more inquiries" (2025 NAR data, still the standard cited figure in 2026 real-estate-marketing research) — validates the existing YouTube/video-first Phase 1 direction already in this blueprint; keep leaning into video over static-post generation, don't dilute focus into text-first content to chase generic-scheduler feature parity. **New idea from research:** BombBomb's model (personalized video-email triggered automatically off CRM/lead events, e.g. via Curaytor's Zapier integration) is a genuinely different distribution channel than social posting — worth founder consideration as a companion feature (auto-generated personalized video follow-up on a LifeRE lead-status change), reusing this product's existing video generation pipeline rather than building new capability. Chair status: cross-client learning and personalized-video-on-lead-event are `escalate_to_founder` (product-direction calls); the video-first prioritization is already correctly reflected in the existing roadmap — no change needed there. |
