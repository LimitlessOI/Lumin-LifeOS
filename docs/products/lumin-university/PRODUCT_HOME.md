...
| Date | Change |
|---|---|
| 2026-08-02 | **Factory advance: step 5 `routes/credentialVerificationRoutes.js` built and auto-registered.** The route module was generated by the governed factory (`e35b12289`) with the correct `registerCredentialVerificationRoutes(app, deps)` export, `POST /api/v1/credentials/verify`, `deps.requireKey`/`deps.pool`/`deps.logger`, and a real import of `services/credentialVerification.js` `verifyCredential`. Added the auto-register entry to `config/auto-registered-product-modules.json` so the boot process mounts the route and the module-health functional-proof gate can pass. Step remains pending until the redeployed container serves the route LIVE. |
