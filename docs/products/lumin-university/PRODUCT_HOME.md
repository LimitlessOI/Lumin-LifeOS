...
| Date | Change |
|---|---|
| 2026-08-02 | **Factory advance: step 5 `routes/credentialVerificationRoutes.js` auto-registered; step 6 `db/migrations/004_create_accreditation_legal.sql` file_contains aligned to built artifact.** Added `routes/credentialVerificationRoutes.js` to `config/auto-registered-product-modules.json` with `registerCredentialVerificationRoutes` so the boot process mounts the route and module-health can pass. Reset step 5 status to `pending` and cleared stale `last_error`/`blocker_class`. Aligned step 6 `file_contains` and `spec` to the actual generated `accreditation_legal_structures` table (plural) with `entity_type TEXT` and reset stale failure metadata so the factory can complete via pre-existing artifact. |
