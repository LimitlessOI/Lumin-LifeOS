...
| Date | Change |
|---|---|
| 2026-08-02 | **Factory advance: steps 5–6 done; step 8 route targeted for rebuild with upstream service import and auto-register.** Steps 5 (`routes/credentialVerificationRoutes.js`) and 6 (`db/migrations/004_create_accreditation_legal.sql`) completed after adding the route's auto-register entry and aligning the migration `file_contains`. Updated `services/never-stop-product-factory.js` to dynamically inject the matching upstream service file (e.g. `services/accreditationService.js`) into `/build` `files[]` for `routes/` targets so route modules import real deps instead of hallucinating. Hardened step 8 (`routes/accreditationRoutes.js`) `spec` and `file_contains` to require `getAccreditationStructure` from `services/accreditationService.js`, `req.query.institutionId`, and correct error logging. Added `routes/accreditationRoutes.js` to `config/auto-registered-product-modules.json` with `registerAccreditationRoutes`. |
