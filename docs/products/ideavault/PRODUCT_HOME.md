<!-- SYNOPSIS: Canonical product home — IdeaVault -->

# IdeaVault Product Home

**Formerly called:** Amendment 38 — IDEA VAULT
...
| **Last Updated** | 2026-08-07 — GAP-FILL: reactivated `POST /api/v1/ideas` (and the rest of `idea-queue-routes.js`) in production — it was mounted only in the full-runtime lane Railway never loads, confirmed via a live 404 before the fix. Prior: 2026-08-06 — Charter clarified: IdeaVault is the canonical intake for every idea from founder/model conversations. Each idea is captured with nuance, features, and source provenance; tagged; prioritized; and promoted to a project/product folder when it matures. `npm run idea-vault:scan-dumps` remains the first-pass bot. |
...
|------|--------------|-----|
| 2026-08-08 | **Filed 10 real ideas via the live submission API from the zero-capital economic engine strategy conversation** (Marketplace Opportunity Scanner, Complaint Arbitrage Engine, Minimum Viable Experiment framework, Business-in-a-Box Factory, Replace-or-Integrate Engine, self-funding capital-bucket engine, SMOS ascending value ladder driven by real problem-detection, bundle arbitrage, "sell the outcome not the software," reasoning-tier cost self-optimization loop) — all confirmed `HTTP 200`/`ok:true` with real UUIDs, none built yet. Also re-ran `scripts/scan-conversation-dumps.mjs --signals` for real (117 files, 5,778 signals) to route the new conversation dump into the Digital Twin signal pipeline. | Direct founder request to capture the conversation, file uncaptured ideas, and route into the Digital Twin archive. | Full source: `docs/conversation_dumps/2026-08-08-zero-capital-economic-engine-strategy.md`. See `docs/products/marketingos/PRODUCT_HOME.md` and `docs/products/ZERO_CAPITAL_ECONOMIC_ENGINE.md` for the strategy synthesis. |
| 2026-08-07 | **GAP-FILL: reactivated the IdeaVault submission API — it was completely dead in production.** Adam directly asked whether IdeaVault actually gets consulted/used. Checked before answering: `GET /api/v1/ideas` returned a live `404` on production, and `routes/idea-queue-routes.js` was only ever imported by `startup/register-runtime-routes.js` (the full-runtime lane `server.js` never loads on Railway) — the same root-cause pattern already fixed multiple times this session for other features. Mounted it in `startup/register-founder-runtime-routes.js`, the real production route file, matching the existing adjacent mount pattern exactly. | Direct founder question ("I'm not sure... the system looks at the idea vault") that turned out to be correct: the submit endpoint itself was unreachable, so nothing could have been consulting it. | `node --check` PASS. Live production re-test after deploy: see next receipt row. |
