<!-- SYNOPSIS: Canonical product home — API Cost Savings -->

# API Cost Savings Product Home

**Formerly called:** Amendment 10 — API COST SAVINGS
...
| **Machine manifest** | `docs/products/api-cost-savings/FILE_MANIFEST.json` |
...
|--------|-------|---------|
| AI Council (provider selection, routing) | Platform | `docs/products/ai-council/PRODUCT_HOME.md` |
...
**Last Updated:** 2026-04-30 — **`TCO_TSOS_75_PERCENT_SAVINGS_BRAINSTORM.md`**: **priority rank (1–50, 1 = ship first)**, **Brief**, **Moves up if** per idea; **Master list** table; methodology note. Prior: **`TCO_TSOS_75_PERCENT_SAVINGS_BRAINSTORM.md`** extended with **Batch 2** (ideas **26–50**): provider prompt-cache layout, cached-token instrumentation, FrugalGPT-style cascade, semantic-cache thresholds, batch/async lanes, output budgets, third-party gateway normalization; external pattern citations. Prior: 2026-05-02 — Brainstorm doc (25 ideas, ≥75% savings target) + manifest `owned_files`. Prior: 2026-04-24 — Monetization view rebuilt: `tsos_savings_report` now exposes `baseline_cost_usd`, `actual_cost_usd`, `savings_pct`, and per-mechanism breakdown (free routing / compression / cache / compact rules). `getSavingsReport` updated to expose all new columns. API summary line now shows BASELINE → ACTUAL → SAVED (%). Prior: 2026-04-25.
...
| 2026-04-30 | **`docs/projects/TCO_TSOS_75_PERCENT_SAVINGS_BRAINSTORM.md`** — **Batch 2:** **25** additional ideas (**26–50**) informed by public LLM cost-optimization practice (prompt caching layout, semantic routing/cascade research, batch discounts, output-token discipline, gateway normalization); **`External patterns consulted`** subsection + expanded **For other AIs** prompt. | Adam: second wave beyond CC’s first **25**; align with industry without pretending shipped. | Editorial + web survey |
| 2026-05-02 | **`docs/projects/TCO_TSOS_75_PERCENT_SAVINGS_BRAINSTORM.md`** — **25** brainstorm items to push **TCO stack + TSOS** telemetry toward **≥75%** measured avg token savings (delta context, TOON, routing, cache, baseline definition, chaos fixtures); grounding table + prompts for other AIs; cross-links to **`TSOS_TEN_UPLIFT_IDEAS.md`**, **`AMENDMENT_10`**, **`BUILDER_COMPOUND_IMPROVEMENT_LOOP.md`**. Manifest **`owned_files`** updated. | Adam: multi-AI review + operator bar on savings; consolidate TCO/TSOS levers in one handoff file. | Editorial |
