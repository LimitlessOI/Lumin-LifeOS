...
| **Last Updated** | 2026-07-04T18:45 — ARC review self-heals when AI returns non-JSON: falls back to deterministic structural validation. Prior: 2026-07-04T18:30 — esm_script steps always regenerated deterministically (prevents wrong-product verify script collision). Prior: 2026-07-04T18:20 — ai-trust endpoint route order fixed (before :id param). Route steps: SQL deps excluded from context files. Prior: 2026-07-04T18:10 — Route steps: SQL deps excluded from context files (prevents HTTP 413), max_output_tokens capped at 8192. Prior: 2026-07-04T14:00 — esm_script steps now generated deterministically (no AI) — file-existence + syntax checks per type (.js/.sql/.html). AI-generated verify scripts were consistently brittle (exact string assertions, doubled paths, silent error swallowing). Progressive trust ledger. Railway GraphQL redeploy. Dynamic product detection (44 products). Prior: 2026-07-04 — Deterministic intent fallback + diagnostics + retry 3x. Prior: 2026-07-04 — Chat-driven execution: "Execute the TC Service blueprint" through UI chat now triggers actual build (not just status check). Chair detects execute/build/run intent and calls executeIntakeBlueprint directly. Backfill onComplete callback added. Async acceptance (no deadlock). Execute returns 202 with poll. Self-heal via GitHub API. esm_script always regenerated; route probes read actual routes. Prior: 2026-07-04 — Codex audit fixes (7 items): (1) P1 gap resolution: removed false-green path where generic AI confirmations ("got it"/"confirmed") auto-resolved all open gaps; now only resolves gaps by explicit gap ID mention or GAPS_RESOLVED sentinel. (2) P1 skeleton→contract: blueprint generation now produces executable step contracts with exact table DDL, factory signatures, endpoint specs, and test assertions — execution is mechanical, no design decisions left for coder. (3) P1 marketingos carveout removed: all products now use the unified per-product intake path. (4) P2 provider centralized: all 7 hardcoded `claude` calls in blueprint-intake switched to `openai` with `product_lane: 'builderos'`; executor model also uses `openai` instead of hardcoded `gemini_flash`. (5) P3 unhandledRejection: health status now reports `degraded` when unhandled rejections have occurred, with fault_receipts (count + last_fault) in health endpoint. (6) P3 stale Docker comment: updated to reflect that `docs/products/**` IS included in the Railway image. |
