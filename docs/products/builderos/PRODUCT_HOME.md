...
| **Last Updated** | 2026-07-14 — BuilderOS governed autonomous loop now sources from `BUILD_QUEUE.json` as the executable blueprint, ordered by `PRODUCT_BUILD_PRIORITY.json` (LifeOS first), revives stale blocked steps, infers `expected_exports` from each spec, and commits shipped files. Added CommonJS-to-ESM normalization (`normalizeCommonJsToEsm`) and remote `BUILD_QUEUE.json` merge with `commit_sha` persistence so the loop never stops building unless it runs out of tokens or SENTRY fails. Model routing hardening: `MAX_DAILY_SPEND` default raised to $20/day, `builderExecution: true` + `allowModelDowngrade: false` on load-bearing codegen/planning/analysis, and `services/builderos-improvement-loop.js` now exports `BUILDEROS_IMPROVEMENT_PROPOSALS` for Replicate image templates, persuasion-psychology ad-copy, model-value benchmarking, and LifeOS Perfect Day. provider-key-health accepts `REPLICATE_API` alias alongside `REPLICATE_API_TOKEN`. Prior: BuilderOS governed autonomous loop sources from `BUILD_QUEUE.json`… |
