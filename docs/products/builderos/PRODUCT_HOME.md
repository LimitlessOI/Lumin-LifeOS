...
| **Last Updated** | 2026-07-13 — Added `scripts/verify-gap-fill-honesty.mjs`, wired into `githooks/commit-msg`: blocks a GAP-FILL commit that justifies hand-authoring server code with "no mission pack / blueprint exists" when a real `docs/products/<product>/BUILD_QUEUE.json` already tracks the exact staged file. Closes a real incident (site-builder commit `ac50782319` cited "no BuilderOS mission pack exists for Site Builder... AGENTS.md states none" while a 33-step, mostly-populated BUILD_QUEUE.json existed at that exact commit) — the prior §2.11 hook only checked the commit message existed and wasn't vague, never whether its justification was true. See Change Receipts. Prior: Kill never-stop thrash: no revive-on-discover, route-specific auto-reg revive, config JSON skips UI SENTRY, lanes honor PRODUCT_BUILD_PRIORITY. |
