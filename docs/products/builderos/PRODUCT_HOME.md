...
| **Last Updated** | 2026-07-12 — `mergeQueueRuntimeStatus` preserves `done` steps from stale in-memory snapshots; `command-center` s5 restored to `done`, s3 parked until 2026-07-15; `builder_runtime_config` migration `id` is `text` with seeded sentinel row; `factory/ship-queue` auth header fixed. |
