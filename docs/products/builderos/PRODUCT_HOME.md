...
| **Last Updated** | 2026-07-12 — `command-center` s3 root cause pinned: `builder_runtime_config` migration `id` is `text PRIMARY KEY` with seeded sentinel row (was `uuid`), `factory/ship-queue` auth header fixed to `x-command-key` (was `x-api-key` `401`), `s3` parked until 2026-07-15. `services/governed-autonomous-shipping-loop.js` now records governed/SENTRY failures back to `BUILD_QUEUE`. |
