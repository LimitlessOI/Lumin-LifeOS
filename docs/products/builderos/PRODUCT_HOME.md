...
| **Last Updated** | 2026-07-14 — GAP-FILL: SENTRY can dynamically import modules and reload route modules at runtime. `factory-staging/factory-core/bpb/author-assertions.js` includes `target` in `module_mounts` assertions; `factory-staging/factory-core/sentry/behavior-assertions.js` retries `module_mounts` after `runner.reload()` when a 404 is observed; `routes/factory-mount-routes.js` adds `importModule` and `reload` to the `assertionRunner` so new route modules can be mounted at runtime before SENTRY proves them. `services/never-stop-product-factory.js` `mergeQueueRuntimeStatus` now trusts a deliberate repo `pending` reset over stale in-container `done`/`blocked` snapshots, so a `BUILD_QUEUE.json` reset is not reverted by another container. `services/product-build-orchestrator.js` `evaluateStepExpectations` now has a default `importModule` runner so artifact proof catches `import` failures that static export-regex misses. `startup/auto-register-product-modules.js` supports a cache-busting `reload` flag so the SENTRY `runner.reload()` path re-imports a module that was previously absent/failed at boot. `docs/products/lifeos/BUILD_QUEUE.json` s11 `spec` corrected to use `createDbPool` from `services/db.js` (the service does not export `pool`), and s11/s12 status reset to `pending` with attempts cleared so the governed loop will re-ship s11 and then s12. |
