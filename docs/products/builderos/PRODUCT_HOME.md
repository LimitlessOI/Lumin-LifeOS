...
| **Last Updated** | 2026-07-14 — Chair-counsel diagnosis: missing `import path from 'node:path'` in `routes/factory-mount-routes.js` `codegenRunner` caused `ReferenceError: path is not defined` during syntax check, consuming every tier and returning `codegen_empty` to `POST /factory/ship-queue`. Added the import and surfaced `error` in `run-step.js` evidence. `services/product-build-orchestrator.js` `reviveStaleBlockedSteps` now clears stale runtime evidence on revive, `services/never-stop-product-factory.js` `mergeQueueRuntimeStatus` honors a `revive_count` increase, and `codegen_*` failures are treated as tooling blocks. |
