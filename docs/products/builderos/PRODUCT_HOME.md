...
| 2026-07-06 | **Async founder build timeout truth fix** — `services/founder-build-self-repair.js`: removed the terminal `Promise.race` timeout from `startFounderBuildJob()`. Slow background builds now get a progress step after `FOUNDER_BUILD_JOB_TIMEOUT_MS` but keep running until `runFounderBuildWithSelfRepair()` returns the real receipt; only that receipt updates/persists terminal job state. Regression added in `tests/founder-build-job-store.test.js` for a build that finishes after the timeout notice. | A founder chat build that exceeded the 8-minute job cap could be durably marked FAIL while the uncancelled build kept running and later committed/deployed, creating false failure truth and retry/duplicate-build risk. | ✅ targeted regression + syntax checks | `node --test tests/founder-build-job-store.test.js`; `node --check services/founder-build-self-repair.js tests/founder-build-job-store.test.js` |
...
2026-07-06 handoff: async founder build jobs no longer persist a terminal timeout FAIL while the underlying build continues. `startFounderBuildJob()` now emits a timeout progress step and waits for the real `runFounderBuildWithSelfRepair()` receipt before setting/persisting terminal status. Verified with `node --test tests/founder-build-job-store.test.js tests/founder-build-result-truth.test.js`, `npm test`, and syntax checks. Full `builder:preflight` remains blocked only by the known stale LifeOS `lumin` vs `chair` routing assertion documented in the LifeOS/continuity logs.
