...
| **Last Updated** | 2026-07-12 — `command-center` s3 root cause pinned to missing `pgcrypto` extension for `gen_random_uuid()`: migration fixed; `s3` parked until deploy. `services/governed-autonomous-shipping-loop.js` now records governed/SENTRY failures back to `BUILD_QUEUE` so stale `autoReg`/`codegen_empty` errors do not cause immediate token-burning re-revives. |
...
| Date | Change | Why |
|------|--------|-----|
| 2026-07-12 | **Governed autonomous loop failure receipts.** `services/governed-autonomous-shipping-loop.js` now writes SENTRY/governed failure reasons back into `BUILD_QUEUE` steps (`markFailedStep`, `deriveFailureReason`) and commits the queue status on failure. This prevents stale `autoReg`/`codegen_empty` errors from causing immediate, token-wasting re-revives. `db/migrations/20260601_builder_runtime_config.sql` now enables `pgcrypto` extension so `gen_random_uuid()` works in Neon. `docs/products/command-center/BUILD_QUEUE.json` `s3` is parked until the migration deploys. | `command-center` `s3` was deadlocked: the route was already built and auto-registered, but `GET /api/v1/lifeos/command-center/mode` returned 500 because the `builder_runtime_config` table was missing. The factory must expose the real failure and cool down, not spin on stale text. | `node --check` changed JS files, `npm run builder:preflight`, `npm run verify:ci`, `npm run lifeos:bp-priority:verify`, `npm run factory:ci` |
