...
| **Last Updated** | 2026-07-08 — **GOVERNED BUILD_QUEUE SCHEDULER (cutover STEP 5f):** built the scheduler the Chair required before the flip (live council receipt `LIFERE_COUNCIL_1783453971120` item 3). `services/governed-build-queue-scheduler.js` (PURE PLANNER) — `planGovernedBuildQueueRun({ products, readQueue })` gathers the shippable steps across every product's `docs/products/<id>/BUILD_QUEUE.json` (the legacy loop's throughput source) and converts each via the STEP 5e adapter into a governed ship step, returning `{ by_product:[{product_id,ship_steps,gaps}], total_shippable, total_gaps, runnable }`. The queue reader/lister are INJECTED so the module stays factory-clean; the caller does I/O, hands `ship_steps` to `runGovernedShippingQueue` (BPB→Builder→SENTRY→TSOS→Historian), then marks BUILD_QUEUE steps done. Non-provable steps are surfaced as `gaps` and NEVER shipped (STEP 5e planning gate). Proof: `builderos-reboot/scripts/governed-build-queue-scheduler-proof.mjs` 8/8 including an END-TO-END check that planned `ship_steps` are accepted by the real `runGovernedShippingQueue` (assertion_provenance `bpb`), plus a live census. LIVE TRUTH (unchanged): all real queues currently 0 shippable / all 13 BP_PRIORITY missions complete — `runnable=false` right now, so there is no live autonomous ship to prove until real work is queued. THE FLIP (`GOVERNED_FACTORY_ONLY=1`) is now blocked ONLY on the Chair's precondition "prove one live autonomous ship first" — which cannot be met with empty queues; needs Chair ruling on the empty-queue case + founder go-ahead (it is the cutover switch + a prod env mutation). Prior: 2026-07-08 — **BUILD_QUEUE → GOVERNED SHIP-QUEUE ADAPTER (cutover STEP 5e):** the Chair (live council receipt `LIFERE_COUNCIL_1783453971120`) ruled, on the finding that the legacy never-stop loop draws throughput NOT from `BP_PRIORITY.json` (all 13 complete) but from per-product `docs/products/<id>/BUILD_QUEUE.json` lanes: (1) walk those BUILD_QUEUE steps through `/factory/ship-queue` so only the SHIP PATH changes (legacy `/build` → governed pipe), preserving the throughput source; (2) require `expected_exports`/`route` per step at plan time so BPB can author provable assertions (a PLANNING GATE); (3) build the governed scheduler + prove one live autonomous ship BEFORE flipping `GOVERNED_FACTORY_ONLY=1`. Built the adapter: `factory-staging/factory-core/bpb/build-queue-step-adapter.js` (PURE, NON-FABRICATING) — `toGovernedShipStep(step)` converts a freeform BUILD_QUEUE step (`{id,target_file,task,spec,depends_on,founder_gated}`) into a governed `author_then_write` ship step (codegen = untrusted hands, SENTRY proves), deriving `sandbox_boundary` from the target dir and the `assertion_spec` from the step's DECLARED expectation (`assertion_spec`/`expected_exports`/`route`). THE PLANNING GATE: `assessBuildQueueStepProvability` reuses the same fail-closed `authorAssertionsFromSpec` contract SENTRY enforces — a server-code step with NO declaration is `provable:false` and REFUSES to convert (the planner must add an expectation), so unprovable work can never reach the pipe. `selectShippableSteps` picks pending (not done/blocked/founder_gated) steps with satisfied `depends_on`. Proof: `builderos-reboot/scripts/build-queue-adapter-proof.mjs` 8/8 + LIVE census over the real marketingos + site-builder queues. IMPORTANT LIVE TRUTH: both product BUILD_QUEUEs are currently fully `done` (0 shippable steps), and all 13 BP_PRIORITY missions are complete — so the entire system is built right now and the flip would lose NO active throughput; the governed scheduler will drive NEW queued work as it appears. NEXT: wire the governed BUILD_QUEUE scheduler (gather shippable steps across products → convert → `runGovernedShippingQueue` → mark done) and add the planner planning-gate, prove one live autonomous ship when real work is queued, THEN flip `GOVERNED_FACTORY_ONLY=1`. Prior: 2026-07-08 — **BPB ASSERTION DERIVATION + BP_PRIORITY COVERAGE AUDIT (cutover STEP 5d, pre-flip):** the Chair (live council receipt `LIFERE_COUNCIL_1783453514819`) ruled: keep `GOVERNED_FACTORY_ONLY=0` for now; add a BPB intake layer that DERIVES an `assertion_spec` from each existing mission `BLUEPRINT.json` (with mission-authored overrides) so legacy contracts become provable WITHOUT reauthoring; and — the #1 pre-flip risk — SENTRY/Century must validate spec extraction on a representative batch to surface SILENT COVERAGE GAPS before any flip. Built both. (1) `factory-staging/factory-core/bpb/derive-assertion-spec.js` (PURE, NON-FABRICATING) — `deriveAssertionSpecFromBlueprintStep(step,{sourceContent})` derives assertions ONLY from evidence the blueprint carries: a mission override (`assertion_spec`/`behavior_assertions` → `declared`); inline `exact_content` or `exact_inputs.exact_content` symbols (`derived_exact`); a byte-exact `exact_inputs.content_source_path` copy's source symbols (`derived_source`); non-server-code (`not_required`). A server-code `builder_build` step with no declaration, or a `write_file_exact` whose CONTENT snapshot is missing, is reported `gap` — NEVER given an invented assertion. `extractExportedSymbols` parses `export const/function/class` + `export { a as b }`. `auditBlueprintCoverage` labels each gap `source_missing` | `exact_no_symbols` | `no_declaration`. (2) `builderos-reboot/scripts/bp-priority-coverage-audit.mjs` — walks all of `BP_PRIORITY.json` and prints per-mission provable/gap counts. LIVE CENSUS: **13 missions, 104 steps — 99 provable (95%), 5 gaps** (2 `no_declaration` freeform `builder_build` steps in rank-1 CONVERSATION-COMMITMENTS; 3 `source_missing` byte-exact snapshots in rank-2 ACTION-INBOX). Those 5 MUST get a mission-authored `assertion_spec` (or a restored CONTENT snapshot) before the flip, or the governed runner halts them fail-closed — the Chair's risk, now measured, not guessed. Proof: `builderos-reboot/scripts/derive-assertion-spec-proof.mjs` 8/8 (symbol extraction, override-wins, derived_exact/derived_source, not_required, non-fabricated gap, source_missing vs no_declaration labelling). NEXT: author `assertion_spec` for the 5 gap steps (or restore ACTION-INBOX snapshots), wire a scheduler that feeds BP_PRIORITY steps + derived specs into `/factory/ship-queue`, prove one autonomous ship live, THEN flip `GOVERNED_FACTORY_ONLY=1`. Prior: 2026-07-08 — **GOVERNED SHIPPING RUNNER + BPB ASSERTION AUTHORSHIP (cutover STEP 5b):** the replacement shipping path the Step-5a fence gates on. Two new modules. (1) `factory-staging/factory-core/bpb/author-assertions.js` (PURE) — BPB authors each step's `behavior_assertions` from the BLUEPRINT SPEC at intake (Chair ruling, live council SHA 1783452556804 chose option (a) for clean provenance + zero latency). NON-FABRICATING: it only translates expectations the spec already declared (`expected_exports`→`file_contains`, `route`→`module_mounts`/`http_status`, `db`→`db_row_exists`, `file_contains`) into the SENTRY assertion schema; a server-code target whose spec declares nothing verifiable returns `ok:false` so the STEP-4 provenance gate fails it closed (never ships unprovable code). Provenance stamped `bpb`, never `codegen` — SENTRY still validates independently-authored assertions against model output. (2) `services/governed-shipping-runner.js` (PURE; dispatch/checkpoint/signal injected) — `runGovernedShippingQueue` walks a build queue and ships every step THROUGH the governed pipe (`/factory/execute-step`: BPB→Builder→SENTRY→TSOS→Historian) instead of legacy `/build`. Addresses CENTURY'S FLAG (the runner's own loop is ungoverned code): every step is checkpointed BEFORE+AFTER dispatch, a crash emits a loud governed signal with `resume_from` (never a silent skip), the run is RESUMABLE from the last completed step, and an unprovable step or a SENTRY block halts fail-closed. Proof: `builderos-reboot/scripts/governed-shipping-runner-proof.mjs` 14/14 — BPB authorship + non-fabrication + declared-preservation + full-queue ship + checkpoints + crash-loud-and-resumable + resume-completes + governed-block-halt + unprovable-halt-before-dispatch + **END-TO-END through the REAL dispatcher** (BPB-authored assertion → real /factory/execute-step → SENTRY PASS, same dispatcher proven live in STEP 4). NEXT: wire the runner behind a route/scheduler that reads BP_PRIORITY build steps, prove one real step live on prod, then flip `GOVERNED_FACTORY_ONLY=1` (the cutover). Prior: 2026-07-08 — **GOVERNED-FACTORY ENFORCEMENT FENCE (cutover STEP 5a):** new pure module `services/governed-factory-guard.js` is the ONE chokepoint that decides whether the ungoverned autonomous shipping path (the never-stop product loop that POSTs directly to `/api/v1/lifeos/builder/build`) may ship. `governedFactoryOnly()` reads `GOVERNED_FACTORY_ONLY` (default OFF, so nothing regresses before the governed shipping runner is proven — turning it on IS the cutover switch). `assertUngovernedShippingAllowed(source)` fails closed when the fence is on. Wired at all three ungoverned ship points in `services/never-stop-product-factory.js` (`postBuilderBuild` returns 423 blocked without calling legacy `/build`; `runProductExpansionCycle` + `runProductExpansionLanes` early-return a blocked receipt) and in `services/never-stop-product-factory-scheduler.js` (`startNeverStopProductFactoryScheduler` refuses to start the loop). This makes the Chair's #1 Step-5 concern — "prove no bypass to the legacy build endpoint remains" — provable: flip the flag and every side-channel fails closed. Codegen capability is NOT lost — it lives on inside the fence as the Step-4 authoring sub-step. Proof: `builderos-reboot/scripts/governed-factory-fence-proof.mjs` 15/15 (default-off no-regression, all truthy spellings block, falsy spellings don't, live cycle/lanes fail closed); regression `tests/{never-stop-idempotent-complete,never-stop-queue-merge,product-build-orchestrator,product-expansion-lanes}.test.js` 27/27. NOTE: flipping `GOVERNED_FACTORY_ONLY=1` is gated on Step 5b (the governed shipping runner that walks BP_PRIORITY through `/factory/execute-step` with BPB-authored per-step assertions) so autonomous build throughput is preserved. Prior: 2026-07-08 — **SENTRY BEHAVIORAL PROOF (governed-factory cutover STEP 3):** new pure module `factory-staging/factory-core/sentry/behavior-assertions.js` gives SENTRY sensors — it classifies server-code targets (`routes/ services/ middleware/ startup/ factory-core/`) + steps that declare an effect as REQUIRING `behavior_assertions`, executes them (`db_row_exists` read-only SELECT / `http_status` / `module_mounts` / `file_contains`) through an INJECTED runner, and fails closed (`evaluateBehaviorGate`: missing proof, unavailable runner, or any failed assertion → `pass:false`). Wired into `sentry/verify-step-result.js` (`behavior_proof` is a blocking finding) and the now-async `builder/run-step.js` + `builder/run-mission.js` (thread the runner, await assertions). `historian/append-record.js` writes `behavior_assertions` VERBATIM (raw per-assertion outputs, never a pass/fail summary) — Chair ruling: the receipt is the only ground truth for whether a behavior was verified. All ~11 dispatcher callers now await. Proof: `builderos-reboot/scripts/sentry-behavior-proof.mjs` (8/8: classification, fail-closed-missing, fail-closed-no-runner, pass-on-satisfied, verbatim observed_rows, fail-on-unsatisfied, reject-non-read-only-SQL); dispatch CI subset (integration/greenfield/determinism/full-loop/tsos/tools) green. 2026-07-08 — **BUILDER CAP 1: the planner now authors a queue from a product's documented PHASE specs, not just flat backlog bullets — so a founder never hand-writes BUILD_QUEUE steps for a documented phase** (`services/build-queue-planner.js` new `extractPhaseSpecs` + folded into `extractBacklog`; hardened `buildPrompt` + done-file dedup in `planBuildQueue`). Until now `extractBacklog` only read bullets under a backlog/roadmap heading, so a product whose next work is written as "### Phase N — Title" prose (MarketingOS §5, most product visions) yielded ZERO extractable backlog and the loop was legitimately stuck waiting on me to hand-author steps (Phase 2, the Content Intel batch). Now `extractPhaseSpecs` deterministically lifts each "### Phase N — Title" sub-heading + its prose body — but ONLY inside a "Phased Build Plan / Roadmap" section (via `PHASE_SECTION_HEADING`, so a technical-spec section's "### Phase N Tables" subsections are never mistaken for buildable phases) and ONLY when the phase is (a) not already shipped (no done/✅ heading marker) and (b) buildable on current infra — a phase whose body/heading carries `UNVERIFIED`/`BLOCKED`/"requires app review"/manual-sprint markers is SKIPPED (never build on unverified infra — a slice of infra-gating). Verified against the real MarketingOS doc: extracts exactly Phases 7/8/10 (all buildable), correctly skipping done Phases 1/2 and infra-gated 3/4/5/6/9. `planBuildQueue` now (1) feeds the model the COMPOSITION rules I've been hand-applying (self-contained modules, auto-registration not server.js edits, DB-defaulted ids/no-uuid-import, no top-level browser globals, AI via injected `callCouncilMember`) so autonomously-planned steps actually mount, and (2) passes the list of already-DONE target files with a "do NOT re-create" instruction AND hard-dedups any proposed step whose `target_file` is already done — so a completed phase is never rebuilt even when its spec still lives in the doc. Fail-closed + purely extractive throughout (never fabricates). `tests/build-queue-planner.test.js` 13/13 (+extractPhaseSpecs manual/done/unverified/blocked skips, +phase folding into backlog, +never-rebuild-done-file). Prior: 2026-07-08 — **SELF-REFINEMENT SPINE PR7: the loop re-plans a COMPLETED queue from its own PRODUCT_HOME — it adds more work to itself once a phase ships** (`services/build-queue-planner.js` `backlogSignature`; `services/never-stop-product-factory.js` `discoverPlanWork` + `runPlanBuildQueue`). Until now the auto-planner only enrolled products with NO `BUILD_QUEUE.json`; a product whose queue was fully `done` (e.g. MarketingOS Phase 1 3/3) was skipped forever, so the loop could not pick up the next documented phase without a human hand-authoring it. Now `discoverPlanWork` also surfaces a `plan_build_queue` task (`extend:true`) for a product whose queue is 100% `done` AND whose PRODUCT_HOME documents backlog not yet planned; `runPlanBuildQueue` passes the existing queue to `planBuildQueue`, which appends only genuinely-new steps (deduped by id + `target_file::task`) onto the shipped ones. Guarded hard against waste: extend fires ONLY when EVERY step is `done` (never on top of a building/blocked phase) AND the order-independent `backlog_signature` (sha of the documented backlog) has CHANGED since the queue was last planned — so an unchanged backlog never burns a planner model call per idle cycle (ZERO WASTE rule). Still fail-closed and never fabricates: `extractBacklog` only reads bullets under a backlog/roadmap/next/build-plan heading, and the planner is fed ONLY those real documented items. NOTE (honest state): site-builder + marketingos currently have ZERO extractable backlog (their next phases are written as brainstorm vision, not under a backlog heading), so the loop is legitimately caught up — building the rest of MarketingOS requires the next phases to be DOCUMENTED as backlog (a founder/product decision), after which the loop now picks them up automatically. `npm test` 328/0; `tests/build-queue-planner.test.js` +backlogSignature stability + queue-stamp. Prior: 2026-07-08 — **SELF-REFINEMENT SPINE PR6: correct the PR5 self-repair mechanism — the functional-proof gate is the arbiter, not a forced /build** (`services/never-stop-product-factory.js` `buildFn`). PR5 made the pre-existing-file short-circuit skip (fall through to `/build`) whenever a step carried `last_error`, on the theory the file was known-broken and must be regenerated. That was wrong in practice: the builder REFUSES to regenerate a large existing file (Zone-3 stub guard / empty edit-patch returns no commit), so a repaired-but-still-flagged step (e.g. `mos-session-routes` after its `uuid` import was fixed and the module was already `mounted` live) fell through to `/build`, got no commit, and re-blocked with `no_commit_sha` on every revive — a NEW spin, never reaching `done`. Fix: the short-circuit now ALWAYS fires for an existing file; the FUNCTIONAL-PROOF gate (verify + deploy-proof + module-health `mounted`) is the true arbiter of health, not the queue's stale `last_error` — so a file repaired out-of-band (or by a corrected regeneration) completes to `done`, while a genuinely-broken one still re-fails the gate with its real mount error (no false-done). The prevention for the original bug class (importing an uninstalled package) remains PR5's integration-context allowlist + DB-defaulted-ids rule, which stops the bad code from being generated at all. `npm test` 328/0. Prior: 2026-07-08 — **SELF-REFINEMENT SPINE PR5: self-repair the loop's own broken artifacts + tell the builder which packages exist** (`services/build-integration-context.js` `readInstalledPackages` + context body; `services/never-stop-product-factory.js` `buildFn` short-circuit guard). Closes the last gap that let a false-done survive: the loop autonomously built `routes/marketing-session-routes.js` but it `import`ed the UNINSTALLED `uuid` package, so the module crashed at load (`Cannot find package 'uuid'`) — and the pre-existing-file short-circuit (which completes an existing file via its last commit instead of calling `/build`) meant the broken file could NEVER be regenerated: short-circuit → functional-proof gate fails → carries `last_error` → short-circuit again, forever. Two fixes: (1) **integration context now lists the AVAILABLE NPM PACKAGES** (from `package.json` deps+devDeps) and a **DB-DEFAULTED-ids rule** — "id/created_at/updated_at are DB-defaulted via `gen_random_uuid()`/`NOW()`; do NOT generate them in JS and do NOT import a uuid package; INSERT without them"; importing anything off the list fails at load and the gate rejects it. (2) **self-repair bypass** — `buildFn` only short-circuits an existing file when it carries NO `last_error`; a KNOWN-BROKEN step (a prior verify/deploy-proof/module-health failure wrote `last_error`) FALLS THROUGH to a real `/build` that regenerates it with the verbatim error fed in, so the loop fixes its own broken code instead of re-completing it into the same failing gate. Also repaired the live artifact: removed the `uuid` import + JS-side `session_id` generation from `routes/marketing-session-routes.js` (the schema DEFAULTs `id`), unblocking `tests/spine-import-resolution.test.js` on `main`. Tests `tests/build-integration-context.test.js` 8/8 (+readInstalledPackages dedupe/empty, +available-packages & uuid-forbidden in context). Prior: 2026-07-07 — **SELF-REFINEMENT SPINE PR3: real integration context into /build** (`services/build-integration-context.js` new; wired in `services/never-stop-product-factory.js` `buildFn`). Every autonomous `/build` call now carries the LIVE integration surface so generated code COMPOSES instead of importing things that don't exist (the root of the MarketingOS false-done: it imported `./ai-council.js`, called service exports that were never defined, and assumed a schema that wasn't there). The appended context has three parts: (1) **injected-deps contract** — the exact `deps` object auto-registered modules receive at `register(app, deps)` (`pool`, `requireKey`, `callCouncilMember`, `logger`, `baseUrl`, `commitToGitHub`, `commitManyToGitHub`), with an explicit "do NOT import `./ai-council.js` — use `deps.callCouncilMember`" rule; (2) **live DB schema digest** parsed from `db/migrations/*.sql` (`parseSchemaFromMigrations`) and narrowed to the step's relevant tables (`selectRelevantTables`, token-bounded) so the model uses EXACT table+column names; (3) **auto-mount convention** for `routes/*.js` — derive `register<Name>(app, deps)`, add the `config/auto-registered-product-modules.json` entry, never edit the protected boot file — which is exactly what PR1's registration + PR2's functional-proof gate then verify LIVE. Pure + network-free (reads migrations from disk), fail-open (build proceeds without context if it errors). Tests `tests/build-integration-context.test.js` 6/6 (schema parse w/ constraints+public. prefix+missing dir, relevance scoring bounded, route context has deps+mount+schema, non-route omits mount, deps contract matches boot). Prior: 2026-07-07 — **SELF-REFINEMENT SPINE PR2: functional-proof completion gate + self-diagnosis carry-forward** (`services/product-build-orchestrator.js` `runNextStep` new `moduleHealthFn` seam + pure `evaluateModuleHealthForStep`; wired in `services/never-stop-product-factory.js` `runProductBuildStep`). Kills the "false done" class: after a step's build passes + verify passes + deploy-proof confirms the built SHA is served live, a `routes/*.js` step is STILL not `done` until the boot module-health manifest (`GET /api/v1/lifeos/builder/module-health`, PR1) shows that exact module `mounted`. If it threw on import or was never auto-registered, the step stays retryable (PENDING, or BLOCKED after maxAttempts — no spin) and the VERBATIM mount error (e.g. `Cannot find module './ai-council.js'`) is written to `step.last_error`, which `buildFn` feeds into the next build attempt's repair prompt — so the loop fixes the root cause instead of re-marking a false done. Non-route targets (services, migrations, config) pass through (gated by verify + deploy-proof, not by mount). A missing-from-manifest route yields an actionable reason telling the loop to add the `config/auto-registered-product-modules.json` entry. Tests `tests/functional-proof-gate.test.js` 6/6 (non-route pass-through, mounted=proven, error/missing=verbatim reason, route-not-mounted stays retryable + carries error, mounted→done+functional_proven, repeated failures BLOCK). Prior: 2026-07-07 — **SELF-REFINEMENT SPINE PR1: convention-based auto-registration + boot module-health manifest** (`startup/auto-register-product-modules.js`, `config/auto-registered-product-modules.json`, wired at `startup/register-founder-runtime-routes.js`). The autonomous loop can now ship a NEW route/UI module and have it mount LIVE by adding the module + one opt-in registry entry — no edit to the protected boot composition root. Each module records verbatim import/mount health (`mounted` | `error` + exact reason), exposed at `GET /api/v1/lifeos/builder/module-health`; this manifest is the substrate for the coming functional-proof completion gate + self-repair (a step can't be `done` until its module actually loaded + serves). Fail-open per module (one broken module never aborts boot). Tests `tests/auto-register-product-modules.test.js` 4/4 (good mounts, missing/broken-import/throwing/no-register all record errors + never abort siblings, disabled/empty no-op, serializable manifest). Also removed the false-done MarketingOS Phase-1 artifacts (`routes/marketing-session-routes.js`, `services/marketing-session-ui.js`) that imported non-existent exports (`sendCoachMessage` etc.) + a missing `./ai-council.js` — they were unwired/unreachable and broke `tests/spine-import-resolution.test.js` on `main` (the loop commits with no CI); Phase 1 is re-queued correctly (functional + auto-registered) in the follow-up re-queue PR. Prior: 2026-07-07 — **UNSTUCK: a legacy SMOS acceptance-repair can no longer monopolize the never-stop loop or thrash redeploys** (`services/never-stop-product-factory.js` `discoverProductExpansionWork` + the `acceptance_repair`/`schema_or_crud` remedy branch). After MarketingOS was enrolled (#238) the loop still never touched its queue: `discoverProductExpansionWork` pushed a `smos_session_crud`/`smos_acceptance_repair` item at integer `priority:2`, which permanently OUTRANKS real BUILD_QUEUE steps (`priority: 2 + productRankFraction` = ~2.x), and its ONLY remedy re-ran the idempotent `db/migrations/20260629_socialmediaos_schema_align.sql` build + `tryRedeploy()` every cycle. Empirical: `POST /api/v1/socialmediaos/sessions` returns 404 `Cannot POST` (the route does not exist), so the session probe can NEVER pass — the migration is already applied and cannot create a missing route, so the loop rebuilt+redeployed the same SQL ~40× in 15 min (commits `09b37da3c…c878337b8`), each redeploy resetting the scheduler (`total_runs:0`, `running:false` between boots) and starving every queued product. Fix: (1) both SMOS repair items drop to `priority:6` (lowest) so real BUILD_QUEUE work always wins; (2) `smos_session_crud` is only enqueued when the migration file is ABSENT (genuine schema work) — once it exists a still-failing probe is a route/handler gap, left to the queue; (3) THRASH GUARD in the remedy branch: if the migration already exists, no-op (`schema_align_already_applied_no_op`) instead of rebuild+redeploy, breaking the ~60s churn. Net: the loop now builds MarketingOS Phase 1 (`mos-phase1-schema` → routes → UI) instead of spinning on an unfixable legacy probe. Prior: 2026-07-07 — **New-file build completes via GitHub-API-resolved SHA even when the /build response omits commit_sha and the container checkout lags** (`services/never-stop-product-factory.js` `buildFn`). Empirical: the loop built + committed `services/site-builder-editor.js` (the shell compose step) to GitHub **4 times** (23:02→23:18) — valid ESM, 0 top-level browser globals, passes `node --check` — yet the step stayed `blocked` with `no_commit_sha (claimed pass without proof)`. Root cause: the idempotent-completion branch required `targetFileExists(target_file)` on LOCAL disk, but the prod container's checkout lags the repo by ~17 commits (queue churns every tick), so the file the build JUST committed is absent locally → completion branch skipped → recorded as failure → block → revive → rebuild forever, and dependents (`sb-editor-endpoints`, waiting on the shell) can never start. Fix: drop the local-disk precondition; when `!commit_sha && (build.ok || b.committed || emptyEditNoOp)`, resolve the file's last-touching commit via `lastCommitShaForFile` (which falls back to the GitHub raw/commits API per #229) — if it resolves, the artifact genuinely exists on the repo and the step completes honestly (verify + deploy-proof still gate it); null = file exists nowhere = real failure. This also collapses the block/revive/rebuild churn (steps finish on first successful commit), which lets the lagging Railway deploy finally catch up. Prior: 2026-07-07 — **Queue-status persistence now MERGES onto the latest repo queue instead of clobbering it** (`services/never-stop-product-factory.js` `commitQueueStatusToRepo` + new `mergeQueueRuntimeStatus`). After #232 the loop autonomously completed all 4 editor panes; the shell compose step then failed builder validation and needed a spec tweak, but every external edit to `BUILD_QUEUE.json` (task/spec) was reverted within one tick: the loop wrote the WHOLE queue from its stale boot-time in-container snapshot, overwriting concurrent spec edits. Fix: `commitQueueStatusToRepo` re-fetches the latest repo `BUILD_QUEUE.json`, overlays ONLY the loop-owned runtime fields per step (`status, attempts, commit_sha, built_sha, proof, last_attempt_at/last_attempt, last_error, revive_count, completed_at, no_op, pre_existing`) via `mergeQueueRuntimeStatus`, preserves repo step order + author-owned fields (task/spec/depends_on/target_file/max_output_tokens), and retries on a 409/422 sha race. Tests: `tests/never-stop-queue-merge.test.js` 4/4. Prior: 2026-07-07 — **CRITICAL: deploy-proof compare-status was INVERTED — it rejected every genuinely-live deploy** (`services/deploy-truth.js` `interpretCompareStatus`). Once #231's pre-build short-circuit got a pane (canvas) past build+verify to deploy-proof, it failed `deploy_serves_different_sha`. Empirical proof via the real GitHub compare API: `compare/<built>...<served>` returns `status:"ahead", ahead_by:72, behind_by:0` — i.e. the served deploy is 72 commits AHEAD of the built commit, so it genuinely CONTAINS it and the built code IS live. But `interpretCompareStatus` accepted only `identical`/`behind` as "contains" — it had GitHub's head-relative semantics backwards (`base...head`: `ahead` = head/served is ahead of base/built = contains; `behind` = served is missing built = not live). So containment could essentially never pass, and EVERY built step (not just panes) failed deploy-proof → block forever. This sat UNDER the no-op / Zone-3 / auth bugs. Fix: `interpretCompareStatus` now returns true for `identical`/`ahead` (equivalently `behind_by === 0`); comments + `tests/deploy-truth.test.js` corrected (13/13). This + #231 completes the chain: already-built file → verify 7/7 → deploy-proof containment (ahead) → `done`. Prior: 2026-07-07 — **DEFINITIVE FIX: pre-build short-circuit — an already-existing target file completes via its last commit BEFORE calling /build, because the builder cannot rebuild an existing file** (`services/never-stop-product-factory.js` `buildFn`). Re-probing `/build` on live prod revealed the failure is NON-DETERMINISTIC and the earlier empty-edit signal was only one face of it: the other is `ZONE3_PATCH_REQUIRED` — `{ error: "Pre-commit governance blocked", governance.pipeline.zoneResult: { zone:3, reason:"file is 354 lines — builder will generate stub. Use extract-helper strategy.", patchModeRequired:true } }`. The builder governance **refuses by design to regenerate a large existing file** (it would emit a stub) and yields an empty edit patch when the file is already correct — so `/build` can NEVER return a commit SHA for an already-built pane, and the step spins `no_commit_sha` → block → revive → forever. Fix: at the TOP of `buildFn`, before any `/build` call, if `targetFileExists(target_file)` and `lastCommitShaForFile` resolves (local git → GitHub API), complete the step via that commit as the built SHA. New (non-existent) targets fall through to a normal build. `verifyFn` (`verify-project.mjs --project site_builder`) + deploy-proof containment still gate it — a broken/stub file fails verify with a different error, so no false green. Prior: 2026-07-07 — **TRUE ROOT CAUSE FOUND: an already-built file makes the builder enter EDIT-PATCH mode and return an EMPTY edit array → HTTP 422, which broke idempotent completion** (`services/never-stop-product-factory.js` `isEmptyEditNoOp` + `buildFn`). Diagnosed by calling `/api/v1/lifeos/builder/build` directly with a pane's exact payload: it returns `HTTP 422 { error: "Zone 3 edit-patch failed — edit output is not a non-empty JSON array", output: "[]" }`. Because `target_file` already exists, the builder diffs instead of writing; the model correctly returns `[]` (nothing to change — file is already right), and the edit stage rejects the empty array as a failure. So `build.ok` was FALSE, and the prior idempotent-completion branch (which required `build.ok === true`) never fired → the 4 editor panes cycled `no_commit_sha` → block → revive → forever despite #227/#228/#229. Fix: new exported `isEmptyEditNoOp(buildBody)` recognizes the empty-edit-array 422 shape (`output === '[]'`, the verbatim error, or `gap_recommendation.stage === 'edit_patch'`); `buildFn` now completes the step via the file's last-touching commit when there's no commit AND (`build.ok` OR `emptyEditNoOp`) AND the file exists. `verifyFn` + deploy-proof still gate it (a genuinely broken file fails verify with a different error — no false green). Regression: `tests/never-stop-idempotent-complete.test.js` 3/3 (adds the empty-edit detection: true for the three 422 shapes, false for real failures / normal responses / null). Prior: 2026-07-07 — **IDEMPOTENT COMPLETION WORKS IN PROD: resolve the already-built file's last commit via the GitHub API, not just local git** (`services/never-stop-product-factory.js` `lastCommitShaForFile` + new `githubLastCommitShaForFile`). Root cause the #227 idempotent-completion fix silently no-opped in production: the deployed container **dockerignores `.git/`** (`.dockerignore` L13), so `git log -1 --format=%H -- <file>` returns nothing → `lastCommitShaForFile` returned null → the no-op branch never completed the step → still `no_commit_sha` forever (observed live: panes stayed pending/att-cycling after #227+#228 shipped). Fix: try local git first (dev/test), then fall back to `GET /repos/:o/:r/commits?path=<file>&sha=<branch>&per_page=1` using the same `GITHUB_TOKEN`/`GITHUB_REPO` the queue-status committer uses; fail-closed to null (no fabricated SHA) on missing creds / non-200 / bad shape. With this, an already-built pane resolves its true built SHA in prod, and (with deploy-proof auth from the prior receipt) completes: build no-op → verify → deploy-proof containment → `done`. Tests: `tests/never-stop-idempotent-complete.test.js` 2/2 (hermetic — null without creds). Prior: 2026-07-07 — **DEPLOY-PROOF AUTH: the proof now sends the command key, so `/ready` no longer 401s and steps can actually reach `done`** (`services/never-stop-product-factory.js` `deployProofFn`). Root cause NO step could ever pass deploy-proof: the `/api/v1/lifeos/builder/ready` endpoint is auth-gated (verified live: 401 without `x-command-key`, 200 with), but `deployProofFn` passed a plain `fetch` with no header, so `proveDeployServesSha` got HTTP 401 → `ready_http_401` → `ok:false` on every attempt → the step exhausted `maxAttempts` → `blocked`. This sat UNDER the no-op and revive bugs — even once a step built, it could never be proven live. Fix: `deployProofFn` now builds an `authedFetch(url, opts)` that merges `{ 'x-command-key': commandKey }` onto the prover's own headers (Cache-Control + abort signal) before calling `fetch`, so the ~10-min proof window reads the real served SHA and the containment check (#225) can pass. `deploy-truth.test.js` stays 13/13. Prior: 2026-07-07 — **IDEMPOTENT COMPLETION: a step whose target file is already built now finishes instead of failing `no_commit_sha` forever** (`services/never-stop-product-factory.js` `buildFn`, helpers `targetFileExists` + `lastCommitShaForFile`). Root cause the revived editor panes still could not finish: their files already exist on `main` (built in an earlier session), so re-running the `/build` primitive is a no-op → no new commit → no `commit_sha` → the orchestrator's no-false-green guard (`product-build-orchestrator.js` L168-172) treats it as a failure, so the step exhausts `maxAttempts` → `blocked` → auto-revives → no-op again → forever. Fix: when the builder runs clean but returns no commit AND the target file is present (`fs.statSync().size > 0`), complete the step honestly using the file's LAST-TOUCHING commit as the built SHA (`git log -1 --format=%H -- <file>`) — a stable ancestor the running deploy provably CONTAINS (HEAD would be wrong: later queue-status commits keep advancing HEAD past the served SHA). `verifyFn` + `deployProofFn` still gate it, so this is not a false green — the artifact must exist AND be served live before `done`. Regression: `tests/never-stop-idempotent-complete.test.js` 2/2 (tracked file → 40-char sha + exists true; untracked → null + exists false). Prior: 2026-07-07 — **SELF-HEAL: the never-stop loop now auto-revives stale `blocked` steps** (`services/product-build-orchestrator.js` `reviveStaleBlockedSteps`, wired into `services/never-stop-product-factory.js` at both the discovery scan and the run path). Root cause a fixed bug still stranded work: `BLOCKED` is TERMINAL, so `selectNextStep` skips a blocked step forever — even after the failure cause is fixed. The 4 editor panes (`sb-editor-sidebar/canvas/chat/strategy`) built as real files but were `blocked` by the old deploy-proof false-negative, and because `sb-editor-shell` `depends_on` them the whole editor stalled. Fix revives a blocked, non-founder-gated step back to `pending` (attempts reset) once `DEFAULT_REVIVE_COOLDOWN_MS` (15 min) elapses, bounded by `revive_count < DEFAULT_MAX_REVIVES` (6) so a genuinely broken step still stops (daily cost cap is the second backstop). Founder-gated steps never auto-revive — only Adam clears those. Regression: `tests/product-build-orchestrator.test.js` 10/10 (revives past cooldown, respects cooldown/cap/gating, makes a stranded dependent selectable again). Prior: 2026-07-06 — **DEPLOY-PROOF CONTAINMENT: steps that built fine no longer get wrongly `blocked`** (`services/deploy-truth.js`, `services/never-stop-product-factory.js`). Root cause the loop built editor panes but never marked them `done`: `proveDeployServesSha` required the served SHA to EXACTLY match the built commit, but in a busy repo the served SHA is always a DESCENDANT (the queue-status commit + up to `NEVER_STOP_LANES` sibling lanes advance HEAD past the build), so exact-match could never land → the step exhausted `maxAttempts` and went `blocked` (observed live: `sb-editor-sidebar` built as real 353-line file, then blocked at att:3). Fix: new pure exported helper `interpretCompareStatus(status)` — a deploy whose SHA is `identical`/`behind` relative to the built commit CONTAINS it, so the built code is genuinely live; `proveDeployServesSha` now takes an optional `compareFn` and, on exact-match miss, calls the GitHub compare API (`GET /repos/:o/:r/compare/:built...:served`) to accept containment (reason `deploy_contains_built_sha`) — fail-closed when the comparison errors/absent (no false live). `never-stop-product-factory.js` supplies `githubCompareStatus()` and widens the proof window to 40×15s (~10 min) to outlast a Railway rebuild. Wrongly-`blocked` steps reset to `pending` in `BUILD_QUEUE.json`. Tests: `tests/deploy-truth.test.js` 13/13 (containment OK, behind-fails, fail-closed on error/absent). Prior: **QUEUE-STATUS DURABILITY: the loop now commits BUILD_QUEUE.json status to the repo after each step** (`services/never-stop-product-factory.js` `commitQueueStatusToRepo`), so a step marked `done` (and its `attempts` count) survives the deploy-truth redeploy instead of resetting to `pending` on a fresh checkout. This is the root-cause fix for "the loop never finishes a product" — before it, every completed step reverted after redeploy and was rebuilt forever. Prior: **AUTONOMOUS LOOP NOW BOOTS ON PROD + COST GUARDRAIL.** Root cause the system never built on its own: Railway is hard-locked to the minimal `founder_builder` runtime (`services/runtime-modes.js`), which never booted `startup/boot-domains.js` where the never-stop scheduler lived — so `BUILDEROS_NEVER_STOP=1` did nothing. Fix: `startNeverStopProductFactoryScheduler()` is now started inside `server-founder-runtime.js` (the lane that actually runs on Railway, and which already mounts the `/build` primitive), self-gated on `BUILDEROS_NEVER_STOP`. Added a hard cost guardrail in `services/never-stop-product-factory.js` — `dailyBuildBudget()` / `recordDailyBuildAttempts()` cap builder attempts per UTC day (`NEVER_STOP_DAILY_STEP_CAP`, default 60; 0 = unlimited); the scheduler halts the tick with `reason: daily_budget` once spent. New status route `GET /api/v1/lifeos/never-stop/status`. Tests 23/23 green. Prior: 2026-07-03 — Founder-build progress streaming: `services/founder-build-job-store.js` now tracks an ordered `steps[]` per job (via new `appendFounderBuildJobStep()`, deduped, capped at 60), and `services/founder-build-self-repair.js` threads an `onProgress` callback through the build + CSS-patch pipelines to emit real milestones ("Reading your request", "Building the change", "Searching online for a fix", "Committing code", "Deploying to production", "Checking it worked live"); `startFounderBuildJob()` wires those into the job's steps so the founder UI can show the system's live background work. Prior: 2026-07-04T20:29 — route_wire_failed also continues. |
