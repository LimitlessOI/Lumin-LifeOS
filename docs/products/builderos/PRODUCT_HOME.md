...
| **Last Updated** | 2026-07-13 — Kill never-stop thrash: no revive-on-discover, route-specific auto-reg revive, config JSON skips UI SENTRY, lanes honor PRODUCT_BUILD_PRIORITY. |
...
>>>>>>> 7c64e2a522 (GAP-FILL: never-stop burned tokens reviving LifeOS/Wellness blocked auto-reg forever — discover revived then lanes ignored PRODUCT_BUILD_PRIORITY; fix: no revive-on-discover, route-specific register unlock, exhausted→skipped, prefer PENDING, config JSON skips UI SENTRY, lanes sort by financial priority)
