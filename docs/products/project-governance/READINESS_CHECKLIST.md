<!-- SYNOPSIS: Canonical pre-build readiness checklist -->

# Pre-Build Readiness Checklist
**Every product packet or product-home build plan MUST complete this checklist before `build_ready` is set to TRUE.**
...
1. Copy the checklist section below into the canonical product home or founder packet.
...
**Competitor table** (fill in packet):
...
When all 5 gates are complete, update the canonical packet header:
...
POST /api/v1/projects/:slug/readiness/mark-ready
{
  ...
  "notes": "Gates 1-5 complete. Competitor table in product packet section 4.",
  "competitor_analysis": [
...
The builder supervisor will pick up safe segments on the next run.
