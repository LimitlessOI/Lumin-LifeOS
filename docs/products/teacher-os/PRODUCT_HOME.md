<!-- SYNOPSIS: Canonical product home — Teacher OS -->

# Teacher OS Product Home

**Formerly called:** Amendment 31 — TEACHER OS

| Field | Value |
|---|---|
| **Canonical home** | this file |
| **Product id** | `teacher-os` |
...
|---|---|---|---|
| 2026-07-20 (2nd pass) | **`teacher-os-9`: last cycle's fix worked (step genuinely got selected and attempted twice, real failures not silence), but it hit a 3rd real bug I'd missed.** `file_contains` still required 3 `services/*.js` paths. Read the actual `config/auto-registered-product-modules.json` structure directly: its `modules[]` array only ever contains ROUTE files (`path`/`register`/`mount_path` for Express auto-mounting) — 0 of ~100 real entries reference a `services/` path, by design. No correct implementation could ever satisfy a check requiring service files to appear in a routes-only config. Confirmed live: both real attempts failed on exactly the 3 service entries (and only those). Removed all `services/*.js` entries from `file_contains` (including the `services/ferpaDpaTemplate.js` one added last cycle, which happened to pass by incidental match, not because it was a real requirement — removed for consistency rather than rely on that coincidence). `file_contains` now only requires the 4 real route files this step is actually responsible for. | Verified against the real config schema before editing, not assumed; the loop's own live SENTRY failures pinpointed exactly which 3 entries were wrong. | Read `config/auto-registered-product-modules.json` directly (`grep -c` for `services/` vs `routes/` path entries: 0 vs ~100); confirmed against 2 real, independent SENTRY failure messages naming the same 3 entries both times |
