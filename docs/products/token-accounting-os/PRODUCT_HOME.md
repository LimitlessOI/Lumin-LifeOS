...
| **Last Updated** | 2026-07-29 — S00 spend→outcomes report via BuilderOS control plane |
...
| Date | Change | Why |
|------|--------|-----|
| 2026-07-29 | **S00 join surface.** BuilderOS control-plane `spend-outcomes` / `estimate` / `linkage` report $→shipped builds; kernel calls `linkTokenReceiptToTask` after builds. `SPEND_OUTCOME_STRICT=1` hardens when blind. Does not yet auto-meter Cursor IDE spend. | Adam: track production from money spent before adding more |
