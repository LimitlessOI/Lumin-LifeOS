<!-- SYNOPSIS: Canonical product home — Story Studio -->

# Story Studio Product Home

**Formerly called:** Amendment 22 — STORY STUDIO

| Field | Value |
|---|---|
| **Canonical home** | this file |
| **Product id** | `story-studio` |
| **Constitutional law** | `docs/constitution/NORTH_STAR_SSOT.md` |
| **Machine manifest** | `docs/products/story-studio/FILE_MANIFEST.json` |
...
| **Manifest** | `docs/products/story-studio/FILE_MANIFEST.json` |
...
## Owned Files
```
docs/products/story-studio/PRODUCT_HOME.md
docs/products/story-studio/FILE_MANIFEST.json
...
```bash
 test -f docs/products/story-studio/PRODUCT_HOME.md

# Manifest exists
 test -f docs/products/story-studio/FILE_MANIFEST.json
...
1. `docs/products/story-studio/PRODUCT_HOME.md`
2. `docs/products/video-pipeline/PRODUCT_HOME.md`
3. `docs/products/capability-map/PRODUCT_HOME.md`
...
| 2026-08-02 | **GAP-FILL: annotated step 5 (`services/rightsControl.js`) with an honest `false_done_note` instead of resetting it.** Step 5's `status: done` / `heal_reason: artifact_proof_pass` is accurate only in the narrow sense that the file exports `setRights`/`enforcePrivacyMode` and passes `exports_smoke` — that assertion checks presence, not behavior. Both functions are bare TODO stubs that unconditionally return `{ok:true, result:null}` regardless of input, directly contradicting the file's own comment. Deliberately did **not** reset the step to `pending`: `claimPreExistingSatisfiedSteps`' `pre_existing_artifact_proof` shortcut (`services/product-build-orchestrator.js`) can silently re-claim an already-present file as satisfied without ever running real codegen, which would just re-mark this exact stub "done" again — a known, separate, unfixed governance gap, not something to trigger by accident here. Live risk stays contained regardless: step 8 (the route that would expose this stub) is already `human_hold: true` from the prior receipt below. Used the same `false_done_note` field convention already established in `limitlessos`/`site-builder` BUILD_QUEUE.json files rather than inventing a new one. | Phase 4.2 of `BLUEPRINT_COMMUNICATION_FIRST_2026-08-02.md` — one of the three confirmed false-done instances from tonight's independent audit. Real implementation still requires the governed factory (SO-001); this fix only corrects what the data honestly claims. | JSON diff is 4 insertions / 3 deletions (single field added, no reformatting); `python3 -c "import json; json.load(open(...))"` confirms the file is still valid JSON; step 8's existing `human_hold:true` re-confirmed still present and unchanged. |
| 2026-08-02 | **GAP-FILL: completed the hold on step 8 (`routes/rightsRoutes.js`) that a prior commit declared but never applied.** `services/rightsControl.js` is a bare TODO stub — `setRights()` and `enforcePrivacyMode()` unconditionally return `{ ok: true, result: null }` regardless of input, despite the file's own comment ("Licensed adaptation projects cannot move forward without an explicit rights record"). Commit `6d53a5cb2` ("story-studio: hold rights-routes step 8 (stub/removed dependency)") declared this held but the diff shows no blocking field was actually set — the step remained `status: pending`, and its own `last_error` field literally suggests auto-registering the route into `config/auto-registered-product-modules.json`, which would put this fake-success consent/privacy stub live. Set `human_hold: true` with a reason note, matching the same pattern used earlier tonight for site-builder step-09. Verified against the real orchestrator (`selectNextStep()` on the live queue) that it's now correctly excluded. | Found during real-time monitoring — the commit message and the actual data disagreed about whether this step was held; completing the declared-but-unapplied hold rather than trusting the message. | Live-verified: `selectNextStep()` run against the real `docs/products/story-studio/BUILD_QUEUE.json` returns `gated` including step `8` and no longer selects it. |
