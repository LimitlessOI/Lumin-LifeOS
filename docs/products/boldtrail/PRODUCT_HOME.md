...
| **Last Updated** | 2026-07-20 — **Fixed the BoldTrail money path (was 500 in prod + no access grant).** `POST /api/v1/boldtrail/create-subscription` destructured `getStripeClient` from the auto-register ctx, which never supplied it → live `500 getStripeClient is not a function` (never actually sold). Now imports `getStripeClient` directly, derives a resolvable base URL from the request (was `https://undefined/...`), routes `success_url` to a new fail-closed `GET /api/v1/boldtrail/subscription/success` that retrieves the Stripe session, confirms paid, and only then flips `boldtrail_agents.subscription_status='active'` + stores `stripe_subscription_id`. New agents now start `'incomplete'` (table default was `'active'`, so unpaid agents looked subscribed). Prior: 2026-07-15 — restored `routes/boldtrail-routes.js` from git history after autonomous builder shipped a stub; added `registerBoldtrailRoutes` / `registerBoldTrailRoutes` aliases. |
