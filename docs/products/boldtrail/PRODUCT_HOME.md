...
| **Last Updated** | 2026-08-02 — GAP-FILL `routes/leadScoring.js` (boldtrail-step2): the builder-generated route exported `registerLeadScoringRoutes` via a destructuring `export { ... }` that `exports_smoke` did not detect, lacked the BUILD_QUEUE `file_contains` substring `app.post('/api/v1/leadscoring/segment'`, and mounted at `/api`. Rewrote to export `registerLeadScoringRoutes` as `(app, deps = {})` with safe `requireKey`/`logger` fallbacks, added the exact `app.post('/api/v1/leadscoring/segment', ...)` line, kept `applyScoringRubric`, and added the route to `config/auto-registered-product-modules.json` for module-health proof. |
...
| 2026-08-02 | **GAP-FILL: removed a duplicate `config/auto-registered-product-modules.json` entry for `routes/boldtrail-routes.js`.** Two separate BUILD_QUEUE steps' fixes each added a registry entry for this file (`registerBoldtrailRoutes` no `mount_path`; `registerBoldTrailRoutes` with `mount_path: /api/v1/boldtrail`) — both `enabled:true`, both aliasing the same `createBoldTrailRoutes(app, ctx)`, which mounts routes via `app.post(...)`/`app.get(...)` directly on the shared app (not a scoped sub-router). With both entries live, the auto-register loader (which does not dedupe by path — confirmed by reading `startup/auto-register-product-modules.js`) would call `createBoldTrailRoutes` twice, double-mounting every real endpoint including `POST /api/v1/boldtrail/register` (checks for an existing agent, then inserts) — a real duplicate-write risk on a revenue-facing CRM route. Kept the more complete entry (has `mount_path`), removed the duplicate. Both export names remain in the file itself, so no `exports_smoke` assertion is affected — the registry is purely a live-mounting mechanism. | Found during real-time monitoring while verifying an unrelated `[system-build]` commit to the same config file. | Verified via direct read of `startup/auto-register-product-modules.js`'s registration loop (`for (const spec of modules)`, no path-based dedup) and `routes/boldtrail-routes.js`'s route bodies (real `app.post`/`app.get`, not router-scoped). |
...
| 2026-08-02 | **GAP-FILL `routes/leadScoring.js` (boldtrail-step2).** The builder-generated route exported `registerLeadScoringRoutes` via a destructuring `export { ... }` that `exports_smoke` did not detect, lacked the BUILD_QUEUE `file_contains` substring `app.post('/api/v1/leadscoring/segment'`, and mounted at `/api` instead of `/api/v1/leadscoring/segment`. Rewrote to export `registerLeadScoringRoutes` as `(app, deps = {})` with safe `requireKey`/`logger` fallbacks, added the exact `app.post('/api/v1/leadscoring/segment', ...)` line, kept `applyScoringRubric`, and added the route to `config/auto-registered-product-modules.json` for module-health proof. | `bp-priority:once` artifact proof failed because the substring and export style did not match the BUILD_QUEUE contract. | ✅ node --check | `bp-priority:once` re-verify |
| 2026-07-20 | **BoldTrail subscription money path fixed end-to-end.** `routes/boldtrail-routes.js`: import `getStripeClient` directly (ctx never supplied it → live 500); `resolveBaseUrl(req)` from `x-forwarded-proto/host` (was `https://undefined`); `success_url` → new `GET /api/v1/boldtrail/subscription/success` verify handler that confirms the Stripe session is paid before `subscription_status='active'` + `stripe_subscription_id`; new agents created `'incomplete'`. | "set up the pathway end to end on all products, no theater": BoldTrail checkout was live but 500'd and granted no access. | ✅ node --check + no-lint; redeploy + live verify pending | Re-probe create-subscription returns real `cs_live_` subscription URL |
