<!-- SYNOPSIS: BuilderOS — redirect stub — see PRODUCT_HOME.md -->

# BuilderOS — redirect stub

This file is a **redirect stub**. It is not the canonical product home.

**Canonical product home:** [`docs/products/builderos/PRODUCT_HOME.md`](builderos/PRODUCT_HOME.md)

**Formerly called:** Amendment 04 — AUTO BUILDER

All product truth, owned files, mission state, and agent read order are in the canonical home above.
