...
**Formerly called:** Amendment 18 — CLIENTCARE BILLING RECOVERY
...
| **Last Updated** | 2026-07-22 — Critical deactivation invariant repaired: `do_not_bill`/`metadata.deactivated` claims are excluded from forever-chase, due-work, stage sync, notes reclassification, and open actions; imports and seed sync preserve the stop instead of reviving it. Browser-write paths re-check the claim/pregnancy immediately before filing so an in-flight queue snapshot cannot bill after deactivation. See Change Receipts. |
...
- `do_not_bill` — explicit operator deactivation; terminal for all automated billing, sync, reclassification, and filing until a deliberate reactivation path is used
...
| Date | What Changed |
| 2026-07-22 | **Critical bug fix — claim deactivation now actually stops billing work and survives re-import.** Audit of commit `58f374329` found that `deactivateClaim` only wrote `rescue_bucket='do_not_bill'`; `getForeverChaseQueue` excluded only `resolved`, so an unpaid deactivated claim still matched `paid_amount <= 0` / stale `metadata.forever_chase=true`, flowed through `getDueChaseWork` into `runHandsOffFileCycle`, and could reach live ClientCare browser filing. `seedForeverChaseFromInventory`, explicit reclassification, and `upsertClaim` could also overwrite the stop on the next sync/import. Fixed the service invariant: terminal-bucket helper; SQL exclusions on queues/reclassification/clocks/actions; deactivation clears chase/billable clock metadata and open actions; ON CONFLICT preserves existing deactivation metadata/bucket; seed bulk update cannot revive it; direct claim/pregnancy allow checks now re-check immediately before automated or direct HCFA filing. Added `tests/clientcare-claim-deactivation.test.js` for terminal detection, queue/due exclusion, action cleanup, import preservation, and last-moment claim/pregnancy guards. Concrete trigger: claim 117 (cash-pay, zero paid, stale forever-chase metadata) remained selectable immediately after deactivation. | Prevent a real cash-pay client from being filed/chased after an operator explicitly says never bill insurance. | Focused regression 5/5 PASS; changed-file syntax PASS; `npm test` 745 pass / 4 skip / 0 fail; post-fix `builder:preflight` 389/389 PASS; product-home verifier PASS; ClientCare verifier 7/7 PASS. Broad `ssot-check --all` exits 0 but reports 5 pre-existing unrelated invalid pointers. | verified on `cursor/critical-bug-investigation-3d07` at `e8f9575e8` |
| 2026-07-21 | **GAP-FILL — reversible claim deactivation (`deactivateClaim` + `POST /claims/:claimId/deactivate`), founder-authorized SO-001 bypass for narrow production fixes.** Adam, wife (Sherry) hospitalized from stress the prior night, directly authorized bypassing the governed-factory (SENTRY-verified) build requirement for this incident-recovery work — explicit, repeated, on-the-record founder authority, Chair to be informed per his instruction. First use: `clientcare_claims.id=117` (Justean Labelle) — a cash-pay client who should never have had an insurance claim generated at all, yet shows `submission_status=edi_submitted_awaiting_payment`, `payer_name="Unknown — ask insurer"`, `resolve_fail_count=97`; her `notes` field contains the literal string `"SYSTEM: Sent Bills nameHit — follow-up clock starts (7d await ERA). File path does not stagger."` duplicated 17 times back-to-back — direct proof, on one record, of the repeating-itself bug from the 07-17 incident. `deactivateClaim` sets `rescue_bucket='do_not_bill'` and merges a reason/timestamp/prior-bucket into `metadata` — deliberately never a hard `DELETE`, so the row, its full note history, and an undo path all survive. Distinct and lower-risk from the separate (not yet built) live-ClientCare duplicate-record deletion, which requires new browser-automation write capability against the vendor UI and is gated on human-supervised (Adam + Sherry) verification on 5-6 real records before any unsupervised use, per founder's explicit process. | Real production incident recovery, founder emergency authority invoked directly ("on my authority... we will be careful... inform the chair, I don't mind pushback") | ✅ `node --check` both files PASS | pending live call against production claim id 117 |
| 2026-07-17 | **GAP-FILL — download raw document bytes for direct visual review (pdf-parse confirmed all 9 are scanned images, no text layer).** `extractDocumentText` came back with 2-88 characters of extracted text on every one of Carol's 9 documents — these are photographed/scanned pages with no embedded text layer, so OCR-free text extraction can't identify any of them; a real visual read of the page image is the only remaining path. Added `downloadDocumentBase64`: same authenticated session, plain GET for one document, returns base64 bytes (never persists to disk, never writes to ClientCare — a read of one already-existing file, equivalent to clicking Download). Wired `POST /browser/download-document`. Read-only. | Direct continuation of the Carol Delgadillo receipt search — content extraction ruled out, direct visual review of the page images is what's left. | ✅ `node --check` both files PASS | pending live download + visual review of Carol's smallest document (best receipt candidate) |
| 2026-07-17 | **GAP-FILL — read PDF text content (Carol's 9 documents have no filename or Content-Disposition).** All 9 of Carol's uploaded Consent & Docs downloaded successfully as real, intact `application/pdf` files (34KB–2.3MB) with HTTP 200 — but no `Content-Disposition` header on any of them, so the original filename is unrecoverable and the visible titles are all generic placeholders. The only remaining way to identify which one (if any) is a deductible/self-pay receipt is the document's actual text content. Added `extractDocumentText`: same authenticated session, extracts cookies via `session.page.cookies()`, does a plain Node `fetch` GET (not routed through the browser page — avoids re-serializing binary through Puppeteer's JSON bridge) for each PDF, parses with the `pdf-parse` package already used in `clientcare-ops-service.js`, returns a bounded (2000-char) text preview per document. Never writes to disk, never writes to ClientCare, never downloads more than a text preview into any persisted record. Wired `POST /browser/extract-document-text`. Read-only. | Direct continuation of the Carol Delgadillo receipt search — headers ruled out as a source of the real filename, content is the last identifying signal available. | ✅ `node --check` both files PASS | pending live call against Carol's 9 PDFs |
| 2026-07-17 | **GAP-FILL — probe real filenames via download headers (no file body fetched).** Recovered Consent & Docs row text for Carol's 9 uploaded documents — every visible title is a generic auto-numbered placeholder (`MR`, `MR 2`...`MR 9`, plus one bare `id`), not a real description, so the visible label can't identify which (if any) is the deductible receipt. Added `probeDownloadHeaders`/`probeDocumentDownloadHeaders`: same authenticated session, `fetch(href, {credentials:'include'})` against each `/Report/DownloadConsent/{guid}` URL, reads only the HTTP response headers (Content-Disposition for the real original filename, Content-Type, Content-Length) — equivalent to hovering/clicking Download, never downloads or exposes the file body. Wired `POST /browser/probe-document-headers`. Read-only. | Direct continuation of the Carol Delgadillo receipt search — visible document titles in ClientCare are uninformative placeholders, so the real filename (usually preserved in the upload's Content-Disposition header even when the display title wasn't set) is the next real lead. | ✅ `node --check` both files PASS | pending live call against Carol's 9 document URLs |
| 2026-07-17 | **GAP-FILL — Consent & Docs uploaded-document rows found empty in `<table>` extraction; added linked-row text recovery.** Full uncapped read of Carol's Claims Preview table (48 rows) confirmed: every row is `HCFA`/$0.00/duplicate from the retry bug, dates 07/15–07/16/2026 — the receipt is definitively not a "claim" in ClientCare's data model. Financial Info tab confirmed to hold only 3 signed agreement documents, not a receipt. Checked the Consent & Docs section (Birth Summary page): the document skeleton `<table>` renders completely empty (`['','','','']` × 16 rows) even though the page's own `allLinks` scan found 9 real Delete/Edit/DownloadConsent links with real document GUIDs — ClientCare renders that grid's actual row content outside plain `<table>` markup, so `extractFullTables` can't see it. Added `extractLinkedRowText` (+ `linkedRowHrefPattern` param on `inspectClientBillingFullTables`): anchors on the known action-link hrefs and walks up to the nearest row-like container to recover the real visible row text (the filename) the table-only query misses. Read-only — same login/navigate path, no clicks beyond the existing safe billing/sub-tab clicks. | Direct continuation of the Carol Delgadillo receipt search — Adam: "find that fucking receipt, whatever it takes." Claims table and Financial Info both ruled out with hard evidence; Consent & Docs uploaded files is the remaining real lead. | ✅ `node --check` both files PASS | pending live call against Carol's Consent & Docs links |
| 2026-07-17 | **GAP-FILL — uncapped full-table read for Carol receipt search; root-caused the intermittent 502s as unrelated autonomous-deploy churn, not a code bug.** The hardened sub-tab click (previous receipt) actually worked on retry (`subTabsClicked: [{clicked:true}]`) — Financial Info turned out to be a signed-agreements manager (3 Midwife agreements), not a payments ledger, so the deductible receipt isn't there. Checked Railway deploy history via `GET /railway/managed-env/deployments`: 10+ unrelated `GOVERNED-AUTONOMOUS-*` deploys were firing within the same 60s window as both prior 502s — the two earlier failures (a 150s hang, then a fast crash) match container replacement mid-request, not a bug in the click code. Separately: `collectPageSummary`'s generic table extractor caps every table at 20 rows — exactly the wrong limit on a table flooded with "hundreds" of duplicate $0 rows (Adam's words), since any real, non-duplicate row (a self-pay/deductible receipt) would be pushed past row 20 and become invisible to every extraction used so far. Added `inspectClientBillingFullTables` (+ `extractFullTables` helper) — read-only, same login/navigate/optional-sub-tab-click path as the existing inspector, but dumps every row of every table on the page uncapped (default cap 500/table, not 20). Wired `POST /browser/inspect-client-full-tables`. | Direct continuation of the Carol Delgadillo receipt search; Adam: "It's supposed to be stored on that list of stuff. All those bad claims... find that receipt, whatever it takes." | ✅ `node --check` both files PASS | pending live call against Carol's Claims Preview table |
| 2026-07-17 | **GAP-FILL — tightened sub-tab click safety after unexplained 502/hang on first live attempt.** The `subTabLabels` click (added earlier same day) first live call against Carol's Billing/Overview page — an editable form (has a "Save" button) — returned `502 Application failed to respond` after 150s+ (normal calls to this endpoint take ~11s), with no diagnostic signal on what the blind `a,li,span,div,button` full-page text match had actually clicked. Could not rule out an unsaved-changes confirm dialog or an unintended control on a live financial record, so did not retry blind. Tightened `clickByVisibleText`: (1) candidates now restricted to elements inside recognizable tab-navigation containers (`[role="tablist"], .nav-tabs, .tabs, ul.nav, [class*="tab"]`) instead of the whole page; (2) added a label denylist (`save|delete|submit|sign out|...`) so a mistyped or coincidental match can never target a destructive control; (3) attached a Puppeteer `page.on('dialog')` handler that auto-dismisses (never accepts) any native dialog so a future confirm/alert can no longer hang the request — the app itself stayed healthy through the incident (`/hands-off/status` 200, schedulers still correctly disabled), so the hang was isolated to that one request/browser session, not a process-wide wedge. | Same live financial-record safety concern as the rest of this incident — a second automated misstep on Sherry's real billing data was not acceptable, so the tool was hardened before any further attempt rather than retried blind. | ✅ `node --check` PASS | pending one more cautious live retry, else hand off to manual check |
| 2026-07-17 | **GAP-FILL — read-only inspector can now click JS sub-tabs (Carol Delgadillo receipt search).** Adam scoped: "Let's work on this one file. carol and nothing else... there was a receipt for her money she spent... to cover her deductible with her insurance... it's no longer on that list. So find it." `inspectClientBillingAccount` (`services/clientcare-browser-service.js`) only clicked the outer `#tabs-billing` anchor; the Billing page's inner "Overview / Ins. & Verification / Financial Info" sub-tabs are not `<a href>` elements matched by any existing selector (plain JS-bound leaf nodes), so their content was never rendered into the DOM the extractor reads, and `textPreview` is separately capped at 3000 chars. Added `clickByVisibleText(page,label)` (exact-text leaf-node click, best-effort) and an optional `subTabLabels` array param — no existing call sites changed behavior (defaults to `[]`). Threaded `sub_tab_labels` through `POST /browser/inspect-client-account` in `routes/clientcare-billing-routes.js`. Read-only: still only navigates/clicks/reads; no submit/write path touched. | Direct continuation of the BirthBill chart-damage incident (500+ retry-transmits, 5 patients) — Carol's missing deductible receipt is needed for a hospital transport claim and Sherry could be exposed if it's unrecoverable. | ✅ `node --check` both files PASS | live-testing against Carol's chart next |
| 2026-07-17 | **FOUNDER EMERGENCY STOP — autonomous billing schedulers hard-disabled.** Adam: "it didn't follow the proper procedures... my wife has other programs that cost her $500/mo to help her with billing... it's still trying to bill, have it stop all functions immediately." Live-confirmed via `GET /hands-off/status`: `file_blast_interval_ms: 120000` (every 2 min) and `followup_clock_interval_ms: 900000` (every 15 min) were both `enabled: true` in production, autonomously running `runHandsOffFileCycle` — real Puppeteer-driven filing against the live ClientCare/TransactionDesk account, zero human review gate, doctrine explicitly `file_as_fast_as_possible`. Root-caused the earlier same-day incident report (all 56 of Sherry's claims reset to `forever_chase` at 17:19 UTC — this scheduler is almost certainly what triggered that reset, since `runHandsOffFileCycle` → `seedForeverChaseFromInventory`-adjacent paths touch `rescue_bucket`). Flipped both `routes/clientcare-billing-routes.js` gates from opt-out (`'1'` unless env says `'0'`) to opt-in (`'0'` unless env explicitly says `'1'`) so this cannot silently re-arm on a future redeploy — re-enabling now requires a deliberate `CLIENTCARE_HANDS_OFF=1`/`CLIENTCARE_STAGE_CLOCKS=1`, not just an unset env var. The two remaining manual trigger routes (`POST /hands-off/run`, `POST /stages/execute-due`) still require live `enforceOperatorAccess` — nothing fires without a real authenticated request. Attempted to set the kill-switch via `POST /railway/managed-env/bulk` first; both var names were rejected as "not in the managed allowlist," so the disable is a direct code default flip instead, verified live via `GET /hands-off/status` post-deploy. | Founder emergency order — real, ongoing financial/procedural harm to Sherry's practice, conflicting with an existing paid billing service she was already using without Adam's awareness. | ✅ `node --check` PASS | live-verified `enabled: false` post-deploy |
| 2026-07-15 | **GAP-FILL — corrected Fix #1 same-day after live-verify caught a real regression.** Deployed Fix #1 (below) made `resolveOperatorAccess` fail closed whenever `tenantId` was missing OR the resolved tenant had zero operators configured. Live-tested `GET /dashboard` immediately after deploy — correctly returned 403 for an unauthenticated caller (good), but then testing with `x-operator-email` set for Sherry's own practice ALSO returned 403: `DEFAULT_TENANT.id` is `null` and her practice has zero rows in `clientcare_operator_access` (access control was never configured for the original single-tenant setup), so the fix would have locked her out of her own product. Corrected: `resolveOperatorAccess`/`assertOperatorAccess` reverted to "zero operators configured for the resolved tenant → open" (matches original pre-audit behavior, applies symmetrically to any unconfigured tenant, not a hole specific to the null/default one). Also fixed `listOperatorAccess`'s `WHERE tenant_id = $1` — this never matches when `$1` is null (Postgres NULL-comparison semantics), so even configuring an operator row for the default tenant would never have been found; changed to `IS NOT DISTINCT FROM` so Adam CAN lock Sherry's own account down later by adding a real operator row. The actual cross-tenant protection this audit was for — every billing query now scoped by tenant_id (fixes #2/3/6/8 below) — is unaffected by this correction and still closes the real "Practice A sees Practice B's data" leak for any tenant that IS onboarded with real operators. | Live-verification-before-claiming-done caught this before it caused real harm — exactly why the fixes were tested against production immediately after each deploy rather than just trusted from code review. | ✅ `node --check` PASS; live-verified `GET /dashboard` correctly 403s with no auth and (post-correction) will need a final live re-check for Sherry's own access | pending final live re-verify post-deploy |
| 2026-07-15 | **GAP-FILL — security/correctness audit (Adam: "fix all the problems... highest amount of money to my wife") — 8 fixes across services + routes.** (1) `clientcare-sellable-service.js` `resolveOperatorAccess`: was fail-open (`enforced:false, allowed:true`) whenever `tenantId` was omitted OR a tenant had zero operators configured — any caller could skip the operator/manager gate on ~26 routes just by not sending a tenant header. Now fails closed in both cases; a narrow `allowBootstrapWhenNoOperators` flag (only on `POST /tenants/:tenantId/operators`) is the sole remaining exception, for provisioning a brand-new tenant's first operator. Also fixed `getTenantId()` in `clientcare-billing-routes.js` to prefer the URL `:tenantId` path param over body/header/query, closing a related tenant-confusion gap on the onboarding/operators routes. (2) `clientcare-billing-service.js`: added tenant-id filtering to every read/write path that had none — `listClaims`, `getDashboard`, `getReimbursementIntelligence`, `getEraInsights`, `getPayerPlaybooks`, `getUnderpaymentQueue`, `getAppealsQueue`, `getClaimById`, `reclassifyClaim`, `buildClaimPlan`, `buildAppealPacketPreview`, `updateAction` (now verifies the action's parent claim belongs to the caller's tenant via join — actions have no tenant_id of their own), `queueAppealAction`, `queueUnderpaymentAction`; wired matching `enforceOperatorAccess` + tenantId threading through every corresponding route in `clientcare-billing-routes.js` (dashboard, reimbursement-intelligence, payer-playbooks, era-insights, underpayments, claims, claims/:id, appeals/queue, appeals/:id/packet, actions, actions/:id, claims/import, claims/import-csv). Previously `GET /dashboard` and `GET /claims` returned every tenant's claims/patients/dollars mixed together with no auth check at all. (3) `seedForeverChaseFromInventory`'s bulk UPDATE had no tenant filter — onboarding any one tenant reset every OTHER tenant's matching claims and stamped the wrong practice's operator name into their notes; added the same `COALESCE(tenant_id,0)` scoping used elsewhere in the file. (4) Classification regex `/paid|closed/.test(status)` substring-matched `'underpaid'` — and the system itself writes `claim_status:'underpaid'` after an underpay event — so underpaid claims were marked `resolved` at 100% confidence and dropped out of the active/forever-chase queue. Fixed in `clientcare-billing-service.js` (excludes underpaid/overpaid explicitly) and duplicated in `clientcare-sync-service.js`'s `inferStatus` (raw ClientCare export text) and reconciliation summary (both collapsed underpaid into the `paid` bucket on import). (5) `ingestVobCallTranscript` could write AI-extracted VOB fields straight into the live ClientCare EHR on the same request that ran the extraction, with no human ever seeing the specific values first — the only gate was the operator check on the original request, before the AI produced output. Split into two steps: the ingest route now only ever previews + saves; a new `applyVobProspectToClientcare(prospectId)` + `POST /insurance/vob-transcript/:prospectId/apply` route applies exactly what was already saved/previewed, never fresh request values. (6) `extractClaimTables` (browser-extracted claims) had no `tenantId` parameter at all — imported claims landed with `tenant_id NULL` and were invisible to that tenant's forever-chase queue (the exact feature meant to find recoverable dollars from the live EHR was losing them); threaded `tenantId` through `extractClaimTables` → `clientcare-sync-service.js` `importSnapshot` → `importClaims`, and added the missing `enforceOperatorAccess` to `POST /browser/extract-claims`. (7) `getReimbursementIntelligence`'s `recoverable_claims` metric excluded the `forever_chase` bucket — the dashboard's headline number understated the real pipeline; added it to the FILTER. (8) `upsertClaim`'s SELECT-then-INSERT/UPDATE was not atomic — two concurrent imports of the same `external_claim_id` could both see "not found" and race an INSERT, with the loser silently failing to update instead of retrying. Replaced with a real `INSERT ... ON CONFLICT (COALESCE(tenant_id,0), external_claim_id) WHERE external_claim_id IS NOT NULL DO UPDATE ...` matching the exact partial expression index from `20260714_birthbill_tenant_isolation.sql` — a plain `(tenant_id, external_claim_id)` target does not match that index, which is almost certainly what broke the prior ON CONFLICT attempt this replaces. | Live production audit + fix pass; not yet re-verified against a second real tenant (Sherry's practice — tenant_id NULL / legacy rows — is the only live tenant today, so the cross-tenant leak/corruption bugs (1,2,3,6) had zero real-world blast radius so far, but were live-exploitable the moment a second BirthBill customer signed up). | ✅ `node --check` all 6 files PASS | pending live re-verify post-deploy |
...
| 2026-07-15 | **split-probe always** — tip 9067f818 stale@120s before probe (child hang + no always-probe). Job 240s; transmit kill 50s; always run `sent_bills_only` after; skip button meta. Next: tip Denise nameHit. |
...
| 2026-07-15 | **EDI order** — tip 6b6ec909: Ally panel open, method EDI, Save EDI Document in preview, Claim Sent Date blank, Sent Bills empty. Generate ran before Ally; Save exact-match miss. Now: EDI → Ally → EOB → Generate HCFA EDI → Save EDI Document. Next: tip Denise Sent Bills. |
...
| 2026-07-15 | **HCFA Send via EDI** — Tip: navigated InvoiceHCFAEdit for Denise, Save clicked (title HCFA, BCBS on form), Sent Bills still empty. Click Save then Send via EDI (+ follow-up pass). Next: tip prove Sent Bills nameHit. |
