...
| **Last Updated** | 2026-07-22 — `getForeverChaseQueue` now excludes `rescue_bucket='do_not_bill'` and `metadata.deactivated=true`, so `deactivateClaim` actually stops forever-chase / due-chase (prior gap: unpaid OR clause still pulled deactivated claims). Prior 2026-07-21: Founder-authorized incident recovery — `deactivateClaim` + POST deactivate for cash-pay wrongly billed claims (e.g. claim 117). Live ClientCare duplicate-record deletion still NOT built; pending human-supervised (Adam + Sherry) live verification on 5-6 records per founder's explicit process before any unsupervised use. See Change Receipts. |
...
| 2026-08-02 | **Creative Director review** — generated CREATIVE_BRIEF.md using the Creative Director lens. | ClientCare Billing Recovery reviewed through the BuilderOS creative responsibility; brief written to product home for founder review. | ✅ generated |
| Date | What Changed |
| 2026-07-22 | **Audit fix — deactivate actually leaves chase inventory.** `getForeverChaseQueue` WHERE clause now requires `rescue_bucket NOT IN ('resolved','do_not_bill')` AND `metadata->>'deactivated' <> 'true'`. Without this, unpaid deactivated claims (e.g. claim 117) stayed in forever-chase via the `paid_amount <= 0` OR branch. Covered by `tests/clientcare-deactivate-chase-exclusion.test.js`. | Branch audit P0: deactivateClaim claimed to stop chasing but did not. | ✅ unit tests PASS | ready for live re-deactivate of claim 117 if still open |
