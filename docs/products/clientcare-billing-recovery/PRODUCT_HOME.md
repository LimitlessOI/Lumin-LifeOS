...
| **Last Updated** | 2026-07-21 — Founder-authorized incident recovery in progress (Adam, direct authority, SO-001 governed-factory requirement explicitly bypassed for narrow reversible fixes — Chair informed). Added `deactivateClaim` (reversible, non-destructive — flags `rescue_bucket='do_not_bill'` + metadata reason, never a hard DELETE) for claims that should never have been created, e.g. cash-pay clients wrongly auto-billed to insurance. First target: Justean Labelle (claim id 117) — cash-pay client, `submission_status=edi_submitted_awaiting_payment` with `payer_name="Unknown — ask insurer"` and `resolve_fail_count=97`; notes field contains the same "SYSTEM: Sent Bills nameHit" line duplicated 17x verbatim — direct evidence of the retry-loop bug on a single record. Live ClientCare duplicate-record deletion (separate, higher-risk — browser automation against the vendor UI) intentionally NOT built yet; pending narrow function build + human-supervised (Adam + Sherry) live verification on 5-6 records per founder's explicit process before any unsupervised use. See Change Receipts. |
