...
| **Last Updated** | 2026-07-17 — Restored fail-closed operator access for unprovisioned tenants while retaining an explicit trusted bootstrap path. |
...
| Date | What Changed |
| 2026-07-17 | **Unprovisioned-tenant authorization fix.** `resolveOperatorAccess`/`assertOperatorAccess` now deny zero-operator tenants unless the route grants bootstrap. Route bootstrap is restricted to command-key or admin/founder-admin auth: trusted access remains for the legacy null/default dashboard, while first-operator provisioning is explicit for named tenants. Added service and member-vs-founder regression coverage. | The prior unconditional open state let any authenticated LifeOS member bypass operator/manager checks for default or newly created tenants and reach PHI/mutation routes. | ✅ focused 4/4, npm test 485 pass, builder preflight PASS | Live-verify member JWT 403 + founder command-key 200 after deploy |
