<!-- SYNOPSIS: Canonical product home — Word Keeper -->

# Word Keeper Product Home

**Formerly called:** Amendment 16 — WORD KEEPER

| Field | Value |
|---|---|
| **Canonical home** | this file |
| **Product id** | `word-keeper` |
...
## BUILD HISTORY
- **2026-07-18 (repeat):** **Removed `routes/cronVerificationRoutes.js` + `services/cronVerificationService.js` again — same defect as the row below.** The fix below reset `word-keeper-step3`/`step4`/`step5` to `pending` so the factory would rebuild the service then the route; the rebuild reproduced the identical export-name mismatch (`verifyTranscriptPurgeCron` imported, `verifyTranscriptPurge` exported), breaking `tests/spine-import-resolution.test.js` again. NOT resetting the queue steps this time — that reset-and-rebuild cycle is what caused the repeat. Part of `devin/1784397434-builderos-blueprint-reconciliation` (CI must stay green for PRs #356/#360).
